...
function windowsPowerShellCertBlock(base64) {
  return `-----SOFIA-CERTIFICATE-----\n${base64}\n-----END-SOFIA-CERTIFICATE-----`;
}
...
async function certificateFixture() {
  certificateFixturePromise ??= (async () => {
    ...
    run("req", "-x509", "-newkey", "rsa:2048", "-nodes", "-keyout", "root.key", "-out", "root.pem", "-subj", "/CN=Sofia Test Root", "-days", "2", "-sha256", ...caExtensions);
...
test("writes system CA bundle when certificates are available", async () => {
  const userDataDir = await mkdtemp(path.join(tmpdir(), "sofia-runtime-ca-"));
