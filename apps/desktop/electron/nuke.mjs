...
function isTruthyDevMode(env) {
  return envValue(env, "SOFIA_DEV_MODE") === "1";
}
...
function isIsolatedProfile(env) {
  return (
    isTruthyDevMode(env) ||
    envValue(env, "SOFIA_ELECTRON_APP_IDENTIFIER") !== "" ||
    envValue(env, "SOFIA_ELECTRON_USERDATA") !== ""
  );
}
...
function resolveNukeEnvironment({ env = {}, homedir, platform, userDataPath }) {
  ...
  const userDataOverride = envValue(env, "SOFIA_ELECTRON_USERDATA");
  const resolvedUserDataPath = userDataOverride || userDataPath;

  if (isTruthyDevMode(env)) {
    const root = paths.join(resolvedUserDataPath, "sofia-dev-data");
...
function sofiaServerConfigPath(env, homedir, platform, paths) {
  return resolveSofiaServerConfigPath({ env, homeDir: homedir, platform });
}

function envStorePath(env, homedir, platform, paths) {
  return sofiaEnvStorePath({ env, homeDir: homedir, platform });
}

function tokenStorePath(env, serverConfigPath, homedir, paths) {
  const override = envValue(env, "SOFIA_TOKEN_STORE");
  if (override) return paths.resolve(override);
  const configDir = serverConfigPath ? paths.dirname(serverConfigPath) : paths.join(homedir, ".config", "sofia");
  return paths.join(configDir, "tokens.json");
}

function runtimeDbPath(env, serverConfigPath, homedir, paths) {
  const override = envValue(env, "SOFIA_RUNTIME_DB");
  if (override) return paths.resolve(override);
  const configDir = serverConfigPath ? paths.dirname(serverConfigPath) : paths.join(homedir, ".config", "sofia");
...
function orchestratorDataDir(env, homedir, paths) {
  ...
  return paths.join(homedir, ".sofia", LEGACY_ORCHESTRATOR_DIR_NAME);
}
...
function addSofiaConfigFiles(deletePaths, roots, paths) {
  for (const root of roots) {
    if (!root) continue;
    for (const filename of SOFIA_CONFIG_FILENAMES) {
...
function resolveNukePlan(input) {
  ...
  const serverConfig = sofiaServerConfigPath(env, homedir, platform, paths);
  ...
  const deletePaths = [
    ...
    ...workspaceSofiaStatePaths(
      Array.isArray(input.workspacePaths) ? input.workspacePaths : [],
      paths,
    ),
  ];

  const sofiaConfigRoots = [
    paths.join(desktopConfigHome(env, homedir, platform, paths), "sofia"),
  ...
  const pendingPath = scopeToProfile
    ...
    : paths.join(desktopConfigHome(env, homedir, platform, paths), "sofia", PENDING_NUKE_FILENAME);
...
function nukeWorkerPayloadPath() {
  return path.join(os.tmpdir(), `sofia-nuke-worker-${Date.now()}-${randomBytes(6).toString("hex")}.json`);
}
...
function nukeWorkerSpawnEnv(env = process.env) {
  ...
  delete workerEnv["SOFIA_ELECTRON_REMOTE_DEBUG_PORT"];
  return workerEnv;
}
...
async function writeReceipt(receipt) {
  const receiptPath = path.join(os.tmpdir(), `sofia-nuke-receipt-${Date.now()}.json`);
...
async function quiesceForNuke({ runtimeManager, uiControlServer, removeWindowsBrandShortcut }, errors, options = {}) {
  ...
  if (!options.scopeToProfile) {
    await bestEffort(errors, "sandbox-docker-cleanup", () => runtimeManager.sandboxCleanupSofiaContainers(), 24_000);
  }
...
/** @returns {Promise<import("@sofia/types/desktop-ipc").NukeReceipt>} */
