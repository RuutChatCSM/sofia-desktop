...
function validRecoveryArtifactIdentity(artifact) {
  ...
  const prefix = `/RuutChatCSM/sofia-desktop/releases/download/v${version}/`;
  ...
  return fileName === `sofia-${distributionSlug}${platformSlug}-${assetArch}-${version}${installerExtension(artifact.platform)}`;
}
...
export function selectRecoveryArtifact(files, { version, platform, arch, distribution }) {
  ...
  const baseUrl = `https://github.com/RuutChatCSM/sofia-desktop/releases/download/v${normalizedVersion}/`;
  for (const selected of matching) {
    const url = new URL(selected.url, baseUrl);
    if (url.origin !== "https://github.com" || !url.pathname.startsWith(`/RuutChatCSM/sofia-desktop/releases/download/v${normalizedVersion}/`)) {
      continue;
    }
    const artifact = {
