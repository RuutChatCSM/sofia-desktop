...
function legacyOrchestratorPath(home) {
  return path.join(home, ".sofia", LEGACY_ORCHESTRATOR_DIR_NAME);
}
...
async function withTempDir(fn) {
  const root = await mkdtemp(path.join(tmpdir(), "sofia-nuke-test-"));
  try {
    await fn(root);
  } finally {
...
function pendingNukePath(root) {
  return path.join(root, "xdg", "sofia", ".nuke-pending.json");
}
...
function fakeRuntimeManager() {
  return {
    ...
    sandboxCleanupSofiaContainers: async () => ({ candidates: [], removed: [], errors: [] }),
  };
}
...
test("buildNukeManifest includes default macOS state roots and preserves bootstrap", () => {
  ...
  const userDataPath = "/Users/alice/Library/Application Support/com.differentai.sofia";
  const manifest = buildNukeManifest({ env: {}, homedir: home, platform: "darwin", userDataPath });

  assert.equal(manifest.bootstrapPath, "/Users/alice/.config/sofia/desktop-bootstrap.json");
  assert.equal(manifest.preserveBootstrapPath, "/Users/alice/.config/sofia/desktop-bootstrap.json");
  assert.deepEqual(manifest.partitions, ["default", "persist:sofia-browser"]);
  assert.ok(manifest.deletePaths.includes(userDataPath));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/sofia/server.json"));
...
test("buildNukeManifest can include the bootstrap file in the wipe", () => {
  const bootstrapPath = "/Users/alice/.config/sofia/desktop-bootstrap.json";
  const manifest = buildNukeManifest({
    env: {},
    homedir: "/Users/alice",
    platform: "darwin",
    preserveBootstrap: false,
    userDataPath: "/Users/alice/Library/Application Support/com.differentai.sofia",
  });
...
test("buildNukeManifest includes default Linux state roots", () => {
  const manifest = buildNukeManifest({
    ...
    homedir: "/home/alice",
    platform: "linux",
    userDataPath: "/home/alice/.config/com.differentai.sofia",
  });
...
test("buildNukeManifest includes Windows path shapes", () => {
  ...
  const manifest = buildNukeManifest({
    ...
    platform: "win32",
    userDataPath: "C:\\Users\\Alice\\AppData\\Roaming\\com.differentai.sofia",
  });

  assert.equal(manifest.preserveBootstrapPath, "C:\\Users\\Alice\\AppData\\Local\\sofia\\desktop-bootstrap.json");
  assert.ok(manifest.deletePaths.includes("C:\\Users\\Alice\\AppData\\Roaming\\com.differentai.sofia"));
  assert.ok(manifest.deletePaths.includes("C:\\Users\\Alice\\AppData\\Roaming\\sofia\\server.json"));
...
test("buildNukeManifest honors SOFIA_ELECTRON_USERDATA override", () => {
  const manifest = buildNukeManifest({
    env: { SOFIA_ELECTRON_USERDATA: "/tmp/sofia-userdata" },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/Users/alice/Library/Application Support/com.differentai.sofia",
  });

  assert.ok(manifest.deletePaths.includes("/tmp/sofia-userdata"));
  assert.ok(!manifest.deletePaths.includes("/Users/alice/Library/Application Support/com.differentai.sofia"));
});
...
test("buildNukeManifest redirects HOME/XDG paths in dev mode", () => {
  const manifest = buildNukeManifest({
    ...
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/sofia-dev-userdata",
  });
...
test("buildNukeManifest never reaches production state from a non-dev isolated profile", () => {
  const userDataPath = "/Users/alice/Library/Application Support/com.differentai.sofia.dev.wt2";
  const manifest = buildNukeManifest({
    env: { SOFIA_ELECTRON_APP_IDENTIFIER: "com.differentai.sofia.dev.wt2" },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath,
  });
...
test("buildNukeManifest still wipes a dev profile's own orchestrator data dir", () => {
  ...
  const manifest = buildNukeManifest({
    env: { SOFIA_DEV_MODE: "1", SOFIA_DATA_DIR: devOrchestratorPath },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/sofia-dev-userdata",
  });
...
test("buildNukeManifest ignores an inherited XDG_CONFIG_HOME pointing at production config", () => {
  const manifest = buildNukeManifest({
    env: { SOFIA_DEV_MODE: "1", XDG_CONFIG_HOME: "/Users/alice/.config" },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/sofia-dev-userdata",
  });
...
test("runPendingNukeCleanup retains the choice to remove bootstrap state", async () => {
  await withTempDir(async (root) => {
    ...
    const bootstrapPath = path.join(input.env.XDG_CONFIG_HOME, "sofia", "desktop-bootstrap.json");
...
test("runPendingNukeCleanup refuses replayed paths outside an isolated profile", async () => {
  await withTempDir(async (root) => {
    ...
    const productionPath = path.join(root, "home", ".config", "sofia", "tokens.json");
    const profilePath = path.join(userDataPath, "sofia-server-tokens.json");
...
test("nuke worker payload only serializes safe path inputs", () => {
  const nukeInput = buildNukeWorkerNukeInput({
    env: {
      APPDATA: "C:\\Users\\Alice\\AppData\\Roaming",
      SOFIA_API_KEY: "secret-api-key",
      SOFIA_TOKEN: "secret-token",
      SOFIA_ELECTRON_REMOTE_DEBUG_PORT: "9888",
      SOFIA_TOKEN_STORE: "C:\\Users\\Alice\\AppData\\Roaming\\sofia\\tokens.json",
  ...
  const payload = buildNukeWorkerPayload({
    ...
    pendingPath: "/tmp/config/sofia/.nuke-pending.json",
    nowMs: 1_000,
  });
  ...
  assert.equal(payload.nukeInput.env.SOFIA_TOKEN_STORE, "C:\\Users\\Alice\\AppData\\Roaming\\sofia\\tokens.json");
  ...
  assert.equal(serialized.includes("SOFIA_ELECTRON_REMOTE_DEBUG_PORT"), false);
  assert.equal(serialized.includes("SOFIA_API_KEY"), false);
  assert.equal(serialized.includes("SOFIA_TOKEN\""), false);
...
test("scheduleNukeCleanupWorker launches Electron as Node with a detached safe payload", async () => {
  await withTempDir(async (root) => {
    const input = pendingNukeInput(root);
    input.env.SOFIA_ELECTRON_REMOTE_DEBUG_PORT = "9888";
    const plan = {
    ...
    const result = await scheduleNukeCleanupWorker({
      ...
      env: { SECRET_TOKEN: "do-not-write", XDG_CONFIG_HOME: input.env.XDG_CONFIG_HOME, SOFIA_ELECTRON_REMOTE_DEBUG_PORT: "9888" },
    ...
    assert.equal(spawnOptions.env.SOFIA_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.equal(payload.nukeInput.env.SOFIA_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.deepEqual(payload.appArgv, []);
...
test("nuke cleanup worker waits for parent exit, clears pending path, removes payload, and launches app", async () => {
  await withTempDir(async (root) => {
    ...
    const result = await runNukeCleanupWorker(payloadPath, {
      env: { ELECTRON_RUN_AS_NODE: "1", SOFIA_ELECTRON_REMOTE_DEBUG_PORT: "9888" },
    ...
    assert.equal(launchEnv.SOFIA_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.equal(await exists(targetPath), false);
    assert.equal(await exists(pendingPath), false);
...
test("executeNukeFreshStart skips host-wide container cleanup on an isolated profile", async () => {
  await withTempDir(async (root) => {
    ...
    runtimeManager.sandboxCleanupSofiaContainers = async () => {
      cleanedContainers += 1;
      return { candidates: [], removed: [], errors: [] };
    };
    const input = { ...pendingNukeInput(root), env: { SOFIA_DEV_MODE: "1" } };
