...
test("normal launches remain unchanged", () => {
  ...
  assert.deepEqual(resolveBlankSlateLaunch({ appName: "Sofia", profile }), {
    enabled: false,
    appName: "Sofia",
    userDataPath: null,
  });
...
test("blank-slate launches receive unique temporary roots and a visible name", async () => {
  ...
  const first = resolveBlankSlateLaunch({ appName: "Sofia", profile: firstProfile });
  const second = resolveBlankSlateLaunch({ appName: "Sofia", profile: secondProfile });

  try {
    assert.equal(first.appName, "Sofia - Test profile");
