...
test("normal launches remain unchanged", () => {
  ...
  assert.deepEqual(resolveBlankSlateLaunch({ appName: "Sofia App", profile }), {
    enabled: false,
    appName: "Sofia App",
    userDataPath: null,
  });
...
test("cleanup worker removes the entire temporary root after its parent exits", async () => {
  const rootPath = await mkdtemp(path.join(tmpdir(), "sofia-cleanup-test-"));
  const userDataPath = path.join(rootPath, "electron", "user-data");
  const configPath = path.join(rootPath, "sofia", "config");
...
test("blank-slate launches receive unique temporary roots and a visible name", async () => {
  ...
  const first = resolveBlankSlateLaunch({ appName: "Sofia App", profile: firstProfile });
  const second = resolveBlankSlateLaunch({ appName: "Sofia App", profile: secondProfile });

  try {
    assert.equal(first.appName, "Sofia App - Test profile");
    ...
    assert.ok(!first.rootPath.includes("com.differentai.sofia"));
  } finally {
    await Promise.all([
...
test("process profile hides an installed bootstrap before workspace-store loads", async () => {
  const installedRoot = await mkdtemp(path.join(tmpdir(), "sofia-installed-profile-test-"));
  ...
  try {
    const { stdout } = await execFileAsync(process.execPath, ["--input-type=module", "--eval", script], {
      env: {
        ...process.env,
        SOFIA_DESKTOP_BOOTSTRAP_PATH: installedBootstrapPath,
      },
    });
...
function registerPlatformIsolationTest(platform, temporaryDirectory, rootPath) {
  test(`blank-slate isolates every persisted path on ${platform}`, () => {
    ...
    const env = {
      ...
      SOFIA_DESKTOP_DISTRIBUTION: "enterprise",
      SOFIA_SERVER_CONFIG: paths.join(paths.parse(rootPath).root, "installed", "server.json"),
    };
    ...
    const profile = prepareBlankSlateProfile({
      ...
      createTempRoot: (prefix) => {
        assert.equal(prefix, paths.join(temporaryDirectory, "sofia-test-profile-"));
        return rootPath;
      },
    ...
    assert.ok(profile);
    assert.equal(env.SOFIA_DESKTOP_DISTRIBUTION, "enterprise");
    assert.ok(!Object.hasOwn(env, "SOFIA_DEV_MODE"));
...
registerPlatformIsolationTest("darwin", "/private/tmp", "/private/tmp/sofia-test-profile-macos");
registerPlatformIsolationTest("linux", "/tmp", "/tmp/sofia-test-profile-linux");
registerPlatformIsolationTest("win32", "C:\\Temp", "C:\\Temp\\sofia-test-profile-windows");
