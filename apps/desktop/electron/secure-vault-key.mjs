...
/**
 * Compact filename-safe UTC timestamp mirroring the `*.sofia-backup-<ts>`
 ...
 * until a user opts into Sofia-managed OAuth.
 *
 * @param {{
 *   filePath: string;
...
export function createDesktopVaultKeyProvider({
  ...
  async function loadKey() {
    ...
    if (!safeStorage || !(await safeStorage.isAsyncEncryptionAvailable())) {
      throw new Error("Operating-system secure storage is unavailable for Sofia-managed OAuth.");
    }
    if (platform === "linux" && safeStorage.getSelectedStorageBackend() === "basic_text") {
      throw new Error("A secure Linux password store is required for Sofia-managed OAuth.");
    }
    ...
    if (encrypted) {
      ...
      try {
        ...
        await rename(filePath, `${filePath}.sofia-backup-${backupTimestamp(new Date())}`);
      }
      if (decrypted && key) {
