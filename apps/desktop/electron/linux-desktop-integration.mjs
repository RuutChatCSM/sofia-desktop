...
/** @typedef {import("@sofia/types/desktop-ipc").DesktopIntegrationIssue} DesktopIntegrationIssue */
/** @typedef {import("@sofia/types/desktop-ipc").DesktopIntegrationResult} DesktopIntegrationResult */
/** @typedef {import("@sofia/types/desktop-ipc").DesktopIntegrationStatus} DesktopIntegrationStatus */

export const SOFIA_DESKTOP_ID = "com.differentai.sofia.desktop";
export const SOFIA_DESKTOP_NAME = "com.differentai.sofia";
export const SOFIA_PROTOCOL_MIME = "x-scheme-handler/sofia";

const INTEGRATION_STATE_VERSION = 1;
const OWNERSHIP_MARKER = "X-Sofia-Managed";
const MANAGED_VERSION_MARKER = "X-Sofia-Version";
...
function entryHandlesSofia(fields) {
  return (fields.get("MimeType") ?? "")
    .split(";")
    .map((value) => value.trim())
    .includes(SOFIA_PROTOCOL_MIME);
}
...
async function removeDesktopAssociation(filePath, desktopId) {
  ...
  const next = content.split(/\r?\n/).map((line) => {
    ...
    if (
      ...
      || !trimmed.startsWith(`${SOFIA_PROTOCOL_MIME}=`)
    ) {
      return line;
    }
    const separator = line.indexOf("=");
...
export function buildSofiaDesktopEntry({
  appImagePath,
  appName,
  appVersion,
  distribution,
...
StartupWMClass=${SOFIA_DESKTOP_NAME}
Terminal=false
Categories=Development;Utility;
MimeType=${SOFIA_PROTOCOL_MIME};
...
X-Sofia-Distribution=${cleanDesktopValue(distribution)}
X-Sofia-AppImage=${cleanDesktopValue(appImagePath)}
`;
}
...
export function createLinuxDesktopIntegration({
  ...
  const desktopEntryPath = path.join(dataHome, "applications", SOFIA_DESKTOP_ID);
  const iconPaths = Object.fromEntries(ICON_SIZES.map((size) => [
    size,
    path.join(dataHome, "icons", "hicolor", `${size}x${size}`, "apps", `${SOFIA_DESKTOP_NAME}.png`),
  ]));
  const iconSources = Object.fromEntries(ICON_SIZES.map((size) => [
    size,
    path.join(resourcesPath ?? "", "icons", "linux", `${size}x${size}.png`),
  ]));
  const statePath = path.join(configHome, "sofia", "desktop-integration.json");
  ...
  async function queryDefaultHandler() {
    const result = await runCommand("xdg-mime", ["query", "default", SOFIA_PROTOCOL_MIME]);
  ...
  async function inspectDesktopFile(candidate, desktopId) {
    ...
    return {
      ...
      handlesProtocol: entryHandlesSofia(fields),
    };
  }
  ...
  async function findExternalEntry(handlerDesktopId) {
    ...
    for (const entry of entries) {
      if (!entry.isFile() || !entry.name.endsWith(".desktop") || entry.name === SOFIA_DESKTOP_ID) continue;
  ...
  async function getStatus() {
    ...
    if (ownFields && ownManaged) {
      ...
      if (!entryHandlesSofia(ownFields)) issues.push("desktop-entry");
      ...
      if (handlerDesktopId !== SOFIA_DESKTOP_ID) issues.push("protocol-handler");
      return {
        supported: true,
  ...
  async function install(options = {}) {
    ...
    if (before.ownership === "external") {
      ...
      return {
        ok: false,
        status,
        error: "The external launcher was not selected for sofia:// callbacks.",
      };
    }
    if (
      await fileExists(desktopEntryPath)
      && before.ownership !== "sofia"
    ) {
      return {
        ok: false,
        status: before,
    ...
    try {
      ...
      await atomicWrite(desktopEntryPath, buildSofiaDesktopEntry({
        appImagePath,
        appName,
      ...
      if (before.handlerDesktopId && before.handlerDesktopId !== SOFIA_DESKTOP_ID) {
      ...
      const registration = await runCommand("xdg-mime", ["default", SOFIA_DESKTOP_ID, SOFIA_PROTOCOL_MIME]);
      if (!registration.ok) {
        throw new Error(registration.stderr || "xdg-mime could not register sofia://.");
      }
      const status = await getStatus();
      if (status.state !== "integrated") {
        throw new Error("The desktop entry was installed, but the desktop did not select it as the sofia:// handler.");
      }
  ...
  async function remove() {
    ...
    try {
      ...
      if (before.handlerDesktopId === SOFIA_DESKTOP_ID) {
        const previousPath = await locateDesktopEntry(state.previousProtocolHandler);
        if (state.previousProtocolHandler && previousPath) {
          await runCommand("xdg-mime", ["default", state.previousProtocolHandler, SOFIA_PROTOCOL_MIME]);
        } else {
          await removeDesktopAssociation(path.join(configHome, "mimeapps.list"), SOFIA_DESKTOP_ID);
          await removeDesktopAssociation(path.join(dataHome, "applications", "mimeapps.list"), SOFIA_DESKTOP_ID);
        }
      }
