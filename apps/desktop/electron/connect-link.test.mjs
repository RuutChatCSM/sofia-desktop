 ...
 * @returns {import("@sofia/types/connect-link").ConnectLinkClaims}
 */
function claims(overrides = {}) {
  return {
    iss: "https://api.sofia.acme.example.com",
    ...
    den: {
      baseUrl: "https://sofia.acme.example.com",
      apiBaseUrl: "https://api.sofia.acme.example.com",
    },
    requireSignin: true,
    ...overrides,
  };
...
/** @param {import("@sofia/types/connect-link").ConnectLinkVerifyResult} result */
...
test("extracts only dedicated desktop connect links", () => {
  assert.equal(extractConnectLinkToken("sofia://connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("sofia-dev://connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("sofia:///connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("sofia://den-auth?grant=x"), null);
...
test("verifies an exact signed organization target end to end", () => {
  ...
  const result = verifyConnectLinkUrl(
    `sofia://connect?token=${encodeURIComponent(token)}`,
...
test("resolves a keyless exchange through the exact HTTPS Den endpoint", async () => {
  ...
  const apiBaseUrl = "https://api.sofia.acme.example.com/api/den";
  const expected = claims({
    iss: apiBaseUrl,
    brand: {
      appName: "Acme Work",
  ...
  const rawUrl = `sofia://connect?code=${code}&apiBaseUrl=${encodeURIComponent(apiBaseUrl)}`;
...
test("fails closed for ambiguous, insecure, mismatched, expired, and replayed exchanges", async () => {
  const code = "abcdefghijklmnopqrstuvwxyz123456";
  const apiBaseUrl = "https://api.sofia.acme.example.com";
  const rawUrl = `sofia://connect?code=${code}&apiBaseUrl=${encodeURIComponent(apiBaseUrl)}`;
  assert.equal(extractConnectExchange(`${rawUrl}&token=a.b.c`), null);

  const insecure = `sofia://connect?code=${code}&apiBaseUrl=${encodeURIComponent("http://den.example.com")}`;
