...
// and /command surface. Dispatched to the renderer's window.__sofiaControl.
...
export function createUiControlServer({ appName, appIdentifier, getWindow }) {
  ...
  async function evaluateSofiaControl(expression) {
  ...
  async function runSofiaControlCommand(command, args = {}) {
    const argsJsonLiteral = jsonForJavaScript(args);
    if (command === "snapshot") {
      return evaluateSofiaControl(`(async () => {
        const control = window.__sofiaControl;
        if (!control) return { ok: false, error: "Sofia App control surface is not available yet." };
    ...
    if (command === "query" || command === "command") {
      return evaluateSofiaControl(`(async () => {
        ...
        if (!input || typeof input.id !== "string" || !input.id.trim()) {
          return { ok: false, error: "Missing Sofia App affordance id." };
        }
    ...
    if (command === "execute") {
      return evaluateSofiaControl(`(async () => {
        ...
        if (!input || typeof input.actionId !== "string" || !input.actionId.trim()) {
          return { ok: false, error: "Missing Sofia App actionId." };
        }
        control.setEnabled?.(true);
        return control.execute(input.actionId, input.args ?? {});
      })()`);
    }
    return { ok: false, error: `Unknown Sofia App control command: ${command}` };
  }
  ...
  async function start() {
    ...
    uiControlServer = createServer(async (request, response) => {
      try {
        ...
        if (request.method === "GET" && url.pathname === "/snapshot") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("snapshot"));
          return;
        }
        if (request.method === "GET" && url.pathname === "/actions") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("actions"));
          return;
        }
        if (request.method === "GET" && url.pathname === "/context") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("context"));
          return;
        }
        if (request.method === "POST" && url.pathname === "/query") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("query", await readJsonRequestBody(request)));
          return;
        }
        if (request.method === "POST" && url.pathname === "/command") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("command", await readJsonRequestBody(request)));
          return;
        }
        if (request.method === "POST" && url.pathname === "/execute") {
          sendJsonResponse(response, 200, await runSofiaControlCommand("execute", await readJsonRequestBody(request)));
          return;
        }
    ...
    if (!port) throw new Error("Could not start Sofia App UI control bridge.");
    uiControlDiscoveryPath = path.join(app.getPath("userData"), "sofia-ui-control.json");
    await writeFile(
    ...
    // Make the discovery path available to child processes (server → managed Sofia engine → plugin).
    process.env.SOFIA_UI_CONTROL_DISCOVERY = uiControlDiscoveryPath;
  }
