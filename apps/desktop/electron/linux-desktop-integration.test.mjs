...
async function createHarness(options = {}) {
  const root = await mkdtemp(path.join(os.tmpdir(), "sofia-appimage-integration-"));
...
function relaunchAt(harness, appImagePath, options = {}) {
  let defaultHandler = options.defaultHandler ?? SOFIA_DESKTOP_ID;
  const dialogs = [];
  const integration = createLinuxDesktopIntegration({
...
describe("Linux AppImage desktop integration", () => {
  it("quotes AppImage paths for Desktop Entry Exec fields", () => {
    ...
    assert.match(entry, /^MimeType=x-scheme-handler\/sofia;$/m);
    assert.match(entry, /^StartupWMClass=com\.differentai\.sofia$/m);
  });
  ...
  it("installs its launcher, icons, and sofia handler", async () => {
    const harness = await createHarness();
    const result = await harness.integration.install();
    assert.equal(result.ok, true);
    assert.equal(result.status.state, "integrated");
    assert.equal(harness.getDefaultHandler(), SOFIA_DESKTOP_ID);
    ...
    assert.match(desktopEntry, /^X-Sofia-Managed=true$/m);
  ...
  it("detects a moved AppImage and repairs the launcher for the new path", async () => {
    ...
    const moved = createLinuxDesktopIntegration({
      ...
      runCommand: async (command, args) => {
        if (command === "xdg-mime" && args[0] === "query") {
          return { ok: true, stdout: `${SOFIA_DESKTOP_ID}\n`, stderr: "" };
        }
        return { ok: true, stdout: "", stderr: "" };
      },
    });
  ...
  it("silently repairs an owned launcher after a self-update lands a new filename", async () => {
    ...
    const updatedPath = path.join(harness.root, "sofia-linux-x86_64-0.18.8.AppImage");
    ...
    assert.match(entry, /^X-Sofia-Version=0\.18\.8$/m);
  });
  ...
  it("keeps a failed silent repair quiet and leaves it to Settings", async () => {
    ...
    const movedPath = path.join(harness.root, "sofia-linux-x86_64-0.18.8.AppImage");
  ...
  it("recognizes an external manager and does not duplicate its integration", async () => {
    const managerDesktopId = "it.mijorus.gearlever.sofia.desktop";
...
`);
  ...
  it("registers an externally managed canonical launcher without rewriting it", async () => {
    const harness = await createHarness({ defaultHandler: SOFIA_DESKTOP_ID });
...
`;
  ...
  it("registers an existing manager launcher without creating a duplicate", async () => {
    const managerDesktopId = "appimagelauncher-sofia.desktop";
