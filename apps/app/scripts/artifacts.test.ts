...
describe("getArtifactsFromMessages", () => {
  it("includes verified slide deck targets mentioned in assistant summaries", () => {
    const messages: UIMessage[] = [{
      id: "msg_deck",
      role: "assistant",
      parts: [{ type: "text", text: "Updated file: decks/sofia-vertebrae-deck.pptx", state: "done" }],
    }];
    const targets: OpenTarget[] = [{
      id: "file:decks/sofia-vertebrae-deck.pptx",
      kind: "file",
      value: "decks/sofia-vertebrae-deck.pptx",
      name: "sofia-vertebrae-deck.pptx",
      preview: "slides",
      confidence: 65,
      reason: "message",
      exists: true,
    ...
    expect(getArtifactsFromMessages(messages, targets)[0]).toMatchObject({
      name: "sofia-vertebrae-deck.pptx",
      path: "decks/sofia-vertebrae-deck.pptx",
      type: "slides",
