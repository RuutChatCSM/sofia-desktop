...
function client(overrides: Partial<SofiaServerClient> = {}): SofiaServerClient {
  return {
...
describe("testRemoteWorkspaceConnection", () => {
  ...
  test("reports a missing token after proving the worker endpoint is reachable", async () => {
    const result = await testRemoteWorkspaceConnection(workspace({ sofiaToken: "" }), {
      createClient: () => client(),
    });
  ...
  test("uses fallback Sofia App tokens saved on older workspace records", async () => {
    const result = await testRemoteWorkspaceConnection(
      workspace({
        sofiaToken: "",
        sofiaClientToken: "legacy-client-token",
      }),
      {
        createClient: (target) => {
...
describe("remote diagnostic identity", () => {
  ...
  test("changes when connection credentials change", () => {
    const before = getRemoteWorkspaceConnectionKey(workspace({ sofiaToken: "old-token" }));
    const after = getRemoteWorkspaceConnectionKey(workspace({ sofiaToken: "new-token" }));
