...
async function main() {
  ...
  try {
    await waitFor(client, "Boolean(window.__sofiaControl)", 15000);
...
async function runPreflight(client) {
  ...
  const controlReady = await evaluate(client, "Boolean(window.__sofiaControl)");
  const actions = controlReady
    ? await evaluate(client, "window.__sofiaControl.listActions().map((action) => action.id)")
    : [];
  ...
  if (!controlReady) throw new Error("Sofia control API is not available.");
...
async function ensureVoicePanel(client) {
  await evaluate(client, "window.__sofiaControl.setEnabled(true)");
  await evaluate(client, "window.localStorage.setItem('sofia.extension.enabled.sofia-voice', '1'); window.dispatchEvent(new CustomEvent('sofia:extension-state-changed', { detail: { id: 'sofia-voice', enabled: true } }))");
  let actions = await evaluate(client, "window.__sofiaControl.listActions().map((action) => action.id)");
  ...
  if (actions.includes("voice.panel.open")) {
    ...
    await waitFor(client, "window.__sofiaControl.listActions().some((action) => action.id === 'voice.inject_audio')", 8000);
    return;
  }
...
async function readProof(client) {
  return evaluate(client, `({
    href: location.href,
    narration: window.__sofiaControl?.snapshot?.().narration ?? "",
...
async function executeControl(client, actionId, actionArgs = undefined) {
  const expression = `window.__sofiaControl.execute(${JSON.stringify(actionId)}, ${JSON.stringify(actionArgs)})`;
...
async function synthesizePcm16Base64(input) {
  ...
  const dir = await mkdtemp(join(tmpdir(), "sofia-voice-cdp-"));
...
async function pickTarget(baseUrl) {
  ...
  const target = pages.find((page) => page.title === "Sofia") ??
