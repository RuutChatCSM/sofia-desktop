...
import type { SofiaCloudMcpHealth } from "../src/app/lib/sofia-server";
import {
...
function baseHealth(overrides?: Partial<SofiaCloudMcpHealth>): SofiaCloudMcpHealth {
  return {
    schemaVersion: 1,
    phase: "engine_failed",
    usable: false,
    ...
    engineInspection: {
      ...
      serverCount: 2,
      servers: [
        { name: "sofia-cloud", status: "failed", error: "fetch failed" },
    ...
    tools: {
      ...
      present: [],
      missing: ["sofia-cloud_search_capabilities", "sofia-cloud_execute_capability"],
      direct: {
        checked: false,
...
describe("cloud MCP advanced diagnostics", () => {
  test("surfaces the raw engine error string that Sofia engine collapses to", () => {
  ...
  test("shows the probe transport cause from the direct check", () => {
    const rows = cloudMcpAdvancedRows(baseHealth({
      tools: {
        ...
        direct: {
          ...
          trace: {
            endpoint: "https://api.sofia.test/mcp/agent",
    ...
    expect(rows.find((row) => row.label === "Endpoint")?.value).toBe("https://api.sofia.test/mcp/agent");
  });
  ...
  test("formats probe trace and engine refresh step lines", () => {
    expect(cloudMcpProbeTraceLines({
      ...
      serverInfo: { name: "sofia-cloud", version: "1.0.0" },
      steps: [
  ...
  test("support bundle is JSON, carries context, and redacts bearer material", () => {
    ...
    expect(parsed.kind).toBe("sofia-cloud-mcp-diagnostic");
