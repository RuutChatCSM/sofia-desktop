...
const PROVIDER_MODEL = { provider: "sofia", model: "gpt-5" };

function failure(input?: Partial<SofiaCloudMcpFailure>): SofiaCloudMcpFailure {
  return {
...
function health(input?: {
  ...
  const projected = usable
    ? ["sofia-cloud_search_capabilities", "sofia-cloud_execute_capability"]
    : [];
  ...
  return {
    ...
    tools: {
      ...
      providerProjection: {
        ...
        missing: projectionPresent.length ? [] : ["sofia-cloud_search_capabilities", "sofia-cloud_execute_capability"],
      },
    },
