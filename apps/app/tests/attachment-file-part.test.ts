...
describe("composer attachment file parts", () => {
  ...
  test("generates session-scoped inbox paths under the workspace inbox", () => {
    ...
    expect(workspaceInboxPath(inboxPath)).toBe(".opencode/sofia/inbox/chat-attachments/ses_123/nonce-abc-scan one 李.pdf");
  });
  ...
  test("uploads exact bytes to the endpoint workspace id and exposes a worker file URL plus path note", async () => {
    ...
    expect(textPartText(parts)).toContain(".opencode/sofia/inbox/chat-attachments/ses_abc/nonce-a-image-only scan.pdf");
    expect(textPartText(parts)).toContain("Read/Bash/MCP/Docling");
    expect(filePartUrl(parts, 1)).toBe("file:///workspaces/Worker%20Root/.opencode/sofia/inbox/chat-attachments/ses_abc/nonce-a-image-only%20scan.pdf");
  ...
  test("workspace image attachments use displayable data URLs while keeping a synthetic path note for tools", async () => {
    ...
    expect(textPartText(parts)).toContain(".opencode/sofia/inbox/chat-attachments/ses_img/nonce-img-shot.png");
    expect(textPartText(parts)).toContain("file:///workspaces/Worker%20Root/.opencode/sofia/inbox/chat-attachments/ses_img/nonce-img-shot.png");
  ...
  test("workspace text-like attachments become text/plain file parts pointing at the workspace copy", async () => {
    ...
    expect(filePartUrl(parts, 1)).toBe("file:///workspaces/Worker%20Root/.opencode/sofia/inbox/chat-attachments/ses_xml/nonce-xml-sitemap.xml");
    expect(parts[1]).toMatchObject({
      type: "file",
  ...
  test("workspace binary attachments upload for tool access and emit a Read-mediated text/plain file part", async () => {
    ...
    expect(textPart(parts)).toMatchObject({ type: "text", synthetic: true });
    expect(textPartText(parts)).toContain(".opencode/sofia/inbox/chat-attachments/ses_bin/nonce-bin-recording.zip");
    expect(textPartText(parts)).toContain("Read/Bash/MCP/Docling");
    ...
    expect(filePartUrl(parts, 1)).toBe("file:///workspaces/Worker%20Root/.opencode/sofia/inbox/chat-attachments/ses_bin/nonce-bin-recording.zip");
    expect(parts[1]).toMatchObject({
      type: "file",
      filename: "recording.zip",
      mime: "text/plain",
  ...
  test("uploads duplicate filenames to distinct non-overwriting paths", async () => {
    ...
    expect(filePartUrl(parts, 1)).toBe("file:///C:/Users/Ada%20Lovelace/%E5%B7%A5%E4%BD%9C%E5%8C%BA/.opencode/sofia/inbox/chat-attachments/ses_dupes/nonce-a-scan.pdf");
    expect(filePartUrl(parts, 2)).toBe("file:///C:/Users/Ada%20Lovelace/%E5%B7%A5%E4%BD%9C%E5%8C%BA/.opencode/sofia/inbox/chat-attachments/ses_dupes/nonce-b-scan.pdf");
  });
