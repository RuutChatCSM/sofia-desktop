...
describe("chat tool error attribution", () => {
  ...
  test("extracts a trusted reconnect action from a Cloud capability failure", () => {
    ...
    expect(reconnectActionFromChatToolResult("sofia-cloud_execute_capability", errorText)).toEqual({
  ...
  test("extracts the same reconnect action when live capability discovery detects expired credentials", () => {
    ...
    expect(reconnectActionFromChatToolResult("sofia-cloud_search_capabilities", output)).toEqual({
      connectionId: "emc_knowledge",
      connectionName: "Knowledge Hub",
      label: "Reconnect",
    })
  ...
  test("does not create actions from arbitrary MCP tools or non-reconnect failures", () => {
    ...
    expect(reconnectActionFromChatToolResult("sofia-cloud_execute_capability", providerPayload)).toBeNull()
  })
  ...
  test("does not guess between multiple reconnect targets in one discovery result", () => {
    ...
    expect(reconnectActionFromChatToolResult("sofia-cloud_search_capabilities", output)).toBeNull()
  })
  ...
  test("rejects unversioned, shared, and admin-owned action shapes", () => {
    ...
    expect(reconnectActionFromChatToolResult("sofia-cloud_execute_capability", { connectionStatus: unversioned })).toBeNull()
    expect(reconnectActionFromChatToolResult("sofia-cloud_execute_capability", { connectionStatus: shared })).toBeNull()
  })
})
