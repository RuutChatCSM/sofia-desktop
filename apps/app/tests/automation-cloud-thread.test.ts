...
import type { AutomationExecutionThread } from "@sofia/types/automations"
import {
...
function cloudThread(): AutomationExecutionThread {
  return { ...desktopThread("sofia-cloud-agent-v1"), executionLocation: "cloud" }
}

describe("Automation execution thread UI", () => {
  test("uses Den's persisted thread identity for receipt navigation", () => {
    expect(automationExecutionThreadRoute(desktopThread("sofia-desktop-runner-v1"))).toBe(
  ...
  test("labels a Web-created run as Organization cloud on Desktop", () => {
  ...
  test("opens a linked Desktop run in its native local session", () => {
    expect(automationLocalSessionRoute({
      ...desktopThread("sofia-desktop-runner-v1"),
      workspaceId: "ws with spaces",
      nativeThreadId: "ses/failed",
    })).toBe("/workspace/ws%20with%20spaces/session/ses%2Ffailed")
    expect(automationLocalSessionRoute(desktopThread("sofia-desktop-runner-v1"))).toBeNull()
    expect(automationLocalSessionRoute({
      ...cloudThread(),
