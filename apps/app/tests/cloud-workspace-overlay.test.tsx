...
function instance(input: Partial<DenCloudInstance> = {}): DenCloudInstance {
  return {
    ...
    imageVersion: "imageVersion" in input ? input.imageVersion ?? null : "sofia-0.18.8",
    ...(typeof input.instanceName === "string" ? { instanceName: input.instanceName } : {}),
    latestVersion: "latestVersion" in input ? input.latestVersion ?? null : "sofia-0.18.8",
  };
}
...
describe("cloud workspace overlay state", () => {
  ...
  test("maps stale and legacy workers to Update available", () => {
    const stale = mapCloudWorkspaceState({
      instance: instance({ imageVersion: "sofia-0.18.2", latestVersion: "sofia-0.18.8" }),
      updating: false,
    });
    const legacyInstance = instance({ imageVersion: null, latestVersion: "sofia-0.18.8" });
