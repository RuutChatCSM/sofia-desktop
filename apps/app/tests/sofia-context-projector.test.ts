...
import {
  buildSofiaContext,
  screenFromRoute,
} from "../src/react-app/shell/sofia-context-projector";
import type { WorkbenchSnapshot } from "../src/react-app/domains/session/chat/workbench-store";

type ContextProjectorInput = Parameters<typeof buildSofiaContext>[0];
...
describe("Sofia App context projector", () => {
  ...
  test("does not leak artifact tabs into a non-panel surface", () => {
    const context = buildSofiaContext({
      ...baseInput,
      ui: {
        ...baseInput.ui,
