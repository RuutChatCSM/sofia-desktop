...
describe("cloud provider runtime patch (re-import diff #2346)", () => {
  ...
  test("cloud-managed key predicate guards re-import vs manual clobber", () => {
    ...
    expect(isCloudManagedProviderKey("sofia")).toBe(true);
  ...
  test("provider baseline persistence does not refresh the desktop cloud snapshot", () => {
    ...
    expect(persistSource).toContain("const config = await readWorkspaceSofiaConfigRecord();");
    ...
    expect(persistSource).toContain("const persisted = await writeWorkspaceSofiaConfigRecord(nextConfig);");
  ...
  test("client env mirror includes non-sofia provider credentials", () => {
    const source = readFileSync(providerAuthStoreSourcePath, "utf8");
    const mirrorStart = source.indexOf("const mirrorSofiaModelsVoiceEnv = async");
    const mirrorEnd = source.indexOf("const readWorkspaceSofiaConfigRecord", mirrorStart);
    ...
    expect(mirrorSource).not.toContain('provider.source !== "sofia"');
