...
describe("markdown code blocks", () => {
  test("renders fallback code blocks with subtle theme-aware styling and copy affordance", () => {
    ...
    expect(html).toContain("data-sofia-code-block");
    expect(html).toContain("bg-gray-2/60");
    expect(html).toContain("data-sofia-code-copy");
    expect(html).toContain("data-sofia-code-copy-icon");
    expect(html).toContain("data-sofia-code-copy-check-icon");
  ...
  test("renders highlighted code blocks with the same copy affordance and dual Shiki themes", async () => {
    ...
    expect(html).toContain("data-sofia-code-block");
    expect(html).toContain("data-sofia-shiki");
    expect(html).toContain("data-sofia-code-copy");
    expect(html).toContain("data-sofia-code-copy-icon");
    expect(html).toContain("data-sofia-code-copy-check-icon");
...
describe("markdown safety and links", () => {
  ...
  test("keeps chat file link actions separate from simple surface links", () => {
    ...
    expect(chatHtml).toContain('href="https://sofia.ruut.chat"');

    const surfaceHtml = renderPrimitiveMarkdownHtml(markdown, "surface");
    expect(surfaceHtml).not.toContain("data-sofia-link-chevron");
    expect(surfaceHtml).not.toContain("data-sofia-link-href");
    expect(surfaceHtml).toContain('href="./docs/readme.md"');
    expect(surfaceHtml).toContain('href="https://sofia.ruut.chat"');
  });
});
