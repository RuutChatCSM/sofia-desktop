...
addHost(process.env.SOFIA_PUBLIC_HOST ?? null);
...
const isElectronPackagedBuild = process.env.SOFIA_ELECTRON_BUILD === "1";
...
const headlessDenTarget = (process.env.SOFIA_DEV_HEADLESS_DEN_TARGET ?? "").trim();
...
export default defineConfig({
  ...
  define: {
    ...
    "import.meta.env.VITE_SOFIA_APP_VERSION": JSON.stringify(buildAppVersion),
    "import.meta.env.VITE_SOFIA_BUILD_SHA": JSON.stringify(shortBuildSha),
  },
  plugins: [
    {
      name: "sofia-dev-server-id",
      configureServer(server) {
        server.middlewares.use("/__sofia_dev_server_id", (_req, res) => {
