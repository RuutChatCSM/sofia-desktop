...
const OWNER_LABEL_KEYS: Record<AgentContextDiagnosticOwner, string> = {
  ...
  "sofia-support": "connect.diagnostics_owner_sofia_support",
};
...
const AGENT_STATE_LABEL_KEYS: Record<AgentContextDiagnosticsReport["agent"]["configuredSofiaAgent"]["state"], string> = {
...
function AgentEvidence(props: {
  ...
  const agent = props.report.agent.configuredSofiaAgent;
  return (
...
function CloudCatalog(props: {
  ...
  const cloudMcp = props.report.mcps.find(
    (mcp) => mcp.source === "config.remote" && mcp.name === "sofia-cloud" && mcp.path === "/mcp/agent",
  ) ?? props.report.mcps.find(
    (mcp) => mcp.name === "sofia-cloud" && mcp.path === "/mcp/agent",
  );
...
export function AgentContextDiagnosticsReportView(props: {
  ...
  const connectStateSummary = resolveSofiaConnectStateSummary(
