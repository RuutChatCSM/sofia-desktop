...
const EXTENSION_LAYOUT_KEY = "sofia.extensions.layout";
const EXTENSION_DISABLED_KEY_PREFIX = "sofia.extension.disabled.";
const EXTENSION_ENABLED_KEY_PREFIX = "sofia.extension.enabled.";
const EXTENSION_HIDDEN_KEY_PREFIX = "sofia.extension.hidden.";
export const SOFIA_EXTENSION_STATE_CHANGED = "sofia:extension-state-changed";
...
export function isSofiaExtensionEnabled(entry: McpDirectoryInfo): boolean {
...
export function setSofiaExtensionEnabled(entry: McpDirectoryInfo, enabled: boolean) {
  if (typeof window === "undefined") return;
  const id = getExtensionId(entry);
  ...
  window.dispatchEvent(new CustomEvent(SOFIA_EXTENSION_STATE_CHANGED, {
    detail: { id, enabled },
  }));
}

export function isSofiaExtensionHidden(entryOrId: McpDirectoryInfo | string): boolean {
...
export function setSofiaExtensionHidden(entryOrId: McpDirectoryInfo | string, hidden: boolean) {
  const id = typeof entryOrId === "string" ? entryOrId : getExtensionId(entryOrId);
  if (typeof window === "undefined") return;
