...
function hasDesktopBridge() {
  return typeof window !== "undefined" && Boolean(window.__SOFIA_ELECTRON__?.invokeDesktop);
}
...
export function ComputerUseConfig({
  ...
  const {
    ...
    mutationFn: async () => {
      if (!hasDesktopBridge()) {
        throw new Error("Computer Use is Mac only and requires the Sofia App desktop app on macOS.");
      }
    ...
    <Card variant="outline" size="sm">
      ...
      <CardContent className="space-y-4">
        ...
        <SetupRow
          ...
          description="Opens the Sofia App Computer Use helper. Grant both permissions there, then click Verify below."
          complete={allGranted}
        >
