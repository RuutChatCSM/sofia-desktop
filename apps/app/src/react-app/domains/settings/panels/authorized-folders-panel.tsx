...
export function AuthorizedFoldersPanel(props: AuthorizedFoldersPanelProps) {
  ...
  const sofiaServerReady = props.sofiaServerStatus === "connected";
  const sofiaServerWorkspaceReady = Boolean(props.runtimeWorkspaceId);
  const canReadConfig =
    sofiaServerReady &&
    sofiaServerWorkspaceReady &&
    (props.sofiaServerCapabilities?.config?.read ?? false);
  const canWriteConfig =
    sofiaServerReady &&
    sofiaServerWorkspaceReady &&
    (props.sofiaServerCapabilities?.config?.write ?? false);

  const authorizedFoldersHint = useMemo(() => {
    if (!sofiaServerReady) return t("context_panel.server_disconnected");
    if (!sofiaServerWorkspaceReady) return t("context_panel.no_server_workspace");
    ...
    return null;
  }, [canReadConfig, canWriteConfig, sofiaServerReady, sofiaServerWorkspaceReady]);
  ...
  useEffect(() => {
    const sofiaClient = props.sofiaServerClient;
    const sofiaWorkspaceId = props.runtimeWorkspaceId;

    if (!sofiaClient || !sofiaWorkspaceId || !canReadConfig) {
    ...
    void (async () => {
      try {
        const response = await sofiaClient.listAuthorizedFolders(sofiaWorkspaceId);
        if (cancelled) return;
  ...
  }, [canReadConfig, props.sofiaServerClient, props.runtimeWorkspaceId]);

  const persistAuthorizedFolders = useCallback(async (nextFolders: string[]) => {
    const sofiaClient = props.sofiaServerClient;
    const sofiaWorkspaceId = props.runtimeWorkspaceId;
    if (!sofiaClient || !sofiaWorkspaceId || !canWriteConfig) {
    ...
    try {
      const response = await sofiaClient.setAuthorizedFolders(sofiaWorkspaceId, nextFolders);
  ...
  }, [canWriteConfig, props.onConfigUpdated, props.sofiaServerClient, props.runtimeWorkspaceId]);
