...
import type { SofiaServerClient, SofiaSessionMessage } from "@/app/lib/sofia-server";
...
const DEFAULT_TEXT_COMMAND = "Summarize the current Sofia App session and put the next step in the composer.";
...
const TOOL_LABELS: Record<string, string> = {
  sofia_snapshot: "Checking Sofia App",
  sofia_list_actions: "Listing controls",
  sofia_execute_action: "Running UI action",
};
...
function humanToolLabel(toolName?: string) {
  if (!toolName) return "Sofia App action";
...
function messageText(message: SofiaSessionMessage) {
...
function buildVoiceSessionContext(messages: SofiaSessionMessage[]) {
...
async function loadVoiceSessionContext(client: SofiaServerClient, workspaceId: string | null, sessionId: string | null) {
...
async function requestMacMicrophoneAccess() {
  const ask = window.__SOFIA_ELECTRON__?.system?.askMicrophoneAccess;
  if (!ask) return true;
...
async function executeSofiaTool(name: string, args: Record<string, unknown>) {
  const control = window.__sofiaControl;
  if (!control) return { ok: false, error: "Sofia App control surface is not available." };

  if (name === "sofia_snapshot") return { ok: true, snapshot: control.snapshot() };
  if (name === "sofia_list_actions") return { ok: true, actions: control.listActions() };
  if (name === "sofia_execute_action") {
  ...
  return { ok: false, error: `Unknown Sofia App voice tool: ${name}` };
}
...
export function VoicePanel(props: VoicePanelProps) {
    ...
    setVoiceRuntimeSnapshot((current) => ({
      ...
      statusText: text ?? (
        nextStatus === "connecting" ? "Connecting to OpenAI Realtime..." :
          nextStatus === "listening" ? "Listening. Ask Sofia App to act." :
            nextStatus === "speaking" ? "Sofia App is speaking..." :
  ...
  const handleRealtimeMessage = useCallback(async (raw: string) => {
    ...
    if (type === "response.function_call_arguments.done") {
      ...
      const output = await executeSofiaTool(toolName, args);
  ...
  const connectRealtime = useCallback(async (audioInput = true) => {
    ...
    if (!client) throw new Error("Sofia App host connection is not ready.");
    ...
    if (audioInput) {
      ...
      if (!macPermissionGranted) throw new Error("macOS denied microphone access. Enable Sofia App in System Settings > Privacy & Security > Microphone, then restart Sofia App.");
    ...
    addEntry("system", `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} Sofia App tools.`);
  ...
  const injectTranscript = useCallback(async (args: unknown) => {
    ...
    addEntry("user", text);
    window.dispatchEvent(new CustomEvent("sofia:voice-transcript", { detail: { text } }));
  ...
  const startAction = useMemo<SofiaControlAction>(() => ({
    id: "voice.start",
  ...
  const stopAction = useMemo<SofiaControlAction>(() => ({
    id: "voice.stop",
  ...
  const muteAction = useMemo<SofiaControlAction>(() => ({
  ...
  const injectTranscriptAction = useMemo<SofiaControlAction>(() => ({
  ...
  const sendTextAction = useMemo<SofiaControlAction>(() => ({
    id: "voice.send_text",
  ...
  const injectAudioAction = useMemo<SofiaControlAction>(() => ({
  ...
  const statusAction = useMemo<SofiaControlAction>(() => ({
    id: "voice.status",
    ...
    <div ref={panelRef} className="flex h-full min-h-0 flex-col bg-dls-canvas">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-muted-foreground">Realtime voice over Sofia App UI MCP controls</div>
        </div>
          ...
          {!props.client ? (
            <Card variant="outline" size="sm">
              ...
              <CardContent className="text-sm text-muted-foreground">
                Voice Mode needs the local Sofia App server so it can mint short-lived Realtime client secrets without exposing your API key to the renderer.
              </CardContent>
            </Card>
          ) : null}
