...
const DEFAULT_TEXT_COMMAND = "Summarize the current Sofia session and put the next step in the composer.";
...
function humanToolLabel(toolName?: string) {
  if (!toolName) return "Sofia action";
...
async function executeSofiaTool(name: string, args: Record<string, unknown>) {
  ...
  if (!control) return { ok: false, error: "Sofia control surface is not available." };
  ...
  return { ok: false, error: `Unknown Sofia voice tool: ${name}` };
}
...
export function VoicePanel(props: VoicePanelProps) {
    ...
    setVoiceRuntimeSnapshot((current) => ({
      ...
      statusText: text ?? (
        nextStatus === "connecting" ? "Connecting to OpenAI Realtime..." :
          nextStatus === "listening" ? "Listening. Ask Sofia to act." :
            nextStatus === "speaking" ? "Sofia is speaking..." :
  ...
  const connectRealtime = useCallback(async (audioInput = true) => {
    ...
    if (!client) throw new Error("Sofia host connection is not ready.");
    ...
    if (audioInput) {
      ...
      if (!macPermissionGranted) throw new Error("macOS denied microphone access. Enable Sofia in System Settings > Privacy & Security > Microphone, then restart Sofia.");
    ...
    addEntry("system", `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} Sofia tools.`);
    ...
    <div ref={panelRef} className="flex h-full min-h-0 flex-col bg-dls-canvas">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-muted-foreground">Realtime voice over Sofia UI MCP controls</div>
        </div>
          ...
          {!props.client ? (
            <Card variant="outline" size="sm">
              ...
              <CardContent className="text-sm text-muted-foreground">
                Voice Mode needs the local Sofia server so it can mint short-lived Realtime client secrets without exposing your API key to the renderer.
              </CardContent>
            </Card>
          ) : null}
