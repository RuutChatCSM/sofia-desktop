...
import { isSofiaExtensionEnabled, isSofiaExtensionHidden } from "@/react-app/domains/settings/extension-state";
...
export function isAppMentionAvailable(): boolean {
  if (typeof window === "undefined" || !window.__SOFIA_ELECTRON__?.invokeDesktop) return false;
  if (!isMacPlatform()) return false;
  const entry = SOFIA_EXTENSION_CATALOG.find((candidate) => candidate.id === "computer-use");
  if (!entry) return false;
  return isSofiaExtensionEnabled(entry) && !isSofiaExtensionHidden(entry);
}
