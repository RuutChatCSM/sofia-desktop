...
export type SofiaConnectStateStatus = "available" | "missing" | "invalid" | "unreadable";

export type SofiaConnectStateSummary = {
...
export function resolveSofiaConnectStateSummary(
  status: SofiaConnectStateStatus,
  connectEnabled: boolean,
): SofiaConnectStateSummary {
  if (status === "missing") {
    return {
...
export type SofiaConnectStatus = {
  state: "checking" | "ready" | "needs_attention";
  label: "Checking" | "Ready" | "Needs attention";
  description: string;
};

export function sofiaConnectAttentionTitle(description: string): string {
  return `One possible issue: ${description}`;
}

export function resolveSofiaConnectStatus(
  signedIn: boolean,
  maintenance: SessionCloudMcpMaintenanceState | undefined,
): SofiaConnectStatus | null {
  ...
  if (maintenance.status === "failed" || maintenance.status === "skipped") {
    return {
      ...
      description: maintenance.issue?.message
        ?? "Connections could not verify connected service tools. Run diagnostics for details.",
    };
  }
