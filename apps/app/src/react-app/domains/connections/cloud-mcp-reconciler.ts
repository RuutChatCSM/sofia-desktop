...
type CloudMcpReconcilerInput = {
  ...
  /**
   * Ask the Sofia server to also verify the Cloud endpoint directly
...
export function cloudMcpFailureStageLabel(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Sofia components need updating";
...
export function cloudMcpRecommendedAction(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Update Sofia, then retry.";
  if (code === "extensions_plugin_missing") return "Reload the agent so Sofia instructions are current.";
