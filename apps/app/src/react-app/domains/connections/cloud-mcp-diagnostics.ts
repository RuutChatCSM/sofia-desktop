 ...
 */
export function cloudMcpEngineErrorText(health: SofiaCloudMcpHealth | null): string | null {
...
export function cloudMcpProbeTraceLines(trace: SofiaCloudMcpProbeTrace | null | undefined): string[] {
  if (!trace) return [];
...
export function cloudMcpEngineRefreshLines(refresh: SofiaCloudMcpEngineRefresh | null | undefined): string[] {
  if (!refresh) return [];
 ...
 */
export function cloudMcpAdvancedRows(health: SofiaCloudMcpHealth | null): CloudMcpAdvancedRow[] {
  if (!health) return [];
  ...
  if (inspection?.checked) {
    if (inspection.cloudPresent === false) {
      rows.push({
        ...
        value: "sofia-cloud is not registered in the engine (the dynamic entry is lost after an engine restart) — use Refresh engine connection",
        tone: "error",
      });
    }
...
export function buildCloudMcpSupportBundle(input: {
  ...
  const bundle = {
    kind: "sofia-cloud-mcp-diagnostic",
