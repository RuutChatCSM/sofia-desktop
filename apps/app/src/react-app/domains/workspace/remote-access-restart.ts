...
import { sofiaServerRestart, type SofiaServerInfo } from "../../../app/lib/desktop";
import {
...
type UseRemoteAccessRestartOptions = {
  isEnabled: () => boolean;
  onHostInfo: (info: SofiaServerInfo) => void;
...
export function useRemoteAccessRestart(options: UseRemoteAccessRestartOptions) {
  ...
  const save = useCallback(
    async (enabled: boolean) => {
      ...
      const previous = readSofiaServerSettings();
      ...
      try {
        const info = await sofiaServerRestart({ remoteAccessEnabled: enabled }) as SofiaServerInfo;
        writeSofiaServerSettings({
        ...
        writeSofiaServerSettings(previous);
        options.onSettingsChanged();
