...
function remoteSupportMessage(message: string) {
  return `${message} Upgrade the Sofia host and try again. If this continues, contact team@ruut.chat.`;
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  ...
  if (workspace.remoteType && workspace.remoteType !== "sofia") {
    return {
      ...
      state: {
        status: "error",
        message: "Connection diagnostics are only available for Sofia remote workers.",
        checkedAt: Date.now(),
      },
    };
  }
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid Sofia token.`),
      checkedAt,
    );
  }
