...
type TestOptions = {
  ...
  > | Promise<Pick<SofiaServerClient, "health" | "capabilities" | "status" | "listWorkspaces">>;
};
...
function stripSofiaWorkspaceMount(baseUrl: string) {
  try {
    const url = new URL(baseUrl);
...
function remoteSupportMessage(message: string) {
  return `${message} Upgrade the Sofia App host and try again. If this continues, contact team@ruut.chat.`;
}
...
export function getRemoteWorkspaceConnectionKey(workspace: WorkspaceInfo): string {
  return [
    ...
    trim(workspace.sofiaHostUrl),
    trim(workspace.sofiaWorkspaceId),
    trim(workspace.sofiaToken),
    trim(workspace.sofiaClientToken),
    trim(workspace.sofiaHostToken),
  ].join("\u001f");
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  ...
  if (workspace.remoteType && workspace.remoteType !== "sofia") {
    return {
      ok: false,
      state: {
        status: "error",
        message: "Connection diagnostics are only available for Sofia App remote workers.",
        checkedAt: Date.now(),
      },
    };
  }

  const rawHostUrl = trim(workspace.sofiaHostUrl) || trim(workspace.baseUrl);
  if (!rawHostUrl) {
    return {
      ok: false,
      state: {
  ...
  const normalizedHostUrl = normalizeSofiaServerUrl(rawHostUrl);
  ...
  const workspaceId =
    trim(workspace.sofiaWorkspaceId) ||
    parseSofiaWorkspaceIdFromUrl(normalizedHostUrl) ||
    parseSofiaWorkspaceIdFromUrl(trim(workspace.baseUrl)) ||
    null;
  const hostBaseUrl = stripSofiaWorkspaceMount(normalizedHostUrl);
  const token =
    trim(workspace.sofiaToken) ||
    trim(workspace.sofiaClientToken) ||
    trim(workspace.sofiaHostToken);
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid Sofia App token.`),
      checkedAt,
    );
  }
