...
import type { SofiaServerSettings } from "../../../app/lib/sofia-server";
import { t } from "../../../i18n";
...
function shareWorkspaceLocalReducer(
  ...
  switch (action.type) {
    ...
    case "localSofiaWorkspace":
      return { ...state, shareLocalSofiaWorkspaceId: action.workspaceId };
  }
}

export function useShareWorkspaceState(options: UseShareWorkspaceStateOptions) {
  const [{ shareWorkspaceId, shareLocalSofiaWorkspaceId }, dispatchShareWorkspace] = useReducer(
  ...
  const shareWorkspaceDetail = useMemo(() => {
    ...
    if (workspace.workspaceType === "remote") {
      if (workspace.remoteType === "sofia") {
        const hostUrl = workspace.sofiaHostUrl?.trim() || workspace.baseUrl?.trim() || "";
        const mounted = buildSofiaWorkspaceBaseUrl(
          hostUrl,
          workspace.sofiaWorkspaceId,
        );
  ...
  useEffect(() => {
    const workspace = shareWorkspace;
    const baseUrl = options.sofiaServerHostInfo?.baseUrl?.trim() ?? "";
    const token =
      options.sofiaServerHostInfo?.ownerToken?.trim() ||
      options.sofiaServerHostInfo?.clientToken?.trim() ||
      "";
    ...
    if (
      ...
      dispatchShareWorkspace({ type: "localSofiaWorkspace", workspaceId: null });
      return;
    }
    ...
    void (async () => {
      try {
        const client = createSofiaServerClient({ baseUrl, token });
        ...
        dispatchShareWorkspace({ type: "localSofiaWorkspace", workspaceId: match?.id ?? null });
      } catch {
        if (!cancelled) {
          dispatchShareWorkspace({ type: "localSofiaWorkspace", workspaceId: null });
        }
  ...
  }, [options.sofiaServerHostInfo, shareWorkspace]);
  ...
  const shareFields = useMemo(() => {
    ...
    if (workspace.workspaceType !== "remote") {
      if (options.sofiaServerHostInfo?.remoteAccessEnabled !== true) {
        return [];
      }
      const hostUrl =
        options.sofiaServerHostInfo?.connectUrl?.trim() ||
        options.sofiaServerHostInfo?.lanUrl?.trim() ||
        options.sofiaServerHostInfo?.mdnsUrl?.trim() ||
        options.sofiaServerHostInfo?.baseUrl?.trim() ||
        "";
      const mountedUrl = shareLocalSofiaWorkspaceId
        ? buildSofiaWorkspaceBaseUrl(hostUrl, shareLocalSofiaWorkspaceId)
        : null;
      const url = mountedUrl || hostUrl;
      const collaboratorToken = options.sofiaServerHostInfo?.clientToken?.trim() || "";
      const ownerToken =
        collaboratorToken || options.sofiaServerHostInfo?.ownerToken?.trim() || "";
      return [
        {
    ...
    if (workspace.remoteType === "sofia") {
      const hostUrl = workspace.sofiaHostUrl?.trim() || workspace.baseUrl?.trim() || "";
      const url =
        buildSofiaWorkspaceBaseUrl(hostUrl, workspace.sofiaWorkspaceId) ||
        hostUrl;
      const token =
        workspace.sofiaToken?.trim() ||
        options.sofiaServerSettings.token?.trim() ||
        "";
      return [
        {
          label: t("session.share_worker_url"),
    ...
    options.sofiaServerHostInfo,
    options.sofiaServerSettings,
    shareLocalSofiaWorkspaceId,
    shareWorkspace,
  ]);
  ...
  const shareServiceDisabledReason = useMemo(() => {
    ...
    if (workspace.workspaceType === "remote" && workspace.remoteType !== "sofia") {
      return t("session.share_sofia_workers_only");
    }
    if (workspace.workspaceType !== "remote") {
      const baseUrl = options.sofiaServerHostInfo?.baseUrl?.trim() ?? "";
      const token =
    ...
    return null;
  }, [options.sofiaServerHostInfo, options.sofiaServerSettings, shareWorkspace]);
