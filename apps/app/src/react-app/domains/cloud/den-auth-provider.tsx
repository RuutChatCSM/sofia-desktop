...
import { parseDenAuthDeepLink } from "../../../app/lib/sofia-links";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function pendingServerSwitchForDeepLink(input: {
  ...
  // provisioned one. Otherwise any sofia://den-auth link can repoint the
...
export function DenAuthProvider({ children }: DenAuthProviderProps) {
  ...
  const syncDesktopSentrySession = useCallback((nextUser: DenUser | null) => {
    if (typeof window === "undefined" || !window.__SOFIA_ELECTRON__) return;
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (!window.__SOFIA_ELECTRON__) return;
