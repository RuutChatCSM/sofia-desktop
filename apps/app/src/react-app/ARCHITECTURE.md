...
- The legacy `sofia.react.activeWorkspace` and
  `sofia.react.sessionByWorkspace` values are only restore/fallback memory.
...
`shell/sofia-context-projector.ts` combines route state, the workbench store,
UI chrome state, and panel-tab state into the shared
`SofiaContextSnapshot` contract. `SofiaContextPublisher` publishes that
snapshot through `window.__sofiaControl`; the desktop loopback bridge and
`sofia-ui-mcp` expose the same contract without requiring every element to
