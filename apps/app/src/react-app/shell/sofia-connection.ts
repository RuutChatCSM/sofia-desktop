...
import { sofiaServerInfo, type SofiaServerInfo } from "../../app/lib/desktop";
import { isDesktopRuntime } from "../../app/utils";

export type SofiaConnectionSource = "desktop-runtime" | "stored-settings" | "same-origin" | "gateway" | "empty";

export type ResolvedSofiaConnection = {
  normalizedBaseUrl: string;
  resolvedToken: string;
  resolvedHostToken: string;
  hostInfo: SofiaServerInfo | null;
  source: SofiaConnectionSource;
};
...
export function isStaleStoredDesktopConnection(input: {
  ...
  return (
    isLoopbackSofiaServerUrl(input.storedBaseUrl) ||
    input.storedBaseUrl === input.runtimeReportedBaseUrl
  );
}

/**
 * Resolve the Sofia App server connection for routes that consume the server API.
 *
 ...
 */
export async function resolveSofiaConnection(): Promise<ResolvedSofiaConnection> {
  const gatewayOrigin = getSofiaGatewayOrigin();
  if (gatewayOrigin) {
    return {
      normalizedBaseUrl: normalizeSofiaServerUrl(gatewayOrigin) ?? "",
      resolvedToken: readSofiaGatewayDenToken(),
      resolvedHostToken: "",
      hostInfo: null,
      source: "gateway",
    };
  ...
  if (isDesktopRuntime()) {
    try {
      ...
      const normalizedBaseUrl =
        normalizeSofiaServerUrl(info.baseUrl ?? info.connectUrl ?? info.lanUrl ?? info.mdnsUrl ?? "") ??
        "";
  ...
  const settings = readSofiaServerSettings();
  const normalizedBaseUrl = normalizeSofiaServerUrl(settings.urlOverride ?? "") ?? "";
  const sameOriginBaseUrl =
    !normalizedBaseUrl && !isDesktopRuntime() && isWebDeployment() && typeof window !== "undefined"
      ? normalizeSofiaServerUrl(window.location.origin) ?? ""
      : "";
  const resolvedToken = settings.token?.trim() ?? "";
  const resolvedHostToken =
    normalizedBaseUrl && isLoopbackSofiaServerUrl(normalizedBaseUrl)
