...
import type {
  SofiaConversationLayout,
  SofiaContextSnapshot,
  SofiaPanelTab,
  SofiaResourceDescriptor,
  SofiaScreen,
} from "@sofia/types/sofia-context";
...
type SofiaContextProjectorInput = {
  ...
  availableAffordances: SofiaAffordanceDescriptor[];
};
...
export function screenFromRoute(route: string): SofiaScreen {
...
function panelTab(tab: PanelTabStore["sessions"][string]["tabs"][number]): SofiaPanelTab {
  if (tab.type === "browser") {
    return {
      id: tab.id,
      kind: "browser",
...
export function buildSofiaContext(
  ...
  const provider: SofiaProviderRef = { id: "sofia-ui", kind: "builtin" };
