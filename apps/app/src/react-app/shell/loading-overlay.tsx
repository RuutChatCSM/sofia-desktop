...
export function LoadingOverlay() {
  ...
  useEffect(() => {
    if (!error) return;
    let cancelled = false;
    const bridge = window.__SOFIA_ELECTRON__?.recovery;
    ...
    void bridge.list(cachedPolicy)
      ...
      .catch(() => undefined);
    if (window.__sofiaRecoveryControl) {
      return () => {
        cancelled = true;
      };
    }
  ...
  const useRelease = window.__SOFIA_ELECTRON__?.recovery?.use;
      ...
      <div className="flex w-full max-w-[320px] flex-col items-center gap-4 px-6 text-center">
        {error ? (
          <div className="flex w-full flex-col gap-3 text-[12px] leading-5">
            <div className="text-base font-medium text-dls-primary">Sofia App couldn't start</div>
            ...
            <button
              ...
              onClick={() => void runRecovery(window.__SOFIA_ELECTRON__?.recovery?.restorePrevious)}
            >
