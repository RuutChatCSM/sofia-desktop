...
export function isReadyLocalSofiaServerInfo(
  info: SofiaServerInfo | null | undefined,
): info is SofiaServerInfo {
  return Boolean(
    info?.running === true &&
      info.baseUrl?.trim() &&
...
export const LOCAL_SOFIA_READINESS_MAX_ATTEMPTS = 20;
export const LOCAL_SOFIA_READINESS_RETRY_DELAY_MS = 500;

export function sofiaServerSettingsChanged(
  previous: SofiaServerSettings,
  next: SofiaServerSettings,
): boolean {
 ...
 */
export async function waitForReadyLocalSofiaServerInfo(options?: {
  fetchInfo?: () => Promise<SofiaServerInfo | null>;
  maxAttempts?: number;
  wait?: (delayMs: number) => Promise<void>;
}): Promise<SofiaServerInfo | null> {
  const fetchInfo =
    options?.fetchInfo ?? (() => sofiaServerInfo() as Promise<SofiaServerInfo | null>);
  const maxAttempts = Math.max(1, options?.maxAttempts ?? LOCAL_SOFIA_READINESS_MAX_ATTEMPTS);
  const wait =
    options?.wait ??
    ((delayMs: number) => new Promise<void>((resolve) => window.setTimeout(resolve, delayMs)));

  let lastInfo: SofiaServerInfo | null = null;
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    if (attempt > 0) await wait(LOCAL_SOFIA_READINESS_RETRY_DELAY_MS);
    lastInfo = await fetchInfo().catch(() => null);
    if (isReadyLocalSofiaServerInfo(lastInfo)) return lastInfo;
  }
  return lastInfo;
}
 ...
 * sequence owns starting the server, and probing `sofiaServerInfo`
...
export async function ensureDesktopLocalSofiaConnection(
  options: EnsureDesktopLocalSofiaOptions,
) {
  ...
  recordInspectorEvent("route.local_sofia.ensure.start", {
    route: options.route,
  ...
  try {
    ...
    const nextSettings = writeSofiaServerSettings({
    ...
    if (startedEngine || sofiaServerSettingsChanged(previousSettings, nextSettings)) {
      emitSofiaSettingsChanged();
    }

    recordInspectorEvent("route.local_sofia.ensure.success", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
    ...
    recordInspectorEvent("route.local_sofia.ensure.error", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
      message,
