 ...
 * localStorage.sofia.debug.profilerOverlay = "1" / "0".
 *
 * In prod builds the wrapper is a pass-through (no Profiler overhead) and
 * the overlay renders null.
 *
 * Findings also land on window.__sofia.slice("profiler") so external
 * tools can read them.
 */
...
//
// Explicit opt-ins:
//   - VITE_SOFIA_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("sofia.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    ...
    const flag = env.VITE_SOFIA_PROFILER;
    if (flag === "1" || flag === "true" || flag === true) return true;
  } catch {
    // ignore
  }
  try {
    if (window.localStorage.getItem("sofia.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
// window.__sofia.slice("profiler") — even for operators who aren't
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("sofia.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("sofia.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
