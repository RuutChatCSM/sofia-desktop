...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetSofiaMode) => void;
...
function clearSofiaLocalStorage(mode: ResetSofiaMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (/sofia/.test(key)) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("sofia_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetSofiaMode>("onboarding");
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetSofiaState(resetModalMode);
      }
      clearSofiaLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
