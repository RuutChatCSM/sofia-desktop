...
declare global {
  interface Window {
    __sofiaReadDesktopVersionMetadataEval?: () => DenAppVersionMetadata | Promise<DenAppVersionMetadata>;
  }
}
...
export async function readFreshDenAppVersionMetadata(): Promise<DenAppVersionMetadata> {
  if (import.meta.env.DEV && typeof window !== "undefined" && window.__sofiaReadDesktopVersionMetadataEval) {
    return window.__sofiaReadDesktopVersionMetadataEval();
  }
