...
type PerfRoot = typeof globalThis & {
  __sofiaPerfSeq?: number;
  __sofiaPerfLogs?: PerfLogRecord[];
  __sofiaPerfConsoleAt?: Record<string, number>;
  __sofiaPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__sofiaPerfSeq ?? 0) + 1;
  root.__sofiaPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__sofiaPerfConsoleAt ?? (root.__sofiaPerfConsoleAt = {});
    const suppressedByKey =
      root.__sofiaPerfConsoleSuppressed ?? (root.__sofiaPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__sofiaPerfLogs = [];
  root.__sofiaPerfSeq = 0;
};
