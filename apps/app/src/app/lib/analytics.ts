/**
 * Product analytics for the Sofia App desktop app (PostHog, zero-dependency).
 *
 ...
 *   (`window.__sofia.record("analytics.<event>")`) so coded evals can
...
const ENV_POSTHOG_HOST = String(import.meta.env.VITE_SOFIA_POSTHOG_HOST ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_SOFIA_APP_VERSION ?? "").trim();

const DEFAULT_POSTHOG_HOST = "https://us.i.posthog.com";

// Packaged releases use the default publishable key; dev builds stay silent
// unless VITE_SOFIA_POSTHOG_KEY is set. Set it to "" to disable analytics
// in any build. The inspector mirror still records events locally either way.
const POSTHOG_KEY = resolvePosthogKey(import.meta.env.VITE_SOFIA_POSTHOG_KEY, import.meta.env.DEV);
const POSTHOG_HOST = (ENV_POSTHOG_HOST || DEFAULT_POSTHOG_HOST).replace(/\/+$/, "");

const PREFS_STORAGE_KEY = "sofia.preferences";
const DISTINCT_ID_STORAGE_KEY = "sofia.analytics.distinctId";
