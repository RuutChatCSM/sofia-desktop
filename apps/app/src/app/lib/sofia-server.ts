...
import { normalizeBaseUrl } from "@sofia/types/url";
import {
...
import type { ExecResult, WorkspaceInfo, WorkspaceList } from "./desktop";
...
export type SofiaCloudProviderSyncRun = {
  status: "applied" | "noop" | "failed" | "no_session";
  message?: string;
};

export type SofiaCloudProviderSyncSkippedProvider = {
...
export type SofiaCloudProviderSyncStatus = {
  hasSession: boolean;
  lastRun: { at: string | number; status: SofiaCloudProviderSyncRun["status"]; message?: string } | null;
  ...
  skippedProviders: SofiaCloudProviderSyncSkippedProvider[];
};

function parseCloudProviderSyncRun(value: unknown): SofiaCloudProviderSyncRun {
...
function parseCloudProviderSyncStatus(value: unknown): SofiaCloudProviderSyncStatus {
  ...
  let lastRun: SofiaCloudProviderSyncStatus["lastRun"] = null;
  ...
  const skippedProviders: SofiaCloudProviderSyncSkippedProvider[] = [];
...
export type SofiaServerStatus = "connected" | "disconnected" | "limited";

export type SofiaServerDiagnostics = {
  ok: boolean;
  version: string;
  uptimeMs: number;
  readOnly: boolean;
  ...
  workspace: SofiaWorkspaceInfo | null;
...
export type SofiaRuntimeSnapshot = {
  ok: boolean;
  worker?: {
    workspace: string;
    sandboxMode: string;
  ...
  upgrade?: {
    ...
    services: SofiaRuntimeServiceName[];
  };
  services: SofiaRuntimeServiceSnapshot[];
};

export type SofiaServerSettings = {
  urlOverride?: string;
  portOverride?: number;
  token?: string;
  hostToken?: string;
...
export type SofiaWorkspaceList = {
  items: SofiaWorkspaceInfo[];
  workspaces?: WorkspaceInfo[];
  activeId?: string | null;
};

export type SofiaSessionMessage = {
  info: Message;
  parts: Part[];
};

export type SofiaSessionSnapshot = {
  session: Session;
  messages: SofiaSessionMessage[];
  todos: Todo[];
  status:
    | { type: "idle" }
    | { type: "busy" }
...
export type SofiaSkillContent = {
  item: SofiaSkillItem;
  content: string;
};

export type SofiaWorkspaceFileContent = {
  path: string;
  content: string;
  bytes: number;
  updatedAt: number;
...
export type SofiaWorkspaceFileWriteResult = {
  ok: boolean;
  path: string;
  bytes: number;
  updatedAt: number;
...
export type SofiaWorkspaceFileDeleteResult = {
  ok: boolean;
  path: string;
  code?: string;
};

export type SofiaAuthorizedFoldersResponse = {
  folders: string[];
  hiddenCount: number;
  workspaceRoot: string;
};

export type SofiaAuthorizedFoldersUpdateResponse = {
  folders: string[];
  hiddenCount: number;
  updatedAt: number;
};

export type SofiaRuntimeConfigMigrationResult = {
  migrated: boolean;
  keys: string[];
  legacyKeys: string[];
...
export type SofiaRuntimeDisabledProvidersResult = {
  ok: true;
  disabledProviders: string[];
};

export type SofiaLegacyConfigSweepState = {
  version: 1;
  sweptAt: string;
  files: Array<{
    path: string;
...
export type SofiaRuntimeConfigStatus = {
...
export type SofiaDesktopCloudSyncChange = {
  id: string;
...
export type SofiaDesktopCloudSyncState = {
  entries: Record<string, unknown>;
  updatedAt: number;
  version: 1;
};

export type SofiaDesktopCloudSyncResult = {
  changes: SofiaDesktopCloudSyncChange[];
  state: SofiaDesktopCloudSyncState;
};

export type SofiaCloudPluginInstallResult = {
  item: CloudImportedPlugin;
  warnings: string[];
};

export type SofiaCloudPluginsResult = {
  marketplaces: Record<string, CloudImportedMarketplace>;
  plugins: Record<string, CloudImportedPlugin>;
};

export type SofiaClaudePluginComponent = {
  type: "mcp" | "skill" | "command" | "agent";
  name: string;
  description: string | null;
};

export type SofiaClaudePluginPreview = {
  pluginId: string;
  name: string;
  description: string | null;
  version: string | null;
  source: { owner: string; repo: string; ref: string; dir: string | null };
  components: SofiaClaudePluginComponent[];
  warnings: string[];
};
...
export type SofiaMcpItem = {
  name: string;
  config: Record<string, unknown>;
  source: "config.project" | "config.global" | "config.remote";
  disabledByTools?: boolean;
  managedOAuth?: SofiaManagedMcpConnection | null;
};

export type SofiaMcpAppResource = {
  serverName: string;
  toolName: string;
  resourceUri: string;
  html: string;
...
export type SofiaMcpAppLaunchReference = {
  connectionId?: string;
  toolName: string;
  resourceUri: string;
...
export type SofiaMcpAppToolResult = {
...
export type SofiaMcpAppSandbox = {
  url: string;
...
export type SofiaManagedMcpConnection = {
  name: string;
  serverUrl: string;
  enabled: boolean;
...
export type SofiaManagedOAuthState = {
  available: boolean;
  recovery: { at: number; reason: string; quarantinedTo: string } | null;
};

export type SofiaManagedMcpStartResult =
  | { status: "connected" }
  | { status: "needs_auth"; authorizeUrl: string };

export type SofiaMcpEngineSync = {
  status: "ok" | "failed";
  at: number;
  failures: Array<{ name: string; status?: number; message?: string }>;
};

export type SofiaCloudMcpProviderModelContext = {
  provider: string;
  model: string;
};

export type SofiaCloudMcpFailureStage =
  | "prerequisites"
  | "token_mint"
  | "desired_config"
  | "engine_delivery"
...
export type SofiaCloudMcpFailureCode =
  ...
  | "sofia_cloud_auth_invalid"
  | "sofia_cloud_token_expired"
  | "sofia_cloud_membership_required"
  | "sofia_cloud_scope_missing"
  | "sofia_cloud_resource_forbidden"
  | "sofia_cloud_resource_not_found"
  | "sofia_cloud_client_registration_required"
...
export type SofiaCloudMcpFailure = {
  code: SofiaCloudMcpFailureCode;
  stage: SofiaCloudMcpFailureStage | string;
  retryable: boolean;
...
export type SofiaCloudMcpCompatibility = {
  sofia: {
...
export type SofiaCloudMcpHealthPhase =
  | "missing_desired"
  | "workspace_ambiguous"
  | "engine_unconfigured"
  | "engine_unreachable"
...
export type SofiaCloudMcpDeliverySnapshot = {
  ...
  trigger?: string;
  failure?: SofiaCloudMcpFailure;
};

export type SofiaCloudMcpProbeStep = {
...
export type SofiaCloudMcpProbeTrace = {
  endpoint: string | null;
  startedAt: string;
  latencyMs: number;
  protocolVersion: string | null;
  serverInfo: { name: string | null; version: string | null } | null;
  steps: SofiaCloudMcpProbeStep[];
};

export type SofiaCloudMcpEngineRefreshStep = {
...
export type SofiaCloudMcpEngineRefresh = {
  performed: boolean;
  reason?: "desired_missing" | string;
  trigger: string;
  startedAt: string;
  finishedAt: string;
  steps: SofiaCloudMcpEngineRefreshStep[];
};

export type SofiaCloudMcpEngineRefreshResult = {
  refresh: SofiaCloudMcpEngineRefresh;
  health: SofiaCloudMcpHealth;
};

export type SofiaCloudMcpHealth = {
  schemaVersion: 1;
  phase: SofiaCloudMcpHealthPhase;
  usable: boolean;
  ...
  delivery: SofiaCloudMcpDeliverySnapshot;
  engine: {
  ...
  compatibility: SofiaCloudMcpCompatibility;
  toolDenies: unknown[];
  firstFailure: SofiaCloudMcpFailure | null;
  checkedAt: string;
  durationMs?: number;
};

export type SofiaCloudMcpReconcilePayload = {
  workspaceId: string;
  name: "sofia-cloud";
  config: Record<string, unknown>;
...
export type SofiaWorkspaceImportPreview = {
  fingerprint: string;
  summary: {
    total: number;
    create: number;
  ...
  changes: SofiaWorkspaceImportChange[];
};

export type SofiaWorkspaceExportSensitiveMode = "auto" | "include" | "exclude";

export type SofiaWorkspaceExportWarning = {
  id: string;
  label: string;
  detail: string;
};

export type SofiaBlueprintSessionsMaterializeResult = {
  ok: boolean;
...
export type SofiaArtifactList = {
  items: SofiaArtifactItem[];
};

export type SofiaConnectState = {
  ok: true;
  schemaVersion: 1;
...
export type SofiaExtensionActionCall = {
  extensionId: string;
  action: string;
...
export type SofiaExtensionActionResult =
  | {
    ok: true;
    extensionId: string;
    action: string;
...
export type SofiaResolvedArtifactTarget = {
  id: string;
  kind: "file" | "url";
  value: string;
  name: string;
...
export type SofiaWorkspaceFileStat = {
  ok: boolean;
  path: string;
  exists: boolean;
  kind?: "file" | "dir" | "other";
...
export type SofiaInboxList = {
  items: SofiaInboxItem[];
};

export type SofiaInboxUploadResult = {
  ok: boolean;
  path: string;
  bytes: number;
};

export type SofiaUserEnvItem = {
  key: string;
  updatedAt: number;
  hasValue: boolean;
  value?: string;
...
export type SofiaActor = {
  type: "remote" | "host";
  clientId?: string;
  tokenHash?: string;
};

export type SofiaAuditEntry = {
  id: string;
  workspaceId: string;
  actor: SofiaActor;
  action: string;
...
export type SofiaReloadTrigger = {
...
export type SofiaReloadEvent = {
  id: string;
  seq: number;
  workspaceId: string;
  reason: "plugins" | "skills" | "mcp" | "config" | "agents" | "commands";
  trigger?: SofiaReloadTrigger;
  timestamp: number;
};

export type SofiaSessionGroupDefinition = {
  id: string;
  label: string;
};

export type SofiaSessionGroupState = {
  groups: SofiaSessionGroupDefinition[];
  assignments: Record<string, string>;
};

export type SofiaSessionGroupEvent = {
  id: string;
  seq: number;
  workspaceId: string;
...
export const DEFAULT_SOFIA_SERVER_PORT = 8787;

const STORAGE_URL_OVERRIDE = "sofia.server.urlOverride";
const STORAGE_PORT_OVERRIDE = "sofia.server.port";
const STORAGE_TOKEN = "sofia.server.token";
const STORAGE_HOST_AUTH_KEY = "sofia.server.hostToken";
const STORAGE_REMOTE_ACCESS = "sofia.server.remoteAccessEnabled";
...
declare global {
  interface Window {
    __SOFIA_BOOTSTRAP__?: SofiaBootstrap;
  }
}

export function normalizeSofiaServerUrl(input: string) {
...
export function isLoopbackSofiaServerUrl(input: string) {
  const normalized = normalizeSofiaServerUrl(input) ?? "";
...
export function parseSofiaWorkspaceIdFromUrl(input: string) {
  const normalized = normalizeSofiaServerUrl(input) ?? "";
...
export function buildSofiaWorkspaceBaseUrl(hostUrl: string, workspaceId?: string | null) {
  const normalized = normalizeSofiaServerUrl(hostUrl) ?? "";
  if (!normalized) return null;
...
const SOFIA_INVITE_PARAM_URL = "ow_url";
const SOFIA_INVITE_PARAM_TOKEN = "ow_token";
const SOFIA_INVITE_PARAM_STARTUP = "ow_startup";
const SOFIA_INVITE_PARAM_AUTO_CONNECT = "ow_auto_connect";

export type SofiaConnectInvite = {
  url: string;
  token?: string;
  startup?: "server";
  autoConnect?: boolean;
...
export function readSofiaConnectInviteFromSearch(input: string | URLSearchParams) {
  const search =
    typeof input === "string"
      ? new URLSearchParams(input.startsWith("?") ? input.slice(1) : input)
      : input;

  const rawUrl = search.get(SOFIA_INVITE_PARAM_URL)?.trim() ?? "";
  const url = normalizeSofiaServerUrl(rawUrl);
  if (!url) return null;

  const token = search.get(SOFIA_INVITE_PARAM_TOKEN)?.trim() ?? "";
  const startupRaw = search.get(SOFIA_INVITE_PARAM_STARTUP)?.trim() ?? "";
  const startup = startupRaw === "server" ? "server" : undefined;
  const autoConnect = search.get(SOFIA_INVITE_PARAM_AUTO_CONNECT)?.trim() === "1";
  ...
  } satisfies SofiaConnectInvite;
}

export function stripSofiaConnectInviteFromUrl(input: string) {
  try {
    const url = new URL(input);
    url.searchParams.delete(SOFIA_INVITE_PARAM_URL);
    url.searchParams.delete(SOFIA_INVITE_PARAM_TOKEN);
    url.searchParams.delete(SOFIA_INVITE_PARAM_STARTUP);
    url.searchParams.delete(SOFIA_INVITE_PARAM_AUTO_CONNECT);
    return url.toString();
  } catch {
    return input;
  }
...
export function readSofiaServerSettings(): SofiaServerSettings {
  if (typeof window === "undefined") return {};
  try {
    const urlOverride = normalizeSofiaServerUrl(
...
export function writeSofiaServerSettings(next: SofiaServerSettings): SofiaServerSettings {
  if (typeof window === "undefined") return next;
  try {
    const urlOverride = normalizeSofiaServerUrl(next.urlOverride ?? "");
    ...
    return readSofiaServerSettings();
  } catch {
    return next;
  }
}
...
function readForceEnvSettingsFlag(): boolean {
  const raw =
    typeof import.meta !== "undefined" && typeof import.meta.env?.VITE_SOFIA_FORCE_ENV_SETTINGS === "string"
      ? import.meta.env.VITE_SOFIA_FORCE_ENV_SETTINGS.trim()
      : "";
  return /^(1|true|yes|on)$/i.test(raw);
}

export function hydrateSofiaServerSettingsFromEnv() {
  if (typeof window === "undefined") return;
  if (isSofiaGatewayRuntime()) return;

  const envUrl = typeof import.meta.env?.VITE_SOFIA_URL === "string"
    ? import.meta.env.VITE_SOFIA_URL.trim()
    : "";
  const envPort = typeof import.meta.env?.VITE_SOFIA_PORT === "string"
    ? import.meta.env.VITE_SOFIA_PORT.trim()
    : "";
  const envToken = typeof import.meta.env?.VITE_SOFIA_TOKEN === "string"
    ? import.meta.env.VITE_SOFIA_TOKEN.trim()
    : "";
  const envHostToken = typeof import.meta.env?.VITE_SOFIA_HOST_TOKEN === "string"
    ? import.meta.env.VITE_SOFIA_HOST_TOKEN.trim()
    : "";
  const bootstrapToken = typeof window.__SOFIA_BOOTSTRAP__?.token === "string"
    ? window.__SOFIA_BOOTSTRAP__.token.trim()
    : "";
  ...
  try {
    const current = readSofiaServerSettings();
    const next: SofiaServerSettings = { ...current };
    let changed = false;

    if (envUrl && (forceEnvSettings || !current.urlOverride)) {
      const normalized = normalizeSofiaServerUrl(envUrl);
...
export function clearSofiaServerSettings() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(STORAGE_URL_OVERRIDE);
    window.localStorage.removeItem(STORAGE_PORT_OVERRIDE);
...
export class SofiaServerError extends Error {
  status: number;
  code: string;
  details?: unknown;
...
const SOFIA_STREAM_URL_RE = /\/events(\b|\?)|\/event-stream\b|\/stream\b/;

function isStreamUrl(url: string): boolean {
  return SOFIA_STREAM_URL_RE.test(url);
}
...
const DEFAULT_SOFIA_SERVER_TIMEOUT_MS = 10_000;
...
async function requestJson<T>(
  ...
  const response = await fetchWithTimeout(
    ...
    options.timeoutMs ?? DEFAULT_SOFIA_SERVER_TIMEOUT_MS,
  );
...
async function requestAgentContextDiagnosticsJson(
  ...
  if (!result.response.ok) {
    ...
    const details = payload && typeof payload === "object" && "details" in payload
      ? payload.details
      : undefined;
    throw new SofiaServerError(result.response.status, code, message, details);
  }
...
export function createSofiaServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    runtimeVersions: () =>
      requestJson<SofiaRuntimeSnapshot>(baseUrl, "/runtime/versions", { token, hostToken, timeoutMs: timeouts.status }),
    status: () => requestJson<SofiaServerDiagnostics>(baseUrl, "/status", { token, hostToken, timeoutMs: timeouts.status }),
    capabilities: () => requestJson<SofiaServerCapabilities>(baseUrl, "/capabilities", { token, hostToken, timeoutMs: timeouts.capabilities }),
    getConnectState: (workspaceId?: string | null) => {
      const query = new URLSearchParams();
      if (workspaceId?.trim()) query.set("workspaceId", workspaceId.trim());
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<SofiaConnectState>(baseUrl, `/experimental/connect/state${suffix}`, { token, hostToken, timeoutMs: timeouts.config });
    },
    ...
    setConnectState: (connectEnabled: boolean) => requestJson<SofiaConnectState>(baseUrl, "/experimental/connect/state", { token, hostToken, method: "PUT", body: { connectEnabled }, timeoutMs: timeouts.config }),
    callExtensionAction: (payload: SofiaExtensionActionCall) =>
      requestJson<SofiaExtensionActionResult>(baseUrl, "/experimental/extensions/call", {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listWorkspaces: () => requestJson<SofiaWorkspaceList>(baseUrl, "/workspaces", { token, hostToken, timeoutMs: timeouts.listWorkspaces }),
    ...
    activateWorkspace: (workspaceId: string, options?: { persist?: boolean }) => {
      ...
      return requestJson<{ activeId: string; workspace: SofiaWorkspaceInfo; persisted: boolean }>(
        baseUrl,
    ...
    deleteWorkspace: (workspaceId: string) =>
      requestJson<{ ok: boolean; deleted: boolean; persisted: boolean; activeId: string | null; items: SofiaWorkspaceInfo[]; workspaces?: WorkspaceInfo[] }>(
        baseUrl,
    ...
    getSessionGroups: (workspaceId: string) =>
      requestJson<{ state: SofiaSessionGroupState; updatedAt: number | null }>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/session-groups`,
        { token, hostToken, timeoutMs: timeouts.sessionRead },
      ),
    putSessionGroups: (workspaceId: string, state: SofiaSessionGroupState) =>
      requestJson<{ state: SofiaSessionGroupState; updatedAt: number }>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/session-groups`,
    ...
    listSessionGroupEvents: (workspaceId: string, options?: { since?: number }) => {
      ...
      return requestJson<{ items: SofiaSessionGroupEvent[]; cursor?: number }>(
        baseUrl,
    ...
    getSessionMessages: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ items: SofiaSessionMessage[] }>(
        baseUrl,
    ...
    getSessionSnapshot: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ item: SofiaSessionSnapshot }>(
        baseUrl,
    ...
    exportWorkspace: (
      workspaceId: string,
      options?: { sensitiveMode?: SofiaWorkspaceExportSensitiveMode },
    ) => {
      const query = new URLSearchParams();
      ...
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<SofiaWorkspaceExport>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/export${suffix}`, {
        token,
        hostToken,
    ...
    importWorkspace: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<{ ok: boolean; preview?: SofiaWorkspaceImportPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/import`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    previewWorkspaceImport: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<SofiaWorkspaceImportPreview>(
        baseUrl,
    ...
    materializeBlueprintSessions: (workspaceId: string) =>
      requestJson<SofiaBlueprintSessionsMaterializeResult>(
        baseUrl,
    ...
    migrateRuntimeConfig: (workspaceId: string) =>
      requestJson<SofiaRuntimeConfigMigrationResult>(
        baseUrl,
    ...
    setRuntimeDisabledProviders: (workspaceId: string, providers: string[]) =>
      requestJson<SofiaRuntimeDisabledProvidersResult>(
        baseUrl,
    ...
    getRuntimeConfigStatus: (workspaceId: string) =>
      requestJson<SofiaRuntimeConfigStatus>(
        baseUrl,
    ...
    getDesktopCloudSync: (workspaceId: string) =>
      requestJson<SofiaDesktopCloudSyncState>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/desktop-cloud-sync`, {
        token,
        hostToken,
        timeoutMs: timeouts.config,
      }),
    syncDesktopCloud: (workspaceId: string, snapshot: DenResourceSnapshot) =>
      requestJson<SofiaDesktopCloudSyncResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/desktop-cloud-sync`, {
        token,
        hostToken,
        method: "POST",
        body: { snapshot },
    ...
    listCloudPlugins: (workspaceId: string) =>
      requestJson<SofiaCloudPluginsResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins`, {
        token,
        hostToken,
        timeoutMs: timeouts.config,
      }),
    installCloudPlugin: (workspaceId: string, payload: { marketplaceId: string | null; marketplace?: DenOrgMarketplace | null; resolved: DenOrgPluginResolved }) =>
      requestJson<SofiaCloudPluginInstallResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    removeCloudPlugin: (workspaceId: string, pluginId: string) =>
      requestJson<SofiaCloudPluginInstallResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins/${encodeURIComponent(pluginId)}`, {
        token,
        hostToken,
        method: "DELETE",
        timeoutMs: timeouts.config,
    ...
    previewClaudePlugin: (workspaceId: string, payload: { url: string; ref?: string }) =>
      requestJson<{ preview: SofiaClaudePluginPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/claude-plugins`, {
        token,
        hostToken,
        method: "POST",
    ...
    installClaudePlugin: (workspaceId: string, payload: { url: string; ref?: string }) =>
      requestJson<SofiaCloudPluginInstallResult & { preview: SofiaClaudePluginPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/claude-plugins`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listReloadEvents: (workspaceId: string, options?: { since?: number }) => {
      const query = typeof options?.since === "number" ? `?since=${options.since}` : "";
      return requestJson<{ items: SofiaReloadEvent[]; cursor?: number }>(
        baseUrl,
    ...
    listPlugins: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      ...
      return requestJson<{ items: SofiaPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    addPlugin: (workspaceId: string, spec: string) =>
      requestJson<{ items: SofiaPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    listSkills: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<{ items: SofiaSkillItem[] }>(
        baseUrl,
    ...
    getSkill: (workspaceId: string, name: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<SofiaSkillContent>(
        baseUrl,
    ...
    upsertSkill: (workspaceId: string, payload: { name: string; content: string; description?: string }) =>
      requestJson<SofiaSkillItem>(baseUrl, `/workspace/${workspaceId}/skills`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listMcp: (workspaceId: string) =>
      requestJson<{
        items: SofiaMcpItem[];
        engineSync?: SofiaMcpEngineSync | null;
        managedOAuthState?: SofiaManagedOAuthState | null;
      }>(
        baseUrl,
    ...
    resolveMcpApp: (
      ...
      launch?: SofiaMcpAppLaunchReference,
    ) =>
      requestJson<{ app: SofiaMcpAppResource | null }>(
        baseUrl,
    ...
    mcpAppSandbox: (app: SofiaMcpAppResource, hostOrigin: string): SofiaMcpAppSandbox => {
    ...
    ) => requestJson<SofiaMcpAppToolResult>(
      baseUrl,
    ...
    getSofiaCloudMcpHealth: (
      ...
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<SofiaCloudMcpHealth>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/health${suffix}`,
        { token, hostToken, timeoutMs: options?.probe ? timeouts.cloudMcpProbeHealth : timeouts.cloudMcpHealth },
      );
    },
    reconcileSofiaCloudMcp: (workspaceId: string, payload: SofiaCloudMcpReconcilePayload) =>
      requestJson<SofiaCloudMcpHealth>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/reconcile`,
        {
          token,
          hostToken,
          method: "POST",
    ...
    refreshSofiaCloudMcpEngine: (
      ...
      requestJson<SofiaCloudMcpEngineRefreshResult>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/engine-refresh`,
        {
          token,
          hostToken,
          method: "POST",
    ...
    addMcp: (workspaceId: string, payload: { name: string; config: Record<string, unknown> }) =>
      requestJson<{ items: SofiaMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    addManagedMcp: (
      ...
      requestJson<SofiaManagedMcpStartResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/managed`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    getManagedMcp: (workspaceId: string, name: string) =>
      requestJson<SofiaManagedMcpConnection>(
        baseUrl,
    ...
    connectManagedMcp: (workspaceId: string, name: string) =>
      requestJson<SofiaManagedMcpStartResult>(
        baseUrl,
    ...
    removeMcp: (workspaceId: string, name: string) =>
      requestJson<{ items: SofiaMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp/${encodeURIComponent(name)}`, {
        token,
        hostToken,
        method: "DELETE",
      }),
    setMcpEnabled: (workspaceId: string, name: string, enabled: boolean) =>
      requestJson<{ items: SofiaMcpItem[] }>(
        baseUrl,
    ...
    upsertCommand: (
      ...
      requestJson<{ items: SofiaCommandItem[] }>(baseUrl, `/workspace/${workspaceId}/commands`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    uploadInbox: async (workspaceId: string, file: File, options?: { path?: string }) => {
      ...
      if (body) {
        try {
          const parsed = JSON.parse(body) as Partial<SofiaInboxUploadResult>;
          if (typeof parsed.path === "string" && parsed.path.trim()) {
            return {
              ok: parsed.ok ?? true,
              path: parsed.path.trim(),
              bytes: typeof parsed.bytes === "number" ? parsed.bytes : file.size,
            } satisfies SofiaInboxUploadResult;
          }
        } catch {
    ...
    listInbox: (workspaceId: string) =>
      requestJson<SofiaInboxList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/inbox`, {
        token,
        hostToken,
      }),
    ...
    statWorkspaceFile: (workspaceId: string, path: string) =>
      requestJson<SofiaWorkspaceFileStat>(
        baseUrl,
    ...
    writeWorkspaceFile: (
      ...
      requestJson<SofiaWorkspaceFileWriteResult>(
        baseUrl,
    ...
    ): Promise<SofiaWorkspaceFileDeleteResult[]> => {
    ...
    listArtifacts: (workspaceId: string) =>
      requestJson<SofiaArtifactList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/artifacts`, {
        token,
        hostToken,
      }),
    ...
    resolveArtifacts: (
      ...
      requestJson<{ items: SofiaResolvedArtifactTarget[] }>(
        baseUrl,
    ...
    listUserEnv: () =>
      requestJson<{ items: SofiaUserEnvItem[] }>(
        baseUrl,
    ...
    getUserEnv: (key: string) =>
      requestJson<{ item: SofiaUserEnvItem & { value: string } }>(
        baseUrl,
...
export type SofiaServerClient = ReturnType<typeof createSofiaServerClient>;
