const ENV_FEEDBACK_URL = String(import.meta.env.VITE_SOFIA_FEEDBACK_URL ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_SOFIA_APP_VERSION ?? "").trim();

export const DEFAULT_FEEDBACK_URL =
  ENV_FEEDBACK_URL || "https://sofia.ruut.chat/feedback";
...
export function buildFeedbackUrl(options: FeedbackUrlOptions): string {
  ...
  url.searchParams.set("source", "sofia-app");
