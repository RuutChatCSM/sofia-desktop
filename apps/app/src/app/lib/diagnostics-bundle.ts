...
function pickAppInfo(info: AppBuildInfo | null) {
  ...
  return {
    ...
    sofiaDevMode: info.sofiaDevMode ?? null,
  };
}
...
function pickHostInfo(info: SofiaServerInfo | null) {
  if (!info) return null;
  return {
...
function defaultHostConnectUrl(hostInfo: SofiaServerInfo | null) {
...
function collectSecretValues(input: DiagnosticsBundleInputs) {
  ...
  addSecretValue(secrets, input.sofiaServerSettings.token);
  addSecretValue(secrets, input.sofiaServerSettings.hostToken);
...
export function composeDiagnosticsBundleJson(input: DiagnosticsBundleInputs): string {
  ...
  const urlOverride = input.sofiaServerSettings.urlOverride?.trim() ?? "";
  const token = input.sofiaServerSettings.token?.trim() ?? "";
  ...
  const bundle = {
    ...
    sofiaServer: {
      status: context?.sofiaServerStatus ?? (clientConnected ? "connected" : "disconnected"),
      url: context?.sofiaServerUrl ?? "",
      settings: {
