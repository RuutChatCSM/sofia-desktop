const PENDING_CHANGES_KEY = "sofia.settings.environment.pendingChanges";
...
export function buildSofiaEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "sofia"}::${runtime || "runtime"}`;
}

export function readSofiaEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeSofiaEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
