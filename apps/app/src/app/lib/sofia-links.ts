...
import { normalizeSofiaServerUrl } from "./sofia-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "sofia:"
    || normalized === "sofia-dev:"
    || normalized === "https:"
    || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("sofiaHostUrl") ?? url.searchParams.get("sofiaUrl") ?? "";
  const tokenRaw = url.searchParams.get("sofiaToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeSofiaServerUrl(hostUrlRaw);
  ...
  return {
    sofiaHostUrl: normalizedHostUrl,
    sofiaToken: token,
    directory: null,
...
export function parseConnectDeepLink(rawUrl: string): ConnectDeepLink | null {
  ...
  const protocol = url.protocol.toLowerCase();
  if (protocol !== "sofia:" && protocol !== "sofia-dev:") {
    return null;
  }
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:sofia-dev|sofia|https?):\/\/[^\s"'<>]+/i);
