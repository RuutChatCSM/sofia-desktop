/**
 * Typed helpers for Sofia session operations.
 *
 * The Sofia SDK (v2) exposes `session.abort`, `session.revert`,
...
function logAbortSession(
  ...
  console.info("[engine-session] abort", {
    phase,
...
/**
 * Archive or unarchive a session via Sofia's native `time.archived` field.
 *
