...
import type {
  SofiaDesktopCloudSyncChange,
  SofiaDesktopCloudSyncResult,
  SofiaDesktopCloudSyncState,
  SofiaServerClient,
} from "../lib/sofia-server";
...
function readSyncChange(value: unknown): SofiaDesktopCloudSyncChange | null {
...
export function readPendingCloudSyncChanges(state: SofiaDesktopCloudSyncState): SofiaDesktopCloudSyncChange[] {
...
async function runDesktopCloudSync(input: {
  ...
  workspaceId: string;
}): Promise<SofiaDesktopCloudSyncResult | null> {
  ...
  return input.sofiaClient.syncDesktopCloud(input.workspaceId, snapshot);
}

export function refreshDesktopCloudSync(input: {
  sofiaClient: SofiaServerClient | null | undefined;
  workspaceId: string | null | undefined;
}): Promise<SofiaDesktopCloudSyncResult | null> {
  const sofiaClient = input.sofiaClient ?? null;
  const workspaceId = input.workspaceId?.trim() ?? "";
  if (!sofiaClient || !workspaceId) return Promise.resolve(null);

  const run = desktopCloudSyncQueue.then(() => runDesktopCloudSync({ sofiaClient, workspaceId }));
