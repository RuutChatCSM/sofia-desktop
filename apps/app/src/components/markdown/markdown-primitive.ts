...
const CODE_COPY_ICON = `<svg data-sofia-code-copy-icon="" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-3.5 w-3.5" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>`;
const CODE_COPIED_ICON = `<svg data-sofia-code-copy-check-icon="" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-3.5 w-3.5" aria-hidden="true" hidden><path d="M20 6 9 17l-5-5"/></svg>`;
...
function codeCopyButton() {
  return `<button type="button" data-sofia-code-copy="" class="absolute right-2 top-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-md border border-border/70 bg-background/95 text-muted-foreground shadow-sm transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Copy code block" title="Copy code block">${CODE_COPY_ICON}${CODE_COPIED_ICON}<span data-sofia-code-copy-label="" class="sr-only" aria-live="polite">Copy code block</span></button>`;
}

function chatCodeBlockContainer(html: string, shiki: boolean) {
  const shikiAttribute = shiki ? ` data-sofia-shiki="true"` : "";

  return `<div data-sofia-code-block=""${shikiAttribute} class="relative my-4 overflow-hidden rounded-[18px] border border-border/70 bg-gray-2/60 font-mono text-xs leading-6 text-foreground">${codeCopyButton()}${html}</div>`;
}
...
export function syncMarkdownImagePreviews(root: HTMLElement) {
  const previews = root.querySelectorAll("[data-sofia-image-preview]");
...
export function setCodeCopyButtonState(button: HTMLButtonElement, copied: boolean) {
  const label = button.querySelector("[data-sofia-code-copy-label]");
  if (label) label.textContent = copied ? "Code block copied" : "Copy code block";

  button.querySelector("[data-sofia-code-copy-icon]")?.toggleAttribute("hidden", copied);
  button.querySelector("[data-sofia-code-copy-check-icon]")?.toggleAttribute("hidden", !copied);
...
function sanitizeMarkdownHtml(value: string) {
  ...
  return DOMPurify.sanitize(value, {
    ...
    ADD_ATTR: [
      "checked",
      "class",
      "data-sofia-math-error",
      "data-sofia-code-block",
      "data-sofia-code-copy",
      "data-sofia-code-copy-check-icon",
      "data-sofia-code-copy-icon",
      "data-sofia-code-copy-label",
      "aria-label",
      "data-sofia-image-preview",
      "data-sofia-link-href",
      "data-sofia-link-chevron",
      "data-sofia-shiki",
      "decoding",
      "disabled",
      "hidden",
...
function markdownProfileForPresentation(presentation: MarkdownPresentation): MarkdownProfile {
  if (presentation === "surface") {
    return {
      ...
      shikiContainer: `<div data-sofia-shiki="true" class="my-4 overflow-x-auto rounded-[18px] border border-dls-border/70 bg-gray-1/80 p-4 text-xs leading-6">%s</div>`,
...
function renderLink(profile: MarkdownProfile, href: string, title: string | null | undefined, text: string) {
  ...
  if (profile.linkPresentation === "chat") {
    ...
    if (isFilePath) {
      ...
      return `<span class="inline-flex items-stretch overflow-hidden rounded-md border border-border/60 bg-muted/40 text-xs font-medium text-foreground align-middle"><a href="${safe}" data-sofia-link-href="${originalHref}"${titleAttr} target="_blank" rel="noreferrer noopener" class="inline-flex items-center gap-1 px-1.5 py-0.5 no-underline transition-colors hover:bg-muted">${fileIcon}${text}</a><button type="button" data-sofia-link-chevron="${originalHref}" class="inline-flex items-center border-l border-border/60 px-1 transition-colors hover:bg-muted" aria-label="Open with">${chevron}</button></span>`;
    }
    ...
    return `<a href="${safe}" data-sofia-link-href="${originalHref}"${titleAttr} target="_blank" rel="noreferrer noopener" class="text-indigo-10 no-underline transition-colors hover:text-indigo-8">${faviconHtml}${text}</a>`;
  }
...
function renderImage(profile: MarkdownProfile, href: string, title: string | null | undefined, text: string) {
  ...
  if (profile.imagePresentation === "chat") {
    return `<button type="button" data-sofia-image-preview="" class="my-4 inline-block max-w-full cursor-zoom-in align-top text-left transition-opacity hover:opacity-90" aria-label="Expand ${escapeAttribute(text)}"><img src="${safe}" alt="${escapeAttribute(text)}"${titleAttr} loading="lazy" decoding="async" class="block h-auto w-auto rounded-lg border border-border/70 object-contain" style="max-height: ${MARKDOWN_IMAGE_PREVIEW_MAX_HEIGHT}px; max-width: ${MARKDOWN_IMAGE_PREVIEW_MAX_WIDTH}px"></button>`;
  }
...
function createMarkedOptions(profile: MarkdownProfile, isAsync: boolean) {
  return {
    ...
    renderer: {
      html({ text }) {
        return profile.rawHtmlMode === "shiki-only" && !text.includes('data-sofia-shiki="true"') ? "" : text;
      },
      paragraph({ tokens }) {
