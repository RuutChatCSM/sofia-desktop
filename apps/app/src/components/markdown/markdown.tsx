...
function MarkdownBlockInner({
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      const copyButton = event.target.closest("[data-sofia-code-copy]");
      if (copyButton instanceof HTMLButtonElement) {
        event.preventDefault();
        event.stopPropagation();

        const codeBlock = copyButton.closest("[data-sofia-code-block]");
        const code = codeBlock?.querySelector("code");
        void handleCodeBlockCopy(copyButton, code?.textContent ?? "");
        return;
      }

      const chevron = event.target.closest("[data-sofia-link-chevron]");
      if (chevron instanceof HTMLElement) {
        event.preventDefault();
        event.stopPropagation();
        const href = chevron.dataset.sofiaLinkChevron ?? "";
      ...
      const link = event.target.closest("a[data-sofia-link-href]");
      if (link instanceof HTMLAnchorElement) {
        const href = link.dataset.sofiaLinkHref ?? link.getAttribute("href") ?? "";
        const target = openTargetForHref(href, openTargets);
      ...
      const preview = event.target.closest("[data-sofia-image-preview]");
