...
import { SofiaAnthropicToolSchema } from "./sofia-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await SofiaAnthropicToolSchema();
...
describe("SofiaAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./sofia-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["SofiaAnthropicToolSchema"]);
  });
});
