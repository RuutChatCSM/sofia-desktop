...
describe("Sofia App capabilities knowledge plugin", () => {
  test("injects current Sofia App Connect guidance", async () => {
    ...
    expect(knowledge).not.toContain("First call `sofia-cloud_search_capabilities`");
    expect(knowledge).not.toContain("then call `sofia-cloud_execute_capability`");
  ...
  test("retrieves Slack connection guidance from bundled docs", async () => {
    process.env.SOFIA_DOCS_DIR = resolve(import.meta.dir, "../../../../packages/docs");

    const plugin = await SofiaCapabilitiesKnowledge();
    const search = await plugin.tool.sofia_docs_search.execute({ query: "how can i connect slack", limit: 3 });
    ...
    const read = await plugin.tool.sofia_docs_read.execute({
  ...
  test("retrieves the Connect-first member flow from bundled docs", async () => {
    ...
    const plugin = await SofiaCapabilitiesKnowledge();
    const search = await plugin.tool.sofia_docs_search.execute({ query: "connect gmail calendar slack", limit: 3 });
    ...
    expect(read).toContain("Settings` > `Sofia Connect");
  ...
  test("does not expose the retired local skill import guide", async () => {
    ...
    const plugin = await SofiaCapabilitiesKnowledge();
    const search = await plugin.tool.sofia_docs_search.execute({ query: "import a skill", limit: 10 });
  ...
  test("reads current Cloud MCP endpoint and proxy guidance from bundled docs", async () => {
    ...
    expect(read).toContain("opencode mcp logout sofia");
    expect(read).toContain("codex mcp logout sofia");
