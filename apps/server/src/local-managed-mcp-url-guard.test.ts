...
const previousPrivateOverride = process.env.SOFIA_ALLOW_PRIVATE_MCP_URLS;

afterEach(() => {
  if (previousDevMode === undefined) delete process.env.SOFIA_DEV_MODE;
  else process.env.SOFIA_DEV_MODE = previousDevMode;
  if (previousPrivateOverride === undefined) delete process.env.SOFIA_ALLOW_PRIVATE_MCP_URLS;
  else process.env.SOFIA_ALLOW_PRIVATE_MCP_URLS = previousPrivateOverride;
});
