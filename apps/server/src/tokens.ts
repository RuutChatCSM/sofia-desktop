...
import { sofiaServerConfigDir } from "@sofia/paths";
...
function resolveTokenStorePath(config: ServerConfig): string {
  const override = (process.env.SOFIA_TOKEN_STORE ?? "").trim();
  if (override) return resolve(override);

  const configPath = config.configPath?.trim();
  const configDir = configPath ? dirname(configPath) : sofiaServerConfigDir();
