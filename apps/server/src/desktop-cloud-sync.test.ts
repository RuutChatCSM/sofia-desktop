...
describe("desktop cloud sync", () => {
  test("queues provider update and remove changes", () => {
    ...
    const result = syncDesktopCloudResources({ now: 1780442400000, sofia, snapshot: baseSnapshot });
    ...
    const state = readDesktopCloudSyncState(result.sofia);
  ...
  test("syncs large provider snapshots within an interactive budget", () => {
    ...
    const sofia = { cloudImports: { providers: importedProviders } };

    const start = performance.now();
    const result = syncDesktopCloudResources({ now: 1780442400000, sofia, snapshot });
