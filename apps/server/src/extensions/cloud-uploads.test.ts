...
import { callSofiaCloudUploadAction } from "./cloud-uploads.js";
...
async function tempRoot() {
  const root = join(tmpdir(), `sofia-cloud-uploads-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  roots.push(root);
...
test("drive upload sends exact workspace bytes and server-derived Office metadata", async () => {
  ...
  const result = await callSofiaCloudUploadAction(
    testConfig(root),
    "drive_upload_file",
  ...
  expect(capturedUrl).toBe("https://api.sofia.test/v1/direct-uploads/google-workspace/drive-files");
...
test("direct upload rejects files above the deployed 4 MiB transport ceiling before network I/O", async () => {
  ...
  await expect(callSofiaCloudUploadAction(
    testConfig(root),
    "drive_upload_file",
    { path: "too-large.bin" },
    { directory: root },
