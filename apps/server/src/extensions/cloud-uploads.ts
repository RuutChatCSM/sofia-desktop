...
export const SOFIA_CLOUD_UPLOADS_EXTENSION_ID = "sofia-cloud-uploads";
...
export const SOFIA_CLOUD_UPLOAD_ACTIONS = [
  {
...
async function cloudUploadEndpoint(config: ServerConfig, suffix: string, dependencies: CloudUploadDependencies) {
  ...
  if (!endpoint || !authorization) {
    throw new ApiError(409, "cloud_not_connected", "Sofia Cloud must be connected before uploading files.");
  }
  let url: URL;
  try {
    url = new URL(endpoint);
  } catch {
    throw new ApiError(409, "cloud_endpoint_invalid", "The configured Sofia Cloud endpoint is invalid.");
  }
  const mcpSuffix = "/mcp/agent";
  if (!url.pathname.replace(/\/+$/, "").endsWith(mcpSuffix)) {
    throw new ApiError(409, "cloud_endpoint_invalid", "The configured Sofia Cloud endpoint must end in /mcp/agent.");
  }
...
async function postDirectUpload(
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status || 502, "cloud_upload_failed", `Sofia Cloud could not upload the file: ${message}`);
  }
  return payload;
}
...
export async function callSofiaCloudUploadAction(
  config: ServerConfig,
  action: string,
  args: Record<string, unknown>,
  context: Record<string, unknown>,
