...
function createTestConfig(): ServerConfig {
  const tempDir = join(
    tmpdir(),
    `sofia-google-workspace-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  return {
    host: "127.0.0.1",
    port: 8787,
...
const previousEnv = {
  devMode: process.env.SOFIA_DEV_MODE,
  plaintextVault: process.env.SOFIA_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT,
  clientSecret: process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  legacyClientSecret: process.env.SOFIA_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  brokerUrl: process.env.SOFIA_GOOGLE_WORKSPACE_TOKEN_BROKER_URL,
};
...
afterEach(() => {
  restoreEnv("SOFIA_DEV_MODE", previousEnv.devMode);
  restoreEnv("SOFIA_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT", previousEnv.plaintextVault);
  restoreEnv("GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.clientSecret);
  restoreEnv("SOFIA_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.legacyClientSecret);
  restoreEnv("SOFIA_GOOGLE_WORKSPACE_TOKEN_BROKER_URL", previousEnv.brokerUrl);
...
describe("Google Workspace extension", () => {
  test("reports only the user-configurable OAuth secret as missing", async () => {
    ...
    process.env.SOFIA_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "";
    process.env.SOFIA_GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
  ...
  test("reads multi-account vaults and exposes active account", async () => {
    ...
    process.env.SOFIA_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT = "1";
