...
const INITIALIZE_REQUEST_ID = "sofia-agent-diagnostics-initialize";
...
/**
 * Cached engine-side evidence for the exact managed Sofia Cloud entry.
...
export type ProbeSofiaCloudCatalogInput = {
  workspaceId: string;
...
function configuredTrustedOrigins(activatedEnterpriseOrigin?: string | null): Set<string> {
  ...
  const configured = process.env.SOFIA_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS ?? "";
...
/**
 * Eligibility is deliberately independent of cached Sofia engine registration
 ...
 */
function prepare(input: ProbeSofiaCloudCatalogInput): PreparedProbe | CloudCatalogProbeCode {
 ...
 */
export async function probeSofiaCloudCatalog(
  input: ProbeSofiaCloudCatalogInput,
): Promise<CloudCatalogProbe> {
