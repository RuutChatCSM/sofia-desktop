#!/usr/bin/env node
// OpenWork in-app browser harness for the codex engine. Exposes the browser
...
// through the same OpenWork CDP broker URL.
//
...
const BROKER_URL = process.env.OPENWORK_BROWSER_CDP_URL?.trim() || "";
if (!BROKER_URL) {
  console.error("[openwork-repl] OPENWORK_BROWSER_CDP_URL not set");
  process.exit(1);
}
...
function isOpenWork(url) { return typeof url === "string" && url.includes("localhost:5173"); }

// --- Stealth: hide CDP/Electron automation fingerprints ------------------------
// OpenWork's browser is a real Electron <webview>, but the User-Agent leaks
// "Electron" / "OpenWork-Dev", which bot detectors (e.g. timesdaily) flag. We
...
function makeTab(tabEntry) {
  ...
  const tab = {
    ...
    botDetection: {
      get: async () => { const s = await ensure(); const r = await s("Runtime.evaluate", { expression: stealthCheckObj(), returnByValue: true }); return { detected: (r?.result?.value?.webdriver ?? false) === true || /Electron|OpenWork-Dev/.test(String(r?.result?.value?.ua ?? "")), fingerprint: r?.result?.value ?? null }; },
...
function makeBrowser(browserId) {  const browser = {
    id: browserId,
    tabs: {
      list: async () => { const list = await jsonList(); return list.filter((t) => t.type === "page" && !isOpenWork(t.url)).map((t) => makeTab({ id: t.id, targetId: t.id, url: t.url, title: t.title })); },
...
async function handleRequest(id, method, params) {
  switch (method) {
    case "initialize":
      return reply(id, { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "openwork-browser-repl", version: "1.0.0" } });
...
function jsTool() {
  return {
    name: "js",
    description: `Drive the visible, signed-in OpenWork browser by running JavaScript. Initialize with setupBrowserRuntime(), read browser.documentation() before first use, inspect the latest page state with tab.see() or tab.ax.get(), then act by index/text/role. Re-inspect after navigation or state-changing actions before deciding what to do next. Pattern:
