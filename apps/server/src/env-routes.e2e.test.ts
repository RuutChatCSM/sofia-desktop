...
const dirs: string[] = [];
const priorEnvStore = process.env.SOFIA_ENV_STORE;
const priorTokenStore = process.env.SOFIA_TOKEN_STORE;
const priorOpenAiApiKey = process.env.OPENAI_API_KEY;
const priorSofiaApiKey = process.env.SOFIA_API_KEY;
const priorSofiaInferenceBaseUrl = process.env.SOFIA_INFERENCE_BASE_URL;
...
beforeEach(() => {
  const dir = mkdtempSync(join(tmpdir(), "sofia-env-routes-"));
  dirs.push(dir);
  // Redirect the shared env.json path into a throwaway dir so the test never
  // touches the developer's real ~/.config/sofia/env.json.
  process.env.SOFIA_ENV_STORE = join(dir, "env.json");
  process.env.SOFIA_TOKEN_STORE = join(dir, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorEnvStore === undefined) {
    delete process.env.SOFIA_ENV_STORE;
  } else {
    process.env.SOFIA_ENV_STORE = priorEnvStore;
  }
  ...
  if (priorSofiaApiKey === undefined) {
    delete process.env.SOFIA_API_KEY;
  } else {
    process.env.SOFIA_API_KEY = priorSofiaApiKey;
  }
  if (priorSofiaInferenceBaseUrl === undefined) {
    delete process.env.SOFIA_INFERENCE_BASE_URL;
  } else {
    process.env.SOFIA_INFERENCE_BASE_URL = priorSofiaInferenceBaseUrl;
  }
...
describe("env routes", () => {
  ...
  test("invalid env store returns 409 instead of overwriting on PUT", async () => {
    writeFileSync(process.env.SOFIA_ENV_STORE!, "{ this is not json");
    const { base } = await boot();
  ...
  test("PUT rejects reserved keys with 400", async () => {
    ...
    const put = await fetch(`${base}/env`, {
      method: "PUT",
      headers: hostAuth(),
      body: JSON.stringify({ key: "SOFIA_TOKEN", value: "x" }),
    });
    expect(put.status).toBe(400);
    const body = (await put.json()) as { code: string; message: string };
  ...
  test("voice realtime session prefers Sofia App Models broker when configured", async () => {
    ...
    const envPut = await fetch(`${base}/env`, {
      ...
      body: JSON.stringify({
        entries: [
          { key: "SOFIA_API_KEY", value: "ow_inf_test" },
          { key: "SOFIA_INFERENCE_BASE_URL", value: "https://inference.example.test" },
        ],
      }),
    });
  ...
  test("voice realtime session shows clear error when broker 503 and no local key", async () => {
    ...
    delete process.env.SOFIA_OPENAI_REALTIME_API_KEY;
    const { base } = await boot();
    ...
    expect(body.code).toBe("sofia_models_voice_unavailable");
  ...
  test("voice realtime session does not fall back on non-503 broker errors", async () => {
    ...
    expect(body.code).toBe("sofia_models_voice_failed");
  });
