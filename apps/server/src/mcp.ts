...
const SOFIA_CLOUD_DIAGNOSTIC_TOOL_IDS = [
...
function diagnosticToolIdsForMcp(name: string): string[] {
  return name === "sofia-cloud" ? SOFIA_CLOUD_DIAGNOSTIC_TOOL_IDS : [];
}
...
export async function diagnoseMcpToolDenies(
  ...
  // Sofia owns the engine config: there is no workspace file layer to read.
  return diagnoseMcpToolDeniesFromConfigs({ projectConfig: {}, globalConfig: {}, name, toolIds });
}
...
function deniedToolIds(
  ...
  // Sofia engine merges the deprecated top-level `tools` map and the current
...
function isMcpDisabledByTools(config: Record<string, unknown>, name: string): boolean {
  ...
  return deniedToolIds([config], "", [`${sanitizedName}___sofia_mcp_probe__`]).length > 0;
}
...
export async function listMcpFromRuntimeSnapshot(
  ...
  return Object.entries(runtimeMcpMap(runtimeConfig)).map(([name, config]) => ({
    name,
    config,
...
/**
 * Diagnostics-only passive inventory. Sofia owns the engine config, so the
 * runtime entries are the only layer there is to report.
 */
export async function inspectMcpLayersFromRuntimeSnapshot(
  ...
  const items: McpItem[] = Object.entries(runtimeMcpMap(runtimeConfig)).map(([name, config]) => ({
    name,
    config,
    source: "config.remote" as const,
  }));
  return {
    items,
    layerStatus: { project: "missing", global: "missing" },
    toolPolicy: {
      scope: "passive-static-subset",
      status: "unavailable",
      inspectedToolIds: [],
...
// not apply": missing, non-object, or malformed enough that Sofia engine would
