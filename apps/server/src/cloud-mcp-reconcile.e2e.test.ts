 ...
 */
describe("openwork-cloud MCP desired config", () => {
  test("rejects malformed desired config without persisting it", async () => {
    const root = await createRoot();
    const openwork = await startOpenwork([workspace("ws_1", root)]);
  ...
  test("GET health reports persisted malformed desired config even when an engine is configured", async () => {
    const root = await createRoot();
    const openwork = await startOpenwork([workspace("ws_1", root, { baseUrl: "http://127.0.0.1:1" })]);
