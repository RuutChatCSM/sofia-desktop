...
async function createTempRoot() {
  const root = await mkdtemp(join(tmpdir(), "sofia-reload-guard-"));
  roots.push(root);
  previousRuntimeDb = process.env.SOFIA_RUNTIME_DB;
  process.env.SOFIA_RUNTIME_DB = join(root, "runtime.sqlite");
  previousEnvStore = process.env.SOFIA_ENV_STORE;
...
afterEach(async () => {
  ...
  if (previousDisposeTimeout === undefined) delete process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS;
  else process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS = previousDisposeTimeout;
  if (previousReloadRetry === undefined) delete process.env.SOFIA_ENGINE_RELOAD_RETRY_MS;
  else process.env.SOFIA_ENGINE_RELOAD_RETRY_MS = previousReloadRetry;
});
...
describe("engine reload guard", () => {
  ...
  test("a reload failure mid-pass keeps the materialized providers visible and the failure loud", async () => {
    previousDisposeTimeout = process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS;
    process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS = "500";
    const root = await createTempRoot();
    const engine = startFakeEngine();
    const config = serverConfig(root, `http://127.0.0.1:${engine.port}`);
    const server = await startServer(config);
  ...
  test("a failed reload self-heals through the retry poll and settles the status", async () => {
    previousDisposeTimeout = process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS;
    process.env.SOFIA_ENGINE_DISPOSE_TIMEOUT_MS = "500";
    previousReloadRetry = process.env.SOFIA_ENGINE_RELOAD_RETRY_MS;
    process.env.SOFIA_ENGINE_RELOAD_RETRY_MS = "200";
    const root = await createTempRoot();
    const engine = startFakeEngine();
    const config = serverConfig(root, `http://127.0.0.1:${engine.port}`);
    const server = await startServer(config);
