...
import {
  inspectRuntimeWorkspaceEngineConfig,
...
async function withRuntimePath(
  ...
  const root = await mkdtemp(join(tmpdir(), "sofia-passive-runtime-inspection-"));
...
describe("passive runtime Sofia engine config inspection", () => {
  test("does not create a missing runtime database or state directory", async () => {
    await withRuntimePath(async ({ stateDir, dbPath, config }) => {
      ...
      expect(await inspectRuntimeWorkspaceEngineConfig(config, WORKSPACE_ID)).toEqual({});
  ...
  test("reads an existing row without changing directory contents, bytes, or mtimes", async () => {
    await withRuntimePath(async ({ stateDir, dbPath, config }) => {
      ...
      sqlite.query("INSERT INTO runtime_engine_configs (workspace_id, config_json, updated_at) VALUES (?, ?, ?)")
        .run(WORKSPACE_ID, JSON.stringify({ default_agent: "sofia", plugin: ["safe-plugin"] }), 1234);
      sqlite.close();
  ...
  test("classifies malformed runtime MCP rows without exposing a partial snapshot", async () => {
    await withRuntimePath(async ({ stateDir, dbPath, config }) => {
      ...
      expect(await inspectRuntimeWorkspaceEngineConfigState(config, WORKSPACE_ID)).toEqual({
        status: "invalid-row",
        config: {},
      });
    });
