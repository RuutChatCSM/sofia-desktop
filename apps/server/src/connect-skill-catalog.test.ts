...
async function serverConfig(): Promise<ServerConfig> {
  const root = await mkdtemp(join(tmpdir(), "sofia-connect-skills-"));
  roots.push(root);
...
describe("Sofia App Connect skill catalog", () => {
  test("renders discovery metadata and capability retrieval guidance", () => {
    ...
    expect(instruction).toContain("sofia-cloud_execute_capability");
    ...
    expect(instruction).toContain("Do not call sofia-cloud_search_capabilities first");
  ...
  test("reads the skill catalog from server-scoped Connect MCP config", async () => {
    ...
    const skills = await readSofiaConnectSkillCatalog(config, skillIndexFetcher());
    expect(skills).toHaveLength(1);
  ...
  test("promotes legacy workspace sofia-cloud config into server scope", async () => {
    ...
    const skills = await readSofiaConnectSkillCatalog(config, skillIndexFetcher("skill:skill_promoted"));
  ...
  test("skips revoked or dead configs and promotes the first working candidate", async () => {
    ...
    const skills = await readSofiaConnectSkillCatalog(config, fetcher);
  ...
  test("returns empty when every candidate config is unusable", async () => {
    ...
    expect(await readSofiaConnectSkillCatalog(config, fetcher)).toEqual([]);
