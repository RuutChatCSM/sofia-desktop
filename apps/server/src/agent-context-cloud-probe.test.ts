...
import {
  ...
  type ProbeSofiaCloudCatalogInput,
...
function input(overrides: Partial<ProbeSofiaCloudCatalogInput> = {}): ProbeSofiaCloudCatalogInput {
...
function initializeResponse(extraHeaders: Record<string, string> = {}): Response {
  return new Response(JSON.stringify({
    jsonrpc: "2.0",
    id: "sofia-agent-diagnostics-initialize",
    result: {
      capabilities: {},
...
afterEach(() => {
  delete process.env.SOFIA_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS;
});

describe("Sofia Cloud catalog probe", () => {
  test("performs initialize, initialized notification, bounded tools/list, and session cleanup with allowlisted headers", async () => {
    ...
    const fetchImpl: CloudCatalogProbeFetch = async (url, init) => {
      ...
      if (isInitialize) {
        expect(body).toEqual({
          ...
          params: {
            capabilities: {},
            clientInfo: { name: "sofia-server-agent-context-diagnostics", version: "1.0.0" },
            protocolVersion: PROTOCOL_VERSION,
          },
        });
    ...
    const observed = await probeSofiaCloudCatalog(probeInput);
    expect(calls).toBe(4);
  ...
  test("accepts a finite JSON tools/list response", async () => {
    const observed = await probeSofiaCloudCatalog(input({
  ...
  test("requires both expected tools and never reflects a provider-controlled tool ID", async () => {
    ...
    const missing = await probeSofiaCloudCatalog(input({
    ...
    const reversed = await probeSofiaCloudCatalog(input({
  ...
  test("blocks remote workspaces, unavailable state, and unsafe endpoints before fetch", async () => {
    ...
    const cases: Array<[Partial<ProbeSofiaCloudCatalogInput>, CloudCatalogProbeCode]> = [
    ...
    for (const [overrides, code] of cases) {
      const blocked = await probeSofiaCloudCatalog(input({ ...overrides, fetchImpl }));
  ...
  test("allows exact loopback and explicitly configured HTTPS origins only", async () => {
    let blockedCalls = 0;
    const blocked = await probeSofiaCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
    ...
    const loopback = await probeSofiaCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
    ...
    process.env.SOFIA_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS = "https://den.customer.example";
    const trusted = await probeSofiaCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: "https://den.customer.example/custom/mcp/agent",
  ...
  test("rejects duplicate or malformed authorization without forwarding configured headers", async () => {
    ...
    const duplicate = await probeSofiaCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: ENDPOINT,
    ...
    const injected = await probeSofiaCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: ENDPOINT,
  ...
  test("rejects redirects and cancels a response that exceeds 64 KiB", async () => {
    const redirected = await probeSofiaCloudCatalog(input({
    ...
    const tooLarge = await probeSofiaCloudCatalog(input({
  ...
  test("keeps the absolute deadline active while a response body stalls", async () => {
    ...
    const timedOut = await probeSofiaCloudCatalog(input({
  ...
  test("classifies abort-aware fetch and body failures as deadline timeouts", async () => {
    const fetchTimedOut = await probeSofiaCloudCatalog(input({
    ...
    const bodyTimedOut = await probeSofiaCloudCatalog(input({
  ...
  test("does not start authenticated egress when the parent signal is already aborted", async () => {
    ...
    controller.abort();
    let calls = 0;
    const cancelled = await probeSofiaCloudCatalog(input({
  ...
  test("parent abort settles a hostile fetch that ignores its signal", async () => {
    ...
    const pending = probeSofiaCloudCatalog(input({
  ...
  test("keeps each caller cancellation-scoped and does not cache settled results", async () => {
    ...
    const first = probeSofiaCloudCatalog(input({ requestId: "single-flight-one", fetchImpl }));
    const joined = probeSofiaCloudCatalog(input({ requestId: "single-flight-two", fetchImpl }));
    ...
    const afterSettlement = await probeSofiaCloudCatalog(input({
  ...
  test("caps global active probes without sharing a caller-owned request", async () => {
    ...
    const active = Array.from({ length: 16 }, (_, index) => probeSofiaCloudCatalog(input({
    ...
    const sameFingerprintBusy = await probeSofiaCloudCatalog(input({
      workspaceId: "ws_busy_0",
      requestId: "busy-joined",
      fetchImpl,
    }));
    const busy = await probeSofiaCloudCatalog(input({
  ...
  test("returns only a safe network code when fetch throws a secret-bearing error", async () => {
    const failed = await probeSofiaCloudCatalog(input({
  ...
  test("attributes initialize-phase failures to initialize stages", async () => {
    ...
    const failed = await probeSofiaCloudCatalog(probeInput);
    expect(failed).toMatchObject({
      performed: true,
      status: "failed",
  ...
  test("splits invalid UTF-8, invalid JSON, and malformed envelopes into distinct codes", async () => {
    const utf8 = await probeSofiaCloudCatalog(input({
      requestId: "bad-utf8",
    ...
    const json = await probeSofiaCloudCatalog(input({
      requestId: "bad-json",
    ...
    const envelope = await probeSofiaCloudCatalog(input({
  ...
  test("exports a reference ID only from a strictly validated safe header", async () => {
    const safe = await probeSofiaCloudCatalog(input({
    ...
    const unsafe = await probeSofiaCloudCatalog(input({
  ...
  test("reports boolean-only proxy and extra-CA posture from the environment seam", async () => {
    const configured = await probeSofiaCloudCatalog(input({
  ...
  test("trusts the activated enterprise origin and reports the trust source", async () => {
    ...
    // so an on-prem probe must never contact Sofia-hosted Cloud.
    ...
    const activated = await probeSofiaCloudCatalog(activatedInput);
    ...
    expect(requestedUrls.some((url) => url.includes("sofia.ruut.chat"))).toBe(false);
    ...
    let mismatchCalls = 0;
    const mismatch = await probeSofiaCloudCatalog(input({
    ...
    const unactivated = await probeSofiaCloudCatalog(input({
