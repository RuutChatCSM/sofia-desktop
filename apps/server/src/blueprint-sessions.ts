...
export function normalizeBlueprintSessionTemplates(sofia: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(sofia?.blueprint);
...
export function readMaterializedBlueprintSessions(sofia: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(sofia?.blueprint);
...
export function sanitizeSofiaTemplateConfig(sofia: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(sofia ?? {});
...
export function applyMaterializedBlueprintSessions(
  sofia: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeSofiaTemplateConfig(sofia);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
