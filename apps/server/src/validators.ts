...
const RESERVED_USER_MCP_NAMES = new Set(["sofia-cloud"]);
const RESERVED_USER_MCP_PREFIXES = ["sofia-connect-"];
...
export function validateUserMcpName(name: string): void {
  ...
  if (RESERVED_USER_MCP_NAMES.has(name.toLowerCase())
    ...
    throw new ApiError(409, "reserved_mcp_name", `${name} is reserved for Sofia App Connect`);
  }
}
