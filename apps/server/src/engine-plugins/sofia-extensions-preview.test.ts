...
import { SofiaExtensionsPreview } from "./sofia-extensions-preview.js";
import * as SofiaExtensionsPreviewEntry from "./sofia-extensions-preview.js";
import {
...
async function transformedSystem(plugin: Awaited<ReturnType<typeof SofiaExtensionsPreview>>): Promise<string> {
  const output: { system: string[] } = { system: [] };
  await plugin["experimental.chat.system.transform"]({}, output);
  return output.system.join("\n");
}

function startFakeSofiaServer() {
  ...
  process.env.SOFIA_SERVER_URL = `http://127.0.0.1:${server.port}`;
  process.env.SOFIA_SERVER_TOKEN = "test-token";
  return { requests };
}

describe("SofiaExtensionsPreview MCP Apps result preservation", () => {
...
describe("SofiaExtensionsPreview session tools", () => {
  test("plugin entry exposes only the factory export for the Sofia engine loader", () => {
    expect(Object.keys(SofiaExtensionsPreviewEntry)).toEqual(["SofiaExtensionsPreview"]);
  });

  test("projects built-in, extension, and Connect providers into one agent context", async () => {
    startFakeSofiaServer();
    const plugin = await SofiaExtensionsPreview({
      client: {
        mcp: {
          status: async () => ({
    ...
    const output = await plugin.tool.sofia_context.execute();
  ...
  test("routes semantic session queries without navigating the UI", async () => {
    ...
    const output = await plugin.tool.sofia_query.execute({
      id: "session.read",
  ...
  test("merges factory directory into transform steering when hook input omits it", async () => {
    ...
    expect(output.system.join("\n")).toContain(SOFIA_CLOUD_SKILL_AUTHORING_INSTRUCTION);
    expect(output.system.join("\n")).not.toContain(SOFIA_LOCAL_SKILL_AUTHORING_INSTRUCTION);
  ...
  test("uses the factory engine client as transform steering source of truth", async () => {
    ...
    const mcp = {
      result: { data: { "sofia-cloud": { status: "connected" } } },
      async status(request: unknown) {
        requests.push(request);
        return this.result;
      },
    };
    const plugin = await SofiaExtensionsPreview({ client: { mcp }, directory: "/tmp/archive" });
    const output: { system: string[] } = { system: [] };
  ...
  test("uses neutral transform steering when the engine reports failed Cloud status", async () => {
    const requests: unknown[] = [];
    const mcp = {
      result: { data: { "sofia-cloud": { status: "failed" } } },
      async status(request: unknown) {
        requests.push(request);
        return this.result;
      },
    ...
    expect(requests).toEqual([{ query: { directory: "/tmp/archive" } }]);
    expect(output.system[0]).toBe(SOFIA_EXTENSION_DISCOVERY_INSTRUCTION);
    expect(output.system.join("\n")).toContain(SOFIA_LOCAL_SKILL_AUTHORING_INSTRUCTION);
  ...
  test("creates and starts multiple sessions through the Sofia backend", async () => {
    ...
    const output = await plugin.tool.sofia_execute.execute({
      id: "session.create",
      args: {
        sessions: [
...
describe("SofiaExtensionsPreview semantic tool surface", () => {
  test("exposes only the three semantic tools", async () => {
    const plugin = await SofiaExtensionsPreview();
    const tools = Object.keys(plugin.tool).sort();

    expect(tools).toEqual(["sofia_context", "sofia_execute", "sofia_query"]);

    const system = await transformedSystem(plugin);
    expect(system).not.toContain("## Default Skill: skill-creator");
    expect(system).not.toContain("<sofia_default_skill");
    expect(system).not.toContain("sofia_ui_");
    expect(system).not.toContain("sofia_session_");
    expect(system).not.toContain("sofia_extension_");
    expect(system).not.toContain("sofia_browser_");
    expect(system).toContain("Use sofia_context");
  ...
  test("rejects a proposal whose schedule is not a supported kind", async () => {
    ...
    await expect(plugin.tool.sofia_execute.execute({
      id: "automation.propose",
      args: {
