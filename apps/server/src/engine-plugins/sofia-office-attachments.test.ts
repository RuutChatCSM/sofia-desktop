...
import { sofiaOfficeAttachmentsPluginPath } from "../sofia-extensions-plugin-path.js";
import { SofiaOfficeAttachments } from "./sofia-office-attachments.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "sofia-office-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string, messages: unknown[]) {
  const plugin = await SofiaOfficeAttachments({ directory: root });
...
describe("SofiaOfficeAttachments", () => {
  ...
  test("rejects file URLs that resolve outside the workspace through a symlink", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "sofia-office-outside-"));
      try {
        const docx = docxFixture();
  ...
  test("uses factory directory and ignores Daytona non-git worktree root", async () => {
    await withWorkspace(async (root) => {
      const docx = docxFixture();
      const plugin = await SofiaOfficeAttachments({ directory: root, worktree: "/" });
      ...
      expect(materialized).toContain(".sofia/sofia/inbox/chat-attachments/");
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    const runtime = await buildSofiaRuntimeConfigObject();
    const plugin = runtime.plugin;
    if (!Array.isArray(plugin)) throw new Error("Expected plugin list");
    expect(plugin).toContain(sofiaOfficeAttachmentsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("sofia-office-attachments.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./sofia-office-attachments.js");
    expect(Object.keys(mod)).toEqual(["SofiaOfficeAttachments"]);
  });
});
