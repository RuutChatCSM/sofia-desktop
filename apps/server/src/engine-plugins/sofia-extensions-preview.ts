...
function normalizeEngineContext(value: unknown): EngineContext {
...
function mergeTransformInputWithFactoryContext(input: unknown, factoryContext: EngineContext): unknown {
...
function contextPayload(context: EngineContext) {
  return {
    agent: context.agent,
  ...
  "experimental.chat.system.transform": async (input: unknown, output: { system: string[] }) => {
    ...
    if (process.env.SOFIA_DEV_MODE === "1") {
      console.log("[sofia:skill-authoring] system prompt selected", {
        ...
        directory: normalizeEngineContext(mergedInput).directory ?? factoryContext.directory ?? null,
      });
    }
