...
const MATERIALIZED_DIR = join(".sofia", "sofia", "inbox", "chat-attachments");
...
function normalizeEngineContext(value: unknown): RuntimeContext {
...
function normalizedText(part: OfficeFilePart, materialized: MaterializedAttachment | null, extractedText: string, error?: string): string {
  return [
    "Sofia App normalized an Office attachment before sending this request to the model.",
...
// Single export: the Sofia engine plugin loader treats every export of a plugin
// module as a plugin factory, so helpers must stay module-private.
export const SofiaOfficeAttachments = async (factoryInput?: unknown) => {
  const factoryContext = normalizeEngineContext(factoryInput);
  return {
