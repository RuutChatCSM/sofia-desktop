import type {
  SofiaAffordanceArgument,
  SofiaAffordanceDescriptor,
  SofiaAffordanceEffects,
  SofiaProviderRef,
} from "@sofia/types/sofia-affordance";
import type {
  SofiaFeatureContribution,
  SofiaGuidanceDescriptor,
} from "@sofia/types/sofia-provider";
...
const noEffects: SofiaAffordanceEffects = {
  data: "none",
  ui: "none",
  external: false,
};
const readEffects: SofiaAffordanceEffects = {
  data: "read",
  ui: "none",
  external: false,
};
const writeEffects: SofiaAffordanceEffects = {
  data: "write",
  ui: "none",
  external: false,
};
...
function argument(
  name: string,
  type: SofiaAffordanceArgument["type"],
  required: boolean,
  description: string,
): SofiaAffordanceArgument {
...
function affordance(input: {
  ...
  title: string;
  description: string;
  provider: SofiaProviderRef;
  arguments?: SofiaAffordanceArgument[];
...
function sessionContribution(): SofiaFeatureContribution {
  const provider: SofiaProviderRef = { id: "sofia-server", kind: "builtin" };
  return {
    featureId: "sessions",
    provider,
    affordances: [
...
function automationContribution(): SofiaFeatureContribution {
  const provider: SofiaProviderRef = { id: "sofia-automations", kind: "builtin" };
  return {
...
function extensionContribution(): SofiaFeatureContribution {
  const provider: SofiaProviderRef = { id: "sofia-extensions", kind: "extension" };
  return {
    ...
    affordances: [
      affordance({
        ...
        description: "List actions exposed by enabled local Sofia App extensions.",
        provider,
      ...
      affordance({
        ...
        description: "Execute one action exposed by a local Sofia App extension.",
        provider,
        arguments: [
...
): SofiaFeatureContribution | null {
  if (skills.length === 0 && !cloudMcp) return null;
  const provider: SofiaProviderRef = { id: "sofia-cloud", kind: "connect" };
  const guidance: SofiaGuidanceDescriptor[] = skills.map((skill) => ({
  ...
  return {
    ...
    affordances: [
      affordance({
        ...
        tool: "sofia-cloud_search_capabilities",
      }),
      affordance({
...
function mcpContribution(mcp: EngineMcpDescriptor): SofiaFeatureContribution {
  return {
...
export function buildSofiaProviderContributions(
  skills: ConnectSkillDescriptor[],
  mcps: EngineMcpDescriptor[] = [],
): SofiaFeatureContribution[] {
  const cloudMcp = mcps.find((mcp) => mcp.name === "sofia-cloud");
  ...
  return [
    ...
    ...mcps
      .filter((mcp) => mcp.name !== "sofia-cloud")
      .map(mcpContribution),
    ...(connect ? [connect] : []),
  ];
}
