 ...
 * bundled Sofia engine forwards them unchanged, so a single bad tool breaks
 * every chat turn for the whole session.
 *
 * No Sofia engine plugin hook can rewrite MCP tool schemas before dispatch, so
...
export const SofiaAnthropicToolSchema = async () => {
