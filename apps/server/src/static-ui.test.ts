...
const WEB_BOOTSTRAP_TOKEN_ENV = "SOFIA_WEB_BOOTSTRAP_TOKEN";

async function createWebRoot() {
  const root = join(tmpdir(), `sofia-static-ui-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  await mkdir(join(root, "assets"), { recursive: true });
  await writeFile(join(root, "index.html"), "<html><head><title>Sofia App</title></head><body>App shell</body></html>");
...
async function withWebRoot(root: string | null, run: () => Promise<void>) {
  const previous = process.env.SOFIA_WEB_ROOT;
  if (root) {
    process.env.SOFIA_WEB_ROOT = root;
  } else {
    delete process.env.SOFIA_WEB_ROOT;
  }
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SOFIA_WEB_ROOT = previous;
    }
  }
}
...
describe("serveStaticUi", () => {
  test("is a no-op when SOFIA_WEB_ROOT is unset", async () => {
    await withWebRoot(null, async () => {
      const response = await serveStaticUi(new Request("http://sofia.test/"), staticConfig());
  ...
  test("rejects path traversal", async () => {
    ...
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://sofia.test/safe%2F..%2F..%2Fsecret.txt"), staticConfig());
  ...
  test("serves configured MIME types", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const js = await serveStaticUi(new Request("http://sofia.test/assets/app.123.js"), staticConfig());
      const css = await serveStaticUi(new Request("http://sofia.test/assets/style.123.css"), staticConfig());
      const json = await serveStaticUi(new Request("http://sofia.test/data.json"), staticConfig());
      const svg = await serveStaticUi(new Request("http://sofia.test/icon.svg"), staticConfig());
  ...
  test("uses immutable cache headers for assets", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://sofia.test/assets/app.123.js"), staticConfig());
  ...
  test("falls back to index.html for SPA routes", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://sofia.test/settings/general"), staticConfig(""));
  ...
  test("serves overlay.html directly without using it as the fallback", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const overlay = await serveStaticUi(new Request("http://sofia.test/overlay.html"), staticConfig());
      const fallback = await serveStaticUi(new Request("http://sofia.test/missing-route"), staticConfig(""));
  ...
  test("injects escaped bootstrap JSON into index.html", async () => {
    ...
    await withBootstrapTokenEnv(null, async () => {
      await withWebRoot(root, async () => {
        const response = await serveStaticUi(new Request("http://sofia.test/"), staticConfig("tok<\u2028>\u2029&"));
        if (!response) throw new Error("expected index response");
        const body = await response.text();
        expect(body).toContain("<script>window.__SOFIA_BOOTSTRAP__ = {\"token\":\"tok\\u003c\\u2028\\u003e\\u2029\\u0026\"}</script></head>");
  ...
  test("can disable bootstrap token injection", async () => {
    ...
    for (const value of ["0", "false"]) {
      await withBootstrapTokenEnv(value, async () => {
        await withWebRoot(root, async () => {
          const response = await serveStaticUi(new Request("http://sofia.test/"), staticConfig("client-token"));
          if (!response) throw new Error("expected index response");
          const body = await response.text();
          expect(body).toContain("App shell");
          expect(body).not.toContain("__SOFIA_BOOTSTRAP__");
        });
      });
    }
  });
