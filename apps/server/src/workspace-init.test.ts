...
import { sofiaExtensionsPreviewPluginPath, sofiaPluginPath } from "./sofia-extensions-plugin-path.js";

async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "sofia-workspace-init-"));
  try {
    await fn(root);
  } finally {
...
describe("ensureWorkspaceFiles", () => {
  test("does not write an sofia.json file (config is DB-backed now)", async () => {
    await withWorkspace(async (root) => {
      const result = await ensureWorkspaceFiles(root, "starter");
      // sofia config no longer lands on disk; it is seeded into the runtime
  ...
  test("defaultWorkspaceSofiaConfig carries authorizedRoots + workspace metadata", async () => {
    await withWorkspace(async (root) => {
      const config = defaultWorkspaceSofiaConfig(root, "starter");
  ...
  test("does not create workspace extension preview plugin", async () => {
    await withWorkspace(async (root) => {
      ...
      await expect(stat(join(root, ".sofia", "plugins", "sofia-extensions-preview.ts"))).rejects.toThrow();
    });
  });
...
describe("ensureLocalWorkspaceFiles", () => {
  ...
  test("provisions local workspaces and skips remote ones", async () => {
    await withWorkspace(async (root) => {
      ...
      // No sofia.json file is written; provisioning does not crash on the
