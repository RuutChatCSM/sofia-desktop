...
Read-only admin analytics MCP server for the Sofia Den database. Ask Sofia
...
> connect "Sofia Admin Analytics" from Settings -> Connections -> MCP
...
GRANT SELECT ON sofia_den.* TO 'den_readonly'@'%';
```
...
```jsonc
{
  "mcp": {
    "den-admin": {
      ...
      "environment": { "DATABASE_URL": "mysql://den_readonly:...@host:3306/sofia_den" },
      "enabled": true
    }
  }
}
...
pnpm --filter @sofia-ee/den-admin-mcp test
```
