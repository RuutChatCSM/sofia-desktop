...
describe("Den DB migration readiness wiring", () => {
  ...
  test("Helm migration defaults execute the precompiled dist runner", () => {
    const values = readRepoFile("packaging/helm/sofia-ee/values.yaml")
  ...
  test("Dockerfile.den builds Den DB dist assets before the Den API image build", () => {
    ...
    assert.notEqual(denDbBuildIndex, -1, "Dockerfile.den builds @sofia-ee/den-db")
    assert.notEqual(denApiBuildIndex, -1, "Dockerfile.den builds @sofia-ee/den-api")
  ...
  test("hosted Den API build includes den-db assets but start does not run migrations", () => {
    ...
    assert.match(denApiPackage, /"build:den-db": "pnpm --filter @sofia-ee\/den-db build"/)
  ...
  test("hosted migration workflow remains the explicit PlanetScale migration owner", () => {
    ...
    assert.match(workflow, /run_with_ddl_retry pnpm --filter @sofia-ee\/den-db db:migrate/)
  ...
  test("PR guardrails run readiness tests and smoke Den DB assets in the Den API image", () => {
    ...
    assert.match(checkWorkflow, /pnpm --filter @sofia-ee\/den-db test/)
    ...
    assert.match(checkWorkflow, /"packaging\/helm\/sofia-ee\/templates\/migration-job\.yaml"/)
