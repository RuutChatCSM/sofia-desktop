...
process.env.DATABASE_URL = "mysql://root:password@127.0.0.1:3306/sofia_den"
...
function inferenceRequest(input: { method: string; headers: Headers; body?: string; path?: string }) {
  return new Request(`http://sofia.test${input.path ?? "/api/v1/chat/completions"}`, {
    method: input.method,
...
test("rewrites approved model aliases before forwarding JSON requests", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    ...
    body: JSON.stringify({ model: "sofia/z-ai/glm-5.2", messages: [] }),
  }))
  ...
  assert.equal(body.session_id, upstream.headers.get("x-sofia-request-id"))
  ...
  assert.equal(report.sofiaRequestId, upstream.headers.get("x-sofia-request-id"))
  assert.equal(report.route, "/api/v1/chat/completions")
  assert.equal(report.method, "POST")
  assert.equal(report.incomingModel, "sofia/z-ai/glm-5.2")
...
test("returns model_not_found for unknown JSON model aliases", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    method: "POST",
    headers: authHeaders("application/json"),
    body: JSON.stringify({ model: "sofia/unknown-model", messages: [] }),
  }))
  ...
  assert.equal(upstreamRequests.length, 0)
  const report = requireRequestReport(reports)
  assert.equal(report.incomingModel, "sofia/unknown-model")
...
test("returns usage-limit 429 without reporting a handled error or contacting provider/upstream", async () => {
  ...
  try {
    ...
    assert.equal(response.headers.get("x-sofia-limit-bucket-id"), "bucket_123")
    assert.equal(response.headers.get("x-sofia-limit-window-type"), "monthly")
...
test("reports handled upstream errors with searchable request context", async () => {
  ...
  assert.equal(errorReport.sofiaRequestId, requestReport.sofiaRequestId)
...
test("accepts application/*+json media types", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    method: "POST",
    headers: authHeaders("application/vnd.sofia.request+json; charset=utf-8"),
    body: JSON.stringify({ model: "sofia/z-ai/glm-5.2", messages: [] }),
  }))
...
test("returns the authenticated local model catalog without forwarding", async () => {
  ...
  assert.ok(!model.id.startsWith("sofia/"))
  assert.equal(calls.findActiveInferenceKey, 1)
  assert.equal(calls.ensureUsableBuckets, 0)
  assert.equal(calls.getOpenRouterProviderKey, 0)
  assert.equal(upstreamRequests.length, 0)
