import { and, eq, sql } from "@sofia-ee/den-db/drizzle"
import type { Hono } from "hono"
import { InferenceKeyTable, InferenceUsageLedgerBucketChargeTable, InferenceUsageLedgerEntryTable, InferenceOrgUsageBucketTable } from "@sofia-ee/den-db"
import { createDenTypeId, normalizeDenTypeId } from "@sofia-ee/utils/typeid"
import type { DenTypeId } from "@sofia-ee/utils/typeid"
import { INFERENCE_USAGE_CONVERSION_FACTOR } from "@sofia/types/den/inference"
...
type WebhookDependencies = {
  ...
  findOpenRouterUsageLedgerEntry(sofiaRequestId: string): Promise<UsageLedgerEntryRef | null>
...
function parseSpan(span: JsonRecord, resourceAttrs: JsonRecord, scopeAttrs: JsonRecord): ParsedSpan | null {
  ...
  const sofiaRequestId = stringAttr(attrs, ["trace.metadata.sofia_request_id", "trace.sofia_request_id", "metadata.sofia_request_id", "sofia_request_id", "trace_id"])
  ...
  if (!orgMembershipId || !inferenceKeyId || !sofiaRequestId || !reportedModel || inputCost === null || outputCost === null) {
    return null
  }
  ...
  unknownModel(report) {
    Sentry.captureMessage("OpenRouter usage webhook could not infer cost for reported model", {
      ...
      tags: {
        ...
        sofia_request_id: report.sofiaRequestId,
...
const defaultWebhookDependencies: WebhookDependencies = {
  ...
  async findOpenRouterUsageLedgerEntry(sofiaRequestId) {
    const [existing] = await db.select({ id: InferenceUsageLedgerEntryTable.id }).from(InferenceUsageLedgerEntryTable)
      .where(and(eq(InferenceUsageLedgerEntryTable.external_job_id, sofiaRequestId), eq(InferenceUsageLedgerEntryTable.event_type, "openrouter_usage"))).limit(1)
  ...
  async insertOpenRouterUsageLedgerEntry(input) {
    ...
    await db.insert(InferenceUsageLedgerEntryTable).values({
      ...
      external_job_id: input.span.sofiaRequestId,
...
function reportUnknownPricedModel(input: { span: ParsedSpan; inferenceKey: WebhookInferenceKey; reporter: OpenRouterUsageWebhookReporter }) {
  logWebhookError("skipped span for unknown priced model", {
    ...
    sofiaRequestId: input.span.sofiaRequestId,
...
async function ingestSpan(span: ParsedSpan, dependencies: WebhookDependencies) {
  ...
  const existing = await dependencies.findOpenRouterUsageLedgerEntry(span.sofiaRequestId)
