...
Sofia Diagnostics is a deliberately small, Vercel-native MCP compatibility
...
record of metadata-only Sofia Connect incidents. It correlates what a
...
NetworkPolicy path. Sofia support can filter the dashboard by the resulting
...
The same editor is visible on the Sofia Connect sign-in surface while signed
out. Run the standalone local MCP smoke journey with:

```bash
pnpm --filter @sofia-ee/diagnostics smoke:debug-proxy
```
...
DEN_DIAGNOSTICS_BEARER_TOKEN=SofiaDiagnosticsToken!
```
...
shared `@sofia/types` package. Link an Upstash Redis database from the
...
Attach `sofia-diagnostic.ruut.chat` in the project's Vercel **Domains**
...
```text
https://sofia-diagnostic.ruut.chat/mcp
```
...
mint its short-lived MCP token, and manage the hidden `sofia-cloud` MCP
entry automatically.
...
one-time `sofia://den-auth` handoff also has its encoded Den base URL
...
```bash
pnpm --filter @sofia-ee/diagnostics test
pnpm --filter @sofia-ee/diagnostics build
pnpm --filter @sofia-ee/diagnostics smoke:debug-proxy
```
...
1. `GET https://sofia-diagnostic.ruut.chat/health` returns HTTP 200 and
   `{"service":"sofia-diagnostics","status":"ok"}`.
...
- app, Sofia server, engine, and platform versions;
...
HTTP reached Sofia. If a row exists, its response status and next missing
...
returns it to the browser. Den uses `https://sofia-diagnostic.ruut.chat` by
...
```dotenv
DEN_DIAGNOSTICS_ORIGIN=https://sofia-diagnostic.ruut.chat
