...
declare global {
  var __sofiaConnectDebugProxyFlakyWindows: Map<string, FlakyWindow> | undefined
}

const flakyWindows = globalThis.__sofiaConnectDebugProxyFlakyWindows ??= new Map()
...
export function connectDebugProxyFault(input: {
  ...
  if (input.scenario.fault === "auth-expired") {
    return {
      kind: "response",
      label: "auth-expired",
      response: jsonError(401, "invalid_token", "The debug scenario expired the Sofia Connect token.", {
        "www-authenticate": "Bearer error=\"invalid_token\", error_description=\"Sofia Connect debug token expired\"",
      }),
    }
  }
