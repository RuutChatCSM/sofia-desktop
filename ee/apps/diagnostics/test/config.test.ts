...
function configureHostedEnvironment(): void {
  ...
  process.env.NEXT_PUBLIC_DIAGNOSTICS_ORIGIN = "https://sofia-diagnostic.ruut.chat"
...
describe("Diagnostics deployment configuration", () => {
  ...
  test("uses the deployment-specific Vercel URL for previews", () => {
    ...
    process.env.VERCEL_URL = "sofia-diagnostics-git-feature.vercel.app"

    expect(validateProductionConfig()).toEqual([])
    expect(diagnosticsConfig().publicOrigin).toBe("https://sofia-diagnostics-git-feature.vercel.app")
  })

  test("fails closed when a preview deployment URL is malformed", () => {
    configureHostedEnvironment()
    process.env.VERCEL_ENV = "preview"
    process.env.VERCEL_URL = "sofia-diagnostics.vercel.app/not-a-root"
