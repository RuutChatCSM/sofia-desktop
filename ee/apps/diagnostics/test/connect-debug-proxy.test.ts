...
describe("Connect debug proxy rewriting and streaming", () => {
  test("rewrites upstream Location, absolute JSON URLs, and cookie scope", async () => {
    const upstreamUrl = new URL("https://sofia-api.ruut.chat/v1/session")
    ...
    expect(rewriteSetCookieHeader("session=value; Domain=sofia-api.ruut.chat; Path=/; Secure; HttpOnly", "/via/default/access-key"))
    ...
    const response = await buildConnectDebugProxyResponse({
      proxyBase,
      tamperMode: null,
      upstream: Response.json({ redirect: "https://sofia-api.ruut.chat/next" }),
  ...
  test("keeps the desktop auth handoff on the selected proxy base", async () => {
    ...
    expect(rewriteLocationHeader(deepLink, upstreamUrl, proxyBase)).toBe(
      `sofia://den-auth?grant=one-time-grant&denBaseUrl=${encodeURIComponent(`${proxyBase}/api/den`)}`,
    )

    const response = await buildConnectDebugProxyResponse({
      proxyBase,
      tamperMode: null,
      upstream: Response.json({ sofiaUrl: deepLink }),
      upstreamRequestUrl: upstreamUrl,
    })
    const payload = await response.json() as { sofiaUrl: string }
    expect(new URL(payload.sofiaUrl).searchParams.get("denBaseUrl")).toBe(`${proxyBase}/api/den`)
  })
  ...
  test("keeps the proxy key private while translating same-origin browser headers", () => {
    const headers = forwardRequestHeaders(new Headers({
      ...
      upstreamOrigin: "https://sofia-api.ruut.chat",
    })
    expect(headers.has("x-connect-debug-proxy-key")).toBe(false)
    expect(headers.get("origin")).toBe("https://sofia-api.ruut.chat")
    expect(headers.get("referer")).toBe("https://sofia-api.ruut.chat/login?next=1")
  ...
  test("passes SSE through without waiting for the stream to finish", async () => {
    ...
    const response = await buildConnectDebugProxyResponse({
      ...
      upstreamRequestUrl: new URL("https://sofia-api.ruut.chat/mcp/agent"),
    })
  ...
  test("rewrites chunked JSON without buffering the complete response", async () => {
    ...
    const upstreamBody = new ReadableStream<Uint8Array>({
      async start(controller) {
        controller.enqueue(encoder.encode(`{"padding":"${"x".repeat(80)}","url":"https://sofia-api.ruut.chat`))
        await secondChunk
