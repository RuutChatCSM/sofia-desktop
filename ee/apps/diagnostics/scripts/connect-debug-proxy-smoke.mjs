...
const upstream = createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/den/v1/auth/desktop-handoff" && request.method === "POST") {
    ...
    const sofiaUrl = new URL("sofia://den-auth")
    sofiaUrl.searchParams.set("grant", "one-time-smoke-grant")
    sofiaUrl.searchParams.set("denBaseUrl", `${upstreamOrigin}/api/den`)
    return json(response, 200, { sofiaUrl: sofiaUrl.toString() })
  }
...
try {
  ...
  const routeCookie = browserCookies.find((cookie) => cookie.startsWith("sofia_connect_debug_route="))?.split(";", 1)[0]
  ...
  const handoffDenBaseUrl = new URL(handoffPayload.sofiaUrl).searchParams.get("denBaseUrl")
