...
import { DEFAULT_SOFIA_WEB_URL } from "../app/(den)/_lib/runtime-config";
...
const originalEnv = {
  ...
  DEN_WEB_SOFIA_WEB_URL: process.env.DEN_WEB_SOFIA_WEB_URL,
};
...
afterEach(() => {
  ...
  restoreEnvValue("DEN_WEB_SOFIA_WEB_URL");
});
...
describe("Web dashboard page", () => {
  ...
  test("renders the external Web button with the configured href", () => {
    ...
    expect(html).toContain(`href="${DEFAULT_SOFIA_WEB_URL}"`);
    expect(html).toContain('target="_blank"');
    expect(html).toContain('rel="noopener noreferrer"');
    expect(html).toContain("Open Sofia Web");
  });

  test("runtime config exposes the default Web URL and deployment override", async () => {
    delete process.env.DEN_API_BASE;
    delete process.env.DEN_WEB_SOFIA_WEB_URL;

    const defaultPayload: unknown = await (await GET()).json();
    expect(readStringProperty(defaultPayload, "sofiaWebUrl")).toBe(DEFAULT_SOFIA_WEB_URL);

    process.env.DEN_WEB_SOFIA_WEB_URL = "https://self-hosted.example.test";
    const overridePayload: unknown = await (await GET()).json();
    expect(readStringProperty(overridePayload, "sofiaWebUrl")).toBe("https://self-hosted.example.test");
  });
});
