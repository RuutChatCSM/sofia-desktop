...
import { buildDenFeedbackUrl, SOFIA_FEEDBACK_URL } from "../app/(den)/_lib/feedback";
...
describe("Den feedback links", () => {
  test("point to the public landing feedback form with dashboard context", () => {
    ...
    expect(url.startsWith(`${SOFIA_FEEDBACK_URL}?`)).toBe(true);
    expect(url).toContain("source=sofia-web-app");
