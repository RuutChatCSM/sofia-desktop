...
describe("mcpAuthorizationPendingDocument", () => {
  test("renders a concise accessible connection screen before provider redirect", () => {
    ...
    expect(document).toContain("Sofia Connect")
  ...
  test("keeps OAuth failures visible with the exact redirect URI", () => {
    const document = mcpAuthorizationErrorDocument({
      ...
      details: {
        ...
        redirectUri: "https://api.sofia.example/v1/mcp-connections/oauth/callback",
        clientMetadataUrl: "https://api.sofia.example/.well-known/oauth-client",
        responseJson: JSON.stringify({
          error: "mcp_oauth_configuration_required",
          callbackUrl: "https://api.sofia.example/v1/mcp-connections/oauth/callback",
        }, null, 2),
      },
    })
    ...
    expect(document).toContain("https://api.sofia.example/v1/mcp-connections/oauth/callback")
