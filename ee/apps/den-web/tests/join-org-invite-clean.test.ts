...
describe("join organization invite clean layout contract", () => {
  ...
  test("removes stacked frames while using the shared wide welcome card hierarchy", () => {
    ...
    expect(cardSource).toContain("/sofia-mark.svg");
    expect(cardSource).toContain("Sofia Cloud");
  ...
  test("keeps onboarding focused and reuses the responsive platform grid", () => {
    ...
    expect(successSource).toContain("Already have Sofia? Open it.");
    ...
    expect(successSource).toContain("Return to Sofia");
    ...
    expect(successSource).not.toContain("Open Sofia");
    ...
    expect(installSource).toContain("Set up Sofia Enterprise");
    expect(installSource).toContain("Download Sofia");
