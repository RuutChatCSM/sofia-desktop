...
function getReauthCompleteMessage(data: unknown): ReauthCompleteMessage | null {
  ...
  if (!("type" in data) || data.type !== "sofia:reauth-complete") {
    return null;
  }
  ...
  return { type: "sofia:reauth-complete", nonce: data.nonce, error };
}
...
export function ReauthDialog({
  ...
  async function continueSocial(provider: SocialAuthProvider) {
    const popup = window.open("", "sofia-reauth", "popup,width=480,height=640");
    if (!popup) {
      setError("Sofia could not open the sign-in window. Allow popups for Sofia, then try again.");
      return;
    }
        ...
        <div className="relative border-b border-slate-100 bg-slate-50/70 px-6 pb-5 pt-6">
          ...
          <div className="grid gap-2 pr-8">
            ...
            <p className="text-[14px] leading-6 text-slate-600">
              Sofia retries the pending action automatically after you confirm.
            </p>
          </div>
        </div>
