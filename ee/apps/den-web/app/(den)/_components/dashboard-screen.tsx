...
export function DashboardScreen({ showSidebar = true }: { showSidebar?: boolean }) {
  ...
  const webDisabled = !sofiaAppConnectUrl || !isReady;
  const desktopDisabled = !sofiaDeepLink || !isReady;
  const showConnectionHint = !sofiaDeepLink || !hasWorkspaceScopedUrl;
                ...
                <div className="flex max-w-xl flex-col gap-6">
                  ...
                  <p className="text-[16px] leading-relaxed text-[var(--dls-text-secondary)] md:text-[17px]">
                    {isReady
                      ...
                      : "We are allocating resources and preparing the Sofia connection before unlocking the rest of the controls."}
                  </p>
                ...
                <div className="flex w-full flex-col gap-3 md:w-[200px] md:shrink-0 md:items-end">
                  ...
                  <div className="hidden w-full flex-col items-center md:flex">
                    <button
                      ...
                      onClick={() => {
                        if (!desktopDisabled && sofiaDeepLink) {
                          window.location.href = sofiaDeepLink;
                        }
                      }}
                    ...
                    <span className="mt-2 text-[11px] font-medium text-[var(--dls-text-secondary)]">requires the Sofia desktop app</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="px-8 pb-8 pt-0">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        disabled={!sofiaDeepLink || !isReady}
                      >
                        {sofiaDeepLink ? "Open in Desktop" : "Preparing connection..."}
                      </button>
                      <button
                        type="button"
                    ...
                    <div className="mt-6 space-y-4">
                      <CredentialRow
                        ...
                        value={activeWorker?.sofiaUrl ?? activeWorker?.instanceUrl ?? null}
                        placeholder="Connection URL is still preparing..."
                        hint={showConnectionHint ? (!sofiaDeepLink ? "Getting connection details ready..." : "Finishing your workspace URL...") : undefined}
                        canCopy={Boolean(activeWorker?.sofiaUrl ?? activeWorker?.instanceUrl)}
                        copied={copiedField === "sofia-url"}
                        onCopy={() => void copyToClipboard("sofia-url", activeWorker?.sofiaUrl ?? activeWorker?.instanceUrl ?? null)}
                        muted={!isReady}
                      />
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg font-semibold tracking-tight text-[var(--dls-text-primary)]">Sofia</span>
              </div>
              <button
                type="button"
