...
${authorizationDocumentStyles}
  ...
  <body>
    <main class="card connect-card" role="status" aria-live="polite">
      <div class="brand"><span class="brand-mark">OW</span>Sofia Connect</div>
      <h1>Connect your account</h1>
      <p>Sofia is preparing the secure provider sign-in.</p>
...
<html lang="en">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <meta name="color-scheme" content="light" />
    <title>Connection failed — Sofia</title>
    <style>
${authorizationDocumentStyles}
    ...
    <main class="card error-card" role="alert" aria-live="assertive">
      <div class="error-header">
        ...
        <div class="status">
          ...
          <p>Sofia couldn’t start the provider sign-in.</p>
        </div>
...
export function openMcpAuthorizationWindow(): Window {
  const popupName = `sofia-mcp-authorization-${crypto.randomUUID()}`
  const popup = window.open("", popupName, "popup,width=960,height=720")
  if (!popup) {
    throw new Error("Sofia could not open the sign-in window. Allow popups for Sofia, then try again.")
  }
  try {
    popup.opener = null
    popup.document.open()
