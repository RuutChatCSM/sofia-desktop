...
import { DownloadSofiaCard, type DownloadCardInstallers } from "@sofia/ui/react";
...
const APP_INSTALLED_KEY = "sofia:onboarding:app-installed";
...
function SofiaMark({ className = "h-5 w-5" }: { className?: string }) {
  return (
    // eslint-disable-next-line @next/next/no-img-element
    <img src="/sofia-mark.svg" alt="" aria-hidden className={className} />
  );
}
    ...
    <div className="mx-auto max-w-4xl px-4 pb-16 pt-12 sm:px-6" data-testid="marketplace-onboarding">
      <header className="mx-auto max-w-xl text-center">
        ...
        <h1 className="mt-3 text-[30px] font-semibold leading-[1.15] tracking-[-0.04em] text-gray-950 sm:text-[34px]">
          {requiredDone ? `${orgName} is ready.` : `Get the Sofia app`}
        </h1>
        <p className="mx-auto mt-3 max-w-lg text-[15px] leading-6 text-gray-500">
          {requiredDone
            ...
            : "Sofia runs on the desktop app. Install it, sign in, and this workspace syncs automatically."}
        </p>
      </header>

      <section className="mt-8 grid gap-4">
        <DownloadSofiaCard installers={installers} releaseTag={releaseTag} />
      ...
      <section className="mt-12 grid gap-5">
        <DenSectionHeader
          align="center"
          title="Then bring your own keys, or use Sofia Models"
          description={
            modelsLoading
              ? "Checking whether Sofia Models are already on…"
              : modelsEnabled
                ? "Sofia Models are on for this workspace."
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <DenChoiceCard
            testId="onboarding-choice-sofia-models"
            icon={<SofiaMark />}
            title="Sofia Models"
