...
export function SsoScreen() {
  ...
  function getOrgScopedHeaders() {
    ...
    if (orgId) {
      headers.set("x-sofia-legacy-org-id", orgId);
    }
    return headers;
  }
                ...
                {formMode === "saml" ? (
                  <>
                    ...
                    <label className="block text-[14px] text-gray-700 md:col-span-2">
                      ...
                      <input className="w-full rounded-[18px] border border-gray-200 px-4 py-3" value={audience} onChange={(event) => setAudience(event.target.value)} placeholder="Defaults to the Sofia auth URL" disabled={ssoFormDisabled} />
                    </label>
                    <label className="block text-[14px] text-gray-700 md:col-span-2">
                    ...
                    <div className="rounded-[18px] border border-gray-200 px-4 py-3 text-[14px] leading-6 text-gray-600 md:col-span-2">
                      Sofia always requires signed SAML assertions, timestamps, and SP-initiated responses for organization SAML connections.
                    </div>
                  </>
                ) : (
                  <>
