...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_test_gwscaps"
...
let schema: typeof import("@sofia-ee/den-db/schema")
let drizzle: typeof import("@sofia-ee/den-db/drizzle")
...
beforeAll(async () => {
  mock.restore()
  const realDb = (await import("@sofia-ee/den-db")).createDenDb({
  ...
  const [appMod, dbMod, schemaMod, drizzleMod, sessionMod, credentialsMod, catalogMod, searchMod] = await Promise.all([
    import("../src/app.js"),
    import("../src/db.js"),
    import("@sofia-ee/den-db/schema"),
    import("@sofia-ee/den-db/drizzle"),
...
test("calendar create requests a Google Meet link when asked", async () => {
  ...
  expect(requestId.startsWith("sofia-")).toBe(true)
...
test("drive share grants user access and sends notification by default", async () => {
  ...
  const response = await request("/v1/capabilities/google-workspace/drive-file-share/file_1", {
    method: "POST",
    body: { type: "user", emailAddress: "raghav@ruut.chat" },
  })
  expect(response.status).toBe(200)
  expect(lastDriveSharePayload).toEqual({ type: "user", role: "reader", emailAddress: "raghav@ruut.chat" })
...
test("drive share grants domain access", async () => {
  ...
  const response = await request("/v1/capabilities/google-workspace/drive-file-share/file_1", {
    method: "POST",
    body: { type: "domain", domain: "sofia.ruut.chat", sendNotificationEmail: false },
  })
  expect(response.status).toBe(200)
  expect(lastDriveSharePayload).toEqual({ type: "domain", role: "reader", domain: "sofia.ruut.chat" })
  const url = new URL(expectString(lastDriveShareUrl, "drive share URL"))
...
test("no connected account returns needs_connection", async () => {
  ...
  expect(body).toEqual({
    ...
    message: "Connect your Google account first: open Settings > Connect and use Connect your account on the Google Workspace row, or connect from the Sofia Cloud dashboard.",
  })
})
