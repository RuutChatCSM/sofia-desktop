...
const origin = "https://diagnostic.sofia.test"
...
describe("Den private-cloud egress diagnostic", () => {
  test("defaults to the Sofia Labs diagnostic host and accepts an operator override", () => {
    expectConfiguredOrigin("https://sofia-diagnostic.ruut.chat")
  ...
  test("identifies DNS failure before HTTP reaches Sofia", async () => {
  ...
  test("identifies a proxy-replaced response that lacks Sofia receipt proof", async () => {
    const result = await runEgressDiagnostic({
      bearerToken: "synthetic-diagnostics-secret",
