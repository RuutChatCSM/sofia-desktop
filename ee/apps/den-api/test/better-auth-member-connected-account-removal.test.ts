...
beforeAll(async () => {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_test_gwsreconnect"
  ...
  drizzle = await import("@sofia-ee/den-db/drizzle")
  oauthCredentials = await import("../src/capability-sources/oauth-credentials.js")
  schema = await import("@sofia-ee/den-db/schema")
})
...
test("Better Auth's native remove and leave routes delete the member's connected account", async () => {
  ...
  const password = "Sofia-test-password-123!"
