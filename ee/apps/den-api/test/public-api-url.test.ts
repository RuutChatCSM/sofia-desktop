...
describe("public API URL configuration", () => {
  test("callback base resolution keeps the configured prefix", async () => {
    ...
    expect(resolvePublicApiBaseUrl(
      ...
      "https://sofia.example/api/den/",
    )).toBe("https://sofia.example/api/den")
  })

  test("preserves and normalizes an HTTPS pathname prefix", () => {
    expect(normalizeConfiguredPublicApiBaseUrl("https://sofia.example/api/den/", {
      allowInsecureHttp: false,
    })).toBe("https://sofia.example/api/den")
  })

  test("requires HTTPS outside development and localhost", () => {
    expect(() => normalizeConfiguredPublicApiBaseUrl("http://sofia.example/api/den", {
      allowInsecureHttp: false,
    ...
    })).toBe("http://sofia.example/api/den")
  })
  ...
  test("rejects malformed and non-base URLs", () => {
    ...
    expect(() => normalizeConfiguredPublicApiBaseUrl("https://sofia.example/api?tenant=one", {
      allowInsecureHttp: false,
