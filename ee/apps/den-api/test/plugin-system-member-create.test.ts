...
import { and, eq } from "@sofia-ee/den-db/drizzle"
import {
...
process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_test_membercreate"
...
test("a member cannot import plugins from GitHub without starting a fetch", async () => {
  ...
  try {
    expect(await rejectedStatus(() => store.importGithubPluginMcps({
      ...
      githubUrl: "https://github.com/sofialabs/test-plugin",
    }))).toBe(403)
