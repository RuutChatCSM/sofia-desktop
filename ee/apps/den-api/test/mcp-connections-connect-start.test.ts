...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_test_pr7"
...
test("version-two legacy callbacks use enterprise issuer validation", async () => {
  ...
  expect(response.status).toBe(400)
  const html = await response.text()
  expect(html).toContain("Sofia could not register or identify its OAuth client with the authorization server")
