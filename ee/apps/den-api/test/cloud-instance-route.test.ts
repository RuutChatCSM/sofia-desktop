...
function seedRequiredEnv() {
  ...
  process.env.DAYTONA_SNAPSHOT = "sofia-0.18.8"
}
...
describe("Cloud gateway resolve route", () => {
  test("returns 404 when the gateway key is not configured", async () => {
    ...
    const response = await app.request("http://den.local/v1/cloud/gateway/resolve", {
      headers: { "X-Sofia-Gateway-Key": "gateway-secret" },
    })
  ...
  test("returns 404 when the gateway key is wrong", async () => {
    ...
    const response = await app.request("http://den.local/v1/cloud/gateway/resolve", {
      headers: { "X-Sofia-Gateway-Key": "bad-secret" },
    })
...
describe("Cloud instance route lifecycle states", () => {
  test("returns the Daytona sandbox identifier on the member instance response", async () => {
    const worker = { ...fakeWorker("healthy"), image_version: "sofia-0.18.7" }
    const app = new Hono<{ Variables: OrgRouteVariables }>()
  ...
  test("claims and wakes a stale stopped sandbox without checkpoint probing on resolve", async () => {
    const worker = { ...storedWorker({ status: "healthy" }), image_version: "sofia-0.18.7" }
    ...
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual(expectedCloudInstance({ status: "waking", url: null, imageVersion: "sofia-0.18.7" }))
  ...
  test("does not inspect the sandbox for an up-to-date stopped worker wake", async () => {
    const worker = { ...fakeWorker("stopped"), image_version: "sofia-0.18.8" }
    const app = new Hono<{ Variables: OrgRouteVariables }>()
    let wakeCalls = 0
    let inspectCalls = 0
    ...
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual(expectedCloudInstance({ status: "waking", url: null, imageVersion: "sofia-0.18.8" }))
...
describe("Cloud instance update route", () => {
  test("flushes and stops a running stale sandbox", async () => {
    ...
    const worker = { ...storedWorker({ orgId, userId, status: "healthy" }), image_version: "sofia-0.18.7" }
    const store = makeCloudWorkerStore({ initialWorkers: [worker] })
    const app = new Hono<{ Variables: OrgRouteVariables }>()
  ...
  test("refuses to stop an already-current worker", async () => {
    const orgId = createDenTypeId("organization")
    const userId = createDenTypeId("user")
    const worker = { ...storedWorker({ orgId, userId, status: "healthy" }), image_version: "sofia-0.18.8" }
    const store = makeCloudWorkerStore({ initialWorkers: [worker] })
    const app = new Hono<{ Variables: OrgRouteVariables }>()
    let inspectCalls = 0
    let stopCalls = 0
