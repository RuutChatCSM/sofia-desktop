...
class RecordingOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata() {
    return {
      ...
      client_name: "Sofia deadline test",
...
describe("external MCP diagnostics", () => {
  ...
  test("attributes our own lifecycle deadline to Sofia rather than the provider", () => {
  ...
  test("attributes a wrapped lifecycle deadline to Sofia through its cause chain", () => {
  ...
  test("uses an honest fallback message after MCP protocol readiness", () => {
    ...
    expect(diagnostic.message).toBe("The MCP server answered, but Sofia could not interpret its response for the current request.")
  })
  ...
  test("successful callbacks stay in the browser without opening the desktop app", () => {
    ...
    expect(html).toContain("Enterprise MCP &lt;test&gt; is connected to Sofia.")
    expect(html).toContain("window.close()")
    expect(html).toContain("Close window")
    expect(html).toContain("Sofia Connect")
    expect(html).toContain("background: #f8fbff")
    expect(html).not.toContain("@keyframes")
    expect(html).not.toContain("sofia://")
    expect(html).not.toContain("Open Sofia")
  })
