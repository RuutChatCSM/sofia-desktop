...
const vulnerableCrossInstallationMatch = {
  ...
  target: {
    ...
    connectorType: "github",
    remoteId: "RuutChatCSM/sofia-desktop",
    targetConfigJson: { repositoryFullName: "RuutChatCSM/sofia-desktop", repositoryId: 42 },
  },
}
...
test("repository rename updates connector identity, records continuity, queues resync, and acks 202", async () => {
  ...
  try {
    const response = await deliverWebhook("repository", {
      action: "renamed",
      changes: { repository: { name: { from: "sofia-desktop" } } },
      installation: { id: 12345 },
      repository: { full_name: "RuutChatCSM/sofia-desktop-renamed", id: 42 },
    })

    expect(response.status).toBe(202)
    expect(updatedValues).toEqual([
      expect.objectContaining({
        remoteId: "RuutChatCSM/sofia-desktop-renamed",
        targetConfigJson: expect.objectContaining({ repositoryFullName: "RuutChatCSM/sofia-desktop-renamed", repositoryId: 42 }),
      }),
      expect.objectContaining({ remoteId: "RuutChatCSM/sofia-desktop-renamed" }),
    ])
    expect(insertedValues).toEqual([
      expect.objectContaining({
        ...
        summaryJson: {
          ...
          from: "RuutChatCSM/sofia-desktop",
          to: "RuutChatCSM/sofia-desktop-renamed",
          trigger: "webhook",
        },
      }),
...
test("removed installation repository disables its target and instance and acks 202", async () => {
  ...
  try {
    const response = await deliverWebhook("installation_repositories", {
      action: "removed",
      installation: { id: 12345 },
      repositories_removed: [{ full_name: "RuutChatCSM/sofia-desktop", id: 42 }],
    })
...
test("unmatched repository rename is ignored with a 200 ack", async () => {
  ...
  const response = await deliverWebhook("repository", {
    ...
    installation: { id: 12345 },
    repository: { full_name: "RuutChatCSM/renamed", id: 404 },
  })
