...
import { and, eq, inArray } from "@sofia-ee/den-db/drizzle"
import {
  AuthUserTable,
...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_test_pr3"
...
beforeAll(async () => {
  seedRequiredEnv()
  mock.restore()
  db = (await import("@sofia-ee/den-db")).createDenDb({
...
describe("marketplace capabilities source", () => {
  ...
  test("mcp objects return server specs and existing-connection or admin-install hints", async () => {
    ...
    expect(missingConnection.result.hint).toContain("Sofia Cloud -> Connectors")
  ...
  test("team-assigned skill discovery reports missing plugin MCP admin setup deterministically", async () => {
    ...
    expect(match?.action?.surface).toBe("sofia_organization_connections")
    ...
    expect(result.result.action?.surface).toBe("sofia_organization_connections")
