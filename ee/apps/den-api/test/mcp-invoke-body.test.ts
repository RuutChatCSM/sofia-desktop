...
test("normalizeToolBody parses JSON-encoded string bodies into objects", () => {
  expect(invokeModule.normalizeToolBody('{"email":"ben+demogods@ruut.chat","role":"member"}')).toEqual({
    email: "ben+demogods@ruut.chat",
    role: "member",
  })
...
test("invitation POST forwarded with an object body passes route validation", async () => {
  const result = await invokeModule.invokeMcpOperation({
    ...
    principal,
    toolInput: { body: { email: "ben+demogods@ruut.chat", role: "member" } },
  })

  expect(result.isError).toBe(false)
  expect(JSON.parse(result.content[0]?.text ?? "")).toEqual({
    received: { email: "ben+demogods@ruut.chat", role: "member" },
  })
})
