...
function seedRequiredEnv() {
  ...
  process.env.DAYTONA_RUNTIME_DATA_PATH = "/tmp/sofia-data"
  process.env.DAYTONA_RUNTIME_WORKSPACE_PATH = "/tmp/sofia-workspace"
  process.env.DAYTONA_DATA_MOUNT_PATH = "/persist/sofia"
  process.env.DAYTONA_WORKSPACE_MOUNT_PATH = "/workspace"
  process.env.DAYTONA_SIDECAR_DIR = "/tmp/sofia-sidecars"
...
describe("Daytona Sofia checkpoint start command", () => {
  test("keeps checkpoint seams and safety invariants in the sandbox script", () => {
    const command = daytona.buildSofiaStartCommand({
    ...
    expect(command).toContain("/tmp/sofia-data /tmp/sofia-workspace")
    expect(command).toContain("CHECKPOINT_DIR=")
    expect(command).toContain("/persist/sofia/checkpoints")
    ...
    const serverStart = command.indexOf(" sofia-server --workspace")
