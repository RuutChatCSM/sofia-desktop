...
test("builds the installer asset URL for the configured release", () => {
  expect(installerReleaseAssetUrl("sofia-enterprise-mac-arm64-9.9.9.dmg", {
    releaseTag: "v9.9.9+build 2",
    releaseRepo: "RuutChatCSM/sofia-desktop",
  })).toBe("https://github.com/RuutChatCSM/sofia-desktop/releases/download/v9.9.9%2Bbuild%202/sofia-enterprise-mac-arm64-9.9.9.dmg")
})
...
test("builds the installer asset URL from the resolved default release", async () => {
  ...
  await expect(installerReleaseAssetUrl("sofia-enterprise-win-x64-9.9.9.exe")).resolves.toBe(
    "https://github.com/RuutChatCSM/sofia-desktop/releases/download/v9.9.9/sofia-enterprise-win-x64-9.9.9.exe",
  )
})

test.each([
  ["mac-arm64", "v9.9.9", "sofia-mac-arm64-9.9.9.dmg"],
  ["mac-x64", "9.9.9", "sofia-mac-x64-9.9.9.dmg"],
  ["win-x64", "v9.9.9", "sofia-win-x64-9.9.9.exe"],
  ["linux-x64", "v9.9.9", "sofia-linux-x86_64-9.9.9.AppImage"],
  ["linux-arm64", "v9.9.9", "sofia-linux-arm64-9.9.9.AppImage"],
...
test.each([
  ["mac-arm64", "v9.9.9", "sofia-enterprise-mac-arm64-9.9.9.dmg"],
  ["win-x64", "v9.9.9", "sofia-enterprise-win-x64-9.9.9.exe"],
  ["linux-x64", "v9.9.9", "sofia-enterprise-linux-x86_64-9.9.9.AppImage"],
...
test.each([
  ["mac-arm64", "v9.9.9", "sofia-cloud-mac-arm64-9.9.9.dmg"],
  ["win-x64", "v9.9.9", "sofia-cloud-win-x64-9.9.9.exe"],
  ["linux-x64", "v9.9.9", "sofia-cloud-linux-x86_64-9.9.9.AppImage"],
...
test("resolves only a mounted installer asset and reports its size", async () => {
  ...
  const fileName = "sofia-enterprise-win-x64-9.9.9.exe"
  ...
  await expect(resolveConfiguredInstallerArtifact("sofia-cloud-win-x64-9.9.9.exe")).resolves.toBeNull()
...
test("ignores a directory with the expected filename", async () => {
  const artifactsDir = mkdtempSync(path.join(os.tmpdir(), "ow-installer-artifacts-"))
  const fileName = "sofia-cloud-win-x64-9.9.9.exe"
