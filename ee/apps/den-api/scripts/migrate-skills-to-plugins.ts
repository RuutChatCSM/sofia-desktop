 ...
 *
 * Usage:
 *   pnpm --filter @sofia-ee/den-api migrate:skills-to-plugins        # dry run
 *   pnpm --filter @sofia-ee/den-api migrate:skills-to-plugins -- --yes
 *
 ...
 *        pnpm --filter @sofia-ee/den-api migrate:skills-to-plugins -- --table-prefix recovered_ --yes
 *   4. Drop the recovered_* tables after verifying the migrated plugins.
 */
import { createDenDb, type DenDbMode } from "@sofia-ee/den-db"
import { and, asc, eq, isNull, sql } from "@sofia-ee/den-db/drizzle"
import {
...
import { createDenTypeId, normalizeDenTypeId, type DenTypeId } from "@sofia-ee/utils/typeid"
import {
...
async function ensureDefaultSofiaMarketplace(input: {
  ...
  const existing = (await input.database
    ...
    .where(and(
      ...
      eq(MarketplaceTable.name, DEFAULT_SOFIA_MARKETPLACE_NAME),
...
async function attachPluginToDefaultMarketplace(input: {
  ...
  const marketplaceId = await ensureDefaultSofiaMarketplace(input)
  const existing = (await input.database
    .select()
...
async function main() {
  ...
  const pointerVersions = await db
    ...
    .from(ConfigObjectVersionTable)
    .where(eq(ConfigObjectVersionTable.schemaVersion, "sofia.den_skill.v1"))
