...
export function registerOrgInferenceRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  app.get(
    ...
    describeRoute({
      ...
      description: "Returns Sofia Models enablement and limit context for the active organization.",
      responses: {
  ...
  app.patch(
    ...
    describeRoute({
      ...
      description: "Enables or disables Sofia Models for the active organization.",
      responses: {
      ...
      try {
        ...
        if (error instanceof Error && error.message === "openrouter_management_api_key_missing") {
          return c.json({
            ...
            message: "Set OPENROUTER_MANAGEMENT_API_KEY on Den API before enabling Sofia Models.",
          }, 400)
        }
        throw error
      }
