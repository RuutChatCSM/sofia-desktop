...
function checkoutSuccessUrl(c: { req: { raw: Request } }) {
  // `return=models` sends the user back to the Sofia Models page after a
...
export function registerOrgBillingRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      tags: ["Organizations"],
      hide: true,
      summary: "Create Stripe Checkout session for Sofia Models",
      responses: {
  ...
  app.post(
    ...
    describeRoute({
      tags: ["Organizations"],
      hide: true,
      summary: "Create Stripe billing portal session for Sofia Models",
      responses: {
