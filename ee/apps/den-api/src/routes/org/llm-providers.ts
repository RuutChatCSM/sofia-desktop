import { and, desc, eq, inArray, isNotNull, isNull } from "@sofia-ee/den-db/drizzle"
import {
  AuthUserTable,
  InvitationTable,
...
export function registerOrgLlmProviderRoutes<T extends { Variables: OrgRouteVariables & Partial<MemberTeamsContext> }>(app: Hono<T>) {
  ...
  app.get(
    ...
    async (c) => {
      ...
      // but this member's Sofia provider/key was deleted, re-provision before
      ...
      if (query.scope === "usable") {
        try {
          ...
          // Keep listing other providers even if Sofia re-provision fails.
        }
      }
