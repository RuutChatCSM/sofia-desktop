...
import { and, asc, eq, inArray, isNull } from "@sofia-ee/den-db/drizzle"
import { WorkerTable, WorkerTokenTable } from "@sofia-ee/den-db/schema"
...
const gatewayKeyHeader = "X-Sofia-Gateway-Key"
...
const databaseCloudWorkerStore: CloudWorkerStore = {
  ...
  async insertCloudWorker(input) {
    await db.insert(WorkerTable).values({
      ...
      name: input.name,
      description: "Sofia Cloud browser instance",
      destination: "cloud",
      status: "provisioning",
...
export function registerCloudRoutes<T extends { Variables: OrgRouteVariables }>(
  ...
  app.get(
    ...
    describeRoute({
      ...
      description: "Starts the active organization's Sofia Cloud browser instance when needed and returns its browser URL once ready.",
      responses: {
  ...
  app.get(
    ...
    describeRoute({
      ...
      description: "Starts or wakes the caller's own Sofia Cloud browser instance when needed and returns the collaborator token only to the trusted gateway.",
      responses: {
