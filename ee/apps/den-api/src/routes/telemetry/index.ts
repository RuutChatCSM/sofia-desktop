import { and, desc, eq, gte, isNull, sql, type SQL } from "@sofia-ee/den-db/drizzle"
import {
  TelemetryEventTable,
  TelemetryEventType,
...
export function registerTelemetryRoutes<T extends { Variables: TelemetryRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Receives a batch of telemetry events from the Sofia app or workers. Auth provides org and member identity. Unknown event types and disallowed fields are dropped. Always returns 204.",
      responses: {
