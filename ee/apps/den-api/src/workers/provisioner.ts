import { WorkerTable } from "@sofia-ee/den-db/schema"
...
async function provisionWorkerOnRender(
  ...
  const sofiaServerPackage = env.render.workerSofiaVersion?.trim()
    ? `sofia-server@${env.render.workerSofiaVersion.trim()}`
    : "sofia-server"
  const buildCommand = [
...
while [ "$attempt" -lt 3 ]; do
  ...
  status=$?
  echo "sofia-server failed (attempt $attempt, exit $status); retrying in 3s"
  sleep 3
done
exit 1
`.trim()
  ...
  const payload = {
    ...
    envVars: [
      { key: "SOFIA_TOKEN", value: input.clientToken },
      { key: "SOFIA_HOST_TOKEN", value: input.hostToken },
