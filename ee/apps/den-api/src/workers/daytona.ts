...
import { DaytonaSandboxTable, WorkerTable } from "@sofia-ee/den-db/schema"
...
function sandboxLabels(workerId: WorkerId) {
  return {
    "sofia.den.provider": "daytona",
    "sofia.den.worker-id": workerId,
  }
}
...
function checkpointRestoreMarkerPath() {
  return `${env.daytona.runtimeDataPath}/.sofia-restore-marker`
}
...
function checkpointFlushFunctions(input: { failOnError: boolean }) {
  ...
  for state_path in $SOFIA_STATE_MANIFEST; do
...
export function buildSofiaStartCommand(input: ProvisionInput) {
  ...
  const sofiaServe = [
    ...
    ` --host 0.0.0.0`,
    ` --port ${shellQuote(String(env.daytona.sofiaPort))}`,
    ` --cors '*'`,
...
hydrate_checkpoint() {
  ...
  if ! state_dirs_pristine; then
    echo "checkpoint hydrate skipped; local Sofia state is not pristine or was already restored"
    return 0
  fi
  ...
  echo "checkpoint hydrate failed for $latest_checkpoint; continuing with fresh Sofia state" >&2
...
checkpoint_pid=""
on_term() {
  echo "termination requested; flushing Sofia checkpoint"
  flush_checkpoint
...
while [ "$attempt" -lt 3 ]; do
  ...
  echo "sofia-server failed (attempt $attempt, exit $status); rebuild and republish the Daytona snapshot if this persists; retrying in 3s"
  sleep 3
done
flush_checkpoint
exit 1
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      if (typeof command.exitCode === "number" && command.exitCode !== 0) {
        ...
        throw new Error(
          [
            `sofia session exited with ${command.exitCode}`,
      ...
      if (error instanceof Error && error.message.startsWith("sofia session exited")) {
        throw error
      }
    }
...
async function checkpointExistsOnDaytonaVolume(daytona: Daytona, workerId: WorkerId, sharedVolume: DaytonaVolumeRuntime) {
  ...
  try {
    ...
    const exitCode = await runSandboxShellCommand(
      probeSandbox,
      `sofia-ckpt-probe-${workerHint(workerId)}-${Date.now()}`,
...
async function startSofiaProcessOnDaytonaSandbox(input: {
  ...
  const expiresInSeconds = normalizedSignedPreviewExpirySeconds()
  const preview = await input.sandbox.getSignedPreviewUrl(env.daytona.sofiaPort, expiresInSeconds)
...
async function startSofiaOnDaytonaSandbox(input: {
  provisionInput: ProvisionInput
  runtime: DaytonaProvisioningRuntime
  sandbox: DaytonaSandboxRuntime
  sessionId: string
  ...
  const started = await startSofiaProcessOnDaytonaSandbox(input)
...
async function wakeExistingDaytonaSandbox(input: {
  ...
  return startSofiaOnDaytonaSandbox({
    provisionInput: input.provisionInput,
    runtime: input.runtime,
    sandbox: input.sandbox,
    sessionId: `sofia-wake-${workerHint(input.provisionInput.workerId)}-${Date.now()}`,
    workspaceVolumeId: input.workspaceVolumeId,
    dataVolumeId: input.dataVolumeId,
...
async function recycleDaytonaSandbox(input: {
  ...
  try {
    ...
    const started = await startSofiaProcessOnDaytonaSandbox({
      provisionInput: input.provisionInput,
      runtime: input.runtime,
      sandbox: replacementSandbox,
      sessionId: `sofia-recycle-${workerHint(input.provisionInput.workerId)}-${Date.now()}`,
    })
    const restored = await input.runtime.verifyRestoreMarker(replacementSandbox)
    if (!restored) {
      throw new Error("Daytona replacement did not restore an Sofia checkpoint")
    }
...
export async function provisionWorkerOnDaytonaWithRuntime(
  ...
  try {
    ...
    return startSofiaOnDaytonaSandbox({
      ...
      sessionId: `sofia-${workerHint(input.workerId)}`,
...
export async function flushWorkerCheckpointOnDaytona(workerId: WorkerId) {
  ...
  const exitCode = await runSandboxShellCommand(
    ...
    `sofia-update-flush-${workerHint(workerId)}-${Date.now()}`,
