...
export function connectCallbackPage(input:
  ...
  const body = input.ok
    ? `<div class="status-row success">
        ...
        <span><strong>${escapeHtml(input.name)} is connected to Sofia.</strong><small>Connection complete</small></span>
      </div>
      <p>You can close this window and return to Sofia.</p>
      ${closeButton}`
...
<html lang="en">
  ...
  <body>
    <main role="${input.ok ? "status" : "alert"}" aria-live="${input.ok ? "polite" : "assertive"}">
      <p class="eyebrow">Sofia Connect</p>
      <h1>${title}</h1>
      ${body}
    </main>
  </body>
