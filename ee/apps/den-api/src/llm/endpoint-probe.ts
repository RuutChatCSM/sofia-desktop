...
export function assertProbeUrlAllowed(url: string, options?: { allowLoopback?: boolean }) {
  const allowLoopback = options?.allowLoopback ?? process.env.SOFIA_DEV_MODE === "1"
...
function hintFor(vendor: ProbeVendor, status: number | null): string {
  ...
  return "Could not reach the endpoint. Check the URL, network access, and that the endpoint allows requests from Sofia Cloud."
}
