import { and, desc, eq, inArray, isNull, notExists } from "@sofia-ee/den-db/drizzle"
import {
...
export type MarketplaceMcpRequirementAction = {
  ...
  label: string
  surface: "none" | "sofia_organization_connections" | "sofia_your_connections"
...
async function filterVisibleRows(input: {
  ...
  return input.rows.filter((row) => {
    ...
    // for every role so admins can curate what Sofia exposes to them.
...
function marketplaceRequirementAction(input: {
  ...
  if ((input.state === "needs_connection" || input.state === "reconnect") && input.connectionId) {
    ...
    return {
      ...
      url: sofiaYourConnectionsUrl(input.connectionId),
    }
  }
  ...
  return {
    ...
    retry: "search_capabilities",
    url: sofiaOrganizationConnectionsUrl(),
  }
}
...
function requirementHint(input: {
  ...
  return `${input.capabilityName} belongs to marketplace plugin "${input.requirement.pluginName}", which needs an org admin to configure its required MCP connection before it can run in Sofia Cloud.`
}
...
async function mcpHint(input: {
  ...
  return {
    ...
    hint: `This plugin declares an MCP server but Sofia will not auto-provision it. Ask an org admin to add it in Sofia Cloud -> Connectors, or install "${input.row.plugin.name}" locally.`,
  }
}
...
export async function executeMarketplaceCapability(input: {
  ...
  return {
    ...
    result: {
      ...
      status: "unsupported",
      hint: "Marketplace plugin hooks are not supported on the Sofia capability rail yet.",
    },
  }
}
