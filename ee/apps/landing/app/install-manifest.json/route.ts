...
//
// Resolves the latest Sofia desktop release on GitHub and returns an install
// manifest in the shape the `sofia-bootstrap install app` command expects:
//
...
const GITHUB_REPO = "RuutChatCSM/sofia-desktop";
...
// Only treat Sofia desktop-app installers as artifacts. Historical sidecar
...
function isDesktopAppAsset(name: string): boolean {
  ...
  if (!lower.startsWith("sofia")) return false;
  // exclude update metadata / checksums
  if (/\.(blockmap|yml|yaml|json|txt|sig|sha256)$/i.test(lower)) return false;
  // exclude any sidecar/CLI binary that also happens to start with "sofia".
  ...
  try {
    const response = await fetch(RELEASES_API, {
      headers: {
        ...
        "user-agent": "sofia-install-manifest",
      },
      next: { revalidate: 300 },
    });
    if (!response.ok) {
    ...
    // Sofia desktop installer, not sidecar or CLI bundles.
