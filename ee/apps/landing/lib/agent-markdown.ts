...
- Sofia Connect: one MCP gateway URL for org-wide skills, servers, roles, and policies
- Sofia Web in alpha; central management and private deployment options for teams
...
const pricing = `# Sofia pricing — free, team, and enterprise

> Sofia has three tiers: free open-source desktop, Team Starter with the first 5 seats free then \\$10 per seat/mo, and custom Enterprise.
...
- Managed deployment — self-hosted in your environment or hosted by Sofia
...
- CTA: [Talk to us](https://sofia.ruut.chat/enterprise#book)
...
- Cloud workers — managed by Sofia, sandbox infrastructure via Daytona (EU)

## Next step

- [Book a call](https://sofia.ruut.chat/enterprise#book)
- [Security Review](https://sofia.ruut.chat/trust) — data handling, subprocessors, and incident SLA
- See [Pricing](https://sofia.ruut.chat/pricing) for tier comparison
`

const trust = `# Trust & Security

> How Sofia handles data, what subprocessors are involved, and how to reach the security team.
...
| LLM API keys | Local keychain / env vars | Held by your LLM provider, not by Sofia |
| Prompts & responses | Local only | Sent to your LLM provider; not logged by Sofia |
...
Omar McAdam — team+security@ruut.chat
`

const glm52 = `# GLM 5.2 is now in Sofia — with 2x usage

> GLM 5.2 is available through Sofia Models, and we're doubling your usage so you can run real agent work on an open model at a fraction of the cost.

## What's new

- **GLM 5.2 in Sofia Models** — managed OSS model access with 2x usage, no keys required
...
- **Voice mode** — control the Sofia UI by voice
- **Advanced analytics on Sofia Cloud** — usage, activity, and team behavior in one view
...
Open Sofia, switch to GLM 5.2, and ask the chat to organize your tasks.
...
const download = `# Get Started with Sofia

> Create a free Sofia Cloud account first, then use the guided desktop app access flow.
...
Once the desktop app is running, use the [workspace-guide skill](https://sofia.ruut.chat/.well-known/agent-skills/workspace-guide/SKILL.md) for first-run orientation.
`

const connect = `# Sofia Connect
...
const cloud = `# Sofia Cloud

> The dashboard for running Sofia across your organization.
