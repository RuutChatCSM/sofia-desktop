...
#### Release importance
Minor release: simplifies artifact management and adds faster workspace controls without changing Sofia's overall workflow model.
...
#### Major bug fix details
- Fixed worker recovery so sandbox restarts can reconnect without rotating existing Sofia tokens.
...
#### One-line summary
Expands cloud billing into a self-serve management flow and lets Slack and Telegram carry richer Sofia Router messages.
...
- Added first-class media transport for Slack and Telegram in Sofia Router.
...
#### Release importance
Major release: substantially improves how users run and monitor long Sofia sessions through rendering, status, and compaction changes across core app surfaces.
...
#### Title
Sofia Share turns dropped files into worker packages

#### One-line summary
Adds a real worker-packaging flow in Sofia Share, rebuilds the share site on the Next.js App Router, and makes provider connections easier to manage.

#### Main changes
- Sofia Share now packages dropped skills, agents, commands, and MCP or Sofia config into worker bundles.
...
#### Major improvement details
- Turned Sofia Share into a worker packager.
- Replatformed Sofia Share onto the Next.js App Router.
...
#### Release importance
Minor release: fixes import targeting and worker startup clarity without materially changing Sofia's overall product shape.
...
- Sofia Share adds inline link-success feedback, and the Book a Call form gets clearer topic cards.
...
- Added inline success feedback and richer content handling on Sofia Share surfaces.
...
#### Release importance
Minor release: keeps release artifacts aligned for distribution without changing how users use Sofia.
...
#### Release importance
Minor release: adds a focused operator capability and several UX improvements without broadly reshaping the Sofia product.
...
#### Release importance
Minor release: extends sharing workflows and fixes setup friction without materially changing Sofia's overall architecture.
...
#### One-line summary
Turns Den signup into a calmer guided flow with clearer provisioning states while refocusing Sofia Share on publishing one skill well.

#### Main changes
- Rebuilt Den onboarding as a guided flow with clearer naming, intent, loading, and browser-access states.
- Simplified Sofia Share to publish a single skill, with cleaner frontmatter and fuller shared previews.
...
#### Release importance
Major release: substantially changes both Den onboarding and the Sofia Share publishing flow in ways users will immediately notice.
...
- Simplified Sofia Share to publish a single skill.
...
#### Release importance
Minor release: delivers focused session and settings polish without materially changing Sofia's broader workflows.
