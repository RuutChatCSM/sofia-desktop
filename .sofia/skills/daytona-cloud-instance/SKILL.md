...
description: Daytona cloud instance, Den server, Sofia Cloud, Marketplace onboarding, desktop plus cloud e2e, frame proof. Use when launching, validating, or recording Daytona cloud/Den flows.
---

# Daytona Cloud Instance

Use this skill to launch or validate an Sofia Cloud/Den Daytona server sandbox and collect useful URLs for desktop sign-in and Marketplace onboarding demos.
...
curl -fsS -X POST "$DEN_API_URL/api/auth/sign-in/email" \
  ...
  --data '{"email":"alex@acme.test","password":"SofiaDemo123!"}'
```
...
```bash
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''cd /workspace && pnpm --filter @sofia-ee/den-web build'\'''
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''pkill -f "next dev --hostname 0.0.0.0 --port 3005" || true; pkill -f "next-server" || true'\'''
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''cd /workspace && nohup pnpm --filter @sofia-ee/den-web exec next start --hostname 0.0.0.0 --port 3005 > /tmp/den-web-prod.log 2>&1 &'\'''
```
...
```js
localStorage.setItem('sofia.den.baseUrl', DEN_WEB_URL)
localStorage.setItem('sofia.den.apiBaseUrl', DEN_API_URL)
localStorage.setItem('sofia.den.authToken', TOKEN)
localStorage.setItem('sofia.den.activeOrgId', ORG_ID)
localStorage.setItem('sofia.den.activeOrgSlug', ORG_SLUG)
localStorage.setItem('sofia.den.activeOrgName', ORG_NAME)
window.dispatchEvent(new CustomEvent('sofia-den-settings-changed'))
window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: { token: TOKEN } }))
```
...
5. Desktop signed-out Marketplace nudge says Sofia works without an account.
6. Desktop signs in to Sofia Cloud.
7. Marketplace refresh shows `Sofia Marketplace` and org marketplaces.
...
10. Workspace files materialize under `.sofia`.
