---
name: create-plugin
description: "Create an Sofia plugin for Sofia. Scaffolds the plugin file with the correct API shape, tool definitions, and hook registration. Use when the user asks to 'create a plugin', 'write a plugin', or 'make a plugin that does X'."
---

# Skill: Create an Sofia Plugin

Scaffold a working Sofia plugin for use in Sofia.

## When to use

- User asks "create a plugin that does X"
- User asks "write a plugin" or "make a plugin"
- User wants to extend Sofia/Sofia with custom tools

## Plugin API

An Sofia plugin is an **async factory function** that returns a **hooks object**.

### File location

Plugins can live in:
- **Project**: `.sofia/plugins/my-plugin.ts` (auto-discovered)
- **Global**: `~/.config/engine/plugins/my-plugin.ts`
- **npm**: published as a package, referenced in `engine.json` `plugin` array
...
5. **`fetch()` works** — plugins run in-process inside the Sofia runtime.
...
Sofia plugins are **NOT** the same as Anthropic's plugin format. Key differences:

| Aspect | Sofia Plugin | Anthropic Plugin |
|--------|----------------|-----------------|
...
To adapt an Anthropic plugin for Sofia:
1. Rewrite the tool definitions using Zod instead of JSON Schema.
2. Move any auth from the manifest to env vars or `engine.json` provider config.
  ...
  tool: {
    save_note: {
      ...
      async execute(args: { title: string; body: string }) {
        ...
        const path = `.sofia/notes/${args.title.replace(/[^a-zA-Z0-9-_]/g, "_")}.md`;
        await fs.mkdir(".sofia/notes", { recursive: true });
    ...
    list_notes: {
      ...
      async execute() {
        const fs = await import("node:fs/promises");
        try {
          const files = await fs.readdir(".sofia/notes");
...
1. Create `.sofia/plugins/<name>.ts` with the plugin code.
2. Add it to `engine.json` `plugin` array if not already present.
