...
```bash
daytona exec "$SANDBOX" -- 'cat /workspace/.sofia-daytona/desktop-bootstrap.json'
```
...
```js
window.dispatchEvent(new CustomEvent("sofia:deep-link", {
  detail: { urls: ["sofia://den-auth?grant=" + grant +
