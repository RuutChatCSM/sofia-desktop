...
Launch the Sofia Electron app in a Daytona cloud sandbox. The real desktop
...
- `sofia-eval-secrets:/daytona-secrets` for provider keys and eval-only secrets.
- `sofia-eval-artifacts:/daytona-artifacts` for screenshots, validation notes, and recordings.
...
```bash
daytona preview-url sofia-dev -p 6080
```

Open that URL in your browser. You'll see the real Electron Sofia app.

### 4. Get other URLs

```bash
# Den Web dashboard (if Den stack is running)
daytona preview-url sofia-dev -p 3005

# CDP debugging endpoint
daytona preview-url sofia-dev -p 9825
```
...
# SSH into sandbox
daytona ssh sofia-dev

# Check logs
daytona exec sofia-dev 'tail -50 /tmp/electron.log'
daytona exec sofia-dev 'tail -50 /tmp/vite.log'
daytona exec sofia-dev 'tail -50 /tmp/start-vnc.log'

# Inspect Electron CDP targets
daytona exec sofia-dev 'curl -s http://127.0.0.1:9825/json/list'

# Capture a persistent screenshot artifact
daytona exec sofia-dev 'bash .devcontainer/capture-daytona-screenshot.sh'

# Restart just the Electron app
daytona exec sofia-dev 'bash -lc "pkill -f electron || true; pkill -f electron-dev || true"'
daytona exec sofia-dev 'bash -lc "cd /workspace && bash /opt/sofia-daytona/start-daytona-electron.sh --detach"'

# Stop the sandbox (preserves state)
daytona stop sofia-dev

# Start it again
daytona start sofia-dev

# Delete (destroys everything)
daytona delete sofia-dev
```
...
```bash
cd /workspace/apps/app && SOFIA_DEV_MODE=1 nohup npx vite --host 0.0.0.0 --port 5173 > /tmp/vite.log 2>&1 &
```

**noVNC shows black screen:**
Xvfb/XFCE may have crashed. Restart the desktop stack:
```bash
bash /opt/sofia-daytona/start-daytona-vnc.sh
```

**"no space left on device" when creating sandbox:**
Use `--disk 10`. The default Daytona disk can be 3 GB, which is not enough for
Sofia dependencies and sidecar prep. Also don't use `--context .` — it
