...
description: "do E2E tests, run E2E, test on Daytona, run Electron on Daytona, validate feature, real desktop flow, CDP/noVNC, PR proof. Launch and drive Sofia Electron in Daytona with test evidence."
---

# Skill: Daytona Electron Test

Drive the real Sofia Electron app inside a Daytona sandbox via CDP browser
...
sandbox from the reusable `sofia-eval-vnc` snapshot when present, checks out
...
from `@sofia/testkit`, and records ambient evidence; see `write-a-spec`.

Use `browser_list` to connect when manual inspection is needed.
Refresh the snapshot with `bash .devcontainer/create-daytona-sofia-snapshot.sh`
when dependencies or base setup change. The snapshot excludes `node_modules`;
dependency installs reuse the `sofia-eval-pnpm-store` volume.
...
sandboxes mount `sofia-eval-secrets:/daytona-secrets` automatically and
...
- `daytona-secrets-volume`: provider keys and eval-only secrets in
  `sofia-eval-secrets:/daytona-secrets`.
...
- **Secrets volume:** use `sofia-eval-secrets:/daytona-secrets` for provider
...
- **Artifacts volume:** use `sofia-eval-artifacts:/daytona-artifacts` for
  screenshots, validation notes, and recordings that survive sandbox deletion.

Validation standard: use `daytona-flow-validator`. The `@sofia/testkit`
...
Should show: `[target_id] Sofia  http://localhost:5173/#/welcome`
...
5. **Wait 10-12s.** Verify:
   ...
   - engine process running: `daytona exec "$SANDBOX" -- "bash -lc 'ps aux | grep engine | grep -v grep'"`
...
To test real sessions (not just UI flow), the engine sidecar needs an LLM
provider key. The easiest is OpenAI:

```bash
daytona exec "$SANDBOX" -- "bash -lc 'cd /workspace/hello && node -e \"
  const fs = require(\\\"fs\\\");
  const p = \\\"engine.jsonc\\\";
...
engine to pick up the new provider.
...
```bash
# Minimize
daytona exec "$SANDBOX" -- "bash -lc 'DISPLAY=:99 xdotool search --name Sofia windowminimize'"

# Restore
daytona exec "$SANDBOX" -- "bash -lc 'DISPLAY=:99 xdotool search --name Sofia windowactivate'"
```
...
Every Daytona eval sandbox mounts `sofia-eval-secrets:/daytona-secrets` and
`/opt/sofia-daytona/start-daytona-electron.sh` sources every
...
daytona exec "$SANDBOX" -- "bash -lc 'pkill -f electron || true; pkill -f electron-dev || true; pkill -f engine || true'"

# Step 2: wait, then restart Electron (separate exec call)
sleep 3
daytona exec "$SANDBOX" -- "bash -lc 'cd /workspace && bash /opt/sofia-daytona/start-daytona-electron.sh --detach'"
```
...
`@sofia/email` must be built before the seed imports Den email helpers:
```bash
daytona exec <server-sandbox> -- 'cd /workspace && pnpm --filter @sofia/email build && cd /workspace/ee/apps/den-api && SOFIA_DEV_MODE=1 DATABASE_URL=mysql://root:password@127.0.0.1:3306/sofia_den DEN_DB_ENCRYPTION_KEY=daytona-den-db-encryption-key-please-change-1234567890 BETTER_AUTH_SECRET=local-dev-secret-not-for-production-use!! BETTER_AUTH_URL=http://localhost:3005 pnpm exec tsx scripts/seed-demo-org.ts --reset'
```
...
grant from the Den API, paste the `sofia://den-auth?...` URL into Cloud
Account -> `Paste sign-in code`, and choose `Acme Robotics`:
```bash
TOKEN=$(curl -s -X POST '<DEN_API_URL>/api/auth/sign-in/email' -H 'content-type: application/json' --data '{"email":"alex@acme.test","password":"SofiaDemo123!"}' | node -e 'let s="";process.stdin.on("data",c=>s+=c);process.stdin.on("end",()=>process.stdout.write(JSON.parse(s).token))')
curl -s -X POST '<DEN_API_URL>/v1/auth/desktop-handoff' -H "authorization: Bearer $TOKEN" -H 'content-type: application/json' --data '{"desktopScheme":"sofia"}'
```
...
you also see `DevTools listening on ws://127.0.0.1:9825/...` and an Sofia
window in noVNC.
...
The app log line `[sofia] Electron CDP exposed at http://127.0.0.1:9825`
means Sofia requested CDP. The real success marker is Chromium's own line:
`DevTools listening on ws://127.0.0.1:9825/devtools/browser/...`.

**engine sidecar not restarting after kill:**
...
recordings are saved to the persistent `sofia-eval-artifacts` volume and
...
Save the sandbox name from the output (e.g. `SANDBOX=sofia-test-20260601-165424`).
...
- Recordings are stored on the `sofia-eval-artifacts` Daytona volume (5 GB,
