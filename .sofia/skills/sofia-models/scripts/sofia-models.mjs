...
const paths = {
  ...
  models: path.resolve("ee/apps/inference/src/models/sofia-models.json"),
  aliases: path.resolve("packages/types/src/den/inference.ts"),
}

function usage() {
  console.log(`Usage:
  sofia-models.mjs search <query>
  sofia-models.mjs add <query-or-id>
  sofia-models.mjs remove <id> [<id2>]
  sofia-models.mjs discount <usageFactor> <id> [<id2>]
  sofia-models.mjs sync
  sofia-models.mjs validate`)
}
...
async function readSofiaModels() {
...
function resolveExactSofiaIds(sofiaModels, ids) {
  const resolved = []
  for (const id of ids) {
    if (sofiaModels[id]) {
      resolved.push(id)
    ...
    const matches = findMatches(sofiaModels, id)
    console.error(`Sofia model "${id}" was not exact. Please clarify with exact id:`)
    displayMatches(matches)
    process.exitCode = 2
    return null
  }
...
async function syncAliases(models, usageOverrides = new Map()) {
  ...
  const entries = Object.entries(models).map(([id, model]) => {
    ...
    const displayName = `Sofia: ${model.name}`
...
async function validate() {
  const models = await readSofiaModels()
  ...
  console.log(`validated ${Object.keys(models).length} Sofia model(s)`)
}
...
async function remove(ids) {
  const models = await readSofiaModels()
  const resolved = resolveExactSofiaIds(models, ids)
  if (!resolved) return
...
if (command === "search") {
  ...
  await syncAll(await readSofiaModels())
