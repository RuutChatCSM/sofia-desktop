...
Use the relevant `evals/specs/**/*.test.ts` test with `@sofia/testkit` for a
...
Prefer synthetic paste for the Sofia composer:
...
- The target Sofia/Chrome window is visible and not covered by a native file
...
right window first with `wmctrl -a "Sofia - Dev"` or
`wmctrl -a "Sofia Cloud - Chromium"` so the screenshot shows the intended
journey step.
