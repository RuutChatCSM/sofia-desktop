...
**Default: inspect slides visually before declaring done.** If your model
supports image input, load the rendered PNGs directly and check for text
overflow, overlapping shapes, cut-off labels, wrong-scale icons, and
off-brand colors. Report each issue as 'slide N: <problem>'.

If your model does not support image input, run the structural checks
only (diagnose.py, dump_text.py grep) and tell the user visual inspection
