...
description: "test on Windows, enterprise CA, corporate certificate, GPO cert, TLS fetch failed, Windows sandbox, daytona windows, self-hosted cert. Use when validating Sofia Windows enterprise TLS/OS-trust fixes in a Daytona Windows sandbox."
---

# Skill: Daytona Windows Enterprise Certificate Test

Run the verified Windows repro for Sofia enterprise TLS behavior: install a
...
`@sofia/testkit`; custom VNC screenshots are supplementary. Reuse the repo
support assets instead of copying their logic: `scripts/support/setup-sofia-tls-repro.ps1`,
`scripts/support/sofia-doctor.ps1`, and `docs/support/enterprise-network-doctor.md`.
...
- User reports `TLS fetch failed`, `fetch failed`, or a certificate-specific
  failure when connecting Sofia to a self-hosted control plane.
...
- `gh` must be authenticated to `RuutChatCSM/sofia-desktop` and able to create/delete
  temporary public prereleases.
- Have a Windows Sofia build or installer ready. Keep secrets and customer
...
Connect. Use `exec` for setup and logs; use VNC to drive the installed Sofia
...
```bash
TAG="sofia-win-cert-repro-$(date +%Y%m%d%H%M%S)"
ZIP="/tmp/${TAG}.zip"
# Put your Windows app build under /tmp/sofia-win-cert-upload/sofia/app
# and include scripts/support/setup-sofia-tls-repro.ps1 plus
# scripts/support/sofia-doctor.ps1 under sofia/scripts/support/.
# Include .sofia/skills/daytona-windows-cert/scripts/ca-probe.js as
# sofia/ca-probe.js.
ditto -c -k --keepParent /tmp/sofia-win-cert-upload/sofia "$ZIP"
gh release create "$TAG" "$ZIP" --repo RuutChatCSM/sofia-desktop --prerelease
```
...
```bash
DOWNLOAD_URL="https://github.com/RuutChatCSM/sofia-desktop/releases/download/${TAG}/$(basename "$ZIP")"
...
```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'mkdir C:\ow\sofia\scripts\support 2>NUL'
daytona exec "$SANDBOX_ID" -- cmd /c 'curl.exe -L -o C:\ow\sofia\scripts\support\setup-sofia-tls-repro.ps1 https://raw.githubusercontent.com/RuutChatCSM/sofia-desktop/dev/scripts/support/setup-sofia-tls-repro.ps1'
daytona exec "$SANDBOX_ID" -- cmd /c 'curl.exe -L -o C:\ow\sofia\scripts\support\sofia-doctor.ps1 https://raw.githubusercontent.com/RuutChatCSM/sofia-desktop/dev/scripts/support/sofia-doctor.ps1'
```

## 4. Stand up the enterprise-TLS repro

`scripts/support/setup-sofia-tls-repro.ps1` creates a fake corporate root and
intermediate, trusts the root in `Cert:\LocalMachine\Root`, maps
`poc.sofia.test` to localhost, and serves:

- `https://poc.sofia.test:8443` — healthy chain.
- `https://poc.sofia.test:9443` — broken chain with the intermediate removed.
...
$repo = "C:\ow\sofia"
$cmdPath = "C:\ow\start-sofia-tls-repro.cmd"
$cmd = @"
@echo off
cd /d "$repo"
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\support\setup-sofia-tls-repro.ps1
powershell -NoProfile -ExecutionPolicy Bypass -Command "while (`$true) { Start-Sleep -Seconds 3600 }"
"@
Set-Content -LiteralPath $cmdPath -Value $cmd -Encoding ASCII
schtasks /create /f /sc onstart /ru SYSTEM /tn SofiaTlsRepro /tr $cmdPath
schtasks /run /tn SofiaTlsRepro
'''
...
```bash
daytona exec "$SANDBOX_ID" -- powershell -NoProfile -ExecutionPolicy Bypass -File 'C:\ow\sofia\scripts\support\sofia-doctor.ps1' -WebUrl https://poc.sofia.test:8443 -ApiUrl https://poc.sofia.test:9443 -ExpectedIssuerMatch "Sofia TLS Repro"
```
...
Copy `.sofia/skills/daytona-windows-cert/scripts/ca-probe.js` into the VM as
`C:\ow\ca-probe.js`. Its contents are intentionally small and reusable:

```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'copy C:\ow\sofia\ca-probe.js C:\ow\ca-probe.js'
```
...
const needle = (process.env.SOFIA_TLS_REPRO_CA_MATCH || "Sofia TLS Repro").toLowerCase();
...
```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'set ELECTRON_RUN_AS_NODE=1 && "C:\ow\sofia\app\Sofia.exe" C:\ow\ca-probe.js'
```
...
Drive the installed Sofia Windows app through VNC, not `daytona exec`.

1. Open Daytona Dashboard -> sandbox -> ⋮ menu -> **VNC** -> Connect.
2. Launch or install Sofia as the interactive user.
3. Point the self-hosted/control-plane URL at `https://poc.sofia.test:8443`.
   The request should succeed.
4. Repeat against `https://poc.sofia.test:9443`. The request should fail with
...
```text
C:\Users\<User>\AppData\Roaming\com.differentai.sofia
```

It is **not** `C:\Users\<User>\AppData\Roaming\Sofia`. Because `exec` runs as
SYSTEM, inspect the interactive user path explicitly:

```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'dir "C:\Users\Administrator\AppData\Roaming\com.differentai.sofia\system-ca-bundle.pem"'
daytona exec "$SANDBOX_ID" -- cmd /c 'findstr /c:"Sofia TLS Repro" "C:\Users\Administrator\AppData\Roaming\com.differentai.sofia\system-ca-bundle.pem"'
```
...
```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'dir "C:\Users\Administrator\AppData\Roaming\com.differentai.sofia"'
```
...
ENCODED=$(python3 - <<'PY'
import base64
command = r'Get-ChildItem Cert:\LocalMachine\Root | Where-Object Subject -like "*Sofia TLS Repro*"'
...
```bash
daytona exec "$SANDBOX_ID" -- cmd /c 'schtasks /end /tn SofiaTlsRepro'
# Core repro cleanup shape: setup-sofia-tls-repro.ps1 -Cleanup
daytona exec "$SANDBOX_ID" -- powershell -NoProfile -ExecutionPolicy Bypass -File 'C:\ow\sofia\scripts\support\setup-sofia-tls-repro.ps1' -Cleanup
