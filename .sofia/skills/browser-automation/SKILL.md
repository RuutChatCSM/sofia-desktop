...
description: Local Sofia Electron browser automation with CDP. Use when driving a local Electron dev app, browser_list, browser_snapshot, browser_eval, composer automation, or local UI smoke tests.
---
...
- Attach Sofia browser tools to the Sofia Electron app during local development.
...
```sh
SOFIA_ELECTRON_REMOTE_DEBUG_PORT=${SOFIA_ELECTRON_REMOTE_DEBUG_PORT:-9823}
```

The default browser URL for Sofia browser tools is:
...
```sh
SOFIA_ELECTRON_REMOTE_DEBUG_PORT=9830 pnpm dev
```

To disable Electron CDP for a run:

```sh
SOFIA_ELECTRON_REMOTE_DEBUG_PORT=0 pnpm dev
```
...
```sh
nohup pnpm dev > /var/folders/d9/xqhkvsp94rg0n0n523snqztm0000gn/T/engine/sofia-dev.log 2>&1 &
```
...
Use this `browser_eval` pattern after selecting the Sofia target:
...
- Electron CDP is used for development test tooling. User browser tasks should use the built-in Sofia Browser target.
- A successful local attach should show an `Sofia` target at `http://127.0.0.1:9823`.
