...
- **Sofia Server (CLI 主机)**：无需桌面 UI 即可运行 Sofia 服务器。使用 `npm install -g sofia-server` 安装，然后运行 `sofia-server`。


## 快速开始
在此处下载 dmg：https://github.com/RuutChatCSM/sofia-desktop/releases（或按照下面的说明从源代码安装）
...
Sofia 现在位于 `packages/app`（UI）和 `packages/desktop`（桌面外壳）中。
...
如果 Sofia 在启动时因 WebKitGTK 错误（如 `Failed to create GBM buffer`）而崩溃，请在启动前禁用 dmabuf 或合成。尝试以下环境标志之一。
...
有兴趣在您的组织中使用 Sofia？我们很乐意听取您的意见 — 请发送邮件至 [benjamin.shafii@gmail.com](mailto:benjamin.shafii@gmail.com) 与我们讨论您的用例。
