...
我們為 AI 智能體配備您團隊已經在使用的工具，並讓它們從您的行為中學習。您使用 Sofia 越多，工具之間的連接就越緊密，積累的知識就越多，能夠自動化的工作塊就越大。

Sofia 是 engine 的最簡單介面。雙擊，選擇一個資料夾，即可立即獲得三大核心優勢：
1. 零摩擦設置 — 您現有的 engine 配置開箱即用，無需遷移
...
Sofia 圍繞一個核心理念設計：讓您可以輕鬆地將智能體工作流程作為可重複的、產品化的流程進行交付。

它是一個原生桌面應用程式，底層運行 **Sofia**，但將其呈現為簡潔的引導式工作流程：
- 選擇工作區
- 開始運行
- 觀察進度 + 計劃更新
- 需要時批准權限
...
- **Owpenbot (WhatsApp 機器人)**：為運行中的 Sofia 伺服器提供的輕量級 WhatsApp 橋接器。安裝方法：
  ...
  - 完整設置：https://github.com/RuutChatCSM/owpenbot/blob/dev/README.md
- **Sofia Server (CLI 主機)**：無需桌面 UI 即可運行 Sofia 伺服器。使用 `npm install -g sofia-server` 安裝，然後運行 `sofia-server`。


## 快速開始
在此處下載 dmg：https://github.com/RuutChatCSM/sofia-desktop/releases（或按照下面的說明從源代碼安裝）

## 為什麼選擇 Sofia

當前 engine 的 CLI 和 GUI 都以開發者為中心。這意味著專注於檔案差異、工具名稱，以及在不依賴某種形式的 CLI 的情況下難以擴展的功能。

Sofia 的設計目標是：
- **可擴展**：技能和 engine 插件是可安裝的模組。
- **可審計**：顯示發生了什麼、何時發生以及為什麼發生。
- **權限控制**：訪問特權流程。
- **本地/遠端**：Sofia 可以在本地工作，也可以連接到遠端伺服器。

## 包含的功能

- **主機模式**：在您的電腦上本地運行 engine
- **客戶端模式**：通過 URL 連接到現有的 Sofia 伺服器
- **會話**：創建/選擇會話並發送提示
- **實時流傳輸**：SSE `/event` 訂閱以獲取實時更新
- **執行計劃**：將 Sofia 待辦事項呈現為時間線
- **權限**：顯示權限請求並回覆（允許一次/始終允許/拒絕）
- **模板**：保存並重新運行常見工作流程（本地存儲）
- **技能管理器**：
  - 列出已安裝的 `.sofia/skills` 資料夾
  - 將本地技能資料夾導入到 `.sofia/skills/<skill-name>`
...
- 已安裝 Sofia CLI 且可在 PATH 中使用：`engine`
...
Sofia 現在位於 `packages/app`（UI）和 `packages/desktop`（桌面外殼）中。
...
- 在**主機模式**中，Sofia 啟動：
  ...
  - 以您選擇的專案資料夾作為進程工作目錄。
在主機模式下，Sofia 直接在您的電腦後台啟動 Sofia 伺服器。
當您選擇專案資料夾時，Sofia 使用該資料夾在本地運行 Sofia 並將桌面 UI 連接到它。
這允許您完全在您的機器上運行智能體工作流程、發送提示並查看進度，而無需依賴遠端伺服器。

- UI 使用 `@engine-ai/sdk/v2/client` 來：
  - 連接到伺服器
  - 列出/創建會話
  - 發送提示
...
插件是擴展 Sofia 的**原生**方式。Sofia 現在通過從技能選項卡讀取和寫入 `engine.json` 來管理它們。

- **專案範圍**：`<workspace>/engine.json`
- **全局範圍**：`~/.config/engine/engine.json`（或 `$XDG_CONFIG_HOME/engine/engine.json`）

您仍然可以手動編輯 `engine.json`；Sofia 使用與 Sofia CLI 相同的格式：
...
如果 Sofia 在啟動時因 WebKitGTK 錯誤（如 `Failed to create GBM buffer`）而崩潰，請在啟動前禁用 dmabuf 或合成。嘗試以下環境標誌之一。

```bash
WEBKIT_DISABLE_DMABUF_RENDERER=1 sofia
```

```bash
WEBKIT_DISABLE_COMPOSITING_MODE=1 sofia
```

## 安全說明

- Sofia 默認隱藏模型推理和敏感工具元數據。
- 主機模式默認綁定到 `127.0.0.1`。

## 貢獻

- 在進行更改之前，請查看 `AGENTS.md` 以及 `VISION.md`、`PRINCIPLES.md`、`PRODUCT.md` 和 `ARCHITECTURE.md` 以了解產品目標。
- 在倉庫內工作之前，確保已安裝 Node.js、`pnpm`、Rust 工具鏈和 `engine`。
- 每次檢出後運行一次 `pnpm install`，然後在打開 PR 之前使用 `pnpm typecheck` 加上 `pnpm test:e2e`（或目標腳本子集）驗證您的更改。
- 按照 `AGENTS.md` 中描述的 `.sofia/skills/prd-conventions/SKILL.md` 約定，將新的 PRD 添加到 `packages/app/pr/<name>.md`。

## 面向團隊和企業

有興趣在您的組織中使用 Sofia？我們很樂意聽取您的意見 — 請發送郵件至 [benjamin.shafii@gmail.com](mailto:benjamin.shafii@gmail.com) 與我們討論您的用例。
