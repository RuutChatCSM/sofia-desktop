...
```bash
bash .devcontainer/create-daytona-sofia-snapshot.sh   # one-time / refresh when deps change
bash .devcontainer/test-on-daytona.sh [branch-or-commit]
```

The test script creates a sandbox from the reusable `sofia-eval-vnc` snapshot
...
`sofia-eval-pnpm-store` volume so the image stays under Daytona's 20 GB limit.
...
Future Daytona test sandboxes mount `sofia-eval-secrets:/daytona-secrets`
...
The artifacts flow mounts `sofia-eval-artifacts:/daytona-artifacts`, starts a
...
Do not use the generic `daytona create https://github.com/RuutChatCSM/sofia-desktop`
...
```bash
bash .devcontainer/create-daytona-sofia-server-snapshot.sh  # one-time / refresh when deps change
...
2. `.devcontainer/create-daytona-sofia-snapshot.sh` bakes that image into
   `sofia-eval-vnc` without `node_modules`.
3. `/opt/sofia-daytona/start-daytona-vnc.sh` starts Xvfb, XFCE, x11vnc, and
   noVNC on display `:99`.
4. `test-on-daytona.sh` installs dependencies through the reusable
   `sofia-eval-pnpm-store` volume when `node_modules` is missing or the
   lockfile changed.
5. Vite serves the React UI on port 5173.
6. `/opt/sofia-daytona/start-daytona-electron.sh` sources optional secrets,
