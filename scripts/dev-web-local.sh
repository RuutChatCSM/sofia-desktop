...
: "${DATABASE_URL:=mysql://root:password@127.0.0.1:3306/sofia_den}"
...
pnpm --filter @sofia-ee/den-db db:push <<'EOF'
y
EOF
...
(
  ...
  env \
    ...
    NEXT_PUBLIC_SOFIA_AUTH_CALLBACK_URL="$DEN_WEB_ORIGIN" \
