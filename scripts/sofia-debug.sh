#!/usr/bin/env bash
#
# sofia-debug.sh — one-stop observability + lifecycle control for the
# Sofia dev stack.
#
# Subcommands:
...
#   wait-healthy    block until sofia-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Dev app: pnpm --filter @sofia/desktop dev:electron launches the Electron
...
#   - ~/Library/Application Support/com.differentai.sofia.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/Sofia.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${SOFIA_DEV_LOG_FILE:-$HOME/.sofia/debug/sofia-dev.log}"
PNPM_DEV_LOG="${SOFIA_PNPM_DEV_LOG:-/tmp/sofia-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${SOFIA_PNPM_DEV_PID:-/tmp/sofia-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${SOFIA_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${SOFIA_ELECTRON_REMOTE_DEBUG_PORT:-9823}"

# ---------------------------------------------------------------------------
# Helpers

log() { printf '[sofia-debug] %s\n' "$*"; }
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.sofia/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.SOFIA_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  echo "=== sofia-server ==="
  local port
  port=$(discover_sofia_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev sofia-server running)"
  else
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with SOFIA_DEV_LOG_FILE set)"
  fi
}
...
diagnose_hang() {
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(SOFIA_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_sofia_server_port)
  if [[ -n "$port" ]]; then
    echo "  sofia-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no sofia-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/sofia-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set SOFIA_FORCE_SAMPLE=1 not currently supported."
  else
...
start() {
  ...
  env SOFIA_DEV_LOG_FILE="$DEV_LOG_FILE" \
    nohup pnpm --filter @sofia/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
  pid=$!
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "sofia-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "sofia-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the sofia-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.differentai.sofia.dev"
