...
async function main() {
  ...
  const workDir = join(tmpdir(), `sofia-migrate-validate-${args.tag}`);
  ...
  const appPath = join(unzipDir, "Sofia.app");
  if (!existsSync(appPath)) {
    console.log(`[validate] ✗ Sofia.app not found inside zip (got: ${unzipDir})`);
  ...
  const steps = [
    ...
    "Restart, see the 'Sofia is moving' modal, click 'Install now'.",
