...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "Sofia is moving" prompt on next update |
...
- Pushes to `dev` or `main` refresh the rolling prerelease bucket at
  <https://github.com/RuutChatCSM/sofia-desktop/releases/tag/electron-preview-latest>.
...
- The install handoff requires a pasted Electron artifact URL plus two explicit
  ...
  `Sofia.app.migrate-bak` for rollback.
...
3. Ask testers to download the matching platform artifact:
   - macOS Apple Silicon: `sofia-mac-arm64-*.dmg` or `.zip`
   - macOS Intel: `sofia-mac-x64-*.dmg` or `.zip`
   - Windows: `sofia-win-x64-*.exe`
   - Linux: `sofia-linux-x64-*.AppImage` or `.tar.gz`
...
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/RuutChatCSM/sofia-desktop/releases/download/v0.12.0/Sofia-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_SOFIA_MIGRATION_RELEASE=1` and the per-platform download
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_SOFIA_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `Sofia.app.migrate-bak`. Instruct users
  to `mv Sofia.app.migrate-bak Sofia.app` if the Electron
  launch fails.
