...
const composeProject = "sofia-den-local"
...
const appPort = process.env.SOFIA_APP_PORT?.trim() || process.env.PORT?.trim() || "5173"
const extraAppPorts = (process.env.SOFIA_EXTRA_APP_PORTS?.trim() || "5174")
  .split(",")
  .map((value) => value.trim())
  .filter(Boolean)
const databaseUrl = process.env.DATABASE_URL?.trim() || "mysql://root:password@127.0.0.1:3306/sofia_den"
...
async function main() {
  ...
  await run("bash", ["-c", "pnpm --filter @sofia-ee/den-db build && pnpm --filter @sofia-ee/den-db exec node --import tsx ./node_modules/drizzle-kit/bin.cjs push --config drizzle.config.ts --force"], {
    env: {
      ...process.env,
  ...
  turboChild = spawn(
    ...
    [
      ...
      "dev:local",
      "--output-logs=full",
        "--filter=@sofia-ee/den-api",
        "--filter=@sofia-ee/inference",
        "--filter=@sofia-ee/den-worker-proxy",
        "--filter=@sofia-ee/den-web",
    ],
    {
      cwd: rootDir,
      stdio: "inherit",
