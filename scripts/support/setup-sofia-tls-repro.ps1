param(
    [string]$Hostname = "poc.sofia.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "SofiaTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning Sofia TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "SofiaTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-Sofia-TLS-Repro", $Label)
...
Write-Step "Sofia TLS repro setup"
...
$rootSubject = "CN=Sofia TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=Sofia TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=Sofia TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\sofia-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"Sofia TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-sofia-tls-repro.ps1 -Cleanup"
