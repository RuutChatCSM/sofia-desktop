...
const landingConfig = await readFile(
  new URL("../ee/apps/landing/components/sofia-connect-installer-config.ts", import.meta.url),
  "utf8",
);
const docsInstaller = await readFile(
  new URL("../packages/docs/snippets/sofia-connect-installer.jsx", import.meta.url),
  "utf8",
);
...
assert.equal(serverUrl, "https://sofia-api.ruut.chat/mcp/agent", "Sofia Connect must use the public /mcp/agent endpoint");
...
assert.ok(!cloudDocs.includes("sofia-ui-mcp"), "Cloud MCP docs must not reference the local UI MCP package");
...
assert.ok(!onboardingScreen.includes("sofia-ui-mcp"), "Cloud onboarding must not copy the local UI MCP package");
assert.ok(
...
console.log("Sofia Connect landing and docs installers are in parity.");
