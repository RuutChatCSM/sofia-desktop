...
test("uses a non-production temporary demo root by default", () => {
  ...
  assert.equal(root, path.join(os.tmpdir(), "sofia-two-electron-demo"));
  assert.notEqual(root, path.join(os.homedir(), ".sofia"));
});

test("honors an explicit demo root", () => {
  assert.equal(
    resolveDemoRoot({
      SOFIA_ELECTRON_DEMO_ROOT: " /tmp/sofia-custom-demo "
    }),
    "/tmp/sofia-custom-demo"
  );
});

test("creates fresh, independent folders for every demo launch", async context => {
  const testRoot = await mkdtemp(path.join(os.tmpdir(), "sofia-demo-test-"));
...
test("reopens the same prepared profile pair without falling back to another profile", async context => {
  const testRoot = await mkdtemp(path.join(os.tmpdir(), "sofia-demo-reopen-test-"));
  context.after(() => rm(testRoot, { recursive: true, force: true }));
...
test("reset removes all prior demo runs from the configured root", async context => {
  const testRoot = await mkdtemp(
    path.join(os.tmpdir(), "sofia-demo-reset-test-")
  );
  context.after(() => rm(testRoot, { recursive: true, force: true }));
...
test("points each Electron instance at its own profile folders", async context => {
  const testRoot = await mkdtemp(
    path.join(os.tmpdir(), "sofia-demo-env-test-")
  );
  context.after(() => rm(testRoot, { recursive: true, force: true }));
  const run = await createDemoRun(testRoot);
  const profile = {
  ...
  assert.equal(adminEnv.SOFIA_ELECTRON_USERDATA, run.admin.userDataDir);
  assert.equal(adminEnv.SOFIA_DATA_DIR, run.admin.dataDir);
  ...
  assert.equal(adminEnv.SOFIA_ELECTRON_USE_MOCK_KEYCHAIN, "1");
  assert.equal(adminEnv.SOFIA_ELECTRON_DISABLE_PROTOCOL_REGISTRATION, "1");
  assert.equal(
    consumerEnv.SOFIA_ELECTRON_USERDATA,
    run.consumer.userDataDir
  );
  assert.equal(consumerEnv.SOFIA_DATA_DIR, run.consumer.dataDir);
  assert.notEqual(
    adminEnv.SOFIA_ELECTRON_USERDATA,
    consumerEnv.SOFIA_ELECTRON_USERDATA
  );
  assert.notEqual(adminEnv.SOFIA_DATA_DIR, consumerEnv.SOFIA_DATA_DIR);
