...
const outputDir = resolve(process.env.RUNNER_TEMP || ".", "sofia-electron-manifests");
...
function isReleaseAsset(path) {
  ...
  if (!basename(path).startsWith("sofia-")) return false;
...
function validateManifest(name, manifest) {
  ...
  const expectedPrefix = distribution === "public" ? "sofia-" : `sofia-${distribution}-`;
  if (
    urls.some((url) => !url.startsWith(expectedPrefix))
    || (distribution === "public" && urls.some((url) => /^sofia-(?:cloud|enterprise)-/.test(url)))
  ) {
