...
const pinnedWorkspaceEngineVersion = String(
  readJson(resolve(root, "constants.json")).engineVersion ?? "",
)
  .trim()
  .replace(/^v/, "");
...
const versions = {
  ...
  engine: pinnedWorkspaceEngineVersion || null,
};
...
if (versions.engine) {
  addCheck("Sofia version pin exists", true, String(versions.engine));
} else {
  addWarning("Sofia version is not pinned in constants.json.");
}
