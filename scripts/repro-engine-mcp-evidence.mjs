...
async function bootstrapDen(denApiUrl) {
  ...
  const password = `Sofia-${runTag}-Evidence!`;
...
async function waitForServerListening(child, logStream) {
  let stdout = "";
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("Timed out waiting for Sofia server listening log")), 90_000);
    const finish = (value) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (stdout.includes("Sofia server listening")) finish();
    });
    child.stderr.on("data", (chunk) => writeLog(logStream, chunk));
    child.once("error", fail);
    child.once("exit", (code, signal) => fail(new Error(`Sofia server exited before listening (code ${code}, signal ${signal})`)));
  });
}
...
async function startSofiaServer(paths, serverPort, opencodeBin) {
  ...
  log(`Starting Sofia server on 127.0.0.1:${serverPort}`);
  ...
  log(`Sofia server log: ${serverLog}`);
...
async function pollHealedHealth(baseUrl, workspaceId) {
  ...
  while (Date.now() - started < 30_000) {
    last = await serverJson(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/health`);
...
async function main() {
  ...
  const paths = { workspaceRoot: join(reproDir, "ws"), xdg: join(reproDir, "xdg"), xdgSofia: join(reproDir, "xdg", "sofia"), home: join(reproDir, "home"), logs: join(reproDir, "logs") };
  ...
  log("PHASE 1: seeding delayed sofia-cloud reconcile");
  ...
  const seedReconcile = await serverJson(sofia.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/reconcile`, {
    method: "POST",
    body: JSON.stringify(reconcilePayload),
  });
  const seedMcp = await serverJson(sofia.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp`);
  const seedHealth = await serverJson(sofia.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/sofia-cloud/health`);

  log("PHASE 2: waiting for delayed Den MCP handshake to heal live engine state");
  await sleep(armedUntilMs + 8000 - Date.now());
  const healedHealth = await pollHealedHealth(sofia.baseUrl, workspaceId);
  const healedMcp = await serverJson(sofia.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp`);
  ...
  const diagnosticsReport = await serverJson(sofia.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/diagnostics/agent-context`, {
    method: "POST",
  ...
  const sofiaCloudSyncStatuses = mcps.filter((mcp) => own(mcp, "name") === "sofia-cloud").map((mcp) => ({
  ...
  const evidence = {
    ...
    phases: {
      ...
      diagnostics: { engineMcpSyncCheck, sofiaCloudSyncStatuses, firstFailedCheck: own(diagnosticsReport, "firstFailedCheck") ?? null, overall: own(diagnosticsReport, "overall") ?? null },
    },
