...
Add a Den-owned SCIM compatibility layer that keeps Better Auth's useful connector/token pieces while giving Sofia control over SCIM resource identity, response shape, pagination, and group semantics.
...
- `organizationId`: owning Sofia org
...
SCIM `Group` is closest to Sofia org teams, but it is not exactly the same thing.

A SCIM Group is an IdP-owned membership container. It may represent teams, departments, roles, app assignments, or access policy groups. Sofia org teams are local collaboration/access structures inside an organization.
...
If product wants SCIM groups to manage Sofia teams:
