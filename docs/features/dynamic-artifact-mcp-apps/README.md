...
Sofia exposes Workflow results as portable, standards-based
...
- tool metadata: `_meta.ui.resourceUri` points to
  `ui://sofia/workflow-artifact/v1/view.html` (with the compatibility metadata
...
The shared payload schema lives in `@sofia/types/workflows` so a host
...
The provider side is deliberately usable without an Sofia-specific host.
