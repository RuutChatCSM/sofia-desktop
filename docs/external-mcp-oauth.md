...
Sofia discovers and authorizes Den-managed external MCP connections without
...
deployment. Sofia derives both public OAuth URLs exclusively from this
...
Sofia as a web OAuth client. Each self-hosted Den deployment has its own
callback and metadata URL.

## OAuth runtime and callback compatibility

Den uses `@sofia/enterprise-mcp-client` for discovery, authorization,
...
In Cloud → Connections, enter the MCP server URL. Sofia automatically runs
...
When authorization starts, Sofia uses this registration priority:
...
challenge nor the administrator selects scopes, Sofia falls back to the
...
- **Additional permission required**: review and approve the newly challenged
  scope before reconnecting; Sofia does not silently expand access.
