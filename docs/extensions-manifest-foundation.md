...
Sofia should expose `extension` as the user-facing abstraction. Claude/Anthropic plugins are an initial source format that Den adapts into Sofia extensions, not a separate product concept.
...
- Voice setup saves/uses an OpenAI key, mints a Realtime client secret through the local server, opens the right-side panel, and executes a semantic Sofia UI action.

Den/local endpoint testing should point Electron at the target Den base URL through the existing Cloud account settings or the desktop bootstrap env/config path. OpenAI-key flows should use the Sofia env store, preferring short-lived renderer credentials for Realtime.
