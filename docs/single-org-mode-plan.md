...
Related: `packaging/helm/sofia-ee`, `ee/apps/den-api/src/auth.ts`, `ee/apps/den-api/src/orgs.ts`, `ee/apps/den-web/app/(den)`
...
Make self-hosted Sofia EE deployments behave like one customer-owned
workspace by default, while keeping hosted Sofia Cloud multi-org.
...
- `packaging/helm/sofia-ee/values.yaml`
- `packaging/helm/sofia-ee/templates/configmap.yaml`
...
2. First `pnpm dev:web-local` runtime attempt started MySQL and schema sync but
   `den-api` failed because `@sofia/install-config` had no `dist/index.js`.
3. `pnpm --filter @sofia/install-config build` fixed the missing package
   artifact.
...
```bash
helm template sofia-ee ./packaging/helm/sofia-ee --set image.tag=test | rg "DEN_ORG_MODE|DEN_SINGLE"
helm template sofia-ee ./packaging/helm/sofia-ee --set config.tenancy.mode=multi_org --set image.tag=test | rg "DEN_ORG_MODE"
```
...
1. `pnpm --filter @sofia-ee/den-api exec bun test test/single-org-mode.test.ts test/single-org-route-guards.test.ts`
   passed 8 tests.
2. `pnpm --filter @sofia-ee/den-api exec tsc --noEmit` passed.
3. `pnpm --filter @sofia-ee/den-web exec tsc --noEmit` passed.
...
9. Native `helm version --short` failed because Helm is not installed locally.
   ...
   - `docker run --rm -v /Users/omar/code/sofia/packaging/helm/sofia-ee:/chart:ro alpine/helm:3.15.4 version --short`
     returned `v3.15.4+gfa9efb0`.
   - `docker run --rm -v /Users/omar/code/sofia/packaging/helm/sofia-ee:/chart:ro alpine/helm:3.15.4 lint /chart --set image.tag=test`
     passed with 0 chart failures.
   - `docker run --rm -v /Users/omar/code/sofia/packaging/helm/sofia-ee:/chart:ro alpine/helm:3.15.4 template sofia-ee /chart --set image.tag=test --show-only templates/configmap.yaml`
     rendered `DEN_ORG_MODE: "single_org"` and singleton settings.
   - `docker run --rm -v /Users/omar/code/sofia/packaging/helm/sofia-ee:/chart:ro alpine/helm:3.15.4 template sofia-ee /chart --set image.tag=test --set config.tenancy.mode=multi_org --show-only templates/configmap.yaml`
...
6. How should existing customer DBs with multiple orgs behave when
   ...
   migrate one org?
7. Should self-host docs call Den web "Sofia Cloud" after Helm defaults to
   single-org mode, or rename that surface in docs to "Sofia web" /
   "Sofia admin" for private deployments?
...
- `packages/docs/start-here/self-host.mdx`: distinguish single-org private
  deployments from hosted Sofia Cloud. The current "Cloud-style accounts,
...
- `packaging/docker/README.md`: avoid calling every local/private web surface
  "Sofia Cloud web app" after single-org mode lands.
