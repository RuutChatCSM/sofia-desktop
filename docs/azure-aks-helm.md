# Deploy Sofia EE on Azure with AKS and Helm

Status: self-host operator guide
Related: `packaging/helm/sofia-ee`, `packaging/helm/sofia-ee/examples/values.azure-ingress.yaml`

This is the recommended Azure path for a first production-like Sofia EE
...
add-on's managed NGINX Ingress controller so Sofia gets stable HTTPS host
...
production workloads through November 2026. The current Sofia chart emits
...
Sofia. Azure Load Balancer is a layer 4 load balancer. It is useful for TCP
...
identities, and network security groups. The Sofia Helm chart owns Sofia
...
The Sofia EE release artifact is already a Helm chart, and AKS gives the
...
not a different Sofia packaging format.
...
export RESOURCE_GROUP=sofia-ee-rg
export AKS_CLUSTER=sofia-ee
export VNET_NAME=sofia-ee-vnet
...
database. This is an Azure subscription/region eligibility issue, not Sofia
quota usage.
...
subscription cannot use, not that Sofia has consumed quota. Pick an allowed
...
- Region capacity varies. If AKS Automatic reports SKU/capacity failures, retry
  in a known-good region before debugging the Sofia chart.
...
indicate an Azure MySQL subscription/region eligibility issue before Sofia or
...
```text
mysql://sofia:<password>@<server>.mysql.database.azure.com:3306/sofia_den?sslaccept=accept
```

Use `?sslaccept=accept` for the simple private-MySQL smoke path. This keeps TLS
on without requiring a cloud CA bundle to be mounted into the Sofia image.
...
```bash
cp packaging/helm/sofia-ee/examples/values.azure-ingress.yaml values.azure.yaml
```
...
helm template sofia-ee oci://ghcr.io/ruutchatcsm/charts/sofia-ee \
  ...
  --namespace sofia-ee \
  -f values.azure.yaml > /tmp/sofia-rendered.yaml
...
ingress:
  tls:
    - secretName: sofia-ee-tls
```

Create that secret from a certificate that covers both Sofia hosts:

```bash
kubectl create namespace sofia-ee

kubectl create secret tls sofia-ee-tls \
  --namespace sofia-ee \
...
- `sofia.example.com` -> the application routing public IP.
- `api.sofia.example.com` -> the same application routing public IP.
...
**Identifier (Entity ID)** is the Sofia auth origin and the certificate saved
in Sofia is the active Entra token-signing certificate.
...
| Browser shows certificate warnings | TLS secret is missing, wrong, or does not cover both hosts | Recreate `sofia-ee-tls` or configure the platform certificate automation |
