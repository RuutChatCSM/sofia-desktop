...
2. `enterprise-mcp-mock-server` must not import Den, an EE app, Sofia's MCP client, or a provider SDK.
...
pnpm --filter @sofia-ee/enterprise-mock-lab dev
```
...
| --- | --- | --- |
| Contract/unit | Scenario schema, profile provenance, fault applicability, revisions, redaction, deterministic state | `pnpm --filter @sofia/enterprise-mcp-mock-server test` |
| Package quality | Public TypeScript surface and distributable build | `pnpm --filter @sofia/enterprise-mcp-mock-server check` |
...
| Control-plane security | Loopback configuration, login/rate limit, session cookie, Origin/CSRF, write-only secrets, safe errors | `pnpm --filter @sofia-ee/enterprise-mock-lab test` |
| App build | EE app imports only the public package contract and compiles as a standalone process | `pnpm --filter @sofia-ee/enterprise-mock-lab build` |
