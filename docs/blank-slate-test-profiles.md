...
```bash
open -a "Sofia Enterprise" --args --blank-slate
# or any flavor:
open -a "Sofia" --args --blank-slate
```

Direct binary (useful for CDP-driven testing):

```bash
SOFIA_ELECTRON_REMOTE_DEBUG_PORT=9898 \
  "dist-electron/mac-arm64/Sofia Enterprise.app/Contents/MacOS/Sofia Enterprise" --blank-slate
```
...
Each launch creates one unique temporary root (`$TMPDIR/sofia-test-profile-*`)
...
- skips `sofia://` protocol registration, Windows shortcut writes, brand
