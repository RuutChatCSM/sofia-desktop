# Deploy Sofia EE on AWS with EKS and Helm

Status: self-host operator guide
Related: `packaging/helm/sofia-ee`, `packaging/helm/sofia-ee/examples/values.aws-load-balancer.yaml`, `packaging/helm/sofia-ee/examples/values.aws-load-balancer-http-smoke.yaml`

This is the recommended AWS path for a first production-like Sofia EE
...
DNS, TLS certificates, IAM, and security groups. The Sofia Helm chart owns
Sofia Deployments, Services, ConfigMaps, Secrets, health probes, and the
database migration Job.

## Use Helm or something else?

Use Helm on EKS for AWS unless the customer explicitly cannot run Kubernetes.
The Sofia EE release artifact is already a Helm chart, the service split maps
...
```text
mysql://sofia:<password>@<rds-endpoint>:3306/sofia_den?sslaccept=accept
```

Use `?sslaccept=accept` for the simple private-RDS smoke path. This keeps TLS on
but does not require the RDS CA bundle to be mounted into the Sofia image.
...
Before installing Sofia, verify network access from the cluster. One simple
...
```bash
cp packaging/helm/sofia-ee/examples/values.aws-load-balancer.yaml values.aws.yaml
```
...
```bash
cp packaging/helm/sofia-ee/examples/values.aws-load-balancer-http-smoke.yaml values.aws.yaml
```
...
helm template sofia-ee oci://ghcr.io/ruutchatcsm/charts/sofia-ee \
  ...
  --namespace sofia-ee \
  -f values.aws.yaml > /tmp/sofia-rendered.yaml
...
- `sofia.example.com` -> Den Web load balancer hostname.
- `api.sofia.example.com` -> Den API load balancer hostname.
...
kubectl describe pods -n sofia-ee
