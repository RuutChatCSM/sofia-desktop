# Deploy Sofia EE on Google Cloud with GKE and Helm

Status: self-host operator guide
Related: `packaging/helm/sofia-ee`, `packaging/helm/sofia-ee/examples/values.gcp-ingress.yaml`

This is the recommended Google Cloud path for a first production-like Sofia
...
in maintenance mode. The current Sofia chart emits Ingress resources, so GKE
...
Sofia. GKE `LoadBalancer` Services are useful for TCP services and quick
...
- one single-org Sofia deployment
...
The Sofia Helm chart owns Sofia Deployments, Services, ConfigMaps,
...
Kubernetes. The Sofia EE release artifact is already a Helm chart, and GKE
...
guidance, not a different Sofia packaging format.
...
- A domain you control, such as `sofia.example.com` and
  `api.sofia.example.com`.
...
export SQL_RANGE=sofia-sql-range
...
```bash
export SQL_INSTANCE=sofia-ee-mysql
...
gcloud sql databases create sofia_den \
  --instance="$SQL_INSTANCE"

gcloud sql users create sofia \
  --instance="$SQL_INSTANCE" \
...
```text
mysql://sofia:<password>@<cloud-sql-private-ip>:3306/sofia_den
```

This guide uses direct private IP because the current Sofia chart does not
...
without requiring a cloud CA bundle to be mounted into the Sofia image. Use
...
Before installing Sofia, verify network access from the cluster:
...
```bash
gcloud compute addresses create sofia-ee-ip \
  --global

gcloud compute addresses describe sofia-ee-ip \
  --global \
...
```bash
kubectl create namespace sofia-ee
```

Create a Google-managed certificate resource:

```bash
kubectl apply -n sofia-ee -f - <<'YAML'
...
Create explicit backend health checks for the two Sofia services:
...
The Helm values annotate the Sofia Services so GKE associates these
...
```bash
cp packaging/helm/sofia-ee/examples/values.gcp-ingress.yaml values.gcp.yaml
```
...
secret:
  values:
    emailFrom: "Sofia <no-reply@example.com>"
    smtpHost: "smtp.example.com"
    smtpPort: "587"
    smtpUser: "sofia@example.com"
...
Use a values file, not a long list of `--set` flags. Several Sofia values are
...
```bash
helm template sofia-ee oci://ghcr.io/ruutchatcsm/charts/sofia-ee \
  --version REPLACE_SOFIA_VERSION \
  --namespace sofia-ee \
  -f values.gcp.yaml > /tmp/sofia-rendered.yaml

grep -E 'DATABASE_URL|BETTER_AUTH_URL|DEN_API_PUBLIC_URL|DEN_WEB_PUBLIC_ORIGIN|EMAIL_FROM|SMTP_HOST|SMTP_PORT|SMTP_SECURE' /tmp/sofia-rendered.yaml
```
...
```bash
helm upgrade --install sofia-ee oci://ghcr.io/ruutchatcsm/charts/sofia-ee \
  --version REPLACE_SOFIA_VERSION \
  --namespace sofia-ee \
  --create-namespace \
  -f values.gcp.yaml
...
```bash
helm upgrade --install sofia-ee ./packaging/helm/sofia-ee \
  --namespace sofia-ee \
  --create-namespace \
  -f values.gcp.yaml
```
...
Avoid `kubectl describe job sofia-ee-migrate` in shared reports because the
...
kubectl get jobs,pods -n sofia-ee
kubectl logs -n sofia-ee -l job-name=sofia-ee-migrate --all-containers=true
```
...
- `sofia.example.com` -> the reserved global IP address.
- `api.sofia.example.com` -> the reserved global IP address.
...
```bash
kubectl get ingress -n sofia-ee
kubectl describe managedcertificate sofia-ee-cert -n sofia-ee
kubectl describe ingress sofia-ee -n sofia-ee
```
...
```bash
kubectl rollout restart deployment/sofia-ee-den-api deployment/sofia-ee-den-web -n sofia-ee
kubectl rollout status deployment/sofia-ee-den-api -n sofia-ee --timeout=180s
kubectl rollout status deployment/sofia-ee-den-web -n sofia-ee --timeout=180s
```
...
```bash
helm status sofia-ee -n sofia-ee
kubectl get pods -n sofia-ee
kubectl get jobs -n sofia-ee
kubectl get ingress -n sofia-ee
kubectl describe backendconfig sofia-ee-den-api-backend -n sofia-ee
kubectl describe backendconfig sofia-ee-den-web-backend -n sofia-ee
kubectl logs -n sofia-ee deploy/sofia-ee-den-api
kubectl logs -n sofia-ee deploy/sofia-ee-den-web
```

Check readiness from your machine:

```bash
curl -fsS https://api.sofia.example.com/ready
curl -fsS https://sofia.example.com/api/ready
```
...
ConfigMap. Then open `https://sofia.example.com/setup`, enter the configured
owner email and one-time operator code, and create the first account. Sofia
...
```text
https://sofia.example.com/api/auth/sso/callback/sofia-sso-<org-id>
```

In Sofia, sign in as the owner, open the organization SSO settings, and enter
the IdP issuer/client details. After saving, the organization sign-in path is:

```text
https://sofia.example.com/sso/<singleOrgSlug>
```

For SAML, Sofia shows the generated ACS URL and metadata URL after the SAML
connection is registered. Use those values in the IdP rather than guessing.
Sofia rejects unsigned or weak SAML responses, so configure the IdP to sign
assertions.
...
| Backends are unhealthy | GKE load balancer health checks do not match Sofia readiness endpoints | Apply the `BackendConfig` resources and keep the service annotations from the starter values |
...
| SSO callback rejected | IdP callback URL does not match Sofia | Use the callback/ACS URL shown by Sofia for that org/provider |
...
```bash
helm uninstall sofia-ee -n sofia-ee
gcloud compute addresses delete sofia-ee-ip --global
