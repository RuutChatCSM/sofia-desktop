...
A per-user **memory bank** inside Sofia. The user opts in by chatting (e.g.
...
Append a static, distinct `## Memory Bank` section to `SOFIA_AGENT_PROMPT`
(`apps/server/src/sofia-runtime-config.ts:34-66`) — separate from the existing
...
- **Per-PR verification gate (`AGENTS.md`) — applies to every stage above.** Each stage's
  ...
  `evals/results/test-runs/<test-run-id>/test-run.json` from an `@sofia/testkit` test, each
...
- **Reference files:** `ee/packages/den-db/src/schema/workers.ts`,
  ...
  `apps/server/src/sofia-runtime-config.ts:34-66`,
...
`~/.agentic-workflow/joi-fairshare-sofia/plans/20260702-112912-memory-bank/consolidated-review.md`.
