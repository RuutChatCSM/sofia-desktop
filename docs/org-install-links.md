...
Related: `ee/apps/den-api/src/install-links.ts`, `ee/apps/den-api/src/routes/org/install-links.ts`, `ee/apps/den-api/src/desktop-connect-grants.ts`, `packaging/helm/sofia-ee/README.md`

## What users get

An organization install link opens a three-step Den page:

1. Download and run the standard Sofia installer.
2. Return to Den and click **Open Sofia**.
...
`https://sofia.example.com`. Set `DEN_API_PUBLIC_URL` to the externally
reachable Den API base; in a single-origin setup, use the Den web proxy path,
such as `https://sofia.example.com/api/den`, so desktops reach only the Den
...
|---|---|---|
| Internet-connected | Immediate `302` to the exact standard asset under `SOFIA_INSTALLER_RELEASE_REPO` and `SOFIA_INSTALLER_RELEASE_TAG`. The organization token is not forwarded. | Den web/API, `github.com`, `release-assets.githubusercontent.com`, and `objects.githubusercontent.com` for legacy or rollback paths. |
| Semi-air-gapped | Stream the matching standard installer from `SOFIA_INSTALLER_ARTIFACTS_DIR`. There is no ZIP or in-memory whole-file buffer. | Den web/API only. |
...
Sofia**, Den mints a fresh five-minute bearer code and stores only its
...
(`sofia-enterprise-<os>-<arch>-<version>.<ext>`, published on the
`enterprise` release channel), or the public installer — and writing
`desktop-bootstrap.json` directly:

| OS | Canonical path |
|---|---|
| Windows | `%LOCALAPPDATA%\sofia\desktop-bootstrap.json` (`%XDG_CONFIG_HOME%\sofia\desktop-bootstrap.json` wins if set) |
| macOS/Linux | `$XDG_CONFIG_HOME/sofia/desktop-bootstrap.json`, falling back to `~/.config/sofia/desktop-bootstrap.json` |

```json
{
  "baseUrl": "https://sofia.example.com",
  "apiBaseUrl": "https://api.sofia.example.com",
  "requireSignin": true,
  "writtenAt": "2026-07-14T12:00:00.000Z"
}
```

Current builds still read the older `~/.config/sofia` path for
...
| **Open Sofia** cannot prepare a connection | Verify the browser can reach Den API and `DEN_API_PUBLIC_URL` is the correct public HTTPS origin. |
| The app refuses an expired or used link | Return to the same Den install page and click **Open Sofia** again to mint a fresh code. |
