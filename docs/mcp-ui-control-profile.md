# Control Sofia from any MCP client

Sofia exposes its UI as an MCP server so any MCP-capable app can read what's on screen and run actions — no DOM scraping, no coordinates, no accessibility hacks.
...
Sofia takes a different approach: the app itself tells you what actions are available, what the current state is, and lets you execute actions by name. The MCP server wraps that surface so any MCP client gets a first-class, semantic control experience out of the box.
...
> Want to control Sofia Cloud workers and server APIs instead of the desktop UI? Check out the **Sofia Cloud MCP** (separate package, coming soon).

## Quick start with HandsFree

HandsFree auto-discovers the Sofia MCP server when both apps are running on the same machine. No config needed.

1. Launch **Sofia** (desktop app).
2. Launch **HandsFree**.
3. Open the HandsFree connector panel — you should see **Sofia** with a green "Connected" status and an action count.
...
- `ui_status` — check if Sofia is running and the bridge is reachable.

### Cross-session memory

Sofia's cross-session memory currently comes from saved session history exposed through the UI control surface. It is not a separate long-term memory database.
...
This may navigate Sofia away from the user's current session while the lookup runs. If multiple sessions match, ask which one to inspect.

### Sofia agents

Inside Sofia, agents control the app through the semantic tools (`sofia_context`, `sofia_query`, `sofia_execute`) using affordance ids from context. External MCP clients can also use the hidden **Sofia UI Control** MCP via **Settings -> Extensions -> Show hidden**.

## Install

```bash
npm install -g sofia-ui-mcp
```
...
```
> Use ui_snapshot to see what's on screen in Sofia, then list the available sessions.
```
...
If you're building an app that speaks MCP, you can connect to the Sofia UI server the same way:
...
Check if Sofia is running and reachable. Returns connection status and app info.
...
Get the current Sofia UI state: active route, narration, visible actions, and status. Call this before acting to understand what the user sees.
...
Execute an Sofia UI action by its id.
...
The exact list depends on the current Sofia route and state. Common actions include:
...
- **Sofia desktop** must be running. The MCP server connects to Sofia's local bridge which starts automatically when the desktop app launches.
- **macOS** is the primary supported platform. The bridge uses Electron IPC and writes a discovery file to `~/Library/Application Support/com.differentai.sofia/`.
...
1. Sofia desktop starts a private localhost HTTP bridge on a random port, protected by a bearer token.
2. It writes a discovery file with the port and token so `sofia-ui-mcp` can find it.
3. `sofia-ui-mcp` reads the discovery file, proxies MCP tool calls to the bridge, and returns structured results.
4. The bridge calls `window.__sofiaControl` inside the Electron renderer to snapshot state and execute actions.

The bridge and discovery file are implementation details — you never need to touch them directly. Just point your MCP client at `sofia-ui-mcp`.
