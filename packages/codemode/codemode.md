...
This is the living design and status document for `@engine-ai/codemode` and its existing V2 Sofia adapter.
...
`packages/engine/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
