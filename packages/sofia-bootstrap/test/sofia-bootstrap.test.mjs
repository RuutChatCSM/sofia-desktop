...
const cli = join(root, "bin", "sofia.mjs")
const temp = mkdtempSync(join(tmpdir(), "sofia-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "sofia-bootstrap.cmd" : "sofia-bootstrap"
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First Sofia Skill", output: "SOFIA_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  // and `apiBaseUrl`, so a normal `--base-url https://sofia-api.ruut.chat`
  // run broke the desktop app's Sign In flow (it opened
  // `https://sofia-api.ruut.chat/?mode=sign-in...` and showed raw API JSON).
  await withStubDenApi(
    () => ({
      ok: true,
      organization: { id: "org_test", name: "Stub Org", slug: "org_test", status: "provisional" },
    ...
    async (baseUrl) => {
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ brandAppName: "Sofia Demo A" })}\n`,
        "utf8",
      )
      ...
      assert.equal(writtenExplicit.brandAppName, "Sofia Demo A", "prepared desktop setup must preserve its profile identity")
    },
  )
