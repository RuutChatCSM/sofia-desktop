...
You are an agent helping a user install and set up Sofia.

Goal: install Sofia, create a provisional workspace without requiring email
...
offer to connect the Sofia MCP to the agent the user is already using.

> The bootstrap command is `sofia-bootstrap`. Use that exact command for
> these setup steps so the bootstrap flow stays distinct from other Sofia
> tooling a user may already have on their PATH.

## 1. Install CLI

Install the `sofia-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/sofia-bootstrap/bin/sofia.mjs install \
  --install-dir "$HOME/.sofia/bootstrap" \
...
`sofia-bootstrap` CLI (a single dependency-free Node file) and installs it
...
```bash
sofia-bootstrap install app \
  --manifest https://sofia.ruut.chat/install-manifest.json \
  --json
```
...
```bash
sofia-bootstrap doctor --app --json
```
...
```bash
sofia-bootstrap cloud bootstrap-workspace \
  --base-url https://sofia-api.ruut.chat \
  --workspace-name "<workspace-name>" \
  --skill-name "First Sofia Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that Sofia is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "Sofia
...
4. End with this single question: "Want me to connect the Sofia MCP to this
...
1. A provisional workspace must be claimed before its owner can authenticate
   the Sofia MCP. If it has not been claimed yet, explain that dependency and
...
3. For Codex, run:
   ...
   If an `sofia` entry already exists, do not add a duplicate. Authenticate
   the existing entry instead. To switch organizations or recover stale auth,
   run `codex mcp logout sofia` before `codex mcp login sofia`.
4. For another agent, use its current instructions from
   `https://sofia.ruut.chat/docs/cloud/run-in-the-cloud/cloud-mcp`; do not guess
...
5. Tell the user to restart or reopen the current agent after setup so the new
   ...
   client can see Sofia's `search_capabilities` and `execute_capability`
   tools.
...
```bash
sofia-bootstrap cloud claim-link --role owner --json
```
...
- `sofia-bootstrap doctor --json` returns `ok: true`
- `sofia-bootstrap doctor --app --json` returns `ok: true`
- `sofia-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `SOFIA_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `sofia-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `sofia-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/sofia-bootstrap`).
- If app install fails: run `sofia-bootstrap doctor --app --json` and report failed checks.
