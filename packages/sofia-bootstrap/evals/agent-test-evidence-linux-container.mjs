// Linux E2E test evidence for Sofia bootstrap without shipping to production.
//
// This runs inside a real Linux container and proves:
// - the bootstrap CLI installs on Linux,
// - `sofia install app` installs a Linux tar.gz app artifact from a manifest,
// - `sofia doctor --app` verifies the installed app,
// - `sofia cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/sofia-bootstrap:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:18995 pnpm exec node evals/agent-test-evidence-linux-container.mjs
...
const outDir = join(repoRoot, "evals", "results", "sofia-linux-container")
...
const artifactPath = join(fixtureDir, "sofia-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/SOFIA_OWNER_PASSWORD=[^\s]+/g, "SOFIA_OWNER_PASSWORD=[redacted]")
}

function prove(claim, { action, assert, evidence }, ok) {
  // Record the screenshot regardless of outcome so the test-evidence report still renders.
...
try {
  const appBinary = join(artifactRoot, "sofia")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho Sofia Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "sofia"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/sofia-linux.tar.gz", sha256: digest, appName: "sofia" },
        arm64: { type: "tar.gz", url: "file:///fixture/sofia-linux.tar.gz", sha256: digest, appName: "sofia" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local sofia tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/sofia.mjs install --install-dir /tmp/sofia/install --bin-dir /tmp/sofia/bin --json >/tmp/install.json",
    "/tmp/sofia/bin/sofia-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/sofia/apps --json >/tmp/app-install.json",
    "/tmp/sofia/bin/sofia-bootstrap doctor --install-dir /tmp/sofia/install --bin-dir /tmp/sofia/bin --app --app-dir /tmp/sofia/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run sofia install, sofia install app, and sofia doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/sofia.mjs install --install-dir /tmp/sofia/install --bin-dir /tmp/sofia/bin --json >/tmp/install.json",
    `/tmp/sofia/bin/sofia-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/sofia/desktop-bootstrap.json --skills-dir /tmp/sofia/skills --json >/tmp/onboard.json`,
    "/tmp/sofia/bin/sofia-bootstrap doctor --install-dir /tmp/sofia/install --bin-dir /tmp/sofia/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/sofia/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `SOFIA_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run sofia cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "SOFIA_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
<title>Sofia Linux Bootstrap — test evidence</title>
...
</head><body><main><h1>Sofia Linux Bootstrap — test evidence</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Screenshots: ${frames.length}</div>
...
console.log(`test evidence: ${join(outDir, "fraimz.html")}`)
