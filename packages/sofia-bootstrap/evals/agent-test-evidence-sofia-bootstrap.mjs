// Live E2E test evidence for the script-installable Sofia bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... pnpm exec node evals/agent-test-evidence-sofia-bootstrap.mjs
...
if (!baseUrl) {
  throw new Error("DEN_API_E2E_BASE_URL is required for live E2E test evidence")
}
...
const temp = mkdtempSync(join(tmpdir(), "sofia-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installedSofia = join(binDir, process.platform === "win32" ? "sofia-bootstrap.cmd" : "sofia-bootstrap")
const outDir = join(repoRoot, "evals", "results", "sofia-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the sofia CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedSofia)

  const doctor = run(installedSofia, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "sofia doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedSofia, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
    ...
    skillName,
    "--json",
  ], { env: { ...process.env, SOFIA_OWNER_PASSWORD: randomPassword() }, timeout: 30_000 })
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "sofia cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
  ...
  }, onboard.status === 0 && onboard.json?.ok === true && onboard.json?.organization?.id && onboard.json?.invitation?.invitationId && onboard.json?.skill?.id && onboard.json?.skill?.title === skillName && onboard.json?.skillRun?.triggered === true && onboard.json?.skillRun?.output === "SOFIA_BOOTSTRAP_SKILL_TRIGGERED")
} finally {
...
<title>Sofia Bootstrap CLI — live E2E test evidence</title>
...
<h1>Sofia Bootstrap CLI — live E2E test evidence</h1>
