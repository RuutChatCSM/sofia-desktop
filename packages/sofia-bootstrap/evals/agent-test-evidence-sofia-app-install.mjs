// Live local E2E test evidence for `sofia install app` using a real macOS DMG.
//
// The flow creates a tiny Sofia.app fixture, packages it as a .dmg, writes an
// install manifest with a SHA-256 digest, installs the bootstrap CLI into a temp
// bin dir, then runs the installed `sofia install app` command against the
// manifest and verifies it with `sofia doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("sofia install app DMG test evidence requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "sofia.mjs")
const outDir = join(repoRoot, "evals", "results", "sofia-app-install-dmg")
const temp = join(tmpdir(), `sofia-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
const sourceDir = join(temp, "source")
const appFixture = join(sourceDir, "Sofia.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "Sofia.dmg")
const manifestPath = join(temp, "sofia-install-manifest.json")
const installedSofia = join(binDir, "sofia-bootstrap")
...
<key>CFBundleExecutable</key><string>Sofia</string>
<key>CFBundleIdentifier</key><string>com.sofia.fixture</string>
<key>CFBundleName</key><string>Sofia</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "Sofia"), "#!/usr/bin/env sh\necho Sofia fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "Sofia"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "Sofia", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create Sofia.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/sofia.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an sofia executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installedSofia))

  const installApp = run(installedSofia, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  const appPath = join(appDir, "Sofia.app")
  prove("The installed CLI can download, verify, mount, and install Sofia.app from a DMG", {
    action: "sofia install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and Sofia.app copied into app dir",
  ...
  const doctor = run(installedSofia, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "sofia doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with sofiaApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "sofiaApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
