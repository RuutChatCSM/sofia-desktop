...
// bootstrap actions from other Sofia commands a user may already have.
const COMMAND_NAME = "sofia-bootstrap"
const DEFAULT_SOFIA_MARKETPLACE_NAME = "Sofia Marketplace"
...
function printHelp() {
  console.log([
    ...
    "",
    "Usage:",
    "  sofia-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  sofia-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  sofia-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  SOFIA_OWNER_PASSWORD=<password> sofia-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  sofia-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  sofia-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the sofia-bootstrap CLI into a user bin dir",
...
function defaultInstallDir() {
  return process.env.SOFIA_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".sofia", "bootstrap")
}

function defaultBinDir() {
  return process.env.SOFIA_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.SOFIA_APP_DIR || (process.platform === "darwin"
    ? join(process.env.HOME || process.cwd(), "Applications")
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "Sofia")
      : join(process.env.HOME || process.cwd(), ".local", "share", "sofia"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.SOFIA_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "sofia", "desktop-bootstrap.json")
}
...
function defaultDeviceKeyPath() {
  return process.env.SOFIA_DEVICE_KEY_PATH || join(configHomeDir(), "sofia", "bootstrap-device-key.json")
}
...
// browser opens `https://sofia-api.ruut.chat/?mode=sign-in...` and shows raw
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "sofia-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "sofia.mjs")
  ...
  jsonOut({ ok: true, message: `Sofia CLI installed at ${executable}`, install: manifest }, json)
}
...
function defaultInstalledName(type, manifest, artifact) {
  ...
  if (type === "dmg") return "Sofia.app"
  if (type === "appimage") return "Sofia.AppImage"
  if (type === "exe") return "Sofia.exe"
  if (type === "msi") return "Sofia.msi"
  if (process.platform === "darwin") return "Sofia.app"
  if (process.platform === "win32") return "Sofia.exe"
  return "sofia"
}
...
function installDmg(input) {
  ...
  try {
    ...
    mounted = true
    const appName = input.appName || "Sofia.app"
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.SOFIA_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `sofia-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    const artifactPath = join(workDir, artifact.fileName || "Sofia.dmg")
    ...
    writeFileSync(join(appDir, "sofia-app-install.json"), JSON.stringify(install, null, 2))
    jsonOut({ ok: true, message: `Sofia app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  checks.push({ name: "sofiaExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    const appManifest = join(appDir, "sofia-app-install.json")
    let appPath = process.platform === "darwin"
      ? join(appDir, "Sofia.app")
      : process.platform === "win32"
        ? join(appDir, "Sofia.exe")
        : join(appDir, "sofia")
    ...
    checks.push({ name: "sofiaApp", ok: existsSync(appPath), value: appPath })
  ...
  jsonOut({ ok, message: ok ? "Sofia doctor: ok" : "Sofia doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by sofia bootstrap.\nsofiaBootstrapTrigger: bootstrap.verify\nsofiaBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm Sofia cloud onboarding can create and trigger a deterministic skill.`
}
...
async function createCloudSkillPlugin(baseUrl, auth, input) {
  ...
  const marketplace = marketplaces.body.items.find((item) => item?.name === DEFAULT_SOFIA_MARKETPLACE_NAME) || marketplaces.body.items[0]
  if (!marketplace?.id) {
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "sofiaBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "sofiaBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "sofia-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "SOFIA_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First Sofia Skill")
  const skillOutput = getFlag(args.flags, "skill-output", "SOFIA_BOOTSTRAP_SKILL_TRIGGERED")
  ...
  jsonOut({
    ok: true,
    message: "Sofia cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  const json = hasFlag(args.flags, "json")
  const baseUrl = getFlag(args.flags, "base-url", "https://sofia-api.ruut.chat")?.replace(/\/$/, "")
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "SOFIA_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "SOFIA_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ok: true,
    message: "Sofia workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "sofia-bootstrap cloud claim-link --role ${link.role}" to view`
