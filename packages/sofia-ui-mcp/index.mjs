 ...
 * sofia-ui-mcp
 *
 * MCP server that exposes Sofia's semantic app context and UI control
 * surface as an MCP resource and tools.
 * Speaks MCP stdio and proxies to the Sofia desktop bridge HTTP API.
 *
 * Requires Sofia desktop running with the local UI control bridge active.
 *
 * Usage:
 *   npx sofia-ui-mcp
 *
...
const DISCOVERY_FILE = "sofia-ui-control.json";
...
function discoveryPaths() {
  return [
    ...
    join(userAppDataDir(), "com.differentai.sofia", DISCOVERY_FILE),
    join(userAppDataDir(), "com.differentai.sofia.dev", DISCOVERY_FILE),
  ].filter(Boolean);
}
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "Sofia is not running. Launch the Sofia desktop app first.",
      hint: "The MCP server connects to a running Sofia instance via its local bridge.",
    };
  }
...
// ── sofia://context/current ──
server.resource(
  "current_sofia_context",
  "sofia://context/current",
  async (uri) => {
...
server.tool(
  "ui_context",
  "Read Sofia's semantic app context: current screen, all open conversation tabs, split-screen layout, focused pane, sidebar, side panel, settings, and currently available queries and commands. Reads app state without focusing the window.",
  {},
  async () => {
    const result = await bridgeRequest("/context");
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current Sofia UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    return { content: [{ type: "text", text: lines.join("\n") || "Sofia is reachable, but it did not return visible UI state." }] };
  }
);
...
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in Sofia: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is Sofia on the main screen?" }] };
    }
...
server.tool(
  "ui_execute_action",
  "Execute an Sofia UI action by its id without activating the desktop window. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_query",
  "Run a side-effect-free Sofia query by id. Use ui_context to inspect currently available queries. Queries never focus or navigate the app.",
  {
...
server.tool(
  "ui_command",
  "Execute a semantic Sofia command by id while Sofia remains in the background. Use ui_context first and pass its revision to prevent acting on stale UI state.",
  {
...
server.tool(
  "ui_status",
  "Check if Sofia is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "Sofia is not running.\nLaunch the Sofia desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "Sofia"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nSofia may have quit. Relaunch it.` }], isError: true };
    }
  }
);
