...
Shared UI primitives for Sofia apps.

This package ships one entrypoint: `@sofia/ui/react`, used by `apps/app`
...
The first shared components live under the `paper` namespace and wrap Paper Design shaders with Sofia-specific defaults and deterministic seed support.
...
`SofiaRoadmap` is the shared visual roadmap used by the landing and docs
routes. Its typed sections are exported as `roadmapSections` so other Sofia
