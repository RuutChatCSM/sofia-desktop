...
Native macOS computer-use runtime for Sofia.
Packaged Sofia builds wrap this runtime in a bundled `Computer Use.app`
...
```bash
pnpm --filter @sofia/handsfree check:native
```

Run it as an MCP-compatible adapter:

```bash
pnpm --filter @sofia/handsfree exec sofia-handsfree-computer-use mcp
```
