export const SofiaConnectInstaller = () => {
...
engine mcp auth sofia`;
  const installs = [
    ...
    },
    {
      ...
      supportStatus: "Setup only",
      supportExplanation: "Setup guide only: add Sofia, run codex mcp login sofia, and reconnect with logout then login. Native proof must be rerun on this exact branch.",
      copyText: `codex mcp add sofia --url ${MCP_SERVER_URL}`,
  ...
  "mcp": {
    ...
    },
    {
      ...
      copyText: `code --add-mcp '{"name":"sofia","type":"http","url":"${MCP_SERVER_URL}"}'`,
    },
    {
      id: "any-client",
      label: "Any client",
  ...
  return (
    <div id="connect-mcp-install" className="not-prose my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-950">
      ...
      <div className="overflow-x-auto border-b border-gray-100 px-4 pt-4 dark:border-white/10" role="tablist" aria-label="Sofia MCP client install options">
      ...
      <div className="flex flex-col gap-2 border-t border-gray-100 px-5 py-4 text-xs sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button type="button" aria-label="Copy the Sofia MCP server URL" onClick={() => copy("server-url", MCP_SERVER_URL)} className="shrink-0 font-medium underline">
