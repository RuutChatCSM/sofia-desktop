...
- `sofia-models.ruut.chat` is the default Sofia model catalog. Set `SOFIA_ENGINE_MODELS_URL` to an approved internal mirror if the public catalog is blocked.
...
| `sofia-models.ruut.chat` | Required in practice unless mirrored | Default Sofia model catalog passed as `SOFIA_ENGINE_MODELS_URL`. | Model selection can be stale, incomplete, or unavailable. |
...
| `sofia.ruut.chat`, `sofia.dev`, `discord.gg`, `ollama.com`, `engine.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |
