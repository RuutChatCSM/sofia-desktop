...
description: "Customer IT allowlist guidance for Sofia desktop, Den, deployment artifacts, and opt-in providers."
---

Sofia works best when customer IT approves outbound access by component. The canonical machine-readable source is [`docs/enterprise/outbound-access.json`](https://github.com/RuutChatCSM/sofia-desktop/blob/dev/docs/enterprise/outbound-access.json). CI guards that file with `node scripts/check-outbound-access.mjs`; this page is the published human-readable summary.

## Minimum hosted desktop allowlist

For a practical Sofia Cloud desktop install, allow TCP 443 to these hosts:
...
- `registry.npmjs.org` is used when packaged desktop builds start `npx -y sofia-ui-mcp`. If it is blocked, the desktop can still open, but UI-control MCP is unavailable.
- `sofia-models.ruut.chat` is the default OpenCode model catalog. Set `OPENCODE_MODELS_URL` to an approved internal mirror if the public catalog is blocked.

## Desktop and local runtime destinations

| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `sofia-app.ruut.chat` | Required for Sofia Cloud | Hosted Cloud web origin. The desktop uses one base URL and derives Den API/MCP calls through `/api/den/...`. | Cloud sign-in, org settings, Collections, and remote workspace flows cannot load. |
| `sofia-api.ruut.chat` | Required for hosted install and Sofia Connect | Hosted public API and Sofia Connect MCP endpoint used by install and external-client flows. | Hosted install checks and public Sofia Connect fail. |
| `app.sofia.software`, `api.sofia.software` | Required only for legacy hosted configurations | Older desktop configurations may still derive Cloud and Sofia Connect traffic from these legacy hosted origins. | Legacy configurations cannot reconcile Sofia Connect or native MCP Apps until moved to the current hosted origins. |
| Your self-hosted Den web origin | Required for self-hosted desktop | Replaces `sofia-app.ruut.chat` for signed-in desktop traffic. In the single-origin shape, API and MCP use `/api/den/...` on the same origin. | The desktop cannot sign in or reach Den. |
...
| `registry.npmjs.org` | Required in practice unless npm is mirrored | `npx` package resolution for `sofia-ui-mcp` and development-only helper commands. | Sofia UI-control MCP is unavailable. |
| `sofia-models.ruut.chat` | Required in practice unless mirrored | Default OpenCode model catalog passed as `OPENCODE_MODELS_URL`. | Model selection can be stale, incomplete, or unavailable. |
...
| `sofia.ruut.chat`, `sofia.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |

`code.claude.com` and `schemas.agentskills.io` appear in source or schema metadata. They are findable in the manifest, but Sofia does not fetch them for normal runtime behavior.
...
| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `ghcr.io` | Required unless mirrored | Sofia Helm chart and container images. | Kubernetes cannot pull the chart or `sofia-den-api`, `sofia-den-web`, and optional `sofia-inference` images. |
