...
sign in to Sofia and start with
[Connect your services](/start-here/connect-your-stack/connect-services).

Slack's official MCP server does not support automatic OAuth client registration. To connect it as a custom local MCP in Sofia, ask a Slack admin to create or approve a Slack app for MCP, then add the app's OAuth client credentials in Sofia.
...
In Slack, an admin should create or approve an app for Sofia's MCP access.
...
4. Add the `User Token Scopes` for the Slack MCP tools your team wants Sofia to use.
...
In Sofia, open your workspace and go to `Settings` > `Library`, expand `Advanced settings`, and click `Add workspace MCP`.

<Frame>
  ![Sofia Library page listing skills, apps, and connections](/images/library-slack-connection.png)
</Frame>

## Add the Slack MCP server

In the `Add workspace MCP` dialog:

1. Set `App name` to `slack`.
2. Keep `Type` set to `Remote server (URL)`.
3. Set `Server URL` to `https://mcp.slack.com/mcp`.
4. Open `OAuth on this device`.
...
<Frame>
  ![Add workspace MCP dialog filled in for Slack with OAuth on this device expanded](/images/slack-mcp-advanced-oauth.png)
</Frame>

Sofia writes the MCP entry with the OAuth client credentials, similar to this:
...
If you entered scopes, Sofia also saves them as `oauth.scope`. This is the scope list Slack sees during OAuth consent.

## Sign in to Slack

After the MCP app is added, reload the engine if Sofia asks you to. Then start Slack MCP sign-in from the Slack app card or the MCP auth prompt.

Slack opens in your browser and shows the scopes you configured. Approve the Slack app for the workspace you want Sofia to use, then return to Sofia when authorization finishes.
...
If Slack says a scope is invalid or unavailable, remove that scope from both the Slack app and Sofia's `OAuth scopes` field, then retry. Slack workspace policies and plan settings can limit which scopes an app may request.

If your Slack workspace blocks app installation, ask a Slack admin to approve the app first. Sofia cannot bypass Slack workspace approval policies.
