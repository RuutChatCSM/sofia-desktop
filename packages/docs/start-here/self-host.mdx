---
title: "Self-host"
description: "Run Sofia on your own server or infrastructure."
---

Most users just use the [desktop app](https://sofia.ruut.chat/download). Self-hosting is for teams that want Sofia to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team Collections, or shared providers without running infra? Use [Sofia Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).

## What you deploy

Sofia is split into normal services:

- **Sofia app**: the desktop or web client people use.
...
- **Inference service**: optional Sofia Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want Sofia Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
The chart pulls the matching GHCR images for `sofia-den-api`, `sofia-den-web`, and optional `sofia-inference` unless you mirror them into an internal registry.
...
For containerized services, start from the [Docker packaging](https://github.com/RuutChatCSM/sofia-desktop/tree/dev/packaging/docker) in the repo.

## Databases and storage

- Worker runtimes use filesystem state and engine SQLite data inside the mounted workspace/data paths.
...
- Hosted Sofia uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/sofia_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. Sofia uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
Private deployments should keep public signup disabled and use the one-time [first-administrator setup flow](/self-host/deploy-to-your-cloud/first-administrator). Configuring an owner or platform-admin email does not create an account or password, and Sofia does not ship a default administrator password.
...
If you do not deploy Den, worker access is token-based with `SOFIA_TOKEN` and `SOFIA_HOST_TOKEN`.

## Minimal working stack

### Sofia Cloud and the desktop app

Sofia cloud-style self-hosting is composed of three main pieces:
...
Sofia does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**Sofia Cloud**](/cloud/get-started) runs hosted workers for your team.
...
If you're rolling Sofia out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
