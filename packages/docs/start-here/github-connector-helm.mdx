...
description: "Create a GitHub App and configure the Sofia EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted Sofia deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
The GitHub App needs these Sofia URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://sofia.example.com` |
...
## What Sofia does with it

When an admin clicks **Connect GitHub** in Sofia:
...
```text
https://sofia.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to Sofia, show repositories visible to the installation, and allow discovery on a selected repository.
