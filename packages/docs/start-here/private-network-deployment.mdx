...
description: "Run Sofia when laptops have internet and VPN access while Den stays inside a private network."
---

Most enterprise Sofia installs are not fully air-gapped. The common shape is:
...
`DEN_API_PUBLIC_URL` still must be configured correctly, but it does not need to be a second desktop-facing origin. In the single-origin shape, set it to the proxied API base, for example `https://sofia.example.internal/api/den`. Den uses that value for install-link claims, OAuth callbacks, webhooks, MCP resource/audience values, and links.
...
External MCP clients such as Cursor, Claude Code, VS Code, ChatGPT, and Sofia are separate from the desktop. They must reach the MCP endpoint you configure for them, usually `<DEN_API_PUBLIC_URL>/mcp/agent`; with the single-origin proxy example above, that is `https://sofia.example.internal/api/den/mcp/agent`. See [Sofia Connect MCP](/cloud/run-in-the-cloud/cloud-mcp) for the external-client flow.

Unless you mirror or replace public dependencies internally, laptops may also need outbound access for installers, release assets, npm packages used by `npx`, and the Sofia model catalog. Keep that allowlist in [Outbound network access](/start-here/outbound-network-access), and use [Installer delivery](/start-here/installer-delivery) when installer downloads must stay inside Den.
...
- The Sofia chart and images at deploy or image-pull time, unless you mirror the Helm chart and enabled images into an internal registry.
...
**Why:** `DEN_DIAGNOSTICS_ORIGIN` defaults to the public Sofia diagnostic service.
...
| Cloud MCP works in a real chat | In a chat, the agent can search available Sofia capabilities and execute one that the user is allowed to run. | `Settings → Connect` for provider sign-in or admin setup. Use `Settings → Debug` to interpret diagnostics, especially a skipped catalog probe. |
