...
Organization branding lets an owner customize the Sofia experience for everyone signed in to an on-prem Den organization. Uploaded images stay inside the deployment: Den stores the binary image data in the existing MySQL database and serves signed, versioned URLs from your Den API surface.
...
<Note>
  Branding changes the displayed organization experience. It does not rename or re-sign the installed executable, and the signed application identity remains Sofia.
</Note>
...
config:
  public:
    webOrigin: "https://sofia.example.internal"
    apiOrigin: "https://sofia.example.internal/api/den"
    desktopDenBaseUrl: "https://sofia.example.internal"
    corsOrigins: "https://sofia.example.internal"
    betterAuthTrustedOrigins: "https://sofia.example.internal"
    webAppHosts: "sofia.example.internal"
```
...
- Use `Clear` and save to return to stock Sofia branding.
...
2. Reopen Sofia to force an immediate configuration fetch instead of waiting for the hourly background refresh.
