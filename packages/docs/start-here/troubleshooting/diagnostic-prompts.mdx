...
description: "Paste-able prompts that let your agent debug Sofia issues on its own."
---

When something in Sofia looks wrong, the fastest debugger is usually the agent itself: it can read its own configuration, probe servers, and verify fixes in-band. This page collects paste-able diagnostic prompts. Copy the whole block for your situation into a chat and let the agent work through it.
...
Your agent's `sofia-cloud` MCP entry is a snapshot: it stores a server URL and token from the moment it was connected. If it was created against a local development server or an old deployment — including via a leftover `desktop-bootstrap.json` file that overrides server URLs beneath the Settings UI — the agent keeps querying that old world forever, while the dashboard shows the current one.

**Paste this to your agent**

````text
Debug and fix my Sofia Cloud MCP connection. It may be pointed at a stale or local server instead of my real Sofia Cloud server. Work through ALL phases, show your findings after each, and ask before anything destructive. Never print bearer tokens or Authorization headers — redact them.

My expected cloud web URL: https://sofia-app.ruut.chat (replace with your self-hosted Den web URL if you run your own).

## Phase 1 — Evidence: what am I actually connected to?

1. Print the `sofia-cloud` MCP entry's `url` (redact headers) from every config you can find:
   - `~/.config/engine/engine.json`
   - any `.sofia/engine.json` or `engine.jsonc` in the current project
...
2. Inspect these files if they exist (they can silently override server URLs beneath the Settings UI), but report only `baseUrl`, `apiBaseUrl`, `requireSignin`, and `writtenAt`. Never print `handoff`, `claimLinks`, grants, tokens, or other credential-bearing fields:
   - `~/.config/sofia/desktop-bootstrap.json`
   - `~/Library/Application Support/com.differentai.sofia/desktop-bootstrap.json`
...
- sofia-cloud url is localhost/127.0.0.1, OR search returned only gmail-drafts, OR execute returned unknown_capability → you are bound to a STALE server, not the expected cloud.
...
10. Tell me to do these steps in Sofia, then wait:
    - Restart the Sofia app.
    - In the affected workspace, open Settings → Library and click Refresh (re-mints the token and rewrites the Cloud MCP config).
    - Click Show hidden, then verify Sofia Cloud Control under Your apps.
    - Run Reload Sofia config from the command palette.
...
- The `sofia-cloud` entry's `url` points at your real server and ends in `/mcp/agent` (either a direct API host or the web app's `/api/den` proxy).
...
If the agent completes all phases and verification still fails, [open an issue](https://github.com/RuutChatCSM/sofia-desktop/issues) and paste the agent's final report (it is already redacted).

## 2. Cloud MCP is configured, but its tools are missing

**Symptoms**

- Settings shows an `sofia-cloud` MCP entry, but the agent calls `sofia_docs_search` for requests involving connected services.
...
- A connected service such as ServiceNow appears ready in Sofia Cloud, but the task does not expose `sofia-cloud_search_capabilities` or `sofia-cloud_execute_capability`.

**What this diagnostic checks**

The prompt below runs Sofia's bundled Sofia executable against its generated runtime config and checks whether a fresh Sofia process can connect to `sofia-cloud`. It does not attach to the Sofia server already running inside Sofia, so it combines that result with the live status shown in Sofia and a forced tool call from a new task.

**Paste this to your agent**

````text
Diagnose whether Sofia Cloud MCP tools are available in this workspace. Gather evidence only: do not modify configuration, authenticate, log out, remove anything, restart services, or print configuration-file contents.

## Phase 1 — Test the task's actual tools

1. Attempt to call `sofia-cloud_search_capabilities` with `{"query":"ServiceNow incident","type":"mcp","limit":10}`. Do not use documentation or explain alternatives. Record whether the runtime accepts the tool call and its exact non-secret result or rejection.
2. Attempt to call `sofia_extension_list_actions`. Record whether the runtime accepts the call. Do not infer tool availability from an answer to "list your tools"; models can omit or invent tools.

## Phase 2 — Find Sofia's exact Sofia runtime

3. Detect whether this machine is Windows, macOS, or Linux.
4. Find the exact Sofia executable used by the Sofia desktop app. Prefer the bundled executable in Sofia's `resources/sidecars` directory; do not assume an unrelated `engine` on `PATH` is the same binary. If multiple Sofia builds are running, use the process associated with this task when that can be established safely; otherwise report the ambiguity.
5. Find Sofia's generated `runtime-engine-config.json` beside the active `server.json`. Respect discovered `SOFIA_SERVER_CONFIG`, `SOFIA_RUNTIME_DB`, or XDG overrides. Typical locations are:
   - Windows: `%APPDATA%\sofia\runtime-engine-config.json`
   - macOS/Linux: `${XDG_CONFIG_HOME:-$HOME/.config}/sofia/runtime-engine-config.json`
...
7. From this task's workspace root, set `SOFIA_ENGINE_CONFIG` to the discovered runtime config for the subprocess only, then run:
...
9. Report:
   - operating system
   - exact Sofia executable path and version
   - runtime config path, existence, and last-modified time
   - whether `sofia-cloud` is listed
...
Important: `engine mcp list` starts a fresh process. Do not claim that it attaches to or tests the Sofia server already running inside Sofia. Describe it only as a fresh-process probe using the same binary and generated config.

## Phase 4 — Classify the result

- Fresh probe omits `sofia-cloud`: the generated config is missing, the wrong runtime config was selected, or this is a secondary workspace whose MCPs are registered dynamically.
...
2. Click **Show hidden**, then find **Sofia Cloud Control** under **Your apps**. This row shows live Sofia MCP status; the catalog card only proves that the entry is configured.
3. Expand the row and retain its status and non-secret error: **Ready**, **Issue**, **Sign in needed**, **Offline**, or **Paused**.
4. Return to the task, open the command palette, run **Reload Sofia config**, and create a new task in the same workspace.
5. Repeat the forced Cloud tool call from Phase 1.

**How to interpret both results**

| Fresh `engine mcp list` | Sofia status / forced call | Likely boundary |
| --- | --- | --- |
