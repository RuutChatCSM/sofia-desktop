...
The prompt below runs Sofia's bundled OpenCode executable against its generated runtime config and checks whether a fresh OpenCode process can connect to `sofia-cloud`. It does not attach to the OpenCode server already running inside Sofia, so it combines that result with the live status shown in Sofia and a forced tool call from a new task.
...
## Phase 2 — Find Sofia's exact OpenCode runtime

3. Detect whether this machine is Windows, macOS, or Linux.
4. Find the exact OpenCode executable used by the Sofia desktop app. Prefer the bundled executable in Sofia's `resources/sidecars` directory; do not assume an unrelated `opencode` on `PATH` is the same binary. If multiple Sofia builds are running, use the process associated with this task when that can be established safely; otherwise report the ambiguity.
5. Find Sofia's generated `runtime-opencode-config.json` beside the active `server.json`. Respect discovered `SOFIA_SERVER_CONFIG`, `SOFIA_RUNTIME_DB`, or XDG overrides. Typical locations are:
   - Windows: `%APPDATA%\sofia\runtime-opencode-config.json`
   - macOS/Linux: `${XDG_CONFIG_HOME:-$HOME/.config}/sofia/runtime-opencode-config.json`
...
Important: `opencode mcp list` starts a fresh process. Do not claim that it attaches to or tests the OpenCode server already running inside Sofia. Describe it only as a fresh-process probe using the same binary and generated config.
...
2. Click **Show hidden**, then find **Sofia Cloud Control** under **Your apps**. This row shows live OpenCode MCP status; the catalog card only proves that the entry is configured.
...
| Fresh `opencode mcp list` | Sofia status / forced call | Likely boundary |
| --- | --- | --- |
