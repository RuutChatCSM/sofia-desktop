...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in Sofia Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in Sofia Cloud

1. Open `Connectors` in the Sofia Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in Sofia Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted Sofia Cloud instance, the dashboard is
`https://sofia-app.ruut.chat` and its public Den API origin is
`https://sofia-api.ruut.chat`. Register this exact redirect URL with the OAuth
provider:

```text
https://sofia-api.ruut.chat/v1/mcp-connections/oauth/callback
```

For a self-hosted instance whose public Den API is
`https://api.sofia.example.com`, register:

```text
https://api.sofia.example.com/v1/mcp-connections/oauth/callback
```
...
3. In Sofia Cloud, add the Slack connection and paste the app's client ID
   and client secret.
...
Granted members find the connection in `Settings` > `Sofia Connect`, grouped by what
they need to do:
...
1. Sign in to Sofia, then open `Settings` > `Sofia Connect`.
...
4. The browser shows `Connected` — return to Sofia.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in Sofia Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through Sofia Connect,
...
action and direct you to `Settings` > `Sofia Connect`.
...
services managed through Sofia Cloud.
