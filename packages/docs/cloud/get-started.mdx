---
title: "Get started with Sofia Cloud"
...
<Note>
Sofia Cloud is optional. The local desktop app stays free, requires no Cloud sign-in, and supports your own providers, skills, plugins, and custom MCP servers. Use Cloud when an organization needs to assign and manage those resources for a team.
</Note>

Sofia Cloud is available at [sofia-app.ruut.chat](https://sofia-app.ruut.chat/). It is for teams that want shared identity, Collections, managed connections and models, desktop policies, or hosted workspaces.

First, create or join an organization and connect the desktop app to it.

<Frame>
  ![Sofia Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [sofia-app.ruut.chat](https://sofia-app.ruut.chat/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if Sofia Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://sofia-app.ruut.chat/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to Sofia automatically, click `Paste sign-in code`, paste the `sofia://den-auth?...` link or one-time code from Sofia Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. The hosted
default is `https://sofia-app.ruut.chat`.
...
- Sofia Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
back to `https://sofia-app.ruut.chat`. When you change the Cloud URL in
`Settings > Account`, Sofia writes the new `baseUrl` back to the bootstrap
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted Sofia runtimes that teammates can open from the desktop app.
