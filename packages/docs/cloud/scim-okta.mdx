...
description: "Provision Sofia members and teams from Okta"
---

Use Okta Lifecycle Management to create, update, deactivate, and reactivate
Sofia members. Okta groups can also become SCIM-managed Sofia teams.
...
Keep Sofia SSO enforcement off until both SAML and a narrowly scoped SCIM
test pass. Make sure at least one Sofia owner can sign in through Okta.

## 1. Create the Sofia SCIM connector

In Sofia, select the organization, then open **Settings → SCIM**. Confirm
the page reports that SAML/SSO is active, then:

1. Select **Create connector**.
2. Copy the **SCIM base URL**.
3. Copy the bearer token immediately. Sofia displays the full token only
...
<Note>
Use the base URL Sofia displays. In a split-host Helm deployment it may use
...
1. Open **Applications → Applications → Sofia**.
...
| Okta field | Value |
| --- | --- |
| SCIM connector base URL | Sofia **SCIM base URL** |
...
| Authorization | Sofia SCIM bearer token |
...
## 3. Configure provisioning to Sofia
...
does not create an Sofia password credential.
...
can create a duplicate Sofia member instead of linking SSO and SCIM.
...
5. In Sofia, open **Members** and verify the same member appears without a
...
1. Changing the profile name in Okta updates the Sofia member.
...
3. The user cannot regain access through password sign-up while SCIM marks the
   identity inactive.
4. Reactivating and reassigning the user restores the same Sofia identity.
5. An unrelated Sofia member remains unchanged throughout the test.

Sofia retains a disconnected member record for organization history. The
global user is deleted only after their final active organization membership
is removed.

## 6. Push Okta groups as Sofia teams

After user provisioning works:

1. In Sofia **Settings → SCIM**, enable
...
5. In Sofia, open **Members → Teams** and verify a team labeled
   **Managed by SCIM** appears with the expected members.
6. Add and remove one member in Okta and verify the Sofia team follows.

Manage pushed groups in Okta, not in Sofia. Manually created Sofia teams
remain independent.
...
3. Review Sofia's SCIM health and unresolved-failure count.
...
Use **Settings → SCIM → Run reconciliation** to check Sofia's local
SCIM-managed identities for inconsistent organization membership or missing
provider-account state. Sofia records unresolved drift for retry or manual
review.
...
Assignments page and System Log for outbound provisioning. When Sofia has no
...
| --- | --- | --- |
| **Create connector** is disabled | Sofia does not see an enabled SSO provider | Complete and test Okta SAML first; if SAML returns `invalid_saml_configuration`, upgrade and save the connection again |
| **Test API Credentials** fails | Incorrect base URL, stale token, whitespace, or a rewritten hostname | Copy both values again from Sofia and use the advertised base URL unchanged |
| Okta reports `409 User already exists` | The user existed before SCIM, or more than one create job ran for the same assignment | Inspect the System Log first. If a successful create immediately precedes the conflict and the exact member exists in Sofia, stop triggering **Provision User**. Use a never-before-provisioned user for the clean lifecycle test |
...
| **Run reconciliation** does not provision an assigned user | Reconciliation only checks Sofia's local SCIM identity state | Trigger and inspect outbound provisioning in Okta instead |
| Deactivation does not remove access | **Deactivate Users** is disabled or Okta has not sent the event | Enable it and inspect the Okta System Log and Sofia SCIM health |
| A pushed group does not create a team | Sofia team synchronization is off | Enable **Create teams from SCIM groups**, then retry the Okta group push |
