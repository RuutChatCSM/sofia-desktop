...
description: "Provision Sofia members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate Sofia
members automatically. Entra security groups can also become Sofia teams
...
- An Sofia organization with Enterprise SSO and SCIM.
- An Sofia owner, or a member who can manage security configuration.
- A working Sofia SAML connection. Complete and test
...
sure at least one Sofia owner can sign in through SAML before relying on
Entra as the identity source.

## 1. Get the Sofia connection values

In Sofia, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. Sofia shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as Sofia teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | Sofia **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for Sofia SAML SSO.
3. Open **Provisioning**.
...
6. Paste the Sofia **SCIM base URL** into **Tenant URL**.
7. Paste the Sofia **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to Sofia
...
create a second Sofia member for the same person.
...
not enough; assign it to the Sofia enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In Sofia, open **Members** and confirm that the member appears with the
   expected email.
...
6. In Sofia, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to Sofia. Add members and select them explicitly
...
5. Refresh Sofia **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that Sofia removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected Sofia
   deprovisioning behavior.
3. Add another assigned group and confirm that Sofia creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in Sofia. Sofia keeps manually
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from Sofia, then retest |
...
| On-demand group provisioning reports an internal server error without an Sofia request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no Sofia team appears | Sofia team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate Sofia member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but Sofia still shows the old state | The UI or incremental cycle has not refreshed | Refresh Sofia, check Entra **Provisioning logs**, and review the SCIM health status in Sofia |
