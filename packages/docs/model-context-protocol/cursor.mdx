---
title: "Sofia MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for Sofia Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The Sofia MCP server ([Sofia Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the Sofia capabilities your
...
Linear, Slack, Stripe, Exa, and Context7, shared skills, and Sofia Cloud
...
Add Sofia as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to Sofia and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. Sofia accepts exactly this
...
Other private-use callback schemes remain rejected: Sofia otherwise accepts
...
Sofia, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which Sofia
accepts.

## Switch organizations

Remove the stored MCP auth token for `sofia`, then connect again and choose
...
steps match Sofia Connect's OAuth profile: HTTPS or HTTP loopback callbacks
