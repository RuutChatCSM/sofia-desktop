---
title: "Sofia MCP for Gemini CLI"
sidebarTitle: "Gemini CLI"
description: "Gemini CLI MCP setup for Sofia Connect, with OAuth access to your org's Gmail, Calendar, Drive, shared connections, skills, and Cloud resources."
---

The Sofia MCP server ([Sofia Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Gemini CLI to use the Sofia capabilities your organization has made
...
1. Add Sofia Connect as a remote HTTP MCP server:

   ```sh
   gemini mcp add --transport http sofia https://sofia-api.ruut.chat/mcp/agent
   ```
...
3. Authenticate Sofia inside Gemini:
...
Remove the stored MCP auth token for `sofia`, then run `/mcp auth sofia`
