...
export function buildConnectDeepLink(token: string, scheme = "sofia"): string {
  return `${scheme}://${CONNECT_LINK_ROUTE}?token=${encodeURIComponent(token)}`
}

export function buildConnectExchangeDeepLink(code: string, apiBaseUrl: string, scheme = "sofia"): string {
