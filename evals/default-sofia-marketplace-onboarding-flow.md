# Default Sofia Marketplace onboarding flow
...
Marketplace containing built-in Sofia extension capabilities.

## Acceptance target

- Den download/sign-in copy does not claim that Den creates or loads a desktop workspace.
- Den introduces the built-in Sofia Marketplace and explains it appears after desktop sign-in.
- A signed-in desktop user can open Settings -> Marketplace and see `Sofia Marketplace` from Den.
- `Sofia Marketplace` includes built-in extension entries such as `Sofia Browser`, `Computer Use`, `OpenAI Image Gen`, `Google Workspace`, and `Ollama`.
...
8. Validate dev auth before UI recording: direct Den API sign-in with the seeded user should return `200` when `SOFIA_DEV_MODE=1`.
...
- Page includes `built-in Sofia Marketplace` or `built-in Marketplace`.
...
**Goal:** Sofia remains usable without account, while Marketplace clearly asks
for Cloud sign-in.
...
- The notice says the user can use Sofia without an account.
...
**Goal:** Desktop sign-in causes Den to provision and return the default Sofia
Marketplace for any user/org.

Steps:

1. Sign Electron into Sofia Cloud using the Daytona Den handoff flow.
2. Create or open a local workspace.
3. Open Settings -> Marketplace.
4. Click `Refresh` if the Marketplace list has not loaded yet.
5. Open `Filters` and inspect marketplace options.
6. Search for `Sofia Browser`.
7. Open the `Sofia Browser` card.

Expected outcome:

- `Sofia Marketplace` appears as a marketplace option.
- `Sofia Browser` appears as a Marketplace card from Den.
- The card shows `Built-in` or an equivalent built-in/ready status.
- The detail modal shows Sofia Browser setup/resource details from the Den extension manifest.
- The detail modal does not offer `Add` or `Remove` for built-in Sofia entries.
...
2. Den explains in plain language: Marketplaces contain plugins; Sofia Marketplace is included; org marketplaces appear after desktop sign-in.
...
6. Marketplace is pre-populated without manual refresh, with `Sofia Marketplace` grouped separately from org marketplaces.
...
stores and returns Sofia extension manifests; the desktop still owns local
