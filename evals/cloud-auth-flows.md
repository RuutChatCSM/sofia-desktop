...
End-to-end user flows for signing the desktop app into Sofia Cloud through a
...
3. From Den Web, start a desktop handoff or create one through Den API:
   ...
   curl -H "authorization: Bearer $TOKEN" \
     ...
     --data '{"desktopScheme":"sofia"}' \
     "$DEN_API_URL/v1/auth/desktop-handoff"
   ```
4. Paste the full `sofia://den-auth?...&denBaseUrl=...` link into `Paste sign-in code`.
...
- `localStorage.sofia.den.authToken` is present.
...
- `localStorage.sofia.den.authToken` remains empty.
