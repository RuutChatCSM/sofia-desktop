...
Date: 2026-05-21
PR: https://github.com/RuutChatCSM/sofia-desktop/pull/1887

## Topology

- Server sandbox: `sofia-server-20260521-195504`
- Electron sandbox: `sofia-test-20260521-200726`
...
- Pasted the full `sofia://den-auth?...` link into the desktop Cloud Account manual sign-in flow.
- Electron showed `Sofia Cloud Connected` and `Signed in as daytona-proof-1779419514@example.com`.
- Den API logs showed:
...
- Not fixed in this PR: raw manual sign-in grant paste does not reliably target the Den API in split Den Web/Den API deployments. Use the full `sofia://den-auth?...&denBaseUrl=...` handoff link for now.
...
- `bash -n .devcontainer/create-daytona-sofia-server-snapshot.sh`
- `pnpm --filter @sofia/app typecheck`
