...
Sofia bootstraps workspace files such as `opencode.jsonc`, `.opencode/agents`,
and legacy command frontmatter. Those internal startup writes should refresh the
Sofia/OpenCode baseline silently. The user-facing **Reload required** toast is
...
Pass criteria:
- Sofia creates the default project `opencode.jsonc` and `.opencode/agents/sofia.md`.
