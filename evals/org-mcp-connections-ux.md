...
> ones). It has one live connection — Sofia Cloud — and everything else it
...
- Tools reach the agent through `search_capabilities` / `execute_capability`
  on the single Sofia Cloud connection — not as per-provider live engine
  connections.
...
3. **Local-process entries stay engine-direct.** Sofia Browser, UI
...
4. Den's callback page completes the exchange and renders an arrival page:
   "Connected ✓ — Open Sofia" (`sofia://` deep link + auto-attempt).
...
- Signed-out desktops show a single dismissible hint row in Extensions:
  "Sign in to Sofia Cloud to see apps shared by your organization."
...
- Callback arrival page ("Connected ✓ — Open Sofia").
