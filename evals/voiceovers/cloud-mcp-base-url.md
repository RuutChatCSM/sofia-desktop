...
1. I sign in to Sofia Cloud, and the desktop automatically prepares its cloud-agent connection.

2. The minted token advertises the desktop's configured base URL through `/api/den/mcp`, never the separate direct API origin.

3. After refreshing the connection, `sofia-cloud` uses `<baseUrl>/api/den/mcp/agent`, replacing any prior `api.` endpoint.
