...
Today Sofia's engine config is two-way and guessy: some settings changes
write into the managed runtime file, others write through the engine into
the user's personal Sofia files, and old versions left untraceable
...
Sofia treats the user's personal config as read-only, and a one-time
migration moves Sofia-written leftovers out of personal files into the
managed file, with a backup.

1. When something looks wrong with my agent's config today, I have to guess — changes land partly in Sofia's managed file and partly in my personal Sofia files, and old versions left ghosts in both.

2. After the update, managed state flows one way only: Sofia writes a single runtime file it owns, my personal config becomes read-only to the app, and a one-time migration moves the old leftovers over — with a backup and a notice showing exactly what moved.
