...
2. Sofia previews both files and validates their format, dimensions, and intended use.

3. If the owner's sign-in is no longer recent, saving opens a clear security check instead of exposing a raw server error.

4. The owner verifies once, and Sofia resumes the pending upload automatically without making them choose the files again.
