# cloud-instance — Sofia Cloud opens a full instance in the browser, booted just in time
...
required for them to run. An enabled org's members get a full Sofia instance in
the browser — a Daytona sandbox running sofia-server plus the web UI from our
own snapshot. No orchestrator, no download.

1. I'm signed in to Sofia on the web. There's no Cloud anywhere — not in the
...
4. I open Cloud. It brings an instance up for me on the spot, and in a few
   seconds I'm looking at the full Sofia interface in my browser.
