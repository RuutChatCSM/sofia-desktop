...
1. I open SCIM settings before SSO is configured. Sofia explains that verified SSO is required and keeps SCIM disabled.
...
4. When SSO is explicitly required and the verified provider exists, Sofia redirects matching users through the configured identity provider as expected.
