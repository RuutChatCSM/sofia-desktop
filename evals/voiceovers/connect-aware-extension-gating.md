...
The collision: Sofia ships two Google Workspace integrations. The legacy
...
agent to Sofia Cloud, and never touches a device where the legacy path is
actually configured.
...
4. If an agent still calls a hidden Google Workspace action, it no longer gets an OAuth error to misread. It gets a redirect: use Sofia Cloud's search and execute capabilities, and if the user needs to connect, send them to Settings > Connect — never Settings > Extensions.
