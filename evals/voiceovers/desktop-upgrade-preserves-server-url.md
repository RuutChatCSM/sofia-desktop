...
1. This Windows desktop installation is connected to Example Manufacturing’s on-prem Sofia server, and its organization URL is recorded in the desktop-bootstrap configuration.
...
3. When Sofia launches after the upgrade, it reconnects to the configured on-prem server—not a stale, default, or cloud URL.
...
6. Finally, with cloud access unavailable, Sofia continues using the on-prem URL without attempting to replace it, preserving support for air-gapped deployments.
