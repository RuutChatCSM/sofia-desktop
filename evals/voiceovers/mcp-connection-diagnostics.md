...
Cast is Alex, the Acme Robotics admin, in Sofia Cloud. Alex is adding an
...
2. He enters an intentionally unreachable endpoint with no authentication. Sofia validates it through the full MCP initialize path; this is a real server-side probe, not client-side URL validation.
