...
1. First, the real Sofia connect path builds the authorization URL and shows the exact callback it will send. The product diagnostic for a redirect-whitelist callback error is still generic, so the frame captures that gap honestly.
...
3. When the provider advertises dynamic client registration, Sofia really does attempt it. If the authorization step still returns a DCR-required error, the current product surface is a generic authorization rejection.
...
5. For duplicate amplification, one user tool action creates exactly one Sofia tools/call request. The provider's duplicate-request signal is currently collapsed to a generic provider tool error.
