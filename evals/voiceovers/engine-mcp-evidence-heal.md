...
1. On an isolated Daytona sandbox, we recreate the customer's trap against a real Sofia server, real engine, and real Den: the very first cloud connection attempt is forced to outlast the registration window, so the system records a failure — exactly what a slow first handshake does in the field.
