...
This proof uses an isolated desktop instance, so the bootstrap file shown here is a temporary eval file rather than the user's real Sofia configuration.
...
5. Alex returns to Advanced and clears the server configuration. The Organization server section returns to standard Sofia Cloud, and the temporary bootstrap file is gone without resetting workspaces.
