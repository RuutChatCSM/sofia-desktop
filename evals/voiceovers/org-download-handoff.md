...
("Open Sofia", an auth handoff) assumes the app is already installed, next
...
3. The guide waits with her while she installs, and says it plainly: only continue once Sofia is installed and running on this computer — and if she already has the app, one click says so.

4. She clicks Open Sofia, and the app jumps to the front asking exactly what's about to happen: connect this copy to Acme Robotics, on Acme's server — nothing changed yet.
...
6. One click on Sign in with Sofia Cloud, and she's standing in Acme's workspace — nobody typed a server URL, ever.
