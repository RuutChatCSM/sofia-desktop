...
const DEFAULT_PASSWORD = process.env.SOFIA_EVAL_SUPER_ADMINS_PASSWORD?.trim() || "SofiaDemo123!";
const FIXTURE_ORG_NAME = process.env.SOFIA_EVAL_SUPER_ADMINS_ORG_NAME?.trim() || "Cloud Super Admins Eval";

const OWNER: FixtureActor = {
  email: process.env.SOFIA_EVAL_SUPER_ADMINS_OWNER_EMAIL?.trim() || "casey.owner.super-admins@sofia.test",
  name: "Casey Owner",
  password: process.env.SOFIA_EVAL_SUPER_ADMINS_OWNER_PASSWORD?.trim() || DEFAULT_PASSWORD,
};

const ADMIN: FixtureActor = {
  email: process.env.SOFIA_EVAL_SUPER_ADMINS_ADMIN_EMAIL?.trim() || "riley.admin.super-admins@sofia.test",
  name: "Riley Admin",
  password: process.env.SOFIA_EVAL_SUPER_ADMINS_ADMIN_PASSWORD?.trim() || DEFAULT_PASSWORD,
};

const SECOND_ADMIN: FixtureActor = {
  email: process.env.SOFIA_EVAL_SUPER_ADMINS_SECOND_ADMIN_EMAIL?.trim() || "taylor.admin.super-admins@sofia.test",
  name: "Taylor Admin",
  password: process.env.SOFIA_EVAL_SUPER_ADMINS_SECOND_ADMIN_PASSWORD?.trim() || DEFAULT_PASSWORD,
};

const MEMBER: FixtureActor = {
  email: process.env.SOFIA_EVAL_SUPER_ADMINS_MEMBER_EMAIL?.trim() || "morgan.member.super-admins@sofia.test",
  name: "Morgan Member",
  password: process.env.SOFIA_EVAL_SUPER_ADMINS_MEMBER_PASSWORD?.trim() || DEFAULT_PASSWORD,
};
...
const MODELS_CHILDREN = ["Sofia Models", "LLM Providers"];
