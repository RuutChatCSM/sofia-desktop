...
// (evals/voiceovers/sofia-models-hidden-single-org-dashboard.md).
// The runner fails this flow if the narration drifts from that script.
const FLOW_ID = "sofia-models-hidden-single-org-dashboard";
...
export default {
  id: FLOW_ID,
  title: "Single-org (self-hosted) dashboard hides the Sofia Models nav and page",
  kind: "user-facing",
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The Models nav group contains no Sofia Models entry and lands on LLM Providers", {
          voiceover: vo[1],
          action: async () => {
            await ctx.waitFor(`(() => {
          ...
          assert: async () => {
            const nav = await ctx.eval(`(() => {
              ...
              return {
                ...
                hasSofiaModels: links.some((l) => l.text.includes('Sofia Models')) || (nav?.innerText ?? '').includes('Sofia Models'),
            ...
            ctx.assert(!nav.hasSofiaModels, "The navigation must not mention Sofia Models on a self-hosted deployment.");
      ...
      run: async (ctx) => {
        await ctx.prove("Visiting /dashboard/inference directly redirects to LLM Providers", {
          ...
          screenshot: {
            ...
            requireText: ["LLM Providers"],
            rejectText: ["Subscribe with Stripe", "Sofia Models"],
          },
        });
      },
    },
