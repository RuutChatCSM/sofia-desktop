...
const FLOW_ID = "reliable-sofia-connect";
...
// Narration is loaded from the approved script (evals/voiceovers/reliable-sofia-connect.md).
...
async function ensureLandingConnect(ctx) {
  const url = `${baseUrlFromEnv(ctx, "SOFIA_EVAL_LANDING_URL")}/#connect-mcp`;
  await applyDesktopViewport(ctx);
  await navigateBrowser(ctx, url, "landing Sofia Connect section");
  await ctx.waitFor(
    `(() => {
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Sofia MCP client - ${escapeHtml(input.status)}</title>
    <style>
...
async function startLoopbackServer() {
  ...
  let details = ["The browser is authorizing Sofia MCP access."];
  let resolveCallback;
  ...
  const server = http.createServer((request, response) => {
    ...
    if (requestUrl.pathname === "/callback") {
      ...
      details = [
        "Sofia redirected back to the client loopback callback.",
...
async function prepareNativeSofiaEnvironment() {
  ...
  state.nativeEnv = {
    ...
    SOFIA_EVAL_BROWSER_CAPTURE_ENDPOINT: state.nativeBrowserCaptureEndpoint,
    NO_COLOR: "1",
  };
...
const endpoint = process.env.SOFIA_EVAL_BROWSER_CAPTURE_ENDPOINT || "";
const launcher = process.env.SOFIA_EVAL_BROWSER_LAUNCHER || "BROWSER";
...
SOFIA_EVAL_BROWSER_LAUNCHER=${JSON.stringify(launcherName)} exec ${JSON.stringify(state.nativeBrowserCaptureScript)} "$@"
`;
  await Promise.all([
...
function summarizeCommandResult(result) {
  ...
  return {
    ...
    mentionsSofia: /\bsofia\b/i.test(combined),
...
function commandLooksAuthenticated(result) {
  ...
  return result.exitCode === 0 && summary.mentionsSofia && summary.mentionsAuthenticated && summary.mentionsOAuth && !summary.containsUnredactedToken;
}

function commandLooksConnected(result) {
  const summary = summarizeCommandResult(result);
  return result.exitCode === 0 && summary.mentionsSofia && summary.mentionsConnected && !summary.containsUnredactedToken;
}
...
async function submitSignIn(ctx) {
  ...
  })()`, { timeoutMs: 60_000, label: "Sofia OAuth sign-in or consent" });
...
async function screenshotWithTemporarilyQuerylessUrl(ctx, name, options) {
  ...
  const sanitizedHref = await ctx.eval(`(() => {
    window.__sofiaReliableConnectRawHref = location.href;
    const url = new URL(location.href);
    history.replaceState(history.state, document.title, url.origin + url.pathname);
    return location.href;
  })()`);
...
function renderProofPage(proof) {
  ...
  const commandRows = proof.commands.map((command) => `<tr>
    ...
    <td>exit ${escapeHtml(String(command.exitCode))}; stdout <code>${escapeHtml(command.stdoutHash)}</code>; stderr <code>${escapeHtml(command.stderrHash)}</code>; sofia=${escapeHtml(String(command.mentionsSofia))}; oauth=${escapeHtml(String(command.mentionsOAuth))}; connected=${escapeHtml(String(command.mentionsConnected))}; authenticated=${escapeHtml(String(command.mentionsAuthenticated))}</td>
  </tr>`).join("\n");
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Reliable Sofia Connect proof</title>
    <style>
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Sofia Connect error matrix</title>
    <style>
  ...
  <body>
    <main>
      <h1>Sofia Connect error matrix</h1>
...
async function runMysql(ctx, sql) {
  const container = ctx.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER.trim();
  const database = ctx.env.SOFIA_EVAL_DEN_MYSQL_DATABASE.trim();
  const password = process.env.SOFIA_EVAL_DEN_MYSQL_ROOT_PASSWORD?.trim() || "password";
  try {
...
async function cleanupEvalRateLimits(ctx, phase) {
  ctx.assert(
    ctx.env.SOFIA_EVAL_ISOLATED_DATABASE.trim() === "1",
    "Rate-limit cleanup requires SOFIA_EVAL_ISOLATED_DATABASE=1 and must never run against a shared database.",
  );
...
async function generateLiveRateLimit(ctx) {
  const maxAttempts = Number(process.env.SOFIA_EVAL_OAUTH_RATE_LIMIT_MAX_ATTEMPTS?.trim() || "80");
  const tokenEndpoint = readString(state.authorizationServerMetadata, "token_endpoint");
...
async function navigateDocsPage(ctx) {
  const url = `${baseUrlFromEnv(ctx, "SOFIA_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp`;
  await navigateBrowser(ctx, url, "Sofia Connect docs page");
  await ctx.waitFor(
    `(() => {
      const root = document.querySelector('article, main') || document.body;
      const text = root.innerText || "";
      return text.includes(${JSON.stringify(PUBLIC_MCP_SERVER_URL)}) && text.includes('Sofia Connect MCP');
    })()`,
    { timeoutMs: 30_000, label: "Sofia Connect docs content" },
  );
  return url;
}
...
export default {
  id: FLOW_ID,
  title: "Reliable Sofia Connect is proven from landing page to OAuth, MCP tools, errors, and docs",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: [
    "SOFIA_EVAL_LANDING_URL",
    "SOFIA_EVAL_DOCS_URL",
    "SOFIA_EVAL_DEN_API_URL",
    "SOFIA_EVAL_DEN_WEB_URL",
    "SOFIA_EVAL_DEN_MYSQL_CONTAINER",
    "SOFIA_EVAL_DEN_MYSQL_DATABASE",
    "SOFIA_EVAL_ISOLATED_DATABASE",
  ],
  steps: [
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("Sofia Connect publishes the permanent MCP server URL and labels only native-proven clients as verified.", {
          voiceover: vo[0],
          action: async () => {
          ...
          assert: async () => {
            ...
            recordAssertion(
              ...
              codex.includes(CODEX_COMMAND)
                ...
                && codex.indexOf("codex mcp logout sofia") >= 0
                && codex.indexOf(CODEX_LOGIN_COMMAND, codex.indexOf("codex mcp logout sofia")) > codex.indexOf("codex mcp logout sofia"),
          ...
          await ctx.prove("The browser OAuth journey signs in, selects an organization, shows consent, and returns code plus state only.", {
            ...
            action: async () => {
              ...
              const consentScreenshotUrl = await screenshotWithTemporarilyQuerylessUrl(ctx, "frame-3-consent-before-authorize", {
                claim: "Sofia shows the selected organization on the MCP consent screen before authorization.",
                voiceover: vo[2],
    ...
    },
    {
      ...
      run: async (ctx) => {
        try {
          await ctx.prove("Public OAuth uses JWT access tokens plus rotating opaque refresh tokens while the verified client searches and executes Sofia capabilities for the selected org.", {
            voiceover: vo[3],
            action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        try {
          await ctx.prove("Sofia Connect returns standards-compliant, traceable error responses for the failure modes clients hit.", {
            voiceover: vo[4],
            action: async () => {
