...
const MEMBER_EMAIL = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || ADMIN_EMAIL;
const MEMBER_PASSWORD = process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || ADMIN_PASSWORD;
const MOCK_OAUTH_MCP_URL = (process.env.MOCK_OAUTH_MCP_URL ?? "http://127.0.0.1:3978").trim().replace(/\/+$/, "");
const MOCK_PORT = Number(process.env.SOFIA_EVAL_DISCONNECT_MCP_PORT ?? 4552);
...
async function ensureMemberContext(ctx) {
  ...
  ctx.assert(Boolean(state.memberSession), `Member sign-in failed for ${MEMBER_EMAIL}. Set SOFIA_EVAL_MEMBER_EMAIL/PASSWORD to an existing member or omit them to reuse the admin account.`);
