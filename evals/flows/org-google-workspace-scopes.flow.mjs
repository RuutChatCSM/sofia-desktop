    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The member authorize URL asks for exactly the defaults plus the two selected additions", {
          ...
          assert: async () => {
            assertExactStringSet(ctx, state.authorizeScopes, EXPECTED_MEMBER_SCOPES, "Authorize URL scopes");
            await ctx.waitFor("document.body.innerText.includes('Mock MCP OAuth') && document.body.innerText.includes('Approve Sofia')", {
              timeoutMs: 30_000,
          ...
          screenshot: {
            ...
            requireText: ["Mock MCP OAuth", "Approve Sofia"],
