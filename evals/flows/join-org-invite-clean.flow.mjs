...
const PENDING_INVITE_STORAGE_KEY = "sofia:web:pending-org-invitation";
const AUTH_TOKEN_STORAGE_KEY = "sofia:web:auth-token";

const state = {
  adminToken: "",
  inviteeEmail: process.env.SOFIA_EVAL_JOIN_ORG_INVITEE_EMAIL?.trim() || DEFAULT_INVITEE_EMAIL,
  invitationId: "",
  inviteToken: "",
  organizationId: "",
  organizationName: "",
...
function orgHeaders() {
  ...
  if (state.organizationId) {
    headers["x-sofia-org-id"] = state.organizationId;
    headers["x-sofia-legacy-org-id"] = state.organizationId;
  }
  return headers;
}
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_MULTI_ORG"],
  steps: [
    {
      name: "Setup: real multi-org invitation exists",
      run: async (ctx) => {
        witness(ctx, denWebUrl().length > 0, "SOFIA_EVAL_DEN_WEB_URL points at den-web", denWebUrl());
