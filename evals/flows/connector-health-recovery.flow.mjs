...
export default {
  id: FLOW_ID,
  title: "Sofia Cloud identifies and recovers any unhealthy downstream connector",
  kind: "internal",
  requiresApp: false,
  steps: [
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A JSON-RPC refresh failure becomes a connector-specific reauthorization action, not an Sofia Cloud failure", {
          voiceover: vo[1],
          assert: async () => {
            ...
            witness(ctx, status?.action?.type === "reconnect" && status?.action?.surface === "sofia_organization_connections", "The response points to the exact reconnect surface", JSON.stringify(status?.action));
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Updating the connector credential makes live tools discoverable on the next search without reconnecting Sofia Cloud", {
          voiceover: vo[2],
          assert: async () => {
            const output = combinedOutput(runEndToEndSuite());
