...
export default {
  id: FLOW_ID,
  title: "Settings labels distinguish legacy local extensions from Sofia Connect",
  kind: "user-facing",
  steps: [
    {
    ...
    {
      name: "Sofia Connect is the final sidebar item",
      run: async (ctx) => {
        await ctx.prove("Sofia Connect stays last in Cloud settings even when Memory is enabled", {
          voiceover: vo[1],
          claim: "The Cloud settings group shows Sofia Connect with the beta badge after Memory, and Sofia Connect is the final item in the full settings sidebar.",
          action: async () => {
            await waitForSettingsShell(ctx);
            await ctx.clickText("Sofia Connect", { selector: "button", timeoutMs: 30_000 });
          ...
          assert: async () => {
            ...
            const connectIndex = cloudLabels.indexOf("Sofia Connect");
            const allLabels = nav.groups.flatMap((group) => group.items.map((item) => item.label));
            const connect = cloud.items.find((item) => item.label === "Sofia Connect");

            ctx.assert(memoryIndex !== -1, `Memory tab was not visible after enabling the feature flag: ${JSON.stringify(cloudLabels)}.`);
            ctx.assert(connectIndex !== -1, `Sofia Connect tab was missing: ${JSON.stringify(cloudLabels)}.`);
            ctx.assert(memoryIndex < connectIndex, `Memory was not before Sofia Connect: ${JSON.stringify(cloudLabels)}.`);
            ctx.assert(cloudLabels[cloudLabels.length - 1] === "Sofia Connect", `Sofia Connect was not last in Cloud: ${JSON.stringify(cloudLabels)}.`);
            ctx.assert(allLabels[allLabels.length - 1] === "Sofia Connect", `Sofia Connect was not last in the sidebar: ${JSON.stringify(allLabels)}.`);
            ctx.assert(
              Boolean(connect?.badges.some((badge) => badge.toLowerCase() === "beta")),
              `Sofia Connect was missing the beta badge: ${JSON.stringify(connect)}.`,
            );
          },
          screenshot: {
            name: "settings-sidebar-sofia-connect-last",
            requireText: ["Cloud", "Memory", "Sofia Connect", "BETA"],
            rejectText: ["Something went wrong"],
...
async function enableEnglishMemoryPreview(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000, label: "control API" });
  const changed = await ctx.eval(`(() => {
    const prefKey = "sofia.preferences";
    let prefs = {};
    try {
