 ...
 *
 * Fix under test: den-web pins Connections requests with x-sofia-org-id,
 ...
 * SOFIA_EVAL_DEN_MULTI_ORG so single-org CI setups skip instead of fail.
 */
...
function denWebUrl() {
  return (process.env.SOFIA_EVAL_DEN_WEB_URL ?? "").trim().replace(/\/+$/, "");
}
