 ...
 * from local + cached data immediately instead of waiting for the Sofia
...
const DEN_KEYS_CLEANUP = `(() => {
  ...
  delete window.__SOFIA_GATEWAY__;
  return true;
})()`;
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("Boolean(window.__sofiaControl)", {
          timeoutMs: 60_000,
          label: "window.__sofiaControl after reload",
        });
        await ctx.waitFor(`Boolean(document.querySelector(${JSON.stringify(PLUG_BUTTON)}))`, {
          timeoutMs: 60_000,
          label: "composer plug button",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Extensions settle immediately instead of hanging on a loader", {
          ...
          voiceover:
            "Extensions behave the same way: the catalog is on screen immediately — here the built-in Sofia Browser, already enabled.",
          action: async () => {
            await ctx.eval(`(() => {
            ...
            await ctx.waitFor(`document.body.innerText.includes("Sofia Browser")`, {
              timeoutMs: 10_000,
              label: "extensions catalog entry",
            });
          },
          assert: async () => {
            await ctx.expectText("Sofia Browser");
          },
          screenshot: {
            name: "extensions-section",
            requireText: ["Sofia Browser"],
            rejectText: ["Loading commands"],
          },
        });
      },
        ...
        await ctx.prove("Skills render in milliseconds while the Den capability fan-out hangs", {
          ...
          action: async () => {
            ...
            const result = await ctx.eval(`new Promise((resolve) => {
              ...
              window.__SOFIA_GATEWAY__ = { version: 1 };
              ...
              localStorage.setItem("sofia.den.authToken", "eval-slow-cloud-token");
              localStorage.setItem("sofia.den.activeOrgId", "org_slowcloud_" + Date.now());
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("Boolean(window.__sofiaControl)", {
          timeoutMs: 60_000,
          label: "window.__sofiaControl after cleanup",
        });
      },
    },
  ],
