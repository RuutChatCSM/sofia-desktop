...
async function waitForInstaller(ctx) {
  await ctx.waitFor(
    ...
    { timeoutMs: 30_000, label: "Sofia Connect installer" },
  );
...
async function grantDocsClipboardPermissions(ctx) {
  ...
  await ctx.client.send("Browser.grantPermissions", {
    origin: new URL(baseUrl("SOFIA_EVAL_DOCS_URL")).origin,
    permissions: ["clipboardReadWrite", "clipboardSanitizedWrite"],
  });
}

export default {
  id: "docs-sofia-connect",
  title: "Use the Sofia Connect installer from the docs or landing page",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["SOFIA_EVAL_DOCS_URL", "SOFIA_EVAL_LANDING_URL"],
  steps: [
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The Sofia Connect docs put the complete client installer at the top of the page.", {
          voiceover: vo[0],
          action: async () => {
            await navigate(ctx, `${baseUrl("SOFIA_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp`);
          ...
          assert: async () => {
            const actual = await ctx.eval(`(() => {
              ...
              return {
                ...
                supportRows,
                hasInternalProxy: bodyText.includes("sofia-app.ruut.chat/api/den") && bodyText.includes("internal same-origin desktop proxy"),
                hasAuthServer: bodyText.includes("https://sofia-app.ruut.chat/api/auth"),
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("The landing page keeps the same Sofia Connect installer, including Codex and ChatGPT Desktop.", {
          voiceover: vo[4],
          action: async () => {
            await navigate(ctx, `${baseUrl("SOFIA_EVAL_LANDING_URL")}/#connect-mcp`);
            await waitForInstaller(ctx);
