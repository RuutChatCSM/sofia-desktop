...
const FIXTURE_WORKSPACE = resolve(
  process.env.SOFIA_EVAL_ARTIFACTS_DIR ?? "evals/results",
  "..",
  "workspace-client-primitive-workspace",
);
const SESSION_TITLE = "Workspace client primitive proof";
const ACTIVE_WORKSPACE_KEY = "sofia.react.activeWorkspace";
...
async function maybeSkipOnboardingPrompts(ctx) {
  const dismissedModels = await ctx.eval(`(() => {
    const dialog = Array.from(document.querySelectorAll('[role="dialog"], [data-slot="dialog-content"], [data-radix-dialog-content]'))
      .find((item) => (item.textContent || "").includes("Sofia Models"));
    if (!dialog) return false;
    const button = Array.from(dialog.querySelectorAll("button"))
      .find((item) => {
        const label = (item.textContent || "").trim();
        return label.includes("Continue without Sofia Models") || label === "Close" || item.getAttribute("aria-label") === "Close";
      });
  ...
  if (dismissedModels) {
    await ctx.waitFor(
      `!Array.from(document.querySelectorAll('[role="dialog"], [data-slot="dialog-content"], [data-radix-dialog-content]')).some((item) => (item.textContent || "").includes("Sofia Models"))`,
  ...
  return ctx.waitFor(
    `(() => {
      ...
      return {
        ...
        route: window.__sofiaControl?.snapshot?.().route ?? window.location.hash,
      };
    })()`,
      ...
      run: async (ctx) => {
        await ctx.prove("A new session opens and is listed without switching the workspace server", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              "window.__sofiaControl.listActions().some((action) => action.id === 'session.rename' && !action.disabled)",
      ...
      run: async (ctx) => {
        await ctx.prove("Reloading restores the same workspace, settings route, and session list", {
          ...
          action: async () => {
            ...
            await ctx.control("route.session");
            await ctx.waitFor(
              "window.__sofiaControl.listActions().some((action) => action.id === 'session.open')",
