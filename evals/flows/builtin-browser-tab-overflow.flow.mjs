...
async function ensureWorkspaceAndSession(ctx) {
  ...
  await ctx.eval(`(() => {
    for (const label of ['Continue without Sofia Models', 'Close']) {
      const button = Array.from(document.querySelectorAll('button')).find((candidate) =>
...
const tabMetricsExpression = `(async () => {
  const state = await window.__SOFIA_ELECTRON__.browser.getState();
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.eval("window.__SOFIA_ELECTRON__.browser.closeAllTabs?.()", { awaitPromise: true });
        await ctx.control("browser.open_url", {
          provider: "builtin",
          url: "https://example.com/?sofia-overflow-tab=1",
        });
        ...
        for (let index = 2; index <= TAB_COUNT; index += 1) {
          const result = await ctx.eval(`window.__SOFIA_ELECTRON__.browser.createTab(${JSON.stringify(
            `https://example.com/?sofia-overflow-tab=${index}`,
          )})`, { awaitPromise: true });
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`window.__SOFIA_ELECTRON__.browser.selectTab(${JSON.stringify(firstTabId)})`, { awaitPromise: true });
        let metrics = null;
