...
const MARKDOWN_PROMPT = [
  ...
  "```",
  "",
  "[Sofia docs](https://sofia.dev)",
].join("\n");
const MARKDOWN_TEXT_MARKERS = ["Primitive Markdown Proof", "inlineToken", "console.log", "Sofia docs"];
...
function actionExpression(actionId, enabled = true) {
  return `(() => {
    const action = window.__sofiaControl?.listActions?.().find((item) => item.id === ${json(actionId)});
...
async function waitForControl(ctx, label = "control API") {
  ...
  await ctx.waitFor("window.__sofiaControl.snapshot().status !== 'acting'", {
    timeoutMs: 30_000,
    label: `${label} idle`,
  });
}
...
async function selectedSessionId(ctx) {
  return ctx.eval(`(() => {
    ...
    return (${sessionIdFromRoute.toString()})(window.__sofiaControl?.snapshot?.().route || "");
  })()`);
}
...
async function waitForSelectedSessionId(ctx, previousSessionId, label = "selected session id") {
  return ctx.waitFor(`(() => {
    ...
    const controlRoute = window.__sofiaControl?.snapshot?.().route || "";
...
async function waitForSessionRoute(ctx, sessionId) {
  await ctx.waitFor(`(() => {
    ...
    return (window.__sofiaControl?.snapshot?.().route || "").includes(expected);
  })()`, {
    timeoutMs: 45_000,
...
function markdownDomInfoExpression() {
  return `(() => {
    ...
    const codeBlock = proofMessage.querySelector('[data-sofia-code-block]');
    const inlineCode = Array.from(proofMessage.querySelectorAll('code')).find((code) => !code.closest('[data-sofia-code-block]') && code.textContent.includes('inlineToken'));
    const link = proofMessage.querySelector('a[data-sofia-link-href="https://sofia.dev"], a[href="https://sofia.dev/"]');
...
async function assertBootState(ctx, state) {
  ctx.assert(state.connected === true, `Sofia server is not connected: ${JSON.stringify(state)}`);
  ctx.assert(state.tokenPresent === true, `Sofia token is missing: ${JSON.stringify(state)}`);
...
async function openSettingsPanel(ctx, panel, requiredText) {
  ...
  await ctx.waitFor(`window.__sofiaControl.snapshot().route.includes(${json(`/settings/${panel}`)})`, {
    timeoutMs: 30_000,
...
async function inspectExtensionsSurface(ctx) {
  return ctx.eval(`(() => {
    ...
    return {
      ...
      hasConnectPath: text.includes("Sofia Connect") || text.includes("Open Connect") || text.includes("Marketplace content now lives in Connect") || text.includes("My Extensions"),
    };
  })()`);
}
...
export default {
  id: "primitives-80-20",
  title: "Primitive cleanup leaves the user-facing Sofia journey unchanged",
  kind: "user-facing",
  precondition: async (ctx) => {
    ...
    const state = await ctx.waitFor(`(() => {
      const route = window.__sofia?.slice?.("route");
      if (!route || route.loading) return null;
      const action = window.__sofiaControl.listActions().find((item) => item.id === "session.create_task");
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sofia lands on an existing workspace session surface with no migration or error warning", {
          voiceover: vo[0],
          action: async () => {
            await routeSession(ctx);
      ...
      run: async (ctx) => {
        await ctx.prove("A conversation opens with supported Markdown, link, and code-block rendering intact", {
          ...
          assert: async () => {
            ...
            ctx.assert(info.hasLink === true && info.linkText.includes("Sofia docs"), `Link is missing: ${JSON.stringify(info)}`);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Reopening Sofia restores the workspace, session history, settings route, and server connection", {
          voiceover: vo[4],
          action: async () => {
          ...
          screenshot: {
            ...
            requireText: ["Sofia Connect", "Connect for teams"],
            rejectText: ERROR_TEXT,
