...
const DEN_TOKEN = process.env.SOFIA_EVAL_DEN_TOKEN?.trim() || "";
...
export default {
  ...
  steps: [
        ...
        await withWeb(ctx, async () => ctx.prove("Maya joins Acme and sees one clear desktop-app next step", {
          ...
          assert: async () => {
            ...
            const oldOpenButtonVisible = await ctx.eval(`(() => {
              const oldTestId = document.querySelector('[data-testid=join-org-open-sofia]');
              const exactButton = [...document.querySelectorAll('button')]
                .find((button) => (button.textContent ?? '').replace(/\\s+/g, ' ').trim() === 'Open Sofia');
              return Boolean(oldTestId || exactButton);
            })()`);
            witness(ctx, !oldOpenButtonVisible, "The old Open Sofia button is gone from the join success layout", oldOpenButtonVisible);
          },
          screenshot: {
      ...
      run: async (ctx) => {
        await withWeb(ctx, async () => ctx.prove("The join CTA opens Acme's guided setup and the org-served installer endpoint", {
          ...
          screenshot: {
            ...
            requireText: ["Download and install", "Open Sofia", "Sign in"],
          },
        }));
      },
    },
      ...
      run: async (ctx) => {
        await withWeb(ctx, async () => ctx.prove("The guide waits for installation and has an already-installed escape hatch", {
          ...
          action: async () => {
            ...
            await ctx.expectText("Only continue once Sofia is installed and running on this computer.");
            await ctx.expectText("I already have Sofia");
          ...
          assert: async () => {
            ...
            witness(ctx, await ctx.eval("Boolean(document.querySelector('[data-testid=install-connect-open]'))"), "The guide exposes the Open Sofia action after the affirmation", "install-connect-open");
          },
          screenshot: {
            name: "install-wait-gate-copy",
            requireText: ["Open Sofia", "Open the app and confirm that you want to connect it to Acme Robotics."],
          },
        }));
      },
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Open Sofia shows Acme and its server before anything changes", {
          voiceover: vo[3],
          action: async () => {
            await withWeb(ctx, async () => {
              ...
              state.connectUrl = await ctx.waitFor(`(() => {
                ...
                if (!url.startsWith('sofia://connect')) return null;
                try {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Connect turns the app into Acme's forced sign-in surface", {
          ...
          screenshot: {
            ...
            requireText: ["Welcome to Sofia", "Sign in to Sofia"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Maya signs in and lands in Acme's workspace without typing a server URL", {
          ...
          action: async () => {
            ...
            witness(ctx, handoff.response.ok && typeof handoff.body?.sofiaUrl === "string", "The member session minted a desktop auth handoff", {
              status: handoff.response.status,
              hasSofiaUrl: typeof handoff.body?.sofiaUrl === "string",
            });
            await deliverDeepLinkToDesktop(ctx, handoff.body.sofiaUrl);
          },
          assert: async () => {
            await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 60_000, label: "persisted Den auth token" });
            await ctx.waitFor("(localStorage.getItem('sofia.den.activeOrgName') ?? '').includes('Acme')", { timeoutMs: 60_000, label: "Acme active org" });
...
async function invokeDesktop(ctx, command, input) {
  ...
  return ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop(${JSON.stringify(command)}, ${JSON.stringify(input)})`, { awaitPromise: true });
}
...
async function restoreDesktopAfterFlow(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop ready for cleanup" }).catch(() => null);
  await ctx.eval(`(() => {
    document.querySelector('[data-testid=connect-confirm-cancel]')?.click();
    document.querySelector('[data-testid=connect-error-dismiss]')?.click();
    for (const key of [
  ...
  if (state.originalDesktopBootstrapConfig) {
    ...
    await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop ready after cleanup reload" }).catch(() => null);
  }
}
