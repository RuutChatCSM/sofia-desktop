 ...
 * plain den-web page (Next.js, real path routing, no window.__sofiaControl),
 * so it skips ensureLightMode() (preserveTheme: true) and uses ctx.eval to
 * navigate instead of navigateHash.
 *
 * Prerequisites:
 * - den-api reachable at SOFIA_EVAL_DEN_API_URL, signed in with
 *   SOFIA_EVAL_DEMO_EMAIL / SOFIA_EVAL_DEMO_PASSWORD (defaults to the
 *   seeded demo owner).
 * - den-web reachable at SOFIA_EVAL_DEN_WEB_URL, pointed at that den-api.
