...
async function seedDesktopSession(ctx) {
  await ctx.eval(`(() => {
    localStorage.setItem('sofia.den.authToken', 'eval-token');
    localStorage.setItem('sofia.den.activeOrgId', ${JSON.stringify(ctx.org.id)});
    localStorage.setItem('sofia.den.activeOrgSlug', ${JSON.stringify(ctx.org.slug)});
    localStorage.setItem('sofia.den.activeOrgName', ${JSON.stringify(ctx.org.name)});
    localStorage.removeItem('sofia.den.appliedBrandingFingerprint');
    localStorage.removeItem('sofia.den.brandingRestartResume');
    return true;
  })()`);
  await ctx.control("eval.auth.set-base-url", { baseUrl: ctx.denBaseUrl });
  await ctx.eval("location.hash = '/onboarding'; location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 45_000, label: "control API after onboarding reload" });
  await ctx.eval(`(() => {
    window.__sofiaUpdateDownloadedForOnboarding = false;
    window.__sofiaOnboardingInstallCalled = false;
    ...
    window.__sofiaOnboardingUpdaterEvalBridge = {
      ...
      download: async () => {
        ...
        window.__sofiaUpdateDownloadedForOnboarding = true;
        return { ok: true };
      },
      installAndRestart: async () => {
        window.__sofiaOnboardingInstallCalled = true;
        return { ok: true };
      },
    };
    window.dispatchEvent(new CustomEvent('sofia-den-settings-changed', { detail: {} }));
    window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: { token: 'eval-token' } }));
    return true;
  })()`);
}
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sofia fetches and prepares the selected organization's desktop identity", {
          voiceover: vo[1],
          action: async () => {
          ...
          assert: async () => {
            ...
            const config = await ctx.eval(`(() => {
              ...
              const key = Object.keys(localStorage).find((candidate) => candidate.startsWith('sofia.den.desktopConfig:') && candidate.endsWith(suffix));
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("An eligible application update is downloaded before restart is offered", {
          ...
          assert: async () => {
            ctx.assert(await ctx.eval("window.__sofiaUpdateDownloadedForOnboarding === true"), "The update was not downloaded.");
            await ctx.expectText("Application update downloaded");
          },
          screenshot: {
            name: "application-update-downloaded",
            requireText: ["Application update downloaded", "Restart Sofia"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The member gets one clear restart choice without being trapped", {
          ...
          assert: async () => {
            await ctx.expectText("Restart Sofia");
            await ctx.expectText("Continue without restarting");
            await ctx.expectText("refreshes the workspace name and icon");
          },
          screenshot: {
            name: "one-restart-choice",
            requireText: ["Restart Sofia", "Continue without restarting", "refreshes the workspace name and icon"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The coordinated restart resumes directly into the selected branded workspace", {
          voiceover: vo[4],
          action: async () => {
            await ctx.clickText("Restart Sofia", { timeoutMs: 10_000 });
            await ctx.waitFor("window.__sofiaOnboardingInstallCalled === true", {
              timeoutMs: 10_000,
              label: "coordinated updater install",
            });
            await ctx.eval("location.reload()");
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 45_000, label: "app after restart" });
          ...
          assert: async () => {
            ...
            ctx.assert(
              await ctx.eval("localStorage.getItem('sofia.den.brandingRestartResume') === null"),
