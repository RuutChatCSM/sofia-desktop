...
function envWithoutEvalOverrides(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.SOFIA_EVAL_FLOWS_DIR;
  delete env.SOFIA_EVAL_VOICEOVERS_DIR;
  return env;
}
...
`);
            ...
            assert: async () => {
              ...
              const run = spawnNode(
                ...
                envWith({ SOFIA_EVAL_FLOWS_DIR: flowsDir, SOFIA_EVAL_VOICEOVERS_DIR: voiceoversDir }),
                RUN_TIMEOUT_MS,
              );
...
`);
            ...
            assert: async () => {
              const fixtureEnv = envWith({ SOFIA_EVAL_FLOWS_DIR: flowsDir, SOFIA_EVAL_VOICEOVERS_DIR: voiceoversDir });
