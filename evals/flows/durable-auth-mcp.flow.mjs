...
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? "").trim().replace(/\/+$/, "");
...
const MYSQL_CONTAINER = process.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER?.trim() || "sofia-web-local-mysql";
const MOCK_PORT = Number(process.env.SOFIA_EVAL_DURABLE_AUTH_MCP_PORT ?? 4521);
...
const LOCAL_DRAFT = "Local draft remains available while Sofia Cloud reconnects";
const WORKSPACE_PATH = `/tmp/sofia-durable-auth-mcp-${RUN_TAG}`;
...
function orgHeaders(token) {
  return {
    ...
    "x-sofia-legacy-org-id": state.orgId,
  };
}
...
async function cleanupDesktopEvalWorkspaces(ctx) {
  ...
  const cleanup = await ctx.eval(`(async () => {
    ...
    if (!info?.baseUrl) return { deleted: 0, failed: ['Sofia server info unavailable'] };
    ...
    const stale = (payload.workspaces ?? []).filter((workspace) =>
      typeof workspace.path === 'string' && workspace.path.startsWith('/tmp/sofia-durable-auth-mcp-')
    );
    const failed = [];
    let deleted = 0;
  ...
  while (Date.now() < deadline) {
    ...
    if (view.hasFolder) {
      ...
      const advanced = await ctx.eval(`(() => {
        const labels = ["Continue with organization", "Continue to workspace", "Continue without Sofia Models", "Continue"];
...
async function ensureDedicatedWorkspace(ctx) {
  ...
  await ctx.waitFor(
    "window.__sofiaControl?.listActions?.().find((action) => action.id === 'workspace.create')?.disabled === false",
...
async function signDesktopIntoCloud(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 120_000, label: "desktop control API" });
  ...
  const written = await ctx.eval(`(async () => {
    await window.__SOFIA_ELECTRON__.invokeDesktop("setDesktopBootstrapConfig", ${JSON.stringify(bootstrap)});
  ...
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop after bootstrap reload" });
  ...
  state.desktopToken = await ctx.eval("localStorage.getItem('sofia.den.authToken')");
  await completeDesktopOnboarding(ctx);
...
async function openDenWebTab(ctx) {
  await ctx.eval(`(() => {
    ...
    link.target = '_blank';
    link.textContent = 'Open Sofia dashboard';
  ...
  const switching = ctx.switchToNewTab({ timeoutMs: 20_000, label: "Sofia dashboard" });
  await sleep(750);
  ...
  await ctx.waitFor(
    ...
    { timeoutMs: 30_000, label: "Sofia dashboard document" },
  );
  if (ctx.client?.send) {
...
async function submitSharedConnectionAndConsent(ctx) {
  ...
  const noSofiaReauth = await ctx.eval(`!document.body.innerText.includes(${JSON.stringify(SECURITY_MESSAGE)})`);
  await ctx.switchToNewTab({ timeoutMs: 20_000, label: "provider consent" });
  await ctx.waitForText("Mock MCP OAuth", { timeoutMs: 30_000 });
  await ctx.clickText("Approve Sofia", { timeoutMs: 15_000 });
  await ctx.waitForText("Connected", { timeoutMs: 30_000 });
  return noSofiaReauth;
}
...
async function stageAndRenewDesktopSession(ctx) {
  ...
  `);
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop reopened" });
...
async function openDesktopMcpSettings(ctx) {
  ...
  await ctx.waitForText("Sofia Cloud Control", { timeoutMs: 90_000 });
  await ctx.waitFor(`(() => {
    const leaves = [...document.querySelectorAll('*')].filter((element) => element.children.length === 0 && (element.textContent ?? '').trim() === 'Sofia Cloud Control');
    ...
    return false;
  })()`, { timeoutMs: 120_000, label: "Sofia Cloud Control Ready" });
}
...
async function expireAndRefreshSharedMcp(ctx) {
  ...
  await stopMock(ctx);
  await startMock(ctx);
  await ctx.eval('window.__SOFIA_ELECTRON__.invokeDesktop("engineRestart", {})', { awaitPromise: true });
...
async function ensureLocalDraft(ctx) {
  ...
  if (!hasComposer) {
    ...
    while (Date.now() < deadline && !created) {
      await ctx.waitFor(
        "window.__sofiaControl?.listActions?.().find((action) => action.id === 'session.create_task')?.disabled === false",
    ...
    ctx.assert(created, "Sofia did not create a local task after the engine restart.");
  }
  await ctx.waitFor("window.__sofiaControl?.listActions?.().find((action) => action.id === 'composer.set_text')?.disabled === false", {
    timeoutMs: 60_000,
...
async function simulateCloudPartition(ctx) {
  ...
  await ctx.waitForText("Sofia Cloud is temporarily unavailable.", { timeoutMs: 30_000 });
...
async function restoreCloudConnectivity(ctx) {
  ...
  await ctx.waitFor("!document.body.innerText.includes('Sofia Cloud is temporarily unavailable.')", {
    timeoutMs: 30_000,
...
export default {
  id: FLOW_ID,
  title: "Active Sofia and MCP sessions renew silently while real security boundaries still hold",
  kind: "user-facing",
  spec: "evals/voiceovers/durable-auth-mcp.md",
  requiredEnv: [
    "SOFIA_EVAL_DEN_API_URL",
    "SOFIA_EVAL_DEN_WEB_URL",
    "SOFIA_EVAL_DEN_MYSQL_CONTAINER",
  ],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Maya signs in once, approves one provider consent, and the shared MCP becomes connected", {
          ...
          screenshot: {
            ...
            claim: "The shared MCP appears Connected in Sofia Cloud after one provider consent.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("An active desktop session older than seven days renews without returning Maya to sign-in", {
          ...
          assert: async () => {
            const persisted = await ctx.eval("localStorage.getItem('sofia.den.authToken')");
          ...
          screenshot: {
            ...
            requireText: ["Sofia Cloud", "Sign out"],
            rejectText: ["Paste sign-in code", "Sofia Cloud is temporarily unavailable."],
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        try {
          await ctx.prove("A temporary Cloud outage preserves Maya's local work and session while Sofia reconnects", {
            voiceover: vo[3],
            action: async () => {
            ...
            screenshot: {
              ...
              requireText: ["Sofia Cloud is temporarily unavailable.", "Local work remains available. Reconnecting automatically.", LOCAL_DRAFT],
      ...
      run: async (ctx) => {
        await ctx.prove("A stale admin session goes directly to provider consent for another shared MCP", {
          ...
          assert: async () => {
            ...
            const authorizeCount = requests.filter((entry) => entry.method === "GET" && entry.path === "/authorize").length;
            recordAssertion(ctx, "Sofia did not insert its own identity check before provider consent", state.secondConnectionSkippedReauth === true, null);
          ...
          screenshot: {
            ...
            claim: "The second shared MCP appears Connected without an intervening Sofia security check.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Explicit sign-out revokes both the desktop bearer and its MCP access immediately", {
          ...
          assert: async () => {
            const localToken = await ctx.eval("localStorage.getItem('sofia.den.authToken')");
          ...
          screenshot: {
            ...
            requireText: ["Sofia Cloud", "Paste sign-in code"],
            rejectText: ["Sign out", "Sofia Cloud is temporarily unavailable."],
            hashIncludes: "/settings/cloud-account",
          },
        });
      },
      ...
      run: async (ctx) => {
        await ctx.prove("A legacy MCP client reaches workspace authorization when it adds offline access", {
          ...
          screenshot: {
            ...
            claim: "A previously registered MCP client reaches Sofia workspace authorization after requesting offline access.",
            targetUrlIncludes: "/mcp/select-organization",
            requireText: ["Where should this client work?", "Sofia", "Authorize and continue"],
