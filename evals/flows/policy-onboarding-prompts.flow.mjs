...
const DESKTOP_URL = (process.env.SOFIA_EVAL_DESKTOP_URL ?? "http://localhost:5173").trim().replace(/\/+$/, "");
...
async function openEmptyDesktopSession(ctx, config = {}) {
  ...
  await ctx.eval(`(() => {
    ...
    const apply = window.__sofiaApplyDesktopConfig;
...
export default {
  ...
  title: "Desktop policies replace Sofia starter suggestions for assigned teams",
  kind: "user-facing",
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        state.token = ctx.env.SOFIA_EVAL_DEN_TOKEN;
    ...
    },
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("Sofia supplies useful local suggestions without organization configuration", {
          voiceover: vo[0],
          action: async () => {
          ...
          screenshot: {
            name: "sofia-default-suggestions",
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("Assigned members see organization prompts while Sofia remains the fallback", {
          voiceover: vo[4],
          action: async () => {
