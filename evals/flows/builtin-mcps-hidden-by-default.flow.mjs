/**
 * Built-in Sofia MCPs stay out of the default Extensions list but remain
...
export default {
  ...
  title: "Built-in Sofia MCPs are hidden by default and revealed by Show hidden",
  ...
  steps: [
    ...
    {
      name: "Built-in Sofia MCPs are hidden by default",
      run: async (ctx) => {
        const directoryEntry = await ctx.hasText("Notion") ? "Notion" : "Linear";
        await ctx.prove("Built-in Sofia MCPs are hidden from the default extensions list", {
          voiceover: "The regular MCP directory still renders in Settings, but Sofia's internal control entries are not listed by default.",
          assert: async () => {
            ctx.assert(await ctx.hasText(directoryEntry), "Expected Notion or Linear to prove the catalog rendered.");
            await ctx.expectNoText("Sofia Cloud Control");
            await ctx.expectNoText("Sofia UI Control");
          ...
          screenshot: {
            ...
            claim: "Sofia Cloud Control and Sofia UI Control are hidden from the default extensions list while the rest of the catalog renders.",
            requireText: ["Show hidden", directoryEntry],
            rejectText: ["Sofia Cloud Control", "Sofia UI Control", "Something went wrong"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Show hidden reveals both built-in Sofia MCP entries", {
          voiceover: "When the user asks to show hidden extensions, both built-in Sofia MCP controls appear for inspection or management.",
          action: async () => {
            await revealHidden(ctx);
          },
          assert: async () => {
            await ctx.expectText("Sofia Cloud Control", { timeoutMs: 30_000 });
            await ctx.expectText("Sofia UI Control", { timeoutMs: 30_000 });
            ...
            await ctx.eval(`(() => {
              ...
              const card = buttons.find((el) => (el.textContent ?? "").includes("Sofia UI Control"));
          ...
          screenshot: {
            ...
            claim: "Show hidden reveals Sofia Cloud Control and Sofia UI Control with hidden styling.",
            requireText: ["Sofia Cloud Control", "Sofia UI Control", "Showing hidden"],
