...
const CODE_BLOCK_SELECTOR = '[data-message-role="assistant"] [data-sofia-code-block]';
const COPY_BUTTON_SELECTOR = `${CODE_BLOCK_SELECTOR} [data-sofia-code-copy]`;
const SAMPLE_CODE = [
  "function greet(name) {",
  "  return `Hello, ${name}!`;",
  "}",
...
async function setTheme(ctx, theme) {
  await ctx.eval(
    `(() => {
      ...
      localStorage.setItem("sofia.react.settings.theme-mode", theme);
...
async function waitForHighlightedCodeBlock(ctx) {
  await ctx.waitFor(
    `(() => Array.from(document.querySelectorAll(${JSON.stringify(CODE_BLOCK_SELECTOR)}))
      .some((block) => block.matches("[data-sofia-shiki]")
        && block.textContent.includes("function greet")
        && block.textContent.includes("Sofia")
        && block.querySelector('[data-sofia-code-copy][aria-label="Copy code block"] [data-sofia-code-copy-icon]:not([hidden])')
        && block.querySelector("[data-sofia-code-copy] [data-sofia-code-copy-check-icon][hidden]")))()`,
...
function codeBlockInfoExpression() {
  return `(() => {
    const block = Array.from(document.querySelectorAll(${JSON.stringify(CODE_BLOCK_SELECTOR)}))
      .find((candidate) => candidate.textContent.includes("function greet") && candidate.textContent.includes("Sofia"));
    if (!block) return { ok: false, reason: "code block not found" };
    const button = block.querySelector("[data-sofia-code-copy]");
    const copyIcon = button?.querySelector("[data-sofia-code-copy-icon]");
    const checkIcon = button?.querySelector("[data-sofia-code-copy-check-icon]");
    const copyLabel = button?.querySelector("[data-sofia-code-copy-label]");
            ...
            await ctx.waitFor(
              `(() => {
                ...
                return Boolean(button
                  ...
                  && button.querySelector("[data-sofia-code-copy-icon][hidden]")
                  && button.querySelector("[data-sofia-code-copy-check-icon]:not([hidden])"));
              })()`,
