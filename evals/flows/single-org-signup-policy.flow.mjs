...
function runDenWebTest(pattern) {
  return spawnSync("pnpm", ["--filter", "@sofia-ee/den-web", "exec", "bun", "test", "tests/single-org-signup-ui.test.ts", "--test-name-pattern", pattern], {
    cwd: ROOT,
    encoding: "utf8",
    timeout: 60_000,
    env: process.env,
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("With single-org public signup disabled, the Den Web auth policy resolves sign-up requests to sign-in", {
          ...
          assert: async () => {
            ...
            ctx.output("$ pnpm --filter @sofia-ee/den-web exec bun test tests/single-org-signup-ui.test.ts --test-name-pattern private", output);
          },
        });
      },
    },
