...
const DEN_WEB_URL = cleanBaseUrl(process.env.SOFIA_EVAL_DEN_WEB_URL || DEN_API_URL);
...
const PLATFORM_ADMIN_EMAIL = process.env.SOFIA_EVAL_PLATFORM_ADMIN_EMAIL?.trim() || "";
const PLATFORM_ADMIN_PASSWORD = process.env.SOFIA_EVAL_PLATFORM_ADMIN_PASSWORD?.trim() || "";
const MARK_VERIFIED_CMD = process.env.SOFIA_EVAL_MARK_VERIFIED_CMD?.trim() || "";
const WORKSPACE_PATH = process.env.SOFIA_EVAL_WORKSPACE_PATH?.trim() || "/tmp/sofia-marketplace-connect-only-delivery";
...
export default {
  id: FLOW_ID,
  title: "Organization marketplace plugins are delivered through Sofia Connect only",
  kind: "user-facing",
  requiredEnv: [
    "SOFIA_EVAL_DEN_API_URL",
    "SOFIA_EVAL_DEN_WEB_URL",
    "SOFIA_EVAL_PLATFORM_ADMIN_EMAIL",
    "SOFIA_EVAL_PLATFORM_ADMIN_PASSWORD",
    "SOFIA_EVAL_MARK_VERIFIED_CMD",
    "SOFIA_EVAL_WORKSPACE_PATH",
  ],
  steps: [
    {
      name: "Frame 1",
...
function markEmailVerified(ctx, email) {
  ctx.assert(
    ...
    "Platform-admin provisioning requires SOFIA_EVAL_MARK_VERIFIED_CMD with an {email} placeholder.",
  );
...
function skillSourceText() {
  return [
    ...
    "---",
    "",
    `When invoked, answer with the exact proof phrase ${PROOF_PHRASE} and explain that it came from a Den marketplace capability running through Sofia Connect.`,
  ].join("\n");
}
...
async function signDesktopIntoCloud(ctx) {
  ...
  const written = await ctx.eval(`(async () => {
    ...
    localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(DEN_WEB_URL)});
  ...
  await ctx.waitFor(`localStorage.getItem('sofia.den.activeOrgId') === ${JSON.stringify(requireStateValue(state.orgId, "organization id"))}`, {
    timeoutMs: 60_000,
...
async function clearDesktopConfigCache(ctx) {
  await ctx.eval(`(() => {
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith('sofia.den.desktopConfig:')) localStorage.removeItem(key);
    }
    return true;
  })()`);
}
...
async function completeDesktopCloudOnboardingIfNeeded(ctx) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    await ctx.eval(`(() => {
      ...
      const modelSkip = buttons.find((candidate) => ['Skip and use the free model', 'Continue without Sofia Models'].includes((candidate.textContent ?? '').trim()));
...
async function remountDesktop(ctx) {
  ...
  for (let attempt = 0; attempt < 3 && !remountReady; attempt += 1) {
    await ctx.eval("location.reload()");
    try {
      await ctx.waitFor("Boolean(window.__sofiaControl) && (document.getElementById('root')?.childElementCount ?? 0) > 0", { timeoutMs: 45_000, label: `desktop alive after reload (attempt ${attempt + 1})` });
      remountReady = true;
    } catch {
...
async function waitForCloudMcpSync(ctx) {
  await ctx.waitFor(
    "Boolean(localStorage.getItem('sofia.den.mcp.lastMaintenanceOutcome')) || (document.body?.innerText ?? '').includes('Sofia Connect: Ready')",
    { timeoutMs: 180_000, label: "Sofia Connect maintenance ready" },
  );
}

async function navigateToSettingsTab(ctx, tab) {
  await dismissStaleDialogs(ctx);
  const workspaceId = await ctx.eval("(window.location.hash.match(/\\/workspace\\/([^/]+)/) ?? [])[1] ?? localStorage.getItem('sofia.react.activeWorkspace') ?? ''");
  ...
  try {
    ...
    await ctx.eval("location.reload()");
    await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after settings recovery reload" });
...
async function runSeededCapabilityAgentTurn(ctx) {
  ...
  await ctx.waitFor(
    "Boolean(window.__sofiaControl?.listActions().find((action) => action.id === 'session.create_task' && !action.disabled))",
  ...
  const prompt = `Use the Sofia Cloud Control capability named ${SKILL_NAME}. Search for it first, execute the matched capability, and tell me the exact proof phrase.`;
...
async function seedLegacyLocalImport(ctx) {
  ...
  // Eval-only seeding: call the still-alive local Sofia server cloud-plugin
  ...
  const result = await ctx.eval(`(async () => {
    ...
    if (!baseUrl || !token || !workspaceId) return { ok: false, reason: "Missing Sofia server connection" };
