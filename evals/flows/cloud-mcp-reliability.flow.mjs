...
const WORKSPACE_PATH = join(tmpdir(), `sofia-cloud-mcp-reliability-${RUN_TAG}`);
const CLOUD_MCP_NAME = "sofia-cloud";
const AUTOMATIC_RECONCILE_TRIGGERS = [
  "desktop-settings-background",
  "desktop-background",
  "desktop-connect-autocheck",
];
const GUARD_STORAGE_KEY = "sofia.eval.cloudMcpReliability.guard";
const GUARD_BLOCKED_STORAGE_KEY = "sofia.eval.cloudMcpReliability.blockedDesktopFetches";
...
const PLUGIN_CANARY = "sofia_docs_search";
...
function configureFixtureFromEnv(ctx) {
  const portText = optionalEnv(ctx, "SOFIA_EVAL_MCP_FIXTURE_PORT");
  const listenPort = portText ? Number(portText) : 0;
  ctx.assert(Number.isInteger(listenPort) && listenPort >= 0 && listenPort <= 65535, `SOFIA_EVAL_MCP_FIXTURE_PORT must be a TCP port, got ${quoted(portText)}.`);
  state.fixtureListenPort = listenPort;

  const publicUrl = optionalEnv(ctx, "SOFIA_EVAL_MCP_FIXTURE_URL");
  state.fixturePublicUrl = publicUrl ? normalizeFixtureUrl(publicUrl) : null;
}

function denApiBase(ctx) {
  return cleanBaseUrl(ctx.env.SOFIA_EVAL_DEN_API_URL);
}

function denWebBase(ctx) {
  const webBase = optionalEnv(ctx, "SOFIA_EVAL_DEN_WEB_URL");
  return cleanBaseUrl(webBase || ctx.env.SOFIA_EVAL_DEN_API_URL);
}
...
async function denFetch(ctx, path, options = {}) {
  const response = await fetch(`${denApiBase(ctx)}${path}`, {
    ...
    headers: {
      ...
      ...(state.org?.id ? { "x-sofia-legacy-org-id": state.org.id } : {}),
      ...(options.headers || {}),
    },
  });
...
async function waitForControl(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 90_000, label: "control API" });
}

async function observeSetupView(ctx) {
  return ctx.eval(`(() => {
    const text = document.body?.innerText || "";
    const actions = window.__sofiaControl?.listActions?.() ?? [];
    return {
      hash: window.location.hash,
      hasControl: Boolean(window.__sofiaControl),
...
async function configureDesktopForDen(ctx) {
  ...
  const written = await ctx.eval(`(async () => {
    ...
    localStorage.setItem("sofia.den.baseUrl", ${quoted(baseUrl)});
    localStorage.setItem("sofia.den.apiBaseUrl", ${quoted(apiBaseUrl)});
    return { ok: true };
  })()`, { awaitPromise: true });
...
async function signInWithFreshHandoff(ctx) {
  await loadOrg(ctx);
  await ctx.eval(`(() => {
    localStorage.removeItem("sofia.den.authToken");
    localStorage.removeItem("sofia.den.activeOrgId");
    localStorage.removeItem("sofia.den.activeOrgSlug");
    localStorage.removeItem("sofia.den.activeOrgName");
    localStorage.removeItem("sofia.den.mcp.sync");
    return true;
  })()`);
  ...
  await ctx.eval(`(() => {
    localStorage.setItem("sofia.den.activeOrgId", ${quoted(state.org.id)});
    ${state.org.slug ? `localStorage.setItem("sofia.den.activeOrgSlug", ${quoted(state.org.slug)});` : "localStorage.removeItem(\"sofia.den.activeOrgSlug\");"}
    ${state.org.name ? `localStorage.setItem("sofia.den.activeOrgName", ${quoted(state.org.name)});` : "localStorage.removeItem(\"sofia.den.activeOrgName\");"}
    window.dispatchEvent(new Event("sofia:den-settings-changed"));
    window.dispatchEvent(new Event("sofia:den-session-updated"));
    return true;
  })()`);
  await ctx.waitFor(
    `Boolean((localStorage.getItem("sofia.den.authToken") ?? "").trim()) && localStorage.getItem("sofia.den.activeOrgId") === ${quoted(state.org.id)}`,
...
async function getServerAuth(ctx, requireWorkspace = false) {
  const auth = await ctx.eval(`(() => {
    ...
    return {
      port: (localStorage.getItem("sofia.server.port") ?? "").trim(),
      token: (localStorage.getItem("sofia.server.token") ?? "").trim(),
      hostToken: (localStorage.getItem("sofia.server.hostToken") ?? "").trim(),
      workspaceId: workspaceFromHash || (localStorage.getItem("sofia.react.activeWorkspace") ?? "").trim(),
    };
  })()`);
  ctx.assert(auth?.port && auth.token, `Sofia server credentials missing: ${JSON.stringify(auth)}`);
...
async function serverFetchJson(ctx, path, options = {}) {
  ...
  const response = await fetch(`http://127.0.0.1:${auth.port}${path}`, {
    ...
    headers: {
      ...
      ...(auth.hostToken ? { "x-sofia-host-token": auth.hostToken } : {}),
      ...(options.headers || {}),
    },
...
async function createFreshWorkspace(ctx) {
  ...
  await ctx.eval(`(() => {
    ...
    const prefs = JSON.parse(localStorage.getItem("sofia.preferences") || "{}");
    localStorage.setItem("sofia.preferences", JSON.stringify({ ...prefs, hasCompletedOnboarding: true, providerStepCompleted: true, selectedAgent: "sofia" }));
    return true;
  })()`);
...
async function initialStrictReconcile(ctx) {
  ...
  const health = await serverFetchJson(ctx, `/workspace/${encodeURIComponent(state.workspaceId)}/mcp/sofia-cloud/reconcile`, {
    method: "POST",
    body: payload,
  });
...
async function deleteCloudRuntimeConfig(ctx) {
  ...
  witness(ctx, true, "Deleting only this workspace's sofia-cloud runtime config makes direct health unusable with desired_config/cloud_mcp_missing.", {
...
async function getHealth(ctx) {
  ...
  return serverFetchJson(ctx, `/workspace/${encodeURIComponent(state.workspaceId)}/mcp/sofia-cloud/health${suffix}`);
}
...
async function waitForScopedSyncMarker(ctx, timeoutMs = SETTINGS_SYNC_MARKER_WINDOW_MS) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    last = await ctx.eval(`(() => {
      const raw = localStorage.getItem("sofia.den.mcp.sync");
  ...
  return `
    ...
    if (!probe.invokeWrapper) {
      probe.invokeWrapper = async (command, ...args) => {
        ...
        const shouldBlock = guard.active === true &&
          method === "POST" &&
          url.includes("/mcp/sofia-cloud/reconcile") &&
          workspaceMatches &&
    ...
    if (!probe.retryTimer && probe.desktopPatched !== true) {
      probe.retryTimer = window.setInterval(() => {
        const nextBridge = window.__SOFIA_ELECTRON__;
...
async function readMarker(ctx) {
  return ctx.eval("localStorage.getItem('sofia.den.mcp.sync')");
}
      ...
      run: async (ctx) => {
        await ctx.prove("Test now performs a read-only live health GET and leaves the degraded state untouched", {
          ...
          assert: async () => {
            ...
            const healthGets = summary.requests.filter((request) => request.method === "GET" && request.url.includes(`/workspace/${state.workspaceId}/mcp/sofia-cloud/health`));
            const reconcilePosts = summary.requests.filter((request) => request.method === "POST" && request.url.includes("/mcp/sofia-cloud/reconcile"));
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Repair and test reconciles exactly this workspace, writes the marker only after usable health, and connects the engine", {
          ...
          assert: async () => {
            const summary = await networkSummary(ctx);
            const posts = summary.requests.filter((request) => request.method === "POST" && request.url.includes(`/workspace/${state.workspaceId}/mcp/sofia-cloud/reconcile`));
      ...
      run: async (ctx) => {
        await ctx.prove("Ready health uses live mcp.status plus direct Cloud tools/list, and reports OpenCode tool API limitations honestly", {
          ...
          assert: async () => {
            ...
            witness(ctx, health.tools.missing.length === 0, "No expected Sofia Cloud tools are missing.", { missing: health.tools.missing });
            if (state.model) {
            ...
            witness(ctx, Boolean(cloud) && listed.engineSync?.status === "ok", "Workspace MCP status includes sofia-cloud and engine sync is ok.", {
        ...
        await ctx.prove("Advanced Settings shows safe Cloud MCP diagnostics and copies a sanitized report without credentials", {
          ...
          assert: async () => {
            ...
            const secrets = [
              ...
              state.mcpToken,
              await ctx.eval("localStorage.getItem('sofia.den.authToken')"),
            ];
      ...
      run: async (ctx) => {
        await ctx.prove("A fresh real task searches Cloud capabilities before executing the connected fixture and does not substitute docs search", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              "window.__sofiaControl?.listActions?.().find((item) => item.id === 'session.create_task' && !item.disabled)",
            ...
            await ctx.waitFor(
              "window.__sofiaControl?.listActions?.().find((item) => item.id === 'composer.send' && !item.disabled)",
          ...
          assert: async () => {
            ...
            const searchIndex = transcriptText.indexOf("[tool:sofia-cloud_search_capabilities]");
            const executeIndex = transcriptText.indexOf("[tool:sofia-cloud_execute_capability]");
            witness(ctx, searchIndex >= 0 && executeIndex > searchIndex, "Transcript tool order is sofia-cloud_search_capabilities before sofia-cloud_execute_capability.", { searchIndex, executeIndex });
            witness(ctx, !transcriptText.includes(PLUGIN_CANARY), "Transcript did not substitute Sofia documentation search for the connected-service action.", { containsDocsSearch: transcriptText.includes(PLUGIN_CANARY) });
          ...
          screenshot: {
            ...
            rejectText: ["sofia_docs_search", "Something went wrong"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          ...
          if (probe?.originalInvoke && window.__SOFIA_ELECTRON__) window.__SOFIA_ELECTRON__.invokeDesktop = probe.originalInvoke;
