...
const state = {
  adminSession: process.env.SOFIA_EVAL_DEN_TOKEN?.trim() || null,
  memberSession: null,
  adminMcpToken: null,
  memberMcpToken: null,
  organizationId: null,
...
async function ensureMember(ctx) {
  ...
  if (!accept.response.ok && accept.body?.error === "email_verification_required" && !MARK_VERIFIED_CMD) {
    witness(ctx, false, "Member invitation accept needs SOFIA_EVAL_MARK_VERIFIED_CMD in this environment", accept.body);
  }
