...
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? "http://localhost:3005").trim().replace(/\/+$/, "");
...
const CLOUD_CONNECTION_NAMES = ["Sofia Cloud Control", "Sofia Cloud"];
...
const state: FlowState = {
  ...
  cloudConnectionName: "Sofia Cloud",
  searchMatches: [],
  searchMatchNames: [],
  patchResultText: "",
  memberSessionToken: "",
...
function adminSessionToken(ctx: FlowContext): string {
  return ctx.env.SOFIA_EVAL_DEN_TOKEN?.trim() ?? "";
}
...
async function syncConfigToApp(ctx: FlowContext): Promise<Record<string, unknown>> {
  ...
  await ctx.waitFor("typeof window.__sofiaApplyDesktopConfig === 'function'", {
    timeoutMs: 30_000,
    label: "desktop config bridge",
  });
  await ctx.eval(`(() => {
    localStorage.setItem('sofia.react.settings.theme-mode', 'light');
    window.__sofiaApplyDesktopConfig(${JSON.stringify(config)});
    return true;
  })()`);
  return config;
}
...
async function resolveCloudConnectionName(ctx: FlowContext): Promise<string> {
  ...
  throw new Error("Sofia Cloud connection name was not visible after revealing hidden extensions.");
}
...
async function ensureDesktopSignedIn(ctx: FlowContext): Promise<void> {
  ...
  const signedIn = await ctx.eval("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())");
  if (!signedIn) {
  ...
  if (onWelcome) {
    const workspacePath = ctx.env.SOFIA_EVAL_WORKSPACE_PATH?.trim() ?? "";
    ctx.assert(Boolean(workspacePath), "Desktop landed on /welcome; set SOFIA_EVAL_WORKSPACE_PATH so the eval can create/select a workspace.");
  ...
  if (needsWorkspace) {
    const workspacePath = ctx.env.SOFIA_EVAL_WORKSPACE_PATH?.trim() ?? "";
    ctx.assert(Boolean(workspacePath), "No workspace exists; set SOFIA_EVAL_WORKSPACE_PATH so the eval can create one.");
  ...
  await ctx.waitFor(
    "Boolean(localStorage.getItem('sofia.den.mcp.sync')) || (localStorage.getItem('sofia.den.mcp.lastMaintenanceOutcome') ?? '').includes('\"status\":\"ok\"')",
...
async function enterDenPolicies(ctx: FlowContext): Promise<void> {
  ...
  await ctx.waitFor(
    "window.__sofiaControl.listActions().some((action) => action.id === 'session.create_task' && !action.disabled)",
  ...
  await ctx.waitFor(
    "window.__sofiaControl.listActions().some((action) => action.id === 'browser.open_url' && !action.disabled)",
...
async function closeBuiltInBrowserTabs(ctx: FlowContext): Promise<void> {
  await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__)", { timeoutMs: 10_000, label: "Electron bridge before closing browser tabs" });
  await ctx.eval(`(async () => {
    ...
    if (bridge.invokeDesktop) {
      try {
        return await bridge.invokeDesktop('sofia:browser:closeAllTabs');
      } catch (error) {
...
function markMemberVerifiedIfConfigured(ctx: FlowContext): void {
  const command = ctx.env.SOFIA_EVAL_MARK_VERIFIED_CMD?.trim() ?? "";
  if (!command) return;
  execSync(command.replaceAll("{email}", MEMBER_EMAIL), { stdio: "ignore" });
  ctx.log(`Marked ${MEMBER_EMAIL} verified through SOFIA_EVAL_MARK_VERIFIED_CMD.`);
}
...
async function acceptInvitation(ctx: FlowContext, memberSessionToken: string, inviteToken: string): Promise<void> {
  ...
  if (/email.*verif/i.test(text) && !(ctx.env.SOFIA_EVAL_MARK_VERIFIED_CMD?.trim())) {
    throw new Error(`Member invitation accept requires email verification; set SOFIA_EVAL_MARK_VERIFIED_CMD. Response: ${accept.response.status} ${text}`);
  }
...
export default defineFlow({
  ...
  title: "Desktop Policies are discoverable and enforce permissions through Sofia Cloud MCP",
  kind: "user-facing",
  ...
  steps: [
    ...
    },
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("Capability search returns the desktop policy tools through Sofia Cloud MCP", {
          voiceover: vo[0],
          action: async () => {
      ...
      run: async (ctx) => {
        await ctx.prove("The desktop app picks up the restricted config and shows the organization policy banner", {
          ...
          action: async () => {
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop control API after closing Den web tab" });
            const config = await syncConfigToApp(ctx);
