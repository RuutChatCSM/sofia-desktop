...
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL?.trim() || "http://localhost:3005").replace(/\/+$/, "");
const TEST_EMAIL = "pat@sofia.test";
...
async function openSignedOutRoot(ctx) {
  ...
  await ctx.eval(`(() => {
    window.localStorage.removeItem('sofia:web:auth-token');
  ...
  await ctx.waitFor(
    `document.body.innerText.includes('Start using Sofia') && Boolean(document.querySelector('input[type="email"]'))`,
...
async function installDeferredLoginOptionsMock(ctx, responsePayload) {
  await ctx.eval(`((payload) => {
    const originalFetch = window.__sofiaOriginalFetch ?? window.fetch.bind(window);
    window.__sofiaOriginalFetch = originalFetch;
    window.__sofiaLoginOptionRequests = [];
    window.__sofiaResolveLoginOptions = null;
    window.fetch = (input, init) => {
      const rawUrl = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url;
      const url = new URL(rawUrl, window.location.origin);
      if (url.pathname === '/v1/auth/login-options' || url.pathname === '/api/den/v1/auth/login-options') {
        window.__sofiaLoginOptionRequests.push(url.pathname + url.search);
        return new Promise((resolve) => {
          window.__sofiaResolveLoginOptions = () => {
...
async function resolveLoginOptions(ctx) {
  const resolved = await ctx.eval(`(() => {
    if (typeof window.__sofiaResolveLoginOptions !== 'function') return false;
    return window.__sofiaResolveLoginOptions();
  })()`);
...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("The root auth page opens as a compact split card with only the shader visual and account form", {
          ...
          assert: async () => {
            ...
            recordAssertion(ctx, "The left panel has exactly one shader canvas and the old marketing content is absent", state.visualCanvasCount === 1 && !state.text.includes("Sofia Cloud") && !state.text.includes("One setup, every seat.") && !state.text.includes("Shared config"), state);
          ...
          screenshot: {
            ...
            requireText: ["Start using Sofia", "EMAIL", "Next"],
            rejectText: ["Sofia Cloud", "One setup, every seat.", "Shared config", "Cloud agents", "Your models"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The only required action is entering an email and selecting Next", {
          ...
          assert: async () => {
            const requests = await ctx.eval("window.__sofiaLoginOptionRequests ?? []");
            const state = await readLandingState(ctx);
            recordAssertion(ctx, "Next calls login-options exactly once with the entered email", requests.length === 1 && requests[0]?.endsWith("email=pat%40sofia.test"), { requests, state });
          ...
          screenshot: {
            ...
            requireText: ["Start using Sofia", "EMAIL", "Checking..."],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("On mobile the decorative panel disappears and the compact Sofia logo leads the form", {
          voiceover: vo[3],
          action: async () => {
          ...
          assert: async () => {
            ...
            recordAssertion(ctx, "The compact Sofia mobile brand and account form are centered", Boolean(state.mobileBrand?.visible && state.form?.visible && state.frame) && state.mobileBrandText.includes("Sofia") && state.frame.width <= state.viewport.width && Math.abs((state.frame.left + state.frame.right) / 2 - state.viewport.width / 2) <= 4, state);
          ...
          screenshot: {
            ...
            requireText: ["Sofia", "Start using Sofia", "EMAIL", "Next"],
            rejectText: ["Sofia Cloud", "One setup, every seat.", "Shared config", "Cloud agents", "Your models"],
          },
        });
      },
