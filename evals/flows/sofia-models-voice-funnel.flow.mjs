...
const EMAIL_DOMAIN = "voice-eval.sofia.test";
const PASSWORD = "SofiaVoiceEval123!";
const MOCK_INFERENCE_KEY = "ow_inf_voice_funnel";
const DEFAULT_STRIPE_WEBHOOK_SECRET = "whsec_sofia_eval";
const DEFAULT_STRIPE_PRICE_ID = "price_sofia_models_eval";
const VOICE_TRANSCRIPT_TEXT = "What can you help me with?";

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function apiBase(ctx) {
  return ctx.env.SOFIA_EVAL_DEN_API_URL.trim().replace(/\/+$/, "");
}

function jsonHeaders(token) {
  return {
    "content-type": "application/json",
    origin: process.env.SOFIA_EVAL_DEN_ORIGIN?.trim() || process.env.SOFIA_EVAL_DEN_API_URL?.trim()?.replace(/\/+$/, "") || "http://localhost:8790",
...
export default {
  id: "sofia-models-voice-funnel",
  title: "Sign up, pay for Sofia Models, and start managed Voice Mode",
  spec: "evals/onboarding-welcome-flows.md#flow-28--sofia-models-path-explains-payment-before-value",
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL"],
  steps: [
    {
      name: "Start mock Sofia Models voice broker",
      run: async (ctx) => {
        ctx.mockBroker = await startMockBroker();
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice broker fixture is listening for authenticated Sofia Models session requests.",
    ...
    {
      name: "Record paid Sofia Models subscription",
      run: async (ctx) => {
        const secret = ctx.env.SOFIA_EVAL_STRIPE_WEBHOOK_SECRET?.trim() || DEFAULT_STRIPE_WEBHOOK_SECRET;
        const priceId = ctx.env.SOFIA_EVAL_STRIPE_INFERENCE_PRICE_ID?.trim() || DEFAULT_STRIPE_PRICE_ID;
        ...
        ctx.assert(billing.json?.billing?.stripe?.hasActiveSubscription === true, "Billing status did not show an active Sofia Models subscription.");
        ctx.recordEvidence({ type: "assertion", status: "passed", assertion: "The paid Sofia Models subscription boundary is active for the new organization.", actual: billing.json.billing.stripe.subscription });
      },
    },
    {
      name: "Sign desktop app into the paid Den account",
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "Sofia control API" });
        await ctx.eval(`(() => {
          localStorage.setItem("sofia.den.baseUrl", ${JSON.stringify(apiBase(ctx))});
          localStorage.setItem("sofia.den.apiBaseUrl", ${JSON.stringify(apiBase(ctx))});
          for (const key of ["sofia.den.authToken", "sofia.den.activeOrgId", "sofia.den.activeOrgSlug", "sofia.den.activeOrgName"]) localStorage.removeItem(key);
          window.location.hash = "#/settings/cloud-account";
          window.location.reload();
          return true;
        })()`);
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "Sofia control API after auth reset" });

        const handoff = await denRequest(ctx, "/v1/auth/desktop-handoff", {
          method: "POST",
          token: ctx.den.token,
          body: { desktopScheme: "sofia" },
        });
        ctx.assert(handoff.response.ok, `Desktop handoff creation failed: ${handoff.response.status} ${handoff.text.slice(0, 300)}`);
        ctx.assert(typeof handoff.json?.sofiaUrl === "string", "Desktop handoff did not return sofiaUrl.");
        ...
        if (!(await ctx.hasText("Sign out"))) {
          ...
          await ctx.fill("#den-signin-link", handoff.json.sofiaUrl);
          await ctx.clickText("Finish sign-in", { timeoutMs: 15_000 });
        }
        await ctx.waitFor(`localStorage.getItem("sofia.den.authToken") === ${JSON.stringify(ctx.den.token)}`, {
          timeoutMs: 45_000,
        ...
        await ctx.eval(`(() => {
          ...
          localStorage.setItem("sofia.preferences", JSON.stringify({ ...prefs, hasCompletedOnboarding: true }));
          window.location.hash = "#/settings/cloud-account";
          return true;
        })()`);
        await ctx.expectText("Sign out", { timeoutMs: 30_000 });
        await ctx.screenshot("paid-den-account-signed-in", { claim: "Desktop is signed in to the newly paid Sofia Models account.", requireText: ["Sign out"] });
      },
    },
    {
      name: "Create workspace via UI",
      run: async (ctx) => {
        const workspaceDir = ctx.env.SOFIA_EVAL_WORKSPACE_DIR?.trim() || join(tmpdir(), `sofia-models-voice-${Date.now()}`);
        ...
        await ctx.eval(`(() => {
          ...
          if (nameInput) {
            ...
            setter.call(nameInput, "Sofia Models Voice Eval");
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("sofia.extension.enabled.sofia-voice", "1");
          window.dispatchEvent(new CustomEvent("sofia:extension-state-changed", { detail: { id: "sofia-voice", enabled: true } }));
          return true;
        })()`);

        await ctx.expectText("Sofia Models", { timeoutMs: 30_000 });
        await ctx.screenshot("workspace-created-via-ui", { claim: "Workspace was created through the Add workspace UI flow.", requireText: ["Sofia Models"] });
      },
    },
    {
      ...
      run: async (ctx) => {
        const serverConfig = await ctx.eval(`(() => ({
          baseUrl: localStorage.getItem("sofia.server.urlOverride") || localStorage.getItem("sofia.server.active") || "",
          hostToken: localStorage.getItem("sofia.server.hostToken") || "",
        }))()`);
        ...
        for (const [key, value] of [["SOFIA_API_KEY", MOCK_INFERENCE_KEY], ["SOFIA_INFERENCE_BASE_URL", ctx.mockBroker.baseUrl]]) {
          ...
          if (!savedViaUI) {
            ...
            const envResponse = await fetch(`${serverConfig.baseUrl}/env`, {
              method: "PUT",
              headers: { "x-sofia-host-token": serverConfig.hostToken, "content-type": "application/json" },
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000, label: "control API after env reload" });
            await ctx.clickText("Environment", { timeoutMs: 10_000 });
            await sleep(1_000);
          }
          ctx.log(`Saved ${key}`);
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice credentials (SOFIA_API_KEY + SOFIA_INFERENCE_BASE_URL) were saved through the Settings > Environment UI.",
          actual: ["SOFIA_API_KEY", "SOFIA_INFERENCE_BASE_URL"],
        });
        await ctx.screenshot("managed-voice-env-configured-via-ui", {
          ...
          requireText: ["SOFIA_API_KEY", "SOFIA_INFERENCE_BASE_URL"],
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("window.__sofiaControl.listActions().some((action) => action.id === 'session.create_task' && !action.disabled)", {
          timeoutMs: 60_000,
        ...
        await ctx.waitFor("window.__sofiaControl.listActions().some((action) => action.id === 'voice.panel.open' && !action.disabled)", {
          timeoutMs: 30_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(request.authorization === `Bearer ${MOCK_INFERENCE_KEY}`, "Voice Mode did not authenticate to the Sofia Models broker with the paid inference key.");
        ctx.assert(request.url === "/voice/realtime/session", "Voice Mode did not call the managed realtime session endpoint.");
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Voice Mode requested a managed Sofia Models realtime session.",
        ...
        ctx.assert(
          ...
          "Voice Mode showed a direct OpenAI API key missing error instead of using the managed Sofia Models path.",
        );
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
        ...
        await ctx.screenshot("managed-voice-mode-started", {
          claim: "Voice Mode started through the managed Sofia Models session path via UI click.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const statusResult = await ctx.eval(`window.__sofiaControl.execute("voice.status")`);
        ...
        await ctx.screenshot("voice-mode-active-with-transcript", {
          claim: "Voice Mode is active through the managed Sofia Models path, with timeline and connection state visible.",
          requireText: ["Voice Mode"],
          rejectText: ["OpenAI API key missing"],
        });
