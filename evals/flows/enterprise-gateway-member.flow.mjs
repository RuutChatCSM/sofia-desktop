 ...
 *
 * Optional env:
 * - SOFIA_EVAL_CDP_URL or --cdp-url: CDP endpoint for the member desktop app.
 *   This should be the SECOND app instance when run after the admin flow.
 * - SOFIA_EVAL_ENTERPRISE_MEMBER_EMAIL: member email (default member@example.com).
 * - SOFIA_EVAL_ENTERPRISE_MEMBER_WORKSPACE: workspace folder (default /workspace/enterprise-member).
 * - SOFIA_EVAL_ENTERPRISE_GATEWAY_URL: gateway base URL used only if the transcript asks for JIT login without a full link.
 * - SOFIA_EVAL_ENTERPRISE_MEMBER_GATEWAY_USER: gateway login user override (default member email).
 ...
 * Run this flow separately from the admin flow and point SOFIA_EVAL_CDP_URL
...
const WORKSPACE_ENV = "SOFIA_EVAL_ENTERPRISE_MEMBER_WORKSPACE";
...
export default {
  ...
  steps: [
    ...
    {
      name: "Create member's fresh workspace with Sofia Connect ready",
      run: async (ctx) => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Member's agent discovers my-incidents from cloud capabilities and answers with the member's own incident", {
          action: async () => {
            const timeout = timeoutMs(ctx, "SOFIA_EVAL_ENTERPRISE_MEMBER_TIMEOUT_MS", 300_000);
            ...
            state.latestTranscript = await retryAfterGatewayLoginIfNeeded(
              ...
              "INC0012338",
              PROMPT_RETRY,
              { timeout, gatewayUserEnvName: "SOFIA_EVAL_ENTERPRISE_MEMBER_GATEWAY_USER" },
            );
