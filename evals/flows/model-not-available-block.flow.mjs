...
async function ensureSession(ctx) {
  ...
  const inSession = await ctx.eval(`window.__sofiaControl.snapshot().route.includes("/session/")`);
  if (inSession) return;
...
async function seedUnavailableSelectedModel(ctx) {
  await ctx.waitFor(
    `window.__sofiaControl.listActions().some((item) => item.id === "eval.model_not_available.seed" && !item.disabled)`,
