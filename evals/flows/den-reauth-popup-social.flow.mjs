...
const MYSQL_CONTAINER = process.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER?.trim() ?? "";
...
export default {
  ...
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_MYSQL_CONTAINER"],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The popup completion message retries the queued action without reloading the opener", {
          ...
          action: async () => {
            ...
            await ctx.eval(`(() => { window.open(${JSON.stringify(completionUrl)}, 'sofia-reauth'); return true; })()`);
            await ctx.waitFor(
...
async function runSql(sql) {
  return execFileAsync("docker", ["exec", MYSQL_CONTAINER, "mysql", "-uroot", "-ppassword", "sofia_den", "-e", sql], {
    maxBuffer: 2_000_000,
  });
}
