...
async function resetFixtures(ctx) {
  ...
  for (const fixture of FIXTURES) {
    ...
    await ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop("workspaceCreate", ${JSON.stringify({
  ...
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 90_000, label: "app after fixture setup" });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The empty workspace registry remains authoritative after restart", {
          voiceover: vo[3],
          action: async () => {
            await ctx.eval("window.__SOFIA_ELECTRON__.shell.relaunch()", { awaitPromise: true }).catch(() => null);
            await sleep(2_000);
