...
const ENV_NAMES = [
  "SOFIA_EVAL_WIN_SANDBOX_ID",
  "SOFIA_EVAL_CDP_URL",
  "SOFIA_EVAL_WIN_PROFILE",
];

const SANDBOX_ID = (process.env.SOFIA_EVAL_WIN_SANDBOX_ID ?? "").trim();
const CDP_URL = cleanUrl(process.env.SOFIA_EVAL_CDP_URL);
const INITIAL_INTERNAL_CDP_PORT = safeTcpPort(process.env.SOFIA_EVAL_INITIAL_CDP_PORT) ?? 9222;
const CDP_DISCOVERY_PORTS = [9223, 9224, 9225, 9226, 9227];
const WIN_PROFILE = cleanWinPath(process.env.SOFIA_EVAL_WIN_PROFILE ?? "");
const RUN_TAG = `${Date.now().toString(36)}-${randomBytes(2).toString("hex")}`;
const BRAND_APP_NAME = "Nuke Proof Work";
const BOOTSTRAP_BASE_URL = "https://sofia-poc.example.test";
const SEED_MARKER = `debug-nuke-seed-${RUN_TAG}`;
const FAKE_AUTH_TOKEN = `eval-fake-token-${RUN_TAG}`;
const LOCKER_SCRIPT_NAME = `sofia-nuke-locker-${RUN_TAG}.ps1`;
...
const OUT_OF_BAND_BOOT_TASK_NAME = `SofiaNukeRetry-${RUN_TAG}`;
const OUT_OF_BAND_BOOT_CMD_PATH = winJoin(paths.windowsTemp, `sofia-nuke-retry-${RUN_TAG}.cmd`);
...
do {
  ...
  foreach($conn in $connections){
    ...
    $isSofia=$alive -and $name -eq 'Sofia'
    $last += [pscustomobject][ordered]@{ port=[int]$conn.LocalPort; localAddress=[string]$conn.LocalAddress; pid=$pidValue; processName=$name; processAlive=$alive; isSofia=$isSofia }
  }
  $match=@($last | Where-Object { $_.isSofia -and $_.port -ne $exclude } | Sort-Object port | Select-Object -First 1)
  if($match.Count -gt 0){
...
function discoverAndRelayCdp(ctx, label, previousPort) {
  ...
  if (discovery?.found !== true) {
    throw new Error(`${label}: no active Sofia.exe CDP listener found on ${CDP_DISCOVERY_PORTS.join(", ")} excluding ${previousPort}.`);
  }
...
async function waitForAppShell(ctx, label = "Sofia renderer") {
  await ctx.waitFor("document.readyState === 'complete'", { timeoutMs: 60_000, label: `${label} document complete` });
  await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__)", { timeoutMs: 60_000, label: `${label} Electron bridge` });
}
...
async function enableRendererState(ctx, options = {}) {
  ...
  await ctx.eval(`(() => {
    localStorage.setItem('sofia.developerMode', '1');
    localStorage.setItem('sofia.den.authToken', ${JSON.stringify(FAKE_AUTH_TOKEN)});
    localStorage.setItem('sofia.den.activeOrgId', 'org_eval_debug_nuke');
    localStorage.setItem('sofia.den.activeOrgSlug', 'debug-nuke');
    localStorage.setItem('sofia.den.activeOrgName', 'Debug Nuke Eval');
    localStorage.setItem('sofia.react.settings.theme-mode', 'light');
    if (${includePreferences ? "true" : "false"}) {
      localStorage.setItem('sofia.preferences', JSON.stringify({ hasCompletedOnboarding: true, seededBy: ${JSON.stringify(FLOW_ID)}, runTag: ${JSON.stringify(RUN_TAG)} }));
    }
    return true;
  })()`);
  ...
  await ctx.waitFor(
    `window.__debugNukeReloadMarker !== ${JSON.stringify(marker)} && document.readyState === 'complete' && Boolean(window.__SOFIA_ELECTRON__)`,
  ...
  const authSnapshot = await ctx.eval(`(() => {
    ...
    localStorage.setItem('sofia.den.activeOrgSlug', 'debug-nuke');
    localStorage.setItem('sofia.den.activeOrgName', 'Debug Nuke Eval');
    const token = localStorage.getItem('sofia.den.authToken');
    return {
      authTokenPresent: Boolean(token),
      authTokenMatches: token === ${JSON.stringify(FAKE_AUTH_TOKEN)},
      authToken: token ? '<seeded>' : null,
      activeOrgId: localStorage.getItem('sofia.den.activeOrgId'),
      activeOrgSlug: localStorage.getItem('sofia.den.activeOrgSlug'),
      activeOrgName: localStorage.getItem('sofia.den.activeOrgName'),
    };
  })()`);
  witness(ctx, authSnapshot.authTokenMatches === true, "Renderer localStorage accepted the seeded fake sofia.den.authToken after reload", authSnapshot);
...
function bootstrapFixture() {
  return {
    ...
    brandIconUrl: "https://sofia-poc.example.test/icon.png",
...
function seedSofiaConfigAndUserDataScript() {
  return `
$profilePath=${psQuote(WIN_PROFILE)}
$userData=${psQuote(paths.userData)}
$appSofia=${psQuote(paths.appDataSofia)}
$configHome=${psQuote(paths.configHome)}
$dirs=@($userData,$appSofia,$configHome)
...
Set-Content -Path (Join-Path $appSofia 'server.json') -Value ${seededJson({ server: "dummy" })} -Encoding UTF8
Set-Content -Path (Join-Path $appSofia 'env.json') -Value ${seededJson({ APPDATA_ENV: "dummy" })} -Encoding UTF8
Set-Content -Path (Join-Path $appSofia 'tokens.json') -Value ${seededJson({ APPDATA_TOKEN: "dummy" })} -Encoding UTF8
[IO.File]::WriteAllBytes((Join-Path $appSofia 'runtime.sqlite'), [Text.Encoding]::UTF8.GetBytes('dummy appdata runtime db ${SEED_MARKER} ${RUN_TAG}'))
...
$result=[ordered]@{ profile=$profilePath; userData=$userData; appSofia=$appSofia; configHome=$configHome }
...
$checks['appSofia']['tokens']=Test-Path -LiteralPath (Join-Path $paths.appSofia 'tokens.json')
$checks['appSofia']['runtime']=Test-Path -LiteralPath (Join-Path $paths.appSofia 'runtime.sqlite')
...
$receipts=@()
foreach($dir in $tempCandidates){ if(Test-Path -LiteralPath $dir){ $receipts += @(Get-ChildItem -LiteralPath $dir -Filter 'sofia-nuke-receipt-*.json' -File -ErrorAction SilentlyContinue) } }
...
function discoverMainSofiaExecutableScript() {
  return `
$processes=@(Get-CimInstance Win32_Process -Filter "Name = 'Sofia.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.ExecutablePath -and ($null -eq $_.CommandLine -or $_.CommandLine -notmatch '--type=') } | Sort-Object ProcessId)
...
if(-not $exe -or -not (Test-Path -LiteralPath $exe)){
  $result=[ordered]@{ executablePath=$exe; executableExists=$false; error='Sofia executable path missing before out-of-band boot' }
  Write-Output ($result | ConvertTo-Json -Depth 4 -Compress)
  exit 53
}
try {
  foreach($proc in @(Get-Process -Name Sofia -ErrorAction SilentlyContinue)){
  ...
  $cmdText=('@echo off','set SOFIA_ELECTRON_REMOTE_DEBUG_PORT=','start "" "' + $exe + '"') -join [Environment]::NewLine
...
function assertPostFirstNuke(ctx, data) {
  const localEntries = arrayValue(data.localSofia?.entries).map(String);
  const unexpectedLocal = localEntries.filter((entry) => entry !== "desktop-bootstrap.json");
  const seededAppSofia = arrayValue(data.appSofia?.seededFiles);
  const appSofiaHardDeleteNames = ["server.json", "env.json", "runtime.sqlite"];
  const appSofiaHardDeleteSurvivors = seededAppSofia
    .filter((entry) => appSofiaHardDeleteNames.includes(String(entry?.name)) && entry?.exists === true)
    .map((entry) => entry.name);
  const appSofiaMarkerSurvivors = seededAppSofia.filter(fileContainsSeed).map((entry) => entry.name);
  ...
  witness(ctx, data.localSofia?.exists === true, "%LOCALAPPDATA%\\sofia survives only as the preserved config directory", data.localSofia);
  witness(ctx, unexpectedLocal.length === 0, "%LOCALAPPDATA%\\sofia contains only desktop-bootstrap.json", localEntries);
  witness(ctx, data.localSofia?.pendingExists === false, ".nuke-pending.json is absent after the unlocked nuke", data.localSofia);
  witness(ctx, data.localSofia?.envExists === false && data.localSofia?.tokensExists === false, "Seeded LOCALAPPDATA env.json and tokens.json were removed", data.localSofia);
  ...
  witness(ctx, bootstrap.baseUrl === BOOTSTRAP_BASE_URL, "desktop-bootstrap.json keeps baseUrl https://sofia-poc.example.test", bootstrap);
  ...
  witness(ctx, Number(receipt.deletedCount) > 0, "The newest sofia-nuke-receipt JSON has a non-empty deleted[]", data.receipt);
...
export default {
  ...
  precondition: async (ctx) => {
    ...
    if (missing.length > 0) {
      throw new Error(`debug-nuke-fresh-start requires a running remote Windows packaged app. Missing env: ${missing.join(", ")}. Set SOFIA_EVAL_WIN_SANDBOX_ID, SOFIA_EVAL_CDP_URL, and SOFIA_EVAL_WIN_PROFILE.`);
    }
    if (!/^[a-zA-Z]:\\/.test(WIN_PROFILE)) {
      throw new Error(`SOFIA_EVAL_WIN_PROFILE must be an absolute Windows profile path, got ${JSON.stringify(WIN_PROFILE)}.`);
    }
    ctx.output("debug-nuke-fresh-start-env", JSON.stringify({
      ...
      optionalInitialPortEnv: process.env.SOFIA_EVAL_INITIAL_CDP_PORT?.trim() || null,
        ...
        await ctx.prove("The Windows tester profile has seeded Sofia, Sofia, bootstrap, orchestrator, Chromium, and renderer state", {
          ...
          assert: async () => {
            ...
            const storage = await ctx.eval(`(() => {
              ...
              result.allSofiaKeys = Object.keys(localStorage).filter((key) => key.startsWith('sofia.')).sort();
              return result;
            })()`);
            witness(ctx, storage["sofia.developerMode"] === "1", "Renderer localStorage has sofia.developerMode = 1", storage);
            witness(ctx, String(storage["sofia.preferences"] ?? "").includes("hasCompletedOnboarding"), "Renderer localStorage has sofia.preferences with hasCompletedOnboarding", storage);
            witness(ctx, state.rendererSeedSnapshot?.authTokenMatches === true, "Renderer localStorage had the seeded fake sofia.den.authToken immediately after reload", state.rendererSeedSnapshot);
            const redactedStorage = { ...storage, "sofia.den.authToken": storage["sofia.den.authToken"] ? "<seeded>" : null };
      ...
      run: async (ctx) => {
        await ctx.prove("Debug settings exposes the Danger zone nuke dialog with delete/survive lists and typed confirmation", {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Chromium storage cleared: default, persist:sofia-browser");
          ...
          screenshot: {
            ...
            requireText: [
              ...
              "Chromium storage cleared: default, persist:sofia-browser",
              "Nuke & relaunch",
            ],
          },
        });
      ...
      run: async (ctx) => {
        await ctx.prove("Typing NUKE executes the cleanup, relaunches Electron, preserves required sign-in, and clears seeded Chromium storage", {
          ...
          assert: async () => {
            ...
            const storage = await ctx.eval(`(() => {
              const preferencesRaw = localStorage.getItem('sofia.preferences');
              ...
              return {
                ...
                preferencesParsed,
                developerMode: localStorage.getItem('sofia.developerMode'),
                authToken: localStorage.getItem('sofia.den.authToken'),
                sofiaKeys: Object.keys(localStorage).filter((key) => key.startsWith('sofia.')).sort(),
              };
            })()`);
            witness(ctx, storage.authToken === null, "Seeded sofia.den.authToken is gone after Chromium storage clear", storage);
            witness(ctx, storage.preferencesParsed?.seededBy === undefined && storage.preferencesParsed?.runTag === undefined, "sofia.preferences contains no seededBy or runTag after Chromium storage clear", storage);
            witness(ctx, storage.preferencesParsed?.hasCompletedOnboarding !== true, "sofia.preferences does not preserve completed onboarding after Chromium storage clear", storage);
            witness(ctx, storage.developerMode === null, "Seeded sofia.developerMode is gone after Chromium storage clear", storage);
      ...
      run: async (ctx) => {
        await ctx.prove("A locked runtime.sqlite is recorded for retry, then the boot guard removes it after the locker is killed and the app relaunches", {
          ...
          action: async () => {
            ...
            witness(ctx, state.lockVerified, "PowerShell holds an exclusive FileShare.None handle on %LOCALAPPDATA%\\sofia\\runtime.sqlite", lock);
            ...
            state.executableProbe = daytonaPowerShellJson(ctx, "discover-main-sofia-executable-before-out-of-band-boot", discoverMainSofiaExecutableScript(), { timeoutMs: 30_000 });
            ctx.output("main-sofia-executable-before-out-of-band-boot-json", JSON.stringify(state.executableProbe, null, 2));
          ...
          assert: async () => {
            ...
            witness(ctx, state.executableProbe?.found === true && String(state.executableProbe?.executablePath ?? "").length > 0, "Harness discovered the main Sofia.exe path before stopping the app", state.executableProbe);
            witness(ctx, state.outOfBandBoot?.createExit === 0 && state.outOfBandBoot?.runExit === 0, "Interactive scheduled task launched a fresh Sofia process without inherited remote debug port env", state.outOfBandBoot);
