...
const WORKSPACE_PATH = process.env.SOFIA_EVAL_WORKSPACE_PATH?.trim() || "/tmp/sofia-admin-desktop-skill-grants";
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_WORKSPACE_PATH"],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The administrator can still manage every Skill, plugin capability, and MCP declaration in Den", {
          ...
          screenshot: {
            ...
            requireText: ["Sofia Cloud", ADMIN_EMAIL, "Connected"],
...
async function signDesktopIntoDen(ctx) {
  ...
  await ctx.waitFor(
    "window.__sofiaControl?.listActions().some((action) => action.id === 'auth.exchange-grant' && !action.disabled)",
  ...
  const selectedWorkspace = await ctx.eval(
    "Boolean((localStorage.getItem('sofia.react.activeWorkspace') ?? '').trim())",
  );
  if (!selectedWorkspace) {
    await ctx.waitFor(
      "window.__sofiaControl?.listActions().some((action) => action.id === 'workspace.create' && !action.disabled)",
