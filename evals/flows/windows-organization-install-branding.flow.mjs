...
const INSTALLER = `${BUNDLE_DIR}\\sofia-win-x64-0.17.20.exe`;
const START_MENU = "C:\\Users\\Administrator\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs";
const CONFIG = "C:\\Users\\Administrator\\AppData\\Local\\sofia\\desktop-bootstrap.json";
...
Get-Process Sofia -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
exit 0
`);
  await daytonaComputerUsePress(sandboxId(ctx), "cmd");
  await new Promise((resolve) => setTimeout(resolve, 500));
  await daytonaComputerUseType(sandboxId(ctx), "Sofia");
...
export default {
  ...
  kind: "user-facing",
  requiresApp: false,
  requiredEnv: ["DAYTONA_API_KEY", "SOFIA_EVAL_DAYTONA_SANDBOX"],
  steps: [
    {
...
[pscustomobject]@{
  ...
  StaleShortcut = Test-Path '${START_MENU}\\Sofia.lnk'
} | ConvertTo-Json
`);
...
Get-Process Sofia -ErrorAction SilentlyContinue | Stop-Process -Force
`);
            await launchInstalledApp(ctx);
          },
          assert: async () => {
...
$config = Get-Content '${CONFIG}' -Raw | ConvertFrom-Json
[pscustomobject]@{ BaseUrl = $config.baseUrl; Shortcut = Test-Path '${START_MENU}\\Northwind.lnk'; Stale = Test-Path '${START_MENU}\\Sofia.lnk' } | ConvertTo-Json
`);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Uninstall removes the managed organization shortcut without leaving stale Sofia entries", {
          voiceover: vo[4],
          action: async () => {
            await windowsExec(ctx, "uninstall Northwind", `
Get-Process Sofia -ErrorAction SilentlyContinue | Stop-Process -Force
$cmd = 'C:\\ow\\uninstall-northwind-proof.cmd'
[IO.File]::WriteAllLines($cmd, @('@echo off', '"C:\\Users\\Administrator\\AppData\\Local\\Programs\\@sofiadesktop\\Uninstall Sofia.exe" /S'))
...
[pscustomobject]@{
  ...
  Sofia = Test-Path '${START_MENU}\\Sofia.lnk'
  Marker = Test-Path 'C:\\Users\\Administrator\\AppData\\Roaming\\com.differentai.sofia\\windows-brand-shortcut.txt'
} | ConvertTo-Json
`);
            ctx.assert(result.includes('"Northwind":  false'), result);
            ctx.assert(result.includes('"Sofia":  false'), result);
