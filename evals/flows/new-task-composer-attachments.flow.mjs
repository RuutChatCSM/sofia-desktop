...
async function configureMockProvider(ctx) {
      ...
      opencode: {
        provider: {
          [PROVIDER_ID]: {
            ...
            name: MODEL_NAME,
            options: { baseURL: `http://127.0.0.1:${MOCK_PORT}/v1`, apiKey: "sk-sofia-ntca-eval" },
            models: {
              [MODEL_ID]: {
                name: MODEL_NAME,
                attachment: true,
  ...
  await ctx.eval(`(() => {
    ...
    localStorage.removeItem("sofia.sessionModels." + ${JSON.stringify(ctx.workspaceId)});
    location.reload();
    return true;
  })()`);
      ...
      run: async (ctx) => {
        await ctx.prove("Run task creates the session and the first message carries the attachment", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(`String(window.__sofiaControl.snapshot().route || "").includes("/session/ses_")`, { timeoutMs: 60_000, label: "session route after run task" });
