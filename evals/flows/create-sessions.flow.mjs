...
async function createWorkspace(ctx) {
  const evalWorkspaceRoot = process.env.SOFIA_DATA_DIR || tmpdir();
  state.workspacePath = await mkdtemp(join(evalWorkspaceRoot, "sofia-create-sessions-"));
  const result = await ctx.eval(`(async () => {
    ...
    if (!port || !token || !hostToken) throw new Error("Sofia server auth is unavailable");
    ...
    for (const workspace of previousEvalWorkspaces) {
      ...
      await window.__SOFIA_ELECTRON__.invokeDesktop("workspaceForget", workspace.id).catch(() => null);
    }
    ...
    await window.__SOFIA_ELECTRON__.invokeDesktop("workspaceSetSelected", created.activeId);
    await window.__SOFIA_ELECTRON__.invokeDesktop("workspaceSetRuntimeActive", created.activeId);
    await window.__SOFIA_ELECTRON__.invokeDesktop(
      "engineStart",
    ...
    const serverInfo = await window.__SOFIA_ELECTRON__.invokeDesktop("sofiaServerInfo");
    if (!serverInfo?.baseUrl) throw new Error("Sofia server did not restart for the eval workspace");
    localStorage.setItem("sofia.server.urlOverride", serverInfo.baseUrl);
    if (serverInfo.port) localStorage.setItem("sofia.server.port", String(serverInfo.port));
    ...
    if (nextToken) localStorage.setItem("sofia.server.token", nextToken);
    if (nextHostToken) localStorage.setItem("sofia.server.hostToken", nextHostToken);
    ...
    let preferences = {};
    try { preferences = JSON.parse(localStorage.getItem("sofia.preferences") || "{}"); } catch {}
    localStorage.setItem("sofia.preferences", JSON.stringify({
      ...preferences,
      hasCompletedOnboarding: true,
      providerStepCompleted: true,
    }));
    localStorage.setItem("sofia.react.activeWorkspace", created.activeId);
  ...
  const safeEvalPrefix = join(evalWorkspaceRoot, "sofia-create-sessions-");
...
async function readCreatedSessions(ctx) {
  return ctx.eval(`(async () => {
    const port = localStorage.getItem("sofia.server.port");
    const token = localStorage.getItem("sofia.server.token");
    if (!port || !token) throw new Error("Sofia server connection is unavailable");
    const baseUrl = "http://127.0.0.1:" + port;
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl && window.__SOFIA_ELECTRON__)", {
          timeoutMs: 60_000,
          label: "Sofia desktop control surfaces",
        });
        ...
        await ctx.prove("CREATE-SESSIONS creates and starts every requested chat without navigating the origin", {
          action: async () => {
            const promoOpen = await ctx.eval(
              `document.body.innerText.includes("Continue without Sofia Models")`,
            );
            if (promoOpen) {
              await ctx.clickText("Continue without Sofia Models");
              await ctx.waitFor(
                `!document.body.innerText.includes("Continue without Sofia Models")`,
                { timeoutMs: 10_000, label: "Sofia Models promo dismissed" },
              );
            }
            await ctx.waitFor(
              "window.__sofiaControl.listActions().some((entry) => entry.id === 'composer.set_text' && entry.disabled === false)",
            ...
            const resultCardsExpression = state.createdSessions
              ...
              .join(" && ");
            await ctx.waitFor(
              `document.querySelector('[data-sofia-session-create-card][data-created-session-count="3"]') && ${resultCardsExpression}`,
