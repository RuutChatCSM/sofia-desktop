/**
 * Regression proof: disabling the auto-configured Sofia Cloud Control MCP
 * ("sofia-cloud") sticks.
 *
 ...
 * `sofia.den.mcp.cloudControlUserState`), makes the reconciler respect it
 * (plus an enabled:false guard), and fixes the marker margin. This flow
 * drives the real app:
 *   1. Sign in to Sofia Cloud via desktop handoff.
 ...
 * - SOFIA_EVAL_DEN_TOKEN    Bearer session token for a seeded user
 */

const CLOUD_TITLE = "Sofia Cloud Control";
const USER_STATE_KEY = "sofia.den.mcp.cloudControlUserState";
...
export default {
  ...
  title: "Disabling the Sofia Cloud Control MCP sticks across sync",
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => { localStorage.removeItem(${JSON.stringify(USER_STATE_KEY)}); localStorage.removeItem("sofia.den.mcp.sync"); return true; })()`);
        ...
        await ctx.waitFor(cloudRowExpr(CONFIGURED_STATUSES), {
          timeoutMs: 90_000,
          label: "sofia-cloud configured row",
        });
        ...
        await ctx.prove("Signing in auto-configures the sofia-cloud MCP", {
          assert: async () => {
            await ctx.waitFor(cloudRowExpr(CONFIGURED_STATUSES.filter((status) => status !== "Paused")), {
              timeoutMs: 90_000,
              label: "sofia-cloud configured row (enabled)",
            });
            await ctx.eval(scrollCloudRowExpr);
          },
          screenshot: {
            name: "cloud-mcp-configured",
            claim: "Sofia Cloud Control appears as a configured MCP after cloud sign-in.",
        ...
        await ctx.prove("The user can disable the Cloud Control MCP and it reports Paused", {
          action: async () => {
            await ctx.waitFor(expandAndClickDetailExpr(CONFIGURED_STATUSES, "Disable"), {
              timeoutMs: 30_000,
              label: "expand sofia-cloud row and click Disable",
            });
          },
          assert: async () => {
            await ctx.waitFor(cloudRowExpr(["Paused"]), {
              timeoutMs: 30_000,
              label: "sofia-cloud row shows Paused",
            });
          ...
          screenshot: {
            ...
            claim: "Sofia Cloud Control is Paused after the user disables it.",
      ...
      run: async (ctx) => {
        await ctx.prove("Remounting Settings no longer re-enables the disabled MCP", {
          ...
          assert: async () => {
            await ctx.waitFor(cloudRowExpr(["Paused"]), {
              timeoutMs: 30_000,
              label: "sofia-cloud row still Paused after remount",
            });
            const intent = await ctx.eval(`localStorage.getItem(${JSON.stringify(USER_STATE_KEY)})`);
          ...
          screenshot: {
            ...
            claim: "Sofia Cloud Control stays Paused after the settings remount sync tick.",
            requireText: [CLOUD_TITLE, "Paused"],
            rejectText: ["Something went wrong"],
            hashIncludes: "/settings/extensions/mcp",
          },
      ...
      run: async (ctx) => {
        await ctx.prove("The user can re-enable the Cloud Control MCP", {
          action: async () => {
            await ctx.waitFor(expandAndClickDetailExpr(["Paused"], "Enable"), {
              timeoutMs: 30_000,
              label: "expand paused sofia-cloud row and click Enable",
            });
          },
          assert: async () => {
            await ctx.waitFor(
            ...
            await ctx.waitFor(cloudRowExpr(CONFIGURED_STATUSES.filter((status) => status !== "Paused")), {
              timeoutMs: 60_000,
              label: "sofia-cloud row no longer Paused",
            });
          ...
          screenshot: {
            ...
            claim: "Sofia Cloud Control leaves Paused after re-enable and the intent record is cleared.",
            requireText: [CLOUD_TITLE],
            rejectText: ["Something went wrong"],
            hashIncludes: "/settings/extensions/mcp",
          },
