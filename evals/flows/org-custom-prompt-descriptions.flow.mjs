...
function denWebBase(ctx) {
  return ctx.env.SOFIA_EVAL_DEN_WEB_URL.replace(/\/$/, "");
}
...
async function ensureDesktopSession(ctx) {
  ...
  await ctx.control("eval.auth.set-base-url", { baseUrl: ctx.env.SOFIA_EVAL_DEN_WEB_URL });
  await ctx.eval(`(() => {
    localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_WEB_URL)});
    localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_API_URL)});
    localStorage.setItem('sofia.den.authToken', ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_TOKEN)});
    ...
    localStorage.setItem('sofia.den.activeOrgName', ${JSON.stringify(activeOrg.name ?? "Example Corp")});
    window.dispatchEvent(new CustomEvent('sofia-den-settings-changed', { detail: {} }));
    window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: { token: ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_TOKEN)} } }));
    return true;
  })()`);
...
async function dismissDesktopOverlays(ctx) {
  await ctx.eval(`(() => {
    const continueButton = Array.from(document.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('Continue without Sofia Models')
    );
    if (continueButton) {
...
async function readComposerState(ctx) {
  return ctx.eval(`(() => {
    const composer = window.__sofia?.slice?.('composer');
        ...
        await runWithErrorCleanup(ctx, async () => {
          await ctx.prove("The desktop new-session screen renders the saved description as the organization prompt card title", {
            ...
            action: async () => {
              ...
              await ctx.eval(`(() => {
                const applyDesktopConfig = window.__sofiaApplyDesktopConfig;
        ...
        await runWithErrorCleanup(ctx, async () => {
          await ctx.prove("Clicking the description-titled card inserts the full custom prompt into the composer", {
            ...
            action: async () => {
              ...
              await ctx.waitFor(
                `window.__sofia?.slice?.('composer')?.draft === ${JSON.stringify(PROMPTS[0])}`,
