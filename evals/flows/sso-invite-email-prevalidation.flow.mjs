...
const REQUIRED_DEN_ENV = ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_TOKEN"];

const vo = await loadVoiceoverParagraphs(FLOW_ID);
if (!vo) throw new Error(`Missing approved voice-over script for ${FLOW_ID}.`);

function runSuffix(ctx) {
  return `${ctx.env.SOFIA_EVAL_RUNSTAMP?.trim() || Date.now()}-${process.pid}`
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "") || "run";
}
...
function authHeaders(ctx) {
  ...
  if (!token) throw new Error("SOFIA_EVAL_DEN_TOKEN is required.");
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        try {
          ...
          await ctx.on(surface, async () => {
            await ctx.prove("If the IdP returns a different email, Sofia rejects the invite coherently and does not loop", {
              voiceover: vo[1],
              action: async () => {
