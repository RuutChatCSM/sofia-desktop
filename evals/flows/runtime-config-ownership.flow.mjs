...
const LEGACY_CONTENT = `${LEGACY_HEADER}{
  ...
  "agent": {
    "sofia": {
      "description": "Legacy Sofia agent left by an older build",
      "mode": "primary"
    }
  },
  "default_agent": "sofia",
...
function devDataRoot() {
  const appIdentifier = "com.differentai.sofia.dev";
  if (process.platform === "darwin") {
    return resolve(os.homedir(), "Library", "Application Support", appIdentifier, "sofia-dev-data");
  }
  if (process.platform === "win32") {
    const appDataRoot = process.env.APPDATA?.trim() || join(os.homedir(), "AppData", "Roaming");
    return resolve(appDataRoot, appIdentifier, "sofia-dev-data");
  }
  const configRoot = process.env.XDG_CONFIG_HOME?.trim() || join(os.homedir(), ".config");
  return resolve(configRoot, appIdentifier, "sofia-dev-data");
}
...
function assertDevDataPath(ctx, path) {
  ...
  ctx.assert(
    ...
    `Refusing to touch a path outside sofia-dev-data: ${resolved}`,
  );
  ctx.assert(resolved.includes("sofia-dev-data"), `Resolved path is not under sofia-dev-data: ${resolved}`);
  return resolved;
}
...
function runtimeStorageCandidates(ctx) {
  return [
    join(devDataRoot(), "home", ".config", "sofia"),
    join(devDataRoot(), "xdg", "config", "sofia"),
...
async function getServerInfo(ctx) {
  ...
  const info = await ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('sofiaServerInfo')", {
    awaitPromise: true,
  });
  ...
  ctx.assert(Boolean(baseUrl), "Sofia server base URL is unavailable.");
  ctx.assert(Boolean(token), "Sofia server token is unavailable.");
  return { baseUrl, token, hostToken };
}

async function fetchSofiaJson(ctx, path, options = {}) {
  const info = await getServerInfo(ctx);
  const headers = {
    Authorization: `Bearer ${info.token}`,
    ...(info.hostToken ? { "X-Sofia-Host-Token": info.hostToken } : {}),
...
async function ensureWorkspace(ctx) {
  ...
  let list = await fetchSofiaJson(ctx, "/workspaces");
  let workspace = workspaceItems(list).find((item) => resolve(String(item?.path ?? "")) === workspaceDir);
  if (!workspace) {
    const created = await fetchSofiaJson(ctx, "/workspaces/local", {
      method: "POST",
      body: {
    ...
    list = workspaceItems(created).length > 0 ? created : await fetchSofiaJson(ctx, "/workspaces");
...
async function readRuntimeConfigStatus(ctx) {
  ...
  const status = await fetchSofiaJson(ctx, `/workspace/${encodeURIComponent(workspaceId)}/runtime-config`);
...
async function clearLegacyBackups(ctx) {
  ...
  for (const entry of entries) {
    if (entry.startsWith(`${file}.sofia-backup-`)) {
...
async function enableDeveloperMode(ctx) {
  ...
  await ctx.waitFor(`localStorage.getItem("sofia.developerMode") === "1"`, {
    timeoutMs: 10_000,
...
async function restartSofiaServerFromDebug(ctx) {
  await openDebugSettings(ctx);
  await ctx.clickText("Restart Sofia server", { selector: "button", timeoutMs: 30_000 });
  await ctx.waitFor(`(() => {
    const text = document.body.innerText;
    return text.includes("Restarted Sofia server.") || text.includes("Failed to restart Sofia server.");
  })()`, {
    timeoutMs: 90_000,
    label: "Sofia server restart result",
  });
  const text = await ctx.eval("document.body.innerText");
  ctx.assert(!text.includes("Failed to restart Sofia server."), "Sofia server restart failed.");
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", {
    timeoutMs: 60_000,
    label: "control API after Sofia server restart",
  });
}

async function setDisabledProviderThroughServer(ctx) {
  const { workspaceId } = await ensureWorkspace(ctx);
  ctx.log("Using the Sofia server runtime-config route as the provider-toggle fallback for this isolated eval.");
  const result = await fetchSofiaJson(ctx, `/workspace/${encodeURIComponent(workspaceId)}/runtime-config/disabled-providers`, {
    method: "POST",
...
function assertSweptPersonalConfig(ctx, content) {
  ...
  for (const removed of ["sofia-cloud", '"default_agent"', "sofia-extensions-preview"]) {
...
function assertSweepState(ctx, state) {
  ...
  for (const key of ["mcp.sofia-cloud", "agent.sofia", "default_agent", "plugin"]) {
    ctx.assert(file.removedKeys?.includes(key), `Legacy sweep state did not list removed key ${key}.`);
  }
  ctx.assert(typeof file.backupPath === "string" && file.backupPath.includes(".sofia-backup-"), "Legacy sweep state did not record a backup path.");
  return file;
}
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The server restart sweeps Sofia-owned leftovers, preserves user-owned config, and reports the backup", {
          voiceover: vo[1],
          action: async () => {
            await enableDeveloperMode(ctx);
            await restartSofiaServerFromDebug(ctx);
            await openDebugSettings(ctx);
            await ctx.waitForText("Removed: mcp.sofia-cloud, agent.sofia, default_agent, plugin", { timeoutMs: 30_000 });
          ...
          assert: async () => {
            ...
            await ctx.expectText("Removed: mcp.sofia-cloud, agent.sofia, default_agent, plugin");
