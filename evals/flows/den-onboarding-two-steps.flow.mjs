...
const OWNER_EMAIL = process.env.SOFIA_EVAL_DEMO_EMAIL?.trim() || "alex@acme.test";
const OWNER_PASSWORD = process.env.SOFIA_EVAL_DEMO_PASSWORD?.trim() || "SofiaDemo123!";
...
const INSTALLED_KEY = "sofia:onboarding:app-installed";
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_API_URL"],
  steps: [
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Step two offers Sofia Models or the owner's own provider key", {
          voiceover: vo[2],
          action: async () => {
            await ctx.waitFor(
          ...
          assert: async () => {
            ...
            witness(
              ...
              choices.links.length === 2
                && choices.links.some((link) => link.label === "Use Sofia Models" && link.href === INFERENCE_PATH)
                && choices.links.some((link) => link.label === "Add your own key" && link.href === BYOK_PATH),
              "Step two routes to Sofia Models or to bring-your-own-key, and nowhere else",
              choices.links,
            );
            witness(
              ctx,
            ...
            witness(ctx, choices.recommended, "Sofia Models is the recommended choice", choices.recommended);
          },
          screenshot: {
            name: "model-choices",
            claim: "Step two is two cards: Sofia Models (recommended) or bring your own provider key.",
            requireText: ["Sofia Models", "RECOMMENDED", "Bring your own key", "Add your own key"],
    ...
    {
      name: "The recommended path lands on Sofia Models",
      run: async (ctx) => {
        await ctx.prove("Following the recommended choice opens the workspace's Sofia Models page", {
          voiceover: vo[3],
          action: async () => {
            await ctx.clickText("Use Sofia Models");
            await ctx.waitFor(`window.location.pathname === ${JSON.stringify(INFERENCE_PATH)}`, {
              timeoutMs: 30_000,
              label: "Sofia Models route",
            });
          },
          assert: async () => {
            witness(
          ...
          screenshot: {
            name: "sofia-models-page",
            claim: "The recommended choice lands on Sofia Models, where inference is switched on.",
            requireText: ["Sofia Models", "Frontier intelligence"],
