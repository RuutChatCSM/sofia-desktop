...
async function withGenericBrowser(ctx, fn) {
  const cdpBaseUrl = cleanBaseUrl(ctx.cdpBaseUrl ?? process.env.SOFIA_EVAL_CDP_URL);
  witness(ctx, cdpBaseUrl.length > 0, "A generic CDP browser endpoint is available for rendered-email screenshots", cdpBaseUrl || "missing --cdp-url/SOFIA_EVAL_CDP_URL");
...
function assertRemovedDesktopCopy(ctx, html) {
  ...
  witness(ctx, !lowerHtml.includes("sofia.ruut.chat/download"), "The invitation email has no generic desktop download link", "absent");

  const blockedLinks = htmlHrefs(html).filter((href) => {
    const lowerHref = href.toLowerCase();
    return lowerHref.includes("/install") || lowerHref.includes("sofia.ruut.chat/download") || lowerHref.includes("download") || lowerHref.includes("sofia://");
  });
