 ...
 * point --cdp-url at a Chrome whose single tab is SOFIA_EVAL_DEN_WEB_URL.
...
const RETIRED_TOP_LEVEL = ["MCP Connections", "Integrations", "Sofia Models", "LLM Providers", "Desktop Policies", "API Keys", "SCIM", "SSO", "Billing", "Org Settings"];
...
export default {
  ...
  steps: [
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Models holds Sofia Models and LLM Providers side by side", {
          voiceover: vo[3],
          action: async () => {
          ...
          assert: async () => {
            ...
            const children = await navChildLabels(ctx);
            ctx.assert(children.some((label) => label.startsWith("Sofia Models")), `Models children missing Sofia Models: ${JSON.stringify(children)}`);
          ...
          screenshot: {
            name: "models-group",
            claim: "The Models group shows Sofia Models and LLM Providers as siblings.",
            requireText: ["Models", "Sofia Models", "LLM Providers"],
            rejectText: ["Something went wrong"],
          },
        });
      },
