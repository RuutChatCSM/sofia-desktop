...
async function dismissSofiaModelsDialog(ctx) {
  const clicked = await ctx.eval(`(() => {
    const button = [...document.querySelectorAll('button')]
      .find((candidate) => (candidate.textContent ?? '').trim() === 'Continue without Sofia Models');
    button?.click();
    return Boolean(button);
  })()`);
...
async function ensureDeveloperMode(ctx) {
  const current = await ctx.eval("window.localStorage.getItem('sofia.developerMode')");
  ...
  await ctx.waitFor("window.localStorage.getItem('sofia.developerMode') === '1'", {
    timeoutMs: 10_000,
...
async function openCloudAccount(ctx) {
  ...
  await ctx.waitFor(
    `(() => {
      const text = document.body?.innerText ?? '';
      return text.includes('Sofia Cloud') && (
...
async function returnToApp(ctx) {
  ...
  await dismissSofiaModelsDialog(ctx);
  await ctx.waitFor("(() => { const text = document.body?.innerText ?? ''; return text.includes('Add workspace') || text.includes('Welcome to Sofia'); })()", {
    timeoutMs: 30_000,
...
async function ensureWorkspaceShellForRemote(ctx) {
  ...
  const onWelcome = await ctx.eval("location.hash.includes('/welcome') || (document.body?.innerText ?? '').includes('Welcome to Sofia')");
  if (onWelcome) {
    state.startedFromWelcome = true;
    if (state.originalPreferences === null) {
      state.originalPreferences = await ctx.eval("localStorage.getItem('sofia.preferences')");
    }
...
async function createStarterWorkspaceFromWelcome(ctx) {
  const starterDir = await mkdtemp(join(tmpdir(), "sofia-fraimz-starter-"));
...
async function startSelfSignedSofiaServer() {
  if (state.selfSignedServerUrl) return state.selfSignedServerUrl;

  const dir = await mkdtemp(join(tmpdir(), "sofia-self-signed-"));
  ...
  const server = createHttpsServer({ key, cert }, sofiaDiscoveryHandler("ws_self_signed", "Self-signed remote", "/srv/self-signed"));
  await listen(server);
...
async function stopSelfSignedSofiaServer() {
...
async function startHttpSofiaServer() {
  if (state.httpServerUrl) return state.httpServerUrl;

  const server = createHttpServer(sofiaDiscoveryHandler(HTTP_REMOTE_WORKSPACE_ID, HTTP_REMOTE_WORKSPACE_NAME, "/srv/fraimz-remote"));
  await listen(server);
...
async function stopHttpSofiaServer() {
...
function sofiaDiscoveryHandler(id, name, path) {
...
async function desktopWorkspaceList(ctx) {
  ...
  return ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('workspaceBootstrap')", { awaitPromise: true });
}

async function forgetDesktopWorkspace(ctx, workspaceId) {
  await ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop('workspaceForget', ${JSON.stringify(workspaceId)})`, {
    awaitPromise: true,
  });
}

async function deleteSofiaServerWorkspace(ctx, workspaceId) {
  if (!workspaceId) return;
  await ctx.eval(`(async () => {
    const baseUrl = localStorage.getItem('sofia.server.urlOverride') || localStorage.getItem('sofia.server.active');
    if (!baseUrl || !window.__SOFIA_ELECTRON__?.invokeDesktop) return null;
    const token = localStorage.getItem('sofia.server.token') || '';
    const hostToken = localStorage.getItem('sofia.server.hostToken') || '';
    const headers = {};
    if (token) headers.authorization = 'Bearer ' + token;
    if (hostToken) headers['x-sofia-host-token'] = hostToken;
    return window.__SOFIA_ELECTRON__.invokeDesktop('__fetch', baseUrl.replace(/\/+$/, '') + '/workspaces/' + encodeURIComponent(${JSON.stringify(workspaceId)}), {
      method: 'DELETE',
      headers,
      timeoutMs: 8_000,
    });
...
async function restartSofiaServer(ctx) {
  await ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('sofiaServerRestart', {})", { awaitPromise: true }).catch(() => null);
}

function isEvalStarterWorkspace(workspace) {
  const workspacePath = String(workspace?.path ?? "");
  return workspacePath.includes("/sofia-fraimz-starter-") || workspacePath.includes("\\sofia-fraimz-starter-");
}

function isEvalStarterPath(value) {
  return String(value ?? "").includes("sofia-fraimz-starter-");
}

function desktopUserDataDir() {
  const appId = "com.differentai.sofia.dev";
...
async function pruneEvalStarterRecoveryStores() {
  ...
  const workspaceStatePath = join(userDataDir, "sofia-workspaces.json");
  ...
  const tokenStorePath = join(userDataDir, "sofia-server-tokens.json");
  ...
  const serverStatePath = join(userDataDir, "sofia-server-state.json");
  ...
  for (const serverConfigPath of [
    join(userDataDir, "sofia-dev-data", "xdg", "config", "sofia", "server.json"),
    join(userDataDir, "sofia-dev-data", "home", ".config", "sofia", "server.json"),
  ]) {
...
async function cleanupEvalStarterWorkspaces(ctx) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    for (const workspace of leftovers) {
      await deleteSofiaServerWorkspace(ctx, workspace.id);
...
async function cleanupCreatedWorkspace(ctx) {
  const list = await desktopWorkspaceList(ctx).catch(() => null);
  const created = (list?.workspaces ?? []).find((workspace) => workspace?.sofiaWorkspaceId === HTTP_REMOTE_WORKSPACE_ID)
  ...
  if (starter?.id && starter.id !== state.previousWorkspaceId && starter.id !== created?.id) {
    await deleteSofiaServerWorkspace(ctx, starter.id);
  ...
  if (state.startedFromWelcome && !state.previousWorkspaceId) {
    await restartSofiaServer(ctx);
  }
  if (state.previousWorkspaceId) {
    await ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop('workspaceSetSelected', ${JSON.stringify(state.previousWorkspaceId)})`, {
      awaitPromise: true,
    });
    await ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop('workspaceSetRuntimeActive', ${JSON.stringify(state.previousWorkspaceId)})`, {
      awaitPromise: true,
    });
  }
...
async function restoreFreshWelcome(ctx) {
  ...
  await ctx.eval(`(() => {
    ...
    if (original === null) {
      localStorage.setItem('sofia.preferences', JSON.stringify({ hasCompletedOnboarding: false }));
    } else {
      try {
        const prefs = JSON.parse(original);
        prefs.hasCompletedOnboarding = false;
        localStorage.setItem('sofia.preferences', JSON.stringify(prefs));
      } catch {
        localStorage.setItem('sofia.preferences', JSON.stringify({ hasCompletedOnboarding: false }));
      }
    }
  ...
  await ctx.waitFor("(document.body?.innerText ?? '').includes('Welcome to Sofia')", {
    timeoutMs: 30_000,
  ...
  if (removedLateStarter > 0) {
    await ctx.eval("(() => { localStorage.removeItem('sofia.react.activeWorkspace'); window.location.hash = '/welcome'; window.location.reload(); return true; })()");
    await ctx.waitFor("Boolean(window.__sofiaControl)", {
      timeoutMs: 60_000,
      label: "control API after late starter cleanup reload",
    });
  }
  await ctx.waitFor("location.hash.includes('/welcome') && (document.body?.innerText ?? '').includes('Welcome to Sofia')", {
    timeoutMs: 30_000,
...
async function visibleRemoteError(ctx) {
  return ctx.eval(`(() => {
    ...
    return lines.find((line) => /certificate|ERR_CERT|fetch failed|Sofia server is unavailable/i.test(line)) ?? '';
  })()`);
}
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("The Cloud account settings surface opens cleanly", {
          claim: "Settings → Account renders the Sofia Cloud account controls and does not show a generic fetch failure.",
          voiceover: vo[0],
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const serverUrl = await startSelfSignedSofiaServer();
        ...
        await ctx.prove("Connecting a worker with an untrusted certificate shows the certificate cause", {
          ...
          assert: async () => {
            ...
            ctx.assert(!errorText.includes("Sofia server is unavailable"), `Remote error was swallowed into a generic availability message: ${errorText}`);
          ...
          screenshot: {
            ...
            rejectText: ["Sofia server is unavailable", "TypeError: fetch failed"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        const serverUrl = await startHttpSofiaServer();
        ...
        await ctx.prove("A valid remote worker is discovered, created, selected, and listed", {
          ...
          assert: async () => {
            ...
            const list = await desktopWorkspaceList(ctx);
            const workspace = (list?.workspaces ?? []).find((entry) => entry?.sofiaWorkspaceId === HTTP_REMOTE_WORKSPACE_ID);
            ctx.assert(Boolean(workspace), `Electron workspace store did not include ${HTTP_REMOTE_WORKSPACE_ID}.`);
            ctx.assert(workspace.remoteType === "sofia", `Expected remoteType sofia, got ${workspace.remoteType}.`);
          ...
          screenshot: {
            ...
            rejectText: ["Remote server details", "Sofia server is unavailable", "fetch failed"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("After cleanup, the app returns to a healthy baseline", {
          ...
          action: async () => {
            await cleanupCreatedWorkspace(ctx);
            await stopHttpSofiaServer();
            await stopSelfSignedSofiaServer();
            ...
            if (state.startedFromWelcome && !state.previousWorkspaceId) {
              ...
              await ctx.waitFor("(document.body?.innerText ?? '').includes('Sofia Cloud')", {
                timeoutMs: 30_000,
          ...
          assert: async () => {
            if (state.startedFromWelcome && !state.previousWorkspaceId) {
              await ctx.expectText("Welcome to Sofia");
            } else {
              await ctx.expectText("Sofia Cloud");
            }
          ...
          screenshot: {
            ...
            requireText: state.startedFromWelcome && !state.previousWorkspaceId ? ["Welcome to Sofia"] : ["Sofia Cloud"],
