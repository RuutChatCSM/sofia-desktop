...
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? "http://127.0.0.1:3005").replace(/\/+$/, "");
const ADMIN_CDP_URL = (process.env.SOFIA_EVAL_WEB_CDP_ADMIN ?? "").replace(/\/+$/, "");
    ...
    window.__sofiaUpdaterEvalBridge = {
      ...
      check: async (channel, targetVersion) => {
        ...
        return {
          ...
          channel: 'stable',
          feedUrl: targetVersion ? 'https://github.com/RuutChatCSM/sofia-desktop/releases/download/v' + targetVersion : 'eval://stable',
...
async function ensureDesktopSession(ctx) {
  const apiUrl = ctx.env.SOFIA_EVAL_DEN_API_URL.replace(/\/+$/, "");
  const token = ctx.env.SOFIA_EVAL_DEN_TOKEN;
  ...
  await ctx.eval(`(() => {
    ...
    localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(apiUrl)});
    localStorage.setItem('sofia.den.authToken', ${JSON.stringify(token)});
    localStorage.setItem('sofia.den.activeOrgId', ${JSON.stringify(activeOrg.id)});
    localStorage.setItem('sofia.den.activeOrgSlug', ${JSON.stringify(activeOrg.slug ?? "acme-robotics-demo")});
    localStorage.setItem('sofia.den.activeOrgName', ${JSON.stringify(activeOrg.name ?? "Acme Robotics")});
    window.dispatchEvent(new CustomEvent('sofia-den-settings-changed', { detail: {} }));
    window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: { token: ${JSON.stringify(token)} } }));
    return true;
  })()`);
...
async function openDesktopUpdates(ctx) {
  ...
  await ctx.eval("localStorage.setItem('sofia.react.settings.update-auto-check', '0')");
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_EVAL_WEB_CDP_ADMIN"],
  steps: [
    {
