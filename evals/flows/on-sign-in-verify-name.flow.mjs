...
async function resetSession(ctx) {
  await ctx.eval(
    `(async () => {
      const token = localStorage.getItem("sofia:web:auth-token");
      try {
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A signed-in dashboard user with the default name sees the User Profile dialog", {
          ...
          action: async () => {
            await uiSignIn(ctx);
            await updateProfileName(ctx, "Sofia", "User");
          ...
          screenshot: {
            ...
            claim: "The dashboard opens a User Profile dialog for the default Sofia User name.",
      ...
      run: async (ctx) => {
        await ctx.prove("The dialog shows first and last name fields with Save disabled until a change", {
          ...
          assert: async () => {
            ...
            ctx.assert(state.first === "Sofia", "First name is prefilled from the default name.");
      ...
      run: async (ctx) => {
        await ctx.prove("Cancel closes the dialog without changing the default name during that dashboard visit", {
          ...
          assert: async () => {
            ...
            const name = await readProfileName(ctx);
            ctx.assert(name === "Sofia User", "Cancel leaves the stored default name unchanged.");
          },
          screenshot: {
