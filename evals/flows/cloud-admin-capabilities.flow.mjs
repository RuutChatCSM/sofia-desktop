...
export default {
  id: FLOW_ID,
  title: "Platform admins use Den admin tools through the existing Sofia Cloud connection",
  kind: "internal",
  requiresApp: false,
  steps: [
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The desktop catalog contains sofia-cloud but no sofia-admin entry", {
          voiceover: vo[3],
          assert: async () => {
            ...
            witness(ctx, constants.includes('serverName: "sofia-cloud"'), "Sofia Cloud remains in the desktop catalog");
            witness(ctx, !constants.includes('serverName: "sofia-admin"'), "The separate Sofia Admin catalog entry is absent");
            witness(ctx, !store.includes('entry.serverName === "sofia-admin"'), "Desktop token injection no longer special-cases a local admin connection");
            ctx.output("Desktop connection catalog", "sofia-cloud: present\nsofia-admin: absent\nadmin token special-case: absent");
          },
        });
      },
    },
