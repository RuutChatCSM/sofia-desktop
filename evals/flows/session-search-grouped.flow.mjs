 ...
 * SOFIA_EVAL_SEARCH_QUERY for profiles seeded with different data. The flow
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sidebar exposes a Search sessions entry", {
          ...
          voiceover:
            "This is Sofia. Every workspace and conversation lives in the sidebar on the left. At the very top there is a new entry: Search sessions. Cross-message search used to be hidden behind a keyboard shortcut — now it is one click away, and the shortcut hint is printed right on the button.",
          assert: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const query = ctx.env.SOFIA_EVAL_SEARCH_QUERY?.trim() || "joke";
