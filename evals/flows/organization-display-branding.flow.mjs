...
exit 1
`);
  const preview = await execFileAsync("daytona", ["preview-url", ctx.env.SOFIA_EVAL_DAYTONA_SANDBOX.trim(), "-p", String(ASSET_PORT)], {
    timeout: 30_000,
  });
...
async function mintInstallLink(ctx) {
  ...
  const config = await fetch(`${ctx.env.SOFIA_EVAL_DEN_API_URL.replace(/\/$/, "")}/v1/install-config?token=${encodeURIComponent(state.installToken)}`).then(async (response) => {
...
sleep 3
cd /workspace
SOFIA_WORKSPACE_DIR=/workspace SOFIA_DESKTOP_BOOTSTRAP_PATH=/workspace/.sofia-daytona/desktop-bootstrap.json SOFIA_ELECTRON_REMOTE_DEBUG_PORT=9825 DISPLAY=:99 bash .devcontainer/start-daytona-electron.sh --detach
...
set -euo pipefail
file=$(find "$HOME/.config" -name sofia-ui-control.json -type f | head -n 1)
test -n "$file"
cat "$file"
`);
}
...
export default {
  ...
  title: "Organization display branding reaches download, setup, and desktop while Sofia's signed identity stays stable",
  kind: "user-facing",
  requiredEnv: [
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ensureRendererMounted(ctx);
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000, label: "desktop control surface" });
      ...
      run: async (ctx) => {
        await ctx.prove("An Example Corp owner saves Acme Work and a wordmark through Brand Appearance", {
          ...
          action: async () => {
            ...
            await setInputByPlaceholder(ctx, "Sofia", APP_NAME);
          ...
          assert: async () => {
            ...
            await panelEval(ctx, `(() => {
              const input = Array.from(document.querySelectorAll('input')).find((candidate) => candidate.placeholder === 'Sofia');
              input?.scrollIntoView({ block: 'center' });
      ...
      run: async (ctx) => {
        await ctx.prove("After sign-in, the desktop title and sidebar apply Acme Work live", {
          ...
          assert: async () => {
            ...
            state.updaterBefore = await ctx.eval("window.__SOFIA_ELECTRON__?.updater?.getChannel?.()", { awaitPromise: true });
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Branding survives a fresh desktop process while app and updater identity remain Sofia-compatible", {
          voiceover: vo[3],
          action: async () => {
          ...
          assert: async () => {
            ...
            ctx.assert(/com\.differentai\.sofia(?:\.dev)?/.test(identity), `Unexpected application identity: ${identity}`);
            ctx.assert(!identity.includes("acme-work"), `Display name leaked into signed identity: ${identity}`);
            const updaterAfter = await ctx.eval("window.__SOFIA_ELECTRON__?.updater?.getChannel?.()", { awaitPromise: true });
