 ...
 * - SOFIA_EVAL_DEN_WEB_URL: Den Web origin opened in the browser CDP target.
 *
 * Optional env:
 * - SOFIA_EVAL_CDP_URL or --cdp-url: CDP endpoint for a headless Chrome page target.
 * - SOFIA_EVAL_ENTERPRISE_ORG_NAME: organization display name (default Example Organization).
 * - SOFIA_EVAL_ENTERPRISE_APP_NAME: branded app display name (default Sofia).
 * - SOFIA_EVAL_ENTERPRISE_ADMIN_EMAIL: inviter/admin email (default admin@example.com).
 * - SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_EMAIL: invited member email (default new.member@example.com).
 * - SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_DISPLAY_NAME: invited member display name (default Alex).
 * - SOFIA_EVAL_ENTERPRISE_PASSWORD: account password (default TutorialDemo123!).
 * - SOFIA_EVAL_ENTERPRISE_ACCOUNT_MODE: use sign-in to rerun with an existing invited account (default create).
 *
 * Runner note: evals/runner/run.mjs selects one CDP page per run. For this web
 * flow, point SOFIA_EVAL_CDP_URL (or --cdp-url) at a clean headless Chrome
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("New member joins the organization and lands on the welcome step", {
          action: async () => {
            if (envText(ctx, "SOFIA_EVAL_ENTERPRISE_ACCOUNT_MODE") === "sign-in") {
      ...
      run: async (ctx) => {
        await ctx.prove("New member can get the desktop app installer from the welcome screen", {
          ...
          screenshot: {
            ...
            requireText: ["Download Sofia", "Download the Sofia installer"],
            rejectText: ["Something went wrong"],
          },
        });
      },
...
function newMemberDisplayName(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_DISPLAY_NAME") || DEFAULT_NEW_MEMBER_DISPLAY_NAME;
}

function enterpriseAppName(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_APP_NAME") || "Sofia";
}

function password(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_PASSWORD") || DEFAULT_PASSWORD;
}
...
async function authenticateExistingInvitee(ctx) {
  ...
  await ctx.eval(`(() => {
    localStorage.setItem('sofia:web:auth-token', ${JSON.stringify(state.newMemberToken)});
