...
async function startDiagnostics(ctx) {
  ...
  state.child = spawn(command, ["--filter", "@sofia-ee/diagnostics", "dev"], {
    cwd: ROOT,
...
async function diagnosticRequest(pathname, runId, step, init = {}) {
  ...
  headers.set("x-sofia-diagnostic-run-id", runId);
  headers.set("x-sofia-diagnostic-step", step);
  headers.set(
    "x-sofia-diagnostic-signature",
    createHmac("sha256", BEARER_TOKEN).update(`sofia-diagnostics-v1\n${runId}\n${step}`).digest("hex"),
  );
...
async function runControlledDiagnostic() {
  ...
  const tokenResponse = await diagnosticRequest("/oauth/token", runId, "oauth-token", {
    ...
    headers: {
      authorization: `Basic ${Buffer.from(`sofia-diagnostics:${BEARER_TOKEN}`).toString("base64")}`,
  ...
  const baseHeaders = {
    ...
    "x-sofia-diagnostic-run-id": runId,
    "x-sofia-diagnostic-step": "mcp-initialize",
    "x-sofia-diagnostic-signature": createHmac("sha256", BEARER_TOKEN)
