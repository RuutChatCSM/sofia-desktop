...
const REQUIRED_DEN_ENV = ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_WEB_URL"];
...
function denBootstrap(ctx) {
  return {
    baseUrl: requiredEnv(ctx, "SOFIA_EVAL_DEN_WEB_URL"),
    apiBaseUrl: requiredEnv(ctx, "SOFIA_EVAL_DEN_API_URL"),
    requireSignin: false,
  };
}
...
function expectAgentReply(ctx) {
  return Boolean(ctx.env.SOFIA_EVAL_EXPECT_AGENT_REPLY?.trim());
}

function runSalt(ctx) {
  const existing = optionalStateString(ctx, "runSalt");
  if (existing) return existing;
  const seed = ctx.env.SOFIA_EVAL_RUNSTAMP?.trim() || new Date().toISOString();
...
function uniqueOrgDetails(ctx) {
  const stamp = ctx.env.SOFIA_EVAL_RUNSTAMP?.trim()
...
async function returnToWorkspace(ctx, surface, key) {
  ...
  await ctx.on(surface, async () => {
    ...
    const predicate = route === "/session"
      ? `(() => {
        const current = window.__sofiaControl?.snapshot?.().route || window.location.hash.replace(/^#/, '');
...
async function navigateDenMembers(ctx) {
  const webUrl = cleanBaseUrl(requiredEnv(ctx, "SOFIA_EVAL_DEN_WEB_URL"));
        ...
        await ctx.on(alexDesktop, async () => {
          await ctx.prove("Alex's fresh desktop boots, connects to Den, and shows her cloud account", {
            ...
            action: async () => {
              ...
              await ctx.waitFor(`(() => {
                const route = window.__sofiaControl?.snapshot?.().route || window.location.hash.replace(/^#/, '');
        ...
        await ctx.on(jamieDesktop, async () => {
          await ctx.prove("A second fresh desktop is alive for Jamie mid-scenario", {
            ...
            assert: async () => {
              ...
              await ctx.expectText("Sofia", { timeoutMs: 30_000 });
              await ctx.expectNoText(ctx.actors.alex.email);
            },
            screenshot: { name: "jamie-fresh-desktop", requireText: ["Sofia"], rejectText: [ctx.actors.alex.email] },
          });
        });
      },
    },
