...
const CARD_SELECTOR = '[data-testid="download-sofia-card"]';
const LINK_SELECTOR = '[data-testid="download-sofia-link"]';
const RECOMMENDED_SELECTOR = '[data-testid="download-sofia-link"][data-recommended="true"]';
...
async function navigateToDownload(ctx) {
  ...
  await ctx.waitFor(
    `Boolean(document.querySelector(${JSON.stringify(CARD_SELECTOR)})) && document.body.innerText.includes("Download Sofia")`,
...
export default {
  id: FLOW_ID,
  title: "Landing /download serves the light Download Sofia card",
  kind: "user-facing",
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("/download renders the light-mode Download Sofia card", {
          voiceover: vo[0],
          action: async () => {
          ...
          assert: async () => {
            const actual = await ctx.eval(`(() => {
              ...
              return {
                ...
                bodyHasTitle: document.body.innerText.includes("Download Sofia"),
                backgroundColor,
                channels,
            ...
            recordAssertion(
              ctx,
              "The Download Sofia card exists on /download",
