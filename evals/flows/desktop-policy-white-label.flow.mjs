  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        const route = await ctx.waitFor(
          "window.__sofiaControl.snapshot().route",
        ...
        await ctx.eval(`(() => {
          ...
          const bridge = window.__sofiaApplyDesktopConfig;
...

    {
      ...
      run: async (ctx) => {
        ...
        await ctx.screenshot("baseline", {
          claim: "App renders with default Sofia accent and no brand logo.",
        });
      },
    },

    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__sofiaControl.listActions().some(a => a.id === 'eval.brand_theme.apply')",
