 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_WEB_URL          Den web origin
 * - SOFIA_EVAL_DEN_EMAIL            Seeded admin email
 * - SOFIA_EVAL_DEN_PASSWORD         Seeded admin password
 * - SOFIA_EVAL_AZURE_FOUNDRY_RESOURCE  Azure AI Foundry resource name
 * - SOFIA_EVAL_AZURE_FOUNDRY_API_KEY   Azure AI Foundry key (throwaway)
 */

const RESOURCE_ORIGIN = `https://${process.env.SOFIA_EVAL_AZURE_FOUNDRY_RESOURCE ?? ""}.services.ai.azure.com`;
...
export default {
  ...
  requiredEnv: [
    ...
    "SOFIA_EVAL_AZURE_FOUNDRY_RESOURCE",
    "SOFIA_EVAL_AZURE_FOUNDRY_API_KEY",
  ],
  steps: [
    {
      name: "Signed-in dashboard session (signs in if needed)",
      run: async (ctx) => {
        const origin = ctx.env.SOFIA_EVAL_DEN_WEB_URL.trim().replace(/\/+$/, "");
        ...
        if (me !== 200) {
          const signIn = await ctx.eval(`(async () => {
            const response = await fetch("/api/auth/sign-in/email", {
              ...
              credentials: "include",
              body: JSON.stringify({
                email: ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_EMAIL)},
                password: ${JSON.stringify(ctx.env.SOFIA_EVAL_DEN_PASSWORD)},
              }),
            });
          ...
          "Pasting the bare Azure resource origin + key surfaces the resource's one real deployment, not the model catalog",
          {
            action: async () => {
              ...
              await ctx.eval(fillInputExpr(
                ...
                ctx.env.SOFIA_EVAL_AZURE_FOUNDRY_API_KEY,
              ));
            },
            assert: async () => {
