...
function testIconUrl(ctx) {
  return ctx.env.SOFIA_EVAL_BRAND_ICON_URL.trim();
}
...
async function closeAdminPanel(ctx) {
  await ctx.eval(`(async () => {
    await window.__SOFIA_ELECTRON__?.browser?.closeAllTabs?.();
    await window.__SOFIA_ELECTRON__?.browser?.hide?.();
    return true;
  })()`, { awaitPromise: true });
...
$path = 'C:\\Users\\Administrator\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Sofia.lnk'
...
async function assertSofiaWindow(ctx) {
  const result = await daytonaComputerUseWindows(sandboxId(ctx));
  const windows = Array.isArray(result?.windows) ? result.windows : [];
  const sofia = windows.find((entry) => /sofia/i.test(entry?.title ?? ""));
  ctx.assert(Boolean(sofia), `Daytona Computer Use did not find an Sofia window: ${JSON.stringify(windows)}`);
  ctx.recordEvidence({
    type: "assertion",
    status: "passed",
    assertion: "Daytona Computer Use sees the real Sofia window in the interactive Windows session",
    actual: sofia?.title,
  });
}

async function assertWindowsRuntime(ctx) {
  const info = await ctx.eval("window.__SOFIA_ELECTRON__?.system?.getArchitectureInfo?.()", { awaitPromise: true });
...
export default {
  ...
  steps: [
    ...
    },
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The stock Sofia icon is visible in the real Windows taskbar", {
          voiceover: vo[0],
          action: async () => {
            await assertSofiaWindow(ctx);
          },
          assert: async () => {
