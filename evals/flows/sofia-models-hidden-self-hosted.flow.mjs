...
// (evals/voiceovers/sofia-models-hidden-self-hosted.md).
// The runner fails this flow if the narration drifts from that script.
const FLOW_ID = "sofia-models-hidden-self-hosted";
const vo = await loadVoiceoverParagraphs(FLOW_ID);

const SELF_HOSTED_BASE_URL = "https://den.internal.acme.test";
const STARTUP_DIALOG_TITLE = "Use Sofia Models without API keys";
...
async function waitForControl(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 90_000, label: "Sofia control API" });
}
...
async function dismissOnboardingOverlays(ctx) {
  const ATTRIBUTION_TITLE = "How did you hear about Sofia?";
...
async function ensureWorkspaceSession(ctx) {
  await waitForControl(ctx);
  const workspaceDir = ctx.env.SOFIA_EVAL_WORKSPACE_DIR?.trim() || "/workspace/hello";
...
async function resetPromoLatches(ctx, { startupShown }) {
  await ctx.eval(`(() => {
    ...
    if (${JSON.stringify(Boolean(startupShown))}) {
      localStorage.setItem("sofia.sofiaModelsPromo.startupShown", "1");
    } else {
      localStorage.removeItem("sofia.sofiaModelsPromo.startupShown");
    }
    localStorage.removeItem("sofia.sofiaModelsPromo.hidden");
    localStorage.removeItem("sofia.sofiaModelsPromo.lastShownAt");
    return true;
  })()`);
}
...
export default {
  id: FLOW_ID,
  title: "Self-hosted control planes hide every Sofia Models upsell surface; hosted keeps them",
  kind: "user-facing",
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Setup: session is ready on the default hosted Sofia Cloud control plane with promo latches reset (startup dialog latch pre-marked for the baseline phase).",
        });
      },
    },
    {
      name: "Frame 1 — hosted model picker pitches Sofia Models",
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane the model picker shows the Sofia Models promo group", {
          voiceover: vo[0],
          action: async () => {
            await openModelPicker(ctx);
            await ctx.waitForText("Sofia hosted", { timeoutMs: 15_000 });
          },
          assert: async () => {
            await ctx.expectText("Sofia Models");
            await ctx.expectText("Sofia hosted");
          },
          screenshot: {
            name: "hosted-model-picker-promo",
            requireText: ["Sofia Models", "Sofia hosted"],
          },
        });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane Settings > AI offers the Sofia Models subscription", {
          voiceover: vo[1],
          action: async () => {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Hosted frontier models for Sofia tasks without managing provider API keys.");
          },
          screenshot: {
            name: "hosted-settings-ai-subscribe",
            requireText: ["Sofia Models", "Subscribe"],
    ...
    {
      name: "Frame 3 — hosted workspace setup offers Use Sofia Models",
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane the workspace-setup provider step includes the Use Sofia Models option", {
          voiceover: vo[2],
          action: async () => {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Use Sofia Models");
            await ctx.expectText("Bring your own API key");
            await ctx.expectText("Skip and use the free model");
          },
          screenshot: {
            name: "hosted-provider-step-sofia-models",
            requireText: ["Power your first task", "Use Sofia Models", "Skip and use the free model"],
          },
        });
        await dismissOnboardingOverlays(ctx);
        await ensureWorkspaceSession(ctx);
    ...
    {
      name: "Frame 5 — no startup pitch and no Sofia Models in Settings > AI",
      run: async (ctx) => {
        await ctx.prove("Self-hosted: the startup dialog never auto-opens and Settings > AI drops every Sofia Models row", {
          voiceover: vo[4],
          action: async () => {
            await ensureWorkspaceSession(ctx);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Self-hosted: the model picker lists only real providers, no Sofia Models group", {
          voiceover: vo[5],
          action: async () => {
            await ctx.navigateHash("/");
            await ensureWorkspaceSession(ctx);
          ...
          assert: async () => {
            await ctx.expectNoText("Sofia hosted");
            await ctx.expectNoText("Sofia Models");
          },
          screenshot: {
            name: "self-hosted-model-picker-clean",
            requireText: ["All models"],
            rejectText: ["Sofia Models", "Sofia hosted"],
          },
        });
        await closeModelPicker(ctx);
      },
    ...
    {
      name: "Frame 7 — self-hosted provider step has no Sofia Models option",
      run: async (ctx) => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Clearing the server configuration returns to Sofia Cloud and the subscribe banner comes back", {
          voiceover: vo[7],
          action: async () => {
            await ctx.navigateHash("/settings/advanced");
