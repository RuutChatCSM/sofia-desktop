...
export default {
  ...
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["SOFIA_EVAL_LANDING_URL", "SOFIA_EVAL_LANDING_PROD_URL"],
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Dev server ships no PostHog, and the hero prompt still works.", {
          ...
          action: async () => {
            ...
            htmlScan = await scanHomeHtml(ctx.env.SOFIA_EVAL_LANDING_URL);
            await ctx.eval(`location.href = ${JSON.stringify(pageUrl(ctx.env.SOFIA_EVAL_LANDING_URL))}; true`);
            await ctx.waitFor(
            ...
            await grantClipboardPermissions(ctx, ctx.env.SOFIA_EVAL_LANDING_URL);
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Production build ships PostHog.", {
          ...
          action: async () => {
            await applyDesktopViewport(ctx);
            htmlScan = await scanHomeHtml(ctx.env.SOFIA_EVAL_LANDING_PROD_URL);
            await setPosthogNetworkBlocked(ctx, true);
            await ctx.eval(`location.href = ${JSON.stringify(pageUrl(ctx.env.SOFIA_EVAL_LANDING_PROD_URL))}; true`);
            await ctx.waitFor(
              `Boolean(document.querySelector(${JSON.stringify(COPY_BUTTON_SELECTOR)}))`,
