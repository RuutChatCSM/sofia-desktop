  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sofia lands on the empty new-task composer screen", {
          voiceover: vo[0],
          action: async () => {
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API" });
      ...
      run: async (ctx) => {
        await ctx.prove("Starting a task keeps the session composer docked and width-capped", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(`window.__sofiaControl.snapshot().route.includes("/session/")`, { timeoutMs: 60_000, label: "session route after task creation" });
