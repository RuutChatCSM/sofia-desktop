...
function denBase(ctx) {
  return trimBase(required(ctx, "SOFIA_EVAL_DEN_API_URL"));
}

function spaBase(ctx) {
  return trimBase(required(ctx, "SOFIA_EVAL_SPA_URL"));
}

function mockBase(ctx) {
  return trimBase(required(ctx, "SOFIA_EVAL_MOCK_URL"));
}
...
function hostHeaders(ctx) {
  return {
    ...
    "x-sofia-host-token": required(ctx, "SOFIA_EVAL_INSTANCE_HOST_TOKEN"),
  };
}
...
async function clearDenBrowserSession(ctx) {
  await ctx.eval(`(() => {
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith("sofia.den.")) localStorage.removeItem(key);
    }
    localStorage.removeItem("sofia:web:auth-token");
    return true;
  })()`);
}

function denBrowserBase(ctx) {
  const override = process.env.SOFIA_EVAL_DEN_BROWSER_URL?.trim();
...
async function seedDenSession(ctx) {
  ...
  await ctx.eval(`(() => {
    localStorage.setItem("sofia.den.baseUrl", ${JSON.stringify(denBrowserBase(ctx))});
...
async function signIn(ctx) {
  const signInResult = await denFetch(ctx, "/api/auth/sign-in/email", {
    method: "POST",
    body: JSON.stringify({
      email: required(ctx, "SOFIA_EVAL_DEN_EMAIL"),
      password: required(ctx, "SOFIA_EVAL_DEN_PASSWORD"),
    }),
  });
...
async function resolveSignedInOrg(ctx) {
  ...
  witness(ctx, org.name === required(ctx, "SOFIA_EVAL_ORG_NAME"), "The active organization display name matches the eval org", {
    expected: required(ctx, "SOFIA_EVAL_ORG_NAME"),
    actual: org.name,
  });
  state.org = org;
}
...
async function extractClientToken(ctx) {
  ...
  const match = root.text.match(/window\.__SOFIA_BOOTSTRAP__\s*=\s*(\{[^<]+?\})\s*<\/script>/);
  witness(ctx, Boolean(match), "The instance HTML includes __SOFIA_BOOTSTRAP__", root.text.slice(0, 500));
...
export default defineFlow({
  id: FLOW_ID,
  title: "Cloud web sign-in opens a real instance, completes an approved action, and executes a mock MCP through Sofia Connect",
  kind: "user-facing",
  preserveTheme: true,
  requiresApp: true,
  requiredEnv: [
    ...
    "SOFIA_EVAL_SPA_URL",
    "SOFIA_EVAL_MOCK_URL",
    "SOFIA_EVAL_DEN_EMAIL",
    "SOFIA_EVAL_DEN_PASSWORD",
    "SOFIA_EVAL_ORG_NAME",
    "SOFIA_EVAL_INSTANCE_HOST_TOKEN",
  ],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Admin signs in.", {
          ...
          assert: async () => {
            ...
            witness(ctx, email === required(ctx, "SOFIA_EVAL_DEN_EMAIL"), "The session belongs to the signing-in admin", { email });
            const seeded = await ctx.eval(`(() => localStorage.getItem("sofia.den.authToken") !== null)()`);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The instance is the full Sofia app.", {
          action: async () => {
            ...
            await ctx.waitFor("document.body.innerText.includes('What do you need done?') || document.body.innerText.includes('New session')", {
              timeoutMs: 90_000,
              label: "Sofia app shell in Cloud instance",
            });
          },
          assert: async () => {
            ...
            witness(ctx, marker === "What do you need done?" || marker === "New session", "The Cloud instance renders the Sofia app shell", marker);
          },
          screenshot: {
            name: "frame-4-instance-app",
            claim: "The Cloud instance loads the full Sofia app shell after the same signed-in organization session is restored.",
            rejectText: ["Something went wrong"],
          },
        });
      },
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The mock MCP provably executes through Sofia Connect.", {
          action: async () => {
          ...
          assert: async () => {
            ...
            witness(ctx, state.executeResultText.includes(LOOKUP_OK_TEXT), "The Sofia Connect execution returned the Acme lookup proof", state.executeResultText);
          },
