 ...
 *   2. SOFIA_EVAL_DEN_WEB_URL=http://127.0.0.1:3005 SOFIA_EVAL_WEB_CDP_ADMIN=http://127.0.0.1:9855 pnpm fraimz --flow billing-seats-vs-ai --stack den
        ...
        await withClient(ctx, ADMIN_CDP_URL, async () => {
          await ctx.prove("Billing opens on a single statement that lists Team seats and AI model access as separate line items and sums them into one total", {
            ...
            screenshot: {
              ...
              // "Sofia Users" was the old seats card title; the sidebar still
              // links to Sofia Models, so only the retired card name is rejected.
              requireText: ["Your subscriptions", "Team seats", "AI model access", "Total"],
              rejectText: ["Sofia Users"],
            },
          });
        });
      },
