...
const ORG_ADMIN_EMAIL = process.env.SOFIA_EVAL_DEMO_EMAIL?.trim() || "alex@acme.test";
const ORG_ADMIN_PASSWORD = process.env.SOFIA_EVAL_DEMO_PASSWORD?.trim() || "SofiaDemo123!";

const ORG_FILTER_INPUT = 'input[placeholder="Org name, slug, or id"]';
const NOTION_NAME = "Notion";
const WORKSPACE_PATH = "/tmp/sofia-org-connections-capability";
...
export default {
  ...
  steps: [
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("Sofia Connect shows Acme's Notion org connection as available to connect", {
          voiceover: vo[4],
          action: async () => {
    ...
    },
    {
      name: "Frame 6",
      run: async (ctx) => {
        await ctx.prove("Turning the capability back off removes Notion from Sofia Connect", {
          voiceover: vo[5],
          action: async () => {
            await withClient(ctx, ADMIN_CDP_URL, async () => {
              await signInToDenWebWithoutOrg(ctx, PLATFORM_ADMIN_EMAIL, PLATFORM_ADMIN_PASSWORD);
          ...
          assert: async () => {
            ...
            ctx.assert(!visible, "Notion org connection row still rendered in Sofia Connect after disabling the capability.");
            const usable = await fetchMcpConnections(ctx, "usable");
...
function markEmailVerified(ctx, email) {
  ctx.assert(
    ...
    "Platform-admin provisioning requires a verified email; set SOFIA_EVAL_MARK_VERIFIED_CMD (shell template with {email}).",
  );
...
async function ensureDesktopSignedInAsOrgAdmin(ctx) {
  ...
  const current = await ctx.eval(`(() => ({
    token: (localStorage.getItem('sofia.den.authToken') ?? '').trim(),
    activeOrgId: (localStorage.getItem('sofia.den.activeOrgId') ?? '').trim(),
  }))()`);
  if (current.token) {
    const org = await denApiFetch("/v1/org", {
      method: "GET",
  ...
  ctx.assert(typeof handoff.body?.sofiaUrl === "string", "Desktop handoff response did not include sofiaUrl.");
  ...
  await ctx.fill("#den-signin-link", handoff.body.sofiaUrl);
...
async function completeDesktopCloudOnboardingIfNeeded(ctx) {
  ...
  await ctx.eval(`(() => {
    const button = [...document.querySelectorAll('button')].find((candidate) => (candidate.textContent ?? '').trim() === 'Continue without Sofia Models');
    button?.click();
    return true;
  })()`);
}
...
async function clearDesktopDenSession(ctx) {
  ...
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop control API after sign-out reload" });
}
...
async function remountDesktopConnect(ctx) {
  await closeDesktopDialogs(ctx);
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 120_000, label: "desktop control API after reload" });
  await completeDesktopCloudOnboardingIfNeeded(ctx);
...
async function waitForDesktopConnectOrgConnection(ctx, name) {
  ...
  ctx.assert(false, `${name} org MCP connection did not render in Sofia Connect.`);
}
...
async function waitForDesktopConnectOrgConnectionGone(ctx, name) {
  ...
  ctx.assert(false, `${name} org MCP connection did not disappear from Sofia Connect.`);
}
