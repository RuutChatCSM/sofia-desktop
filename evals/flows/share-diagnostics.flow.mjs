...
async function waitForDiagnosticsDownload(downloadPath) {
  ...
  while (Date.now() < deadline) {
    ...
    const filename = entries.find((entry) => /^sofia-diagnostics-.*\.json$/.test(entry));
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Export diagnostics saves the same kind of bundle as a JSON file", {
          voiceover: vo[2],
          action: async () => {
            ctx.diagnosticsDownloadDir = await mkdtemp(join(tmpdir(), "sofia-diagnostics-"));
        ...
        await ctx.prove("Diagnostics record token presence without exposing token values", {
          ...
          assert: async () => {
            ...
            const secrets = await ctx.eval(`(async () => {
              ...
              const serverInfo = invoke ? await invoke('sofiaServerInfo').catch(() => null) : null;
              const engineInfo = invoke ? await invoke('engineInfo').catch(() => null) : null;
              return {
                serverRunning: Boolean(serverInfo?.running),
                settingsToken: window.localStorage.getItem('sofia.server.token') ?? '',
                settingsHostToken: window.localStorage.getItem('sofia.server.hostToken') ?? '',
            ...
            if (secretValues.length === 0) {
              ctx.assert(bundle.sofiaServer?.settings?.tokenPresent === false, "tokenPresent should be false when no real token values are present");
            }
            await waitForCommandItem(ctx, "Copy diagnostics");
          },
