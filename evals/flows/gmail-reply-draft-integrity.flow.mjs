...
const WORKSPACE_PATH = `/tmp/sofia-gmail-reply-draft-integrity-${RUN_TAG}`;
const GMAIL_THREAD_ID = "thread-q3-launch";
const PROMPT = `Use the Sofia Cloud connection: find the Gmail thread about "Q3 launch" (call search_capabilities, then the Gmail messages capability), then create a reply draft on that thread with subject "Re: Q3 launch" confirming Thursday works, addressed to sarah@acme.test. Do not include quoted history yourself; pass the threadId so Sofia can append it. When done, reply with the draft link the tool returned.`;
...
async function ensureVerifiedUser(ctx, email, name, password) {
  ...
  ctx.assert(MARK_VERIFIED_CMD.length > 0, "Set SOFIA_EVAL_MARK_VERIFIED_CMD to verify eval accounts.");
...
async function connectGoogleWorkspace(ctx) {
  ...
  if (!connected) {
    ...
    await ctx.expectText("Sofia stores this sign-in", { timeoutMs: 15_000 });
...
async function ensureSofiaCloudControl(ctx) {
  await openMcpSettings(ctx);
  ...
  if (!alreadyConnected) {
    ...
    ctx.assert(openedCard, "Could not open the Sofia Cloud Control card.");
...
async function ensureRuntimeCloudMcp(ctx) {
  ...
  while (Date.now() < deadline) {
    runtime = await ctx.eval(`(async () => {
      ...
      const entry = (payload?.items ?? []).find((item) => item.name === 'sofia-cloud');
      return { ok: Boolean(entry?.config?.url?.includes('/mcp/agent') && payload?.engineSync?.status === 'ok') };
    })()`, { awaitPromise: true });
    if (runtime?.ok) break;
    await sleep(1_000);
  }
  ctx.assert(runtime?.ok, `Runtime Sofia Cloud Control MCP never became ready: ${JSON.stringify(runtime)}`);
}
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ensureSofiaCloudControl(ctx);
    ...
    },
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The user asks Sofia to reply to Sarah's Q3 launch thread", {
          voiceover: vo[0],
          // "I'm in Sofia, and Sarah's thread about the Q3 launch has been going back "
          action: async () => {
    ...
    },
    {
      name: "Frame 2",
      run: async (ctx) => {
        await ctx.prove("Sofia reads Gmail before drafting the reply", {
          voiceover: vo[1],
          // "Sofia reads the thread through my connected Google account first, so the "
          action: async () => {},
          assert: async () => {
    ...
    },
    {
      name: "Frame 6",
      run: async (ctx) => {
        await ctx.prove("Sofia relays the Gmail draft link back to the user", {
          voiceover: vo[5],
          // "And Sofia finishes by handing me the link — one click opens the ready-to-"
          action: async () => {},
          assert: async () => {
