 ...
 *
 * The orchestrator supplies SOFIA_EVAL_REPRO_COMMAND. That command runs the
...
const REQUIRED_ENV = ["SOFIA_EVAL_REPRO_COMMAND"];
...
function parseTimeoutMs() {
  const raw = process.env.SOFIA_EVAL_REPRO_TIMEOUT_MS?.trim();
  if (!raw) return DEFAULT_REPRO_TIMEOUT_MS;
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error(`SOFIA_EVAL_REPRO_TIMEOUT_MS must be a positive number of milliseconds (got ${raw}).`);
  }
...
function requiredEnv() {
  ...
  if (snapshot.missing.length > 0) {
    throw new Error(
      ...
      + "Set SOFIA_EVAL_REPRO_COMMAND to a shell command that runs scripts/repro-engine-mcp-evidence.mjs on the target environment and prints the driver's final JSON as the last stdout line.",
    );
  }
  return { command: snapshot.values.SOFIA_EVAL_REPRO_COMMAND, timeoutMs: parseTimeoutMs() };
}
...
function engineSyncFailed(mcpBody) {
  ...
  return own(sync, "status") === "failed"
    ...
    && failures.some((failure) => own(failure, "name") === "sofia-cloud");
}
...
function sofiaCloudConnectedStatus(diagnostics) {
  const statuses = own(diagnostics, "sofiaCloudSyncStatuses");
  if (!Array.isArray(statuses)) return null;
  return statuses.find((status) => own(status, "name") === "sofia-cloud" && own(status, "source") === "config.remote" && own(status, "syncStatus") === "connected") ?? null;
}
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The repro run forced the first Sofia Cloud handshake past the registration window and captured the initial failure evidence", {
          voiceover: vo[0],
          assert: async () => {
            ...
            witness(ctx, engineSyncFailed(seedMcp), "The immediate MCP inventory recorded engineSync failed for sofia-cloud", own(seedMcp, "engineSync"));
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("After the delayed handshake completes, live health shows the managed cloud connection is usable", {
          ...
          assert: async () => {
            ...
            witness(ctx, isHealthConnected(health), "The driver's healed health poll observed a usable or connected Sofia Cloud MCP", health);
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("Agent-context diagnostics no longer report registration_not_connected for a healthy managed cloud MCP", {
          ...
          assert: async () => {
            ...
            const connectedStatus = sofiaCloudConnectedStatus(diagnostics);

            ctx.output("diagnostics-check-and-verdict", JSON.stringify({
              engineMcpSyncCheck: check,
              sofiaCloudSyncStatuses: own(diagnostics, "sofiaCloudSyncStatuses"),
            ...
            witness(ctx, Boolean(connectedStatus), "sofiaCloudSyncStatuses includes config.remote sofia-cloud with syncStatus connected", own(diagnostics, "sofiaCloudSyncStatuses"));
