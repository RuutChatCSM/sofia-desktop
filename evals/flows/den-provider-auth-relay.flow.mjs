 ...
 * supplied by the orchestrator through SOFIA_EVAL_* environment variables.
 */
...
function missingEnvMessage() {
  return `Missing required environment variables for ${FLOW_ID}: ${envSnapshot.missing.join(", ")}. Set SOFIA_EVAL_DEN_API_URL, SOFIA_EVAL_GATEWAY_MCP_URL, and SOFIA_EVAL_GATEWAY_MCP_URL_FOREIGN to the Daytona Den API and mock gateway MCP URLs.`;
}
...
function requiredEnv(ctx) {
  ...
  return {
    denApiUrl: stripTrailingSlashes(envSnapshot.values.SOFIA_EVAL_DEN_API_URL),
    gatewayMcpUrl: stripTrailingSlashes(envSnapshot.values.SOFIA_EVAL_GATEWAY_MCP_URL),
    foreignGatewayMcpUrl: stripTrailingSlashes(envSnapshot.values.SOFIA_EVAL_GATEWAY_MCP_URL_FOREIGN),
  };
}
