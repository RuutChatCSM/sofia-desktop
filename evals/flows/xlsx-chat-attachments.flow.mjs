...
const DOWNLOAD_DIR = "/tmp/sofia-xlsx-attachment-downloads";
...
set -euo pipefail
if [ -f /tmp/sofia-xlsx-attachments-mock.pid ]; then
  kill "$(cat /tmp/sofia-xlsx-attachments-mock.pid)" >/dev/null 2>&1 || true
fi
rm -f /tmp/sofia-xlsx-attachments-mock.log /tmp/sofia-xlsx-attachments-mock.pid
nohup node evals/drivers/office-attachments-mock-provider.mjs --mode xlsx --port ${MOCK_PORT} --workspace ${shellQuote(ctx.workspacePath || "/workspace")} > /tmp/sofia-xlsx-attachments-mock.log 2>&1 < /dev/null &
echo $! > /tmp/sofia-xlsx-attachments-mock.pid
for _ in $(seq 1 80); do
  if curl -sf http://127.0.0.1:${MOCK_PORT}/health >/tmp/sofia-xlsx-attachments-health.json; then
    cat /tmp/sofia-xlsx-attachments-health.json
    exit 0
  fi
  sleep 0.25
done
cat /tmp/sofia-xlsx-attachments-mock.log >&2
exit 1
`).trim();
}
...
if [ -s /tmp/sofia-xlsx-attachments-mock.pid ]; then
  pid="$(cat /tmp/sofia-xlsx-attachments-mock.pid 2>/dev/null || true)"
  if [ -n "$pid" ] && kill -0 "$pid" >/dev/null 2>&1; then
    kill "$pid" >/dev/null 2>&1 || true
  fi
fi
rm -f /tmp/sofia-xlsx-attachments-mock.pid
printf 'mock stopped\n'
`, 30_000).trim();
}
...
async function forceEnglish(ctx) {
  const shouldReload = await ctx.eval(`(() => {
    const current = localStorage.getItem("sofia.language");
    localStorage.setItem("sofia.language", "en");
    return current !== "en" || document.documentElement.getAttribute("lang") !== "en";
  })()`);
  if (!shouldReload) return;
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after language reload" });
}
...
async function appRouteState(ctx) {
  return await ctx.eval(`(() => {
    ...
    const workspaceId = pathSegment(hash, "workspace") || localStorage.getItem("sofia.react.activeWorkspace") || "";
...
async function serverShell(ctx, path, init = {}) {
  const auth = await ctx.eval(`JSON.stringify({ port: localStorage.getItem("sofia.server.port"), token: localStorage.getItem("sofia.server.token") })`);
...
async function configureMockProvider(ctx) {
      ...
      opencode: {
        provider: {
          [PROVIDER_ID]: {
            ...
            options: { baseURL, apiKey: "sk-sofia-xlsx-attachments-eval" },
            models: {
              [MODEL_ID]: {
  ...
  await ctx.eval(`(() => {
    const prefsRaw = localStorage.getItem("sofia.preferences");
    let prefs = {};
    ...
    localStorage.removeItem("sofia.sessionModels.${ctx.workspaceId}");
  })()`);
...
async function createFreshSession(ctx) {
  await ctx.waitFor(`window.__sofiaControl.listActions().some((item) => item.id === "session.create_task" && !item.disabled)`, { timeoutMs: 60_000, label: "session.create_task enabled" });
...
async function dismissSofiaModelsModal(ctx) {
  const result = await ctx.eval(`(() => {
    const dialog = Array.from(document.querySelectorAll('[role="dialog"], [data-slot="dialog-content"], [data-radix-dialog-content]')).find((item) => (item.textContent || "").includes("Sofia Models"));
    if (!dialog) return { dismissed: false };
    const button = Array.from(dialog.querySelectorAll("button")).find((item) => (item.textContent || "").trim().includes("Continue without Sofia Models") || item.getAttribute("aria-label") === "Close");
    if (!button) return { dismissed: false };
    button.click();
    return { dismissed: true };
  })()`);
  if (!result?.dismissed) return;
  await ctx.waitFor(`!Array.from(document.querySelectorAll('[role="dialog"], [data-slot="dialog-content"], [data-radix-dialog-content]')).some((item) => (item.textContent || "").includes("Sofia Models"))`, { timeoutMs: 10_000, label: "Sofia Models modal dismissed" });
}
      ...
      run: async (ctx) => {
        await ctx.prove("The real composer accepts a generic-MIME .xlsx workbook without an unsupported-format warning", {
          ...
          action: async () => {
            ...
            await ctx.eval("location.reload()");
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after provider reload" });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Reloading the session keeps spreadsheet history safe, and a follow-up message succeeds without provider errors", {
          ...
          action: async () => {
            await ctx.eval("location.reload()");
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after session reload" });
