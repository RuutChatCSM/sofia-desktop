...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        const hasSelectedSession = await ctx.eval(`window.__sofiaControl.snapshot().route.includes("/session/")`);
        if (!hasSelectedSession) {
          await ctx.control("session.create_task");
          await ctx.waitFor(`window.__sofiaControl.snapshot().route.includes("/session/")`, {
            timeoutMs: 60_000,
        ...
        const seedReady = async () =>
          ctx.eval(`window.__sofiaControl.listActions().some((a) => a.id === "eval.artifact_tabs.seed_overflow" && !a.disabled)`);
        ...
        for (let attempt = 0; attempt < 2 && !(await seedReady()); attempt += 1) {
          ...
          try {
            await ctx.waitFor(
              `window.__sofiaControl.listActions().some((a) => a.id === "eval.artifact_tabs.seed_overflow" && !a.disabled)`,
