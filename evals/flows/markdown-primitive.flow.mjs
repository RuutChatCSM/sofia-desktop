...
async function activeSessionFromRoute(ctx) {
  return ctx.eval(`(() => {
    const route = String(window.__sofiaControl.snapshot().route || window.location.hash || "");
...
async function ensureSession(ctx) {
  ...
  if (firstSession?.sessionId) {
    ...
    await ctx.waitFor(
      `String(window.__sofiaControl.snapshot().route || "").includes(${JSON.stringify(firstSession.sessionId)})`,
  ...
  const createdSession = await ctx.waitFor(
    `(() => {
      const route = String(window.__sofiaControl.snapshot().route || "");
      const match = route.match(/ses_[A-Za-z0-9]+/);
...
async function ensureActionEnabled(ctx, actionId) {
  await ctx.waitFor(
    `window.__sofiaControl.listActions().some((action) => action.id === ${JSON.stringify(actionId)} && !action.disabled)`,
...
async function mountArtifactPanel(ctx) {
  const ready = await ctx.eval(
    `window.__sofiaControl.listActions().some((action) => action.id === ${JSON.stringify(ARTIFACT_SEED_ACTION)} && !action.disabled)`,
  );
  if (ready) return;
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sofia displays a normal session before the Markdown proof starts", {
          voiceover: vo[0],
          action: async () => {
      ...
      run: async (ctx) => {
        await ctx.prove("The chat transcript renders Markdown headings, emphasis, inline code, code blocks, and links", {
          ...
          assert: async () => {
            ...
            const rendered = await ctx.eval(`(() => {
              ...
              return {
                ...
                hasCodeBlock: Boolean(root.querySelector("[data-sofia-code-block] code")),
                hasLink: Boolean(root.querySelector('a[href="https://sofia.ruut.chat"]')),
                hasCopyButton: Boolean(root.querySelector("[data-sofia-code-copy]")),
                text: root.innerText,
              };
            })()`);
          ...
          screenshot: {
            ...
            requireText: ["Markdown proof heading", "bold proof text", "Sofia link"],
            rejectText: ["Something went wrong"],
          },
        });
      },
      ...
      run: async (ctx) => {
        await ctx.prove("A Markdown artifact preview renders readable formatted content outside the chat", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              `Boolean(document.querySelector('[data-sofia-markdown-preview]'))`,
              { timeoutMs: 30_000, label: "markdown preview root" },
            );
          },
          assert: async () => {
            const rendered = await ctx.eval(`(() => {
              const preview = document.querySelector('[data-sofia-markdown-preview]');
              ...
              const externalAnchor = Array.from(preview.querySelectorAll("a"))
                .find((node) => node.getAttribute("href") === "https://sofia.ruut.chat");
              const shikiCode = Array.from(preview.querySelectorAll('[data-sofia-shiki] code'))
              ...
              return {
                ...
                hasChatCopyButton: Boolean(preview.querySelector("[data-sofia-code-copy]")),
              };
            })()`);
