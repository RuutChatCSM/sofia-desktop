 ...
 *
 * 1. The rich `/mcp` endpoint ("Sofia Cloud Control" as it existed
 ...
 *    desktop app's "Sofia Cloud Control" connection now points here,
 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_API_URL    Den API base (e.g. http://127.0.0.1:8793)
 * - SOFIA_EVAL_DEN_TOKEN      Bearer session token for the demo owner
 */
...
async function denFetch(ctx, path, options = {}) {
  const base = ctx.env.SOFIA_EVAL_DEN_API_URL.trim().replace(/\/+$/, "");
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        if (onWelcome) {
          const wsPath = ctx.env.SOFIA_EVAL_WORKSPACE_PATH.trim();
        ...
        await ctx.waitFor(
          ...
          { timeoutMs: 180_000, label: "sofia.den.mcp.sync marker" },
        );
      },
    },
    {
      name: "Sofia Cloud Control is connected — the unchanged existing surface still works",
      run: async (ctx) => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const minted = await denFetch(ctx, "/v1/mcp/token", {
          method: "POST",
          headers: { authorization: `Bearer ${ctx.env.SOFIA_EVAL_DEN_TOKEN.trim()}` },
          body: JSON.stringify({}),
        });
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Chat-triggered, unprompted: the agent calls search_capabilities then execute_capability because that's all this connection exposes.", {
          action: async () => {
            ...
            await ctx.waitFor(
              "Boolean(window.__sofiaControl?.listActions().find((a) => a.id === 'session.create_task' && !a.disabled))",
            ...
            const pasted = await ctx.eval(`(() => {
              ...
              data.setData('text/plain', ${JSON.stringify(
                "On the Sofia Cloud Control MCP, find and tell me the name of my current organization.",
              )});
