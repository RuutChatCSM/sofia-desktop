export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_SIMULATE_OPEN_EXTERNAL_FAILURE"],
  steps: [
    {
      name: "App booted",
      run: async (ctx) => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor(
          "window.__sofiaControl.execute('auth.status').then((result) => result.result?.status === 'signed_out').catch(() => false)",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(
          typeof signInUrl === "string" && signInUrl.includes("desktopScheme=sofia"),
        ...
        await ctx.screenshot("browser-open-fallback", {
          ...
          voiceover:
            "I click Sign in, and Sofia keeps the exact Cloud URL directly below the button. Even when this machine cannot launch a browser, I can open or copy the link and finish signing in.",
          requireText: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.screenshot("signin-link-copied", {
          claim: "The fallback copy action gives the user a usable Sofia Cloud sign-in link.",
          voiceover:
