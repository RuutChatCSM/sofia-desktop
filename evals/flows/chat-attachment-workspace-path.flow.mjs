      ...
      run: async (ctx) => {
        await ctx.prove("A normal chat attachment appears in the composer before sending", {
          ...
          assert: async () => {
            ...
            const composer = await ctx.eval("window.__sofia?.slice('composer')");
      ...
      run: async (ctx) => {
        await ctx.prove("The uploaded scan path is a real file path for tools such as Docling", {
          ...
          assert: async () => {
            ...
            assertEvidence(ctx, !text.toLowerCase().includes("ocr complete"), "The prompt does not claim Sofia performed native OCR", text);
