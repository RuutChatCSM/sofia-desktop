...
const WORKSPACE_PATH = "/tmp/sofia-connect-tab-beta";
      ...
      run: async (ctx) => {
        await ctx.prove("With the org capability off and zero usable connections, Connect shows the pitch state", {
          ...
          action: async () => {
            ...
            await ctx.waitForText("Connect is the new way Sofia lets you share workflows with your team.", { timeoutMs: 30_000 });
          },
          assert: async () => {
          ...
          screenshot: {
            ...
            requireText: ["Connect is the new way Sofia lets you share workflows with your team.", "Manage in Den web"],
...
async function reloadUntilAlive(ctx, label) {
  ...
  for (let attempt = 0; attempt < 3 && !alive; attempt += 1) {
    ...
    try {
      await ctx.waitFor("Boolean(window.__sofiaControl) && (document.getElementById('root')?.childElementCount ?? 0) > 0", { timeoutMs: 45_000, label: `${label} (attempt ${attempt + 1})` });
      alive = true;
    } catch {
      // next attempt
    }
...
async function signDesktopIntoCloud(ctx) {
  try {
    await ctx.waitFor("Boolean(window.__sofiaControl) && (document.getElementById('root')?.childElementCount ?? 0) > 0", { timeoutMs: 30_000, label: "desktop control API" });
  } catch {
