...
function repairProbeScript(url, caPath, disabled) {
  return `
    ...
    const logs = [];
    const userDataDir = await mkdtemp(path.join(tmpdir(), "sofia-egress-chain-repair-"));
    const parentEnv = ${disabled ? "{ SOFIA_DISABLE_CHAIN_REPAIR: '1' }" : "{}"};
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        let repaired;
        let disabled;
        await ctx.prove("Leaf-only TLS chains fail plainly, product repair succeeds, the kill switch fails again, and Sofia product diagnostics name the chain fault", {
          voiceover: vo[0],
          action: async () => {
          ...
          assert: async () => {
            try {
              ...
              witness(ctx, disabled.status === 0 && disabledJson?.child?.status !== 0, "SOFIA_DISABLE_CHAIN_REPAIR=1 fails again", JSON.stringify(disabledJson));
