...
async function runMysql(ctx, sql) {
  ...
  const args = container
    ? ["exec", container, "mysql", "-uroot", "-ppassword", "sofia_den", "-e", sql]
    : ["-h127.0.0.1", "-uroot", "-ppassword", "sofia_den", "-e", sql];
...
async function prepareSeededMultiOrg(ctx) {
  ...
  if (organization.name.startsWith(ORG_NAME_PREFIX)) {
    const healedName = state.org.slug === "default" ? "Sofia" : "Acme Robotics";
  ...
  await ctx.waitFor(
    `(() => {
      ...
      if (location.pathname !== ${JSON.stringify(ORG_SETTINGS_PATH)}) {
        window.location.href = ${JSON.stringify(`${process.env.SOFIA_EVAL_DEN_WEB_URL?.replace(/\/$/, "") ?? ""}${ORG_SETTINGS_PATH}`)};
      }
      return false;
    })()`,
...
async function openOrgSettingsAsSeededAdmin(ctx) {
  ...
  await ctx.eval("window.sessionStorage.removeItem('sofia:web:pending-org-selection'); true");
...
export default {
  ...
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_MULTI_ORG"],
  steps: [
    {
