 ...
 * - SOFIA_EVAL_DEN_WEB_URL: Den web origin used by the desktop handoff deep link.
 *
 * Optional env:
 * - SOFIA_EVAL_CDP_URL or --cdp-url: CDP endpoint for the admin desktop app.
 * - SOFIA_EVAL_ENTERPRISE_ADMIN_EMAIL: admin email (default admin@example.com).
 * - SOFIA_EVAL_ENTERPRISE_MEMBER_EMAIL: member email used to verify sharing (default member@example.com).
 * - SOFIA_EVAL_ENTERPRISE_ADMIN_WORKSPACE: workspace folder (default /workspace/enterprise-admin).
 * - SOFIA_EVAL_ENTERPRISE_GATEWAY_URL: gateway base URL used only if the transcript asks for JIT login without a full link.
 * - SOFIA_EVAL_ENTERPRISE_ADMIN_GATEWAY_USER: gateway login user override (default admin email).
 * - SOFIA_EVAL_ENTERPRISE_PASSWORD: account password override (default TutorialDemo123!).
 * - SOFIA_EVAL_ENTERPRISE_TASK_TIMEOUT_MS: chat turn timeout in milliseconds.
 * - SOFIA_EVAL_ENTERPRISE_SHARE_TIMEOUT_MS: marketplace/share chat timeout in milliseconds.
 *
 ...
 * SOFIA_EVAL_CDP_URL (or --cdp-url) pointed at that second app instance.
 */
...
const WORKSPACE_ENV = "SOFIA_EVAL_ENTERPRISE_ADMIN_WORKSPACE";
const DEFAULT_WORKSPACE = "/workspace/enterprise-admin";

const PROMPT_INCIDENTS = "Use Sofia Connect capabilities to find the enterprise incident gateway. Search for the right capability, then ask the gateway for my open incidents assigned to me using its enterprise graph query capability. Do not use lookup_incident_records. I need the incident numbers, priorities, and short descriptions.";
...
export default {
  ...
  steps: [
    ...
    {
      name: "Create Admin's fresh workspace with Sofia Connect ready",
      run: async (ctx) => {
        ...
        await waitForSofiaConnectReady(ctx);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Admin's agent uses the enterprise incident gateway and resolves the JIT sign-in path", {
          action: async () => {
            const timeout = timeoutMs(ctx, "SOFIA_EVAL_ENTERPRISE_INCIDENT_TIMEOUT_MS", 300_000);
            ...
            state.latestTranscript = await retryAfterGatewayLoginIfNeeded(
              ...
              { timeout, gatewayUserEnvName: "SOFIA_EVAL_ENTERPRISE_ADMIN_GATEWAY_USER" },
            );
          },
          assert: async () => {
        ...
        await ctx.prove("Admin turns the learned gateway pattern into an org skill", {
          action: async () => {
            state.latestTranscript = await sendPromptAndWait(ctx, PROMPT_CREATE_SKILL, {
              timeout: timeoutMs(ctx, "SOFIA_EVAL_ENTERPRISE_SKILL_TIMEOUT_MS", 300_000),
            });
          },
          assert: async () => {
        ...
        await ctx.prove("Admin assigns the org skill to the member through the organization marketplace", {
          action: async () => {
            state.latestTranscript = await sendPromptAndWait(ctx, shareSkillPrompt(ctx), {
              timeout: timeoutMs(ctx, "SOFIA_EVAL_ENTERPRISE_SHARE_TIMEOUT_MS", 420_000),
            });
          },
          assert: async () => {
...
function adminEmail(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_ADMIN_EMAIL") || DEFAULT_ADMIN_EMAIL;
}

function memberEmail(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_MEMBER_EMAIL") || DEFAULT_MEMBER_EMAIL;
}
