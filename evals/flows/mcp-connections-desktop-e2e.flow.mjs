 ...
 * happens in the Sofia DESKTOP APP, from the member's point of view:
 *
 ...
 *
 *   1. Jordan signs the DESKTOP APP into Sofia Cloud (real handoff
 *      grant exchange against the local Den).
 *   2. The app auto-configures "Sofia Cloud Control" — the /mcp/agent
 ...
 * - den-api at SOFIA_EVAL_DEN_API_URL with the seeded demo org.
...
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? DEN_API_URL).trim().replace(/\/+$/, "");
...
const WORKSPACE_PATH = "/tmp/sofia-mcp-desktop-e2e";
...
export default {
  ...
  steps: [
    ...
    {
      name: "Member signs the desktop app into Sofia Cloud (real handoff grant)",
      run: async (ctx) => {
        const alreadySignedIn = await ctx.eval("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())");
        if (!alreadySignedIn) {
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        // Dismiss the Sofia Models upsell if it appears.
        await ctx.eval(`(() => {
    ...
    {
      name: "Sofia Cloud Control auto-configures with the member's token",
      run: async (ctx) => {
        ...
        await ctx.waitFor(
          "Boolean(localStorage.getItem('sofia.den.mcp.sync'))",
          { timeoutMs: 120_000, label: "sofia.den.mcp.sync marker" },
        );
        ctx.log(`marker: ${await ctx.eval("localStorage.getItem('sofia.den.mcp.sync')")}`);
        await ctx.prove("Sofia Cloud Control is configured in the member's app", {
          assert: async () => {
            await ctx.expectText("Sofia Cloud Control", { timeoutMs: 30_000 });
          },
          screenshot: {
            name: "desktop-cloud-control-configured",
            claim: "The member's desktop app auto-configured the Sofia Cloud Control connection after cloud sign-in.",
            requireText: ["Sofia Cloud Control"],
