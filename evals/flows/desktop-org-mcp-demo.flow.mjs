 ...
 * - The connection appears in Sofia Connect as an org MCP item that needs
 *   Jordan's sign-in.
 * - Jordan clicks through Connect, the OS browser completes a real OAuth
 *   round trip, and the item becomes Ready in Connect.
 * - A real chat turn executes the external MCP tool through Sofia Cloud
 *   Control; the mock server log is the external witness.
 */

import { execSync } from "node:child_process";

const DEN_API_URL = (process.env.SOFIA_EVAL_DEN_API_URL ?? "").trim().replace(/\/+$/, "");
const DEN_WEB_URL = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? DEN_API_URL.replace("127.0.0.1", "localhost")).trim().replace(/\/+$/, "");
...
const MEMBER_EMAIL = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "jordan.demo@acme.test";
const MEMBER_PASSWORD = process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || "SofiaDemo123!";
...
const WORKSPACE_PATH = `/tmp/sofia-desktop-org-mcp-demo-${RUN_TAG}`;
...
async function ensureMember(ctx) {
  ...
  ctx.assert(MARK_VERIFIED_CMD.length > 0, "Set SOFIA_EVAL_MARK_VERIFIED_CMD to verify the member's email.");
...
async function ensureWorkspace(ctx) {
  ...
  while (Date.now() < deadline) {
    const state = await ctx.eval(`(() => {
      ...
      const hasCreateAction = !hasOnboardingStep && window.__sofiaControl?.listActions?.().find((a) => a.id === 'workspace.create')?.disabled === false;
  ...
  await ctx.eval(`(() => {
    const btn = [...document.querySelectorAll('button')].find((el) => el.textContent.trim() === 'Continue without Sofia Models');
    btn?.click();
    return true;
  })()`, { awaitPromise: true });
}
...
async function readRuntimeCloudControlMcp(ctx) {
  ...
  return ctx.eval(`(async () => {
    ...
    const entry = items.find((item) => item.name === 'sofia-cloud');
...
async function waitForRuntimeCloudControlMcp(ctx) {
  ...
  ctx.assert(false, `Runtime Sofia Cloud Control MCP config never became ready: ${JSON.stringify(last)}`);
}

async function createFreshEvalWorkspace(ctx) {
  await ensureWorkspace(ctx);
  await ctx.waitFor(
    "Boolean(localStorage.getItem('sofia.server.port') && localStorage.getItem('sofia.server.token') && localStorage.getItem('sofia.server.hostToken'))",
    { timeoutMs: 30_000, label: "Sofia server auth for workspace setup" },
  );
  let created = null;
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    created = await ctx.eval(`(async () => {
      try {
        ...
        const response = await fetch(base + '/workspaces/local', {
          method: 'POST',
          headers,
          body: JSON.stringify({ folderPath: ${JSON.stringify(WORKSPACE_PATH)}, name: 'sofia-desktop-org-mcp-demo', preset: 'starter' }),
        });
        const text = await response.text();
        let payload = null;
        try { payload = JSON.parse(text); } catch {}
...
export default {
  ...
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL"],
  steps: [
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 120_000 });
        await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__?.invokeDesktop)", { timeoutMs: 30_000, label: "desktop bridge" });
        ...
        await ctx.eval(`(() => {
          localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(DEN_API_URL)});
          localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(DEN_API_URL)});
          const prefs = JSON.parse(localStorage.getItem('sofia.preferences') || '{}');
          localStorage.setItem('sofia.preferences', JSON.stringify({ ...prefs, selectedAgent: 'sofia' }));
          return true;
        })()`);
        await ctx.eval("location.reload()");
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after bootstrap reload" });
        ...
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 45_000, label: "persisted den auth token" });
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.activeOrgId') ?? '').trim())", { timeoutMs: 60_000, label: "active org resolved" });
        await createFreshEvalWorkspace(ctx);
      },
    },
    {
      name: "Jordan discovers the org MCP connection in Sofia Connect",
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan sees the org MCP connection in Sofia Connect, with a connect action", {
          voiceover: "Jordan opens Sofia Connect and sees the org-shared MCP connection under Needs your sign-in, clearly marked as something they can connect with their own account.",
          assert: async () => {
          ...
          screenshot: {
            ...
            claim: "Sofia Connect shows the org-published MCP connection with a 'Connect your account' action.",
    ...
    {
      name: "The item becomes ready in Sofia Connect",
      run: async (ctx) => {
        await openConnectSettings(ctx);
        await waitForConnectReadyRow(ctx, CONNECTION_NAME);
        await ctx.prove("After the browser sign-in completes, the same org MCP item is ready in Sofia Connect", {
          voiceover: "The OAuth callback completed in the browser, and the desktop did not need a manual reload. Jordan's row is now ready in Sofia Connect as a connected MCP integration.",
          assert: async () => {
            await ctx.waitFor(
              `(() => {
                const row = [...document.querySelectorAll('[data-testid="connect-organization-row"]')]
          ...
          screenshot: {
            ...
            claim: "The org MCP connection is marked Ready in Sofia Connect after the browser OAuth round trip.",
    ...
    {
      name: "Sofia Cloud Control is ready",
      run: async (ctx) => {
        await openMcpSettings(ctx);
        await revealHidden(ctx);
        await ctx.expectText("Sofia Cloud Control", { timeoutMs: 30_000 });

        const alreadyConnected = await ctx.eval(`(() => {
          const card = [...document.querySelectorAll('button')].find((el) => el.textContent.includes('Sofia Cloud Control'));
        ...
        if (!alreadyConnected) {
          ...
          ctx.assert(opened, "Could not open the Sofia Cloud Control card.");
          ...
          ctx.assert(clicked, "Could not click Connect for Sofia Cloud Control.");
        }
        ...
        await ctx.waitFor(
          ...
          { timeoutMs: 60_000, label: "Sofia Cloud Control connected card" },
        );
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan's agent can immediately execute the newly connected org MCP tool", {
          voiceover: "The connection is not just a settings card. In a real desktop chat, the agent discovers the organization's MCP capability through Sofia Cloud Control and executes it with Jordan's own connected account.",
          action: async () => {
            await ctx.waitFor(
              "window.__sofiaControl?.listActions?.().find((a) => a.id === 'session.create_task')?.disabled === false",
            ...
            const prompt = `Use the Sofia Cloud Control connection: call search_capabilities with query "echo", then call execute_capability with the exact match name and body {"text":"${ECHO_TEXT}"}. Reply with the exact text the tool returned.`;
            await ctx.control("composer.set_text", { text: prompt });
            await ctx.waitFor(
              "window.__sofiaControl?.listActions?.().find((a) => a.id === 'composer.send')?.disabled === false",
          ...
          screenshot: {
            ...
            claim: "A real desktop chat executed the org MCP tool through Sofia Cloud Control and showed the exact result.",
