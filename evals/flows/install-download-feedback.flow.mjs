...
async function openInstallPage(ctx) {
  ...
  await ctx.waitFor(`document.body.innerText.includes('Download Sofia for ${ORG_NAME}')`, {
    timeoutMs: 45_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The organization install page presents a centered, focused download card", {
          ...
          screenshot: { name: "frame-1-centered-install-card", sandboxCapture: true, textTargetUrlIncludes: "/install?token=", requireText: [`Download Sofia for ${ORG_NAME}`] },
        });
      },
    },
    {
