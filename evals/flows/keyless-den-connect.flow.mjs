...
const DEN_API_URL = clean(process.env.SOFIA_EVAL_DEN_API_URL);
const DEN_WEB_URL = clean(process.env.SOFIA_EVAL_DEN_WEB_URL);
const WEB_CDP_URL = clean(process.env.SOFIA_EVAL_WEB_CDP_ADMIN);
...
const BRAND_LOGO_URL = `${DEN_WEB_URL}/sofia-logo-transparent.svg`;
      ...
      run: async (ctx) => {
        ...
        await withWeb(ctx, async () => ctx.prove("The Den organization download opens one explicit three-step setup", {
          ...
          assert: async () => {
            ...
            await ctx.expectNoText("Sofia Enterprise");
          },
          screenshot: {
            name: "keyless-three-step-guide",
            requireText: ["Download the Sofia installer", "Open the installer and paste this link:", "Sign in", "Acme Robotics"],
            rejectText: ["Sofia Enterprise"],
          },
        }));
      },
    },
    {
      ...
      run: async (ctx) => withWeb(ctx, async () => ctx.prove("Download redirects immediately to the standard release without a custom ZIP", {
        ...
        action: async () => {
          ...
          witness(ctx, location.includes("github.com/RuutChatCSM/sofia-desktop/releases/download/"), "The redirect targets the configured standard GitHub release", location);
        ...
        screenshot: {
          ...
          requireText: ["Download the Sofia installer", "Open the installer and paste this link:", "keep this page open"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await withWeb(ctx, async () => {
          ...
          witness(ctx, connect.protocol === "sofia:" && Boolean(connect.searchParams.get("code")) && !connect.searchParams.has("token"), "The default handoff is a keyless short-lived exchange", state.connectUrl.replace(connect.searchParams.get("code") ?? "", "<redacted>"));
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Confirmation applies the organization wordmark and opens normal sign-in", {
          ...
          assert: async () => {
            ...
            const persisted = await ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('getDesktopBootstrapConfig')", { awaitPromise: true });
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("A reused or altered exchange is refused and leaves desktop configuration untouched", {
          voiceover: vo[4],
          action: async () => {
            const before = await ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('getDesktopBootstrapConfig')", { awaitPromise: true });
            ...
            const after = await ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('getDesktopBootstrapConfig')", { awaitPromise: true });
      ...
      run: async (ctx) => {
        try {
          await withWeb(ctx, async () => ctx.prove("Optional signed handoffs use the same standard installer experience", {
            voiceover: vo[5],
            action: async () => {
              const defaultRender = spawnSync("helm", ["template", "sofia-ee", "packaging/helm/sofia-ee"], {
                cwd: REPO_ROOT,
                encoding: "utf8",
              });
              const signedRender = spawnSync("helm", [
                "template", "sofia-ee", "packaging/helm/sofia-ee",
            ...
            screenshot: {
              ...
              requireText: ["Download the Sofia installer", "Open the installer and paste this link:", "The installer only continues with a valid link"],
              rejectText: ["custom installer", "Sofia Enterprise"],
            },
          }));
        } finally {
          await restoreBrand();
