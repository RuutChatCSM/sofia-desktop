...
function connectionPrefix(ctx) {
  const stamp = ctx.env.SOFIA_EVAL_RUNSTAMP?.trim() || "run";
...
function mockBase(ctx) {
  ...
  return (fixtureUrl || ctx.env.MOCK_OAUTH_MCP_URL?.trim() || ctx.env.SOFIA_EVAL_MOCK_URL?.trim() || "http://127.0.0.1:3978").replace(/\/+$/, "");
}

function reconnectMcpUrl(ctx) {
  const stamp = cleanStamp(ctx.env.SOFIA_EVAL_RUNSTAMP?.trim() || new Date().toISOString());
        ...
        await ctx.on(alexWeb, async () => {
          await ctx.prove("A no-auth mock MCP server is saved org-wide and shown as Connected", {
            ...
            assert: async () => {
              ...
              if (ctx.env.SOFIA_EVAL_MCP_RELIABILITY_STOP_AFTER_CONNECT?.trim()) {
