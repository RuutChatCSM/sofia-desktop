 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_API_URL   Den API base, e.g. https://api.example.com
 * - SOFIA_EVAL_DEN_TOKEN     Bearer session token for a Den account
 *
 * Optional env:
 * - SOFIA_EVAL_PROJECT_DIR   Existing sandbox folder for the workspace
 *                               (default /workspace/atlas-billing). The flow
 *                               assumes this folder already exists.
 *
 * How to run:
 * SOFIA_EVAL_DEN_API_URL=... SOFIA_EVAL_DEN_TOKEN=... pnpm fraimz --flow analytics-project-dimensions --cdp-url <electron-cdp>
 */
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        if (needsReload) {
          await sleep(3_000);
          await ctx.waitFor("Boolean(window.__sofiaControl) && !location.hash.startsWith('#/welcome')", {
            timeoutMs: 120_000,
      ...
      run: async (ctx) => {
        await ctx.prove("Desktop cloud sign-in connects through the paste-code handoff", {
          ...
          assert: async () => {
            ...
            const token = await ctx.eval("localStorage.getItem('sofia.den.authToken') ?? ''");
            recordAssertion(
              ctx,
    ...
    },
    {
      ...
      run: async (ctx) => {
        const projectDir = optionalEnv(ctx, "SOFIA_EVAL_PROJECT_DIR", DEFAULT_PROJECT_DIR);
        await ctx.prove("The first task from the project workspace is tagged for telemetry", {
          ...
          action: async () => {
            ...
            await ctx.eval(`(() => {
              ...
              const key = "sofia.react.workspaceProjectDimension";
              let map = {};
              try {
            ...
            await ctx.waitFor(
              `window.__sofiaControl.listActions().some((action) => action.id === "composer.set_text")`,
              { timeoutMs: 60_000, label: "composer actions registered" },
            );
            await ctx.control("composer.set_text", { text: "Reply with exactly: ATLAS-OK" });
            await ctx.waitFor(
              `window.__sofiaControl.listActions().some((action) => action.id === "composer.send" && !action.disabled)`,
          ...
          assert: async () => {
            const witness = await ctx.eval(`(() => {
              const raw = localStorage.getItem("sofia.react.workspaceProjectDimension");
              let parsed = null;
              try {
