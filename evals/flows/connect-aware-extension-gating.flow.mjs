 ...
 *
 * Protagonist: the Sofia server surface the agent actually consumes
...
/**
 * In-page request against the embedded Sofia server using the app's own
...
export default {
  id: FLOW_ID,
  title: "Connect flag gates dead legacy Google Workspace actions and redirects the agent to Sofia Cloud",
  kind: "internal",
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "window.__sofiaControl" });
        await ctx.waitFor('Boolean(localStorage.getItem("sofia.server.port")) && Boolean(localStorage.getItem("sofia.server.token"))', {
          timeoutMs: 60_000,
      ...
      run: async (ctx) => {
        await ctx.prove("PUT /experimental/connect/state {connectEnabled:true} persists and reports the device snapshot", {
          ...
          assert: async () => {
            ...
            ctx.assert(typeof state.payload?.cloudMcpPresent === "boolean", "snapshot reports whether the sofia-cloud MCP is present");
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A gated call answers use_sofia_cloud with Settings > Connect guidance", {
          voiceover: vo[3],
          assert: async () => {
            ...
            ctx.assert(call.payload?.error === "use_sofia_cloud", `gated call error is use_sofia_cloud (got ${call.payload?.error})`);
