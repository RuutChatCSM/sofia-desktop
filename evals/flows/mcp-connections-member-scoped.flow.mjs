 ...
 * - den-api at SOFIA_EVAL_DEN_API_URL, den-web at SOFIA_EVAL_DEN_WEB_URL.
 * - Mock OAuth+MCP server reachable at MOCK_OAUTH_MCP_URL from den-api.
 * - Admin: SOFIA_EVAL_DEMO_EMAIL/PASSWORD (default seeded alex@acme.test).
 * - Member: SOFIA_EVAL_MEMBER_EMAIL/PASSWORD (default
 *   jordan.demo@acme.test / SofiaDemo123!). If the member can't sign in,
 ...
 *   SOFIA_EVAL_MARK_VERIFIED_CMD — a shell template with {email}, e.g.
      ...
      run: async (ctx) => {
        ...
        if (!state.memberSession) {
          ...
          ctx.assert(
            ...
            "Member email must be verified to accept the invitation; set SOFIA_EVAL_MARK_VERIFIED_CMD (shell template with {email}).",
          );
