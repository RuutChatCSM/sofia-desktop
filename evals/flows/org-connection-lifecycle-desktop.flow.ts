...
const MOCK_PORT = Number(process.env.SOFIA_EVAL_LIFECYCLE_MOCK_PORT || 3979);
...
const MOCK_EXTERNAL_URL = process.env.SOFIA_EVAL_LIFECYCLE_MOCK_PUBLIC_URL?.trim().replace(/\/+$/, "") ?? "";
...
const WORKSPACE_PATH = `/tmp/sofia-org-connection-lifecycle-${RUN_TAG}`;
...
async function waitForMockAuthorizeRequest(ctx: FlowContext, clickedAt: string): Promise<void> {
  ...
  ctx.assert(
    ...
    `Authorize redirect_uri was not an Sofia MCP connection callback: ${redirectUri}`,
  );
}
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          ...
          localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(DEN_WEB_URL)});
          let prefs = {};
        ...
        await ctx.waitFor(
          "Boolean(window.__sofiaControl?.listActions?.().some((a) => a.id === 'auth.exchange-grant'))",
      ...
      run: async (ctx) => {
        await ctx.prove("The connected detail page now offers both Reconnect and Disconnect", {
          ...
          screenshot: {
            ...
            sandboxCapture: Boolean(process.env.SOFIA_EVAL_DAYTONA_SANDBOX?.trim()),
          },
        });
      },
    },
