 ...
 *   1. Signs in to Sofia Cloud via desktop handoff on App B.
 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_API_URL  local Den API (e.g. http://127.0.0.1:8790)
 * Prereqs: member riley@acme.test / SofiaDemo123! exists in the org.
 */
...
async function denFetch(ctx, path, init = {}) {
  ...
  const origin = ctx.env.SOFIA_EVAL_DEN_ORIGIN?.trim() || base.replace("127.0.0.1", "localhost");
...
const serverCallExpr = (pathTemplate) => `(async () => {
  ...
  const active = localStorage.getItem("sofia.react.activeWorkspace");
...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("Member (Riley) is signed in on her own app instance", {
          action: async () => {
            ...
            await ctx.waitFor(
              "window.__sofiaControl.execute('auth.status').then(r => r.result?.status === 'signed_in').catch(() => false)",
    ...
    },
    {
      ...
      run: async (ctx) => {
        const wsPath = join(homedir(), ".sofia", "two-electron-demo", "eval-workspace-b");
      ...
      run: async (ctx) => {
        await ctx.prove("Plugin installs through the real marketplace UI", {
          ...
          assert: async () => {
            ...
            // member's workspace (polled via the Sofia server API).
