...
const INSTALLED_DESKTOP_DIR = "C:\\Users\\Administrator\\AppData\\Local\\Programs\\@sofiadesktop";
const BRANCH_LAUNCHER = "C:\\ow\\desktop-upgrade-url\\launch-sofia.cmd";
const FIREWALL_RULE = "Sofia Upgrade URL Airgap Eval";

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function sandboxId(ctx) {
  return (ctx.env.SOFIA_EVAL_DAYTONA_SANDBOX_ID || ctx.env.SOFIA_EVAL_DAYTONA_SANDBOX).trim();
}

function windowsAppPath(ctx) {
  return ctx.env.SOFIA_EVAL_WINDOWS_APP_PATH.trim();
}

function windowsInstallerPath(ctx) {
  return ctx.env.SOFIA_EVAL_WINDOWS_INSTALLER_PATH.trim();
}

function standardInstallerPath(ctx) {
  return ctx.env.SOFIA_EVAL_WINDOWS_STANDARD_INSTALLER_PATH.trim();
}
...
async function stopSofia(ctx) {
  await windowsExec(ctx, "stop Sofia", `
Get-Process -Name Sofia -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Milliseconds 800
Write-Output 'Sofia stopped'
`);
}
...
`);
  ...
  await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__)", {
    timeoutMs: 60_000,
...
async function showBootstrapDebug(ctx) {
  await ctx.eval(`localStorage.setItem("sofia.developerMode", "1")`);
...
Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowTitle -match '^Sofia Setup' } | Stop-Process -Force
...
Copy-Item -LiteralPath '${standardInstaller}' -Destination (Join-Path $bundle 'sofia-win-x64-0.17.20.exe') -Force
...
export default {
  ...
  requiredEnv: [
    ...
    "SOFIA_EVAL_WINDOWS_APP_PATH",
    "SOFIA_EVAL_WINDOWS_INSTALLER_PATH",
    "SOFIA_EVAL_WINDOWS_STANDARD_INSTALLER_PATH",
  ],
  steps: [
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The installed Windows desktop is configured for Example Manufacturing", {
          ...
          assert: async () => {
            const runtime = await ctx.eval("window.__SOFIA_ELECTRON__?.system?.getArchitectureInfo?.()", { awaitPromise: true });
...
`);
            ...
            installerTitles = await waitForWindow(ctx, /Sofia Setup/i, true, 30_000);
            await sleep(500);
          },
          assert: async () => {
...
if (Test-Path -LiteralPath '${INSTALLED_DESKTOP_DIR}\\Sofia.exe') { Write-Output 'installed' } else { Write-Output 'missing' }
`);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A full app relaunch keeps the organization bootstrap", {
          ...
          action: async () => {
            try {
              await ctx.eval("window.__SOFIA_ELECTRON__.shell.relaunch()", { awaitPromise: true });
            } catch (error) {
              ctx.log(`Expected relaunch disconnect: ${error instanceof Error ? error.message : String(error)}`);
            }
            await sleep(1_000);
            await ctx.reconnect({ timeoutMs: 120_000 });
            await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__)", { timeoutMs: 60_000, label: "Electron bridge after restart" });
...
`);
            ...
            ctx.recordEvidence({ type: "assertion", status: "passed", assertion: "Windows Firewall blocks outbound traffic for the tested Sofia executable", actual: firewall.trim() });
            await ctx.expectText("Bootstrap config");
            await ctx.expectText(ORG_URL);
