...
function mockUrl(ctx, path) {
  return new URL(path, ctx.env.SOFIA_EVAL_POSTHOG_MOCK_URL).toString();
}
...
export default {
  ...
  // The landing website has no theme system or __sofiaControl API; skip the
  // desktop-app light-mode bootstrap instead of waiting for it to time out.
  preserveTheme: true,
  requiredEnv: ["SOFIA_EVAL_LANDING_URL", "SOFIA_EVAL_POSTHOG_MOCK_URL"],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Hero prompt copy captures the analytics event.", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              `Boolean(document.querySelector(${JSON.stringify(COPY_BUTTON_SELECTOR)})) && document.body.innerText.includes("Install Sofia on my computer")`,
          ...
          assert: async () => {
            ...
            const pageScan = await ctx.eval(`(() => {
              ...
              return {
                heroPromptVisible: bodyText.includes("Install Sofia on my computer") && bodyText.includes("start.md?v=hero"),
                headerVisible: bodyText.includes("Paste this prompt — it installs Sofia for you"),
                executionPreviewVisible: bodyText.includes("now paste it into Claude Code") && bodyText.includes("Installs Sofia") && bodyText.includes("Opens ready to run"),
        ...
        await ctx.prove("Agent fetch of start.md is captured server-side.", {
          ...
          action: async () => {
            ...
            guideFetch = {
              ...
              startsWithGuide: body.startsWith("# Sofia Start"),
              offersMcpConnection: body.includes("Want me to connect the Sofia MCP to this agent"),
              includesCodexMcpSetup: body.includes("codex mcp add sofia --url https://sofia-api.ruut.chat/mcp/agent")
                && body.includes("codex mcp login sofia"),
            };
            ...
            await ctx.waitFor(
              `document.body.innerText.includes(${JSON.stringify("Sofia Start")})`,
          ...
          assert: async () => {
            ...
            recordAssertion(
              ctx,
              "Agent fetch returns HTTP 200 and the Sofia start guide markdown",
              guideFetch?.status === 200 && guideFetch.startsWithGuide === true,
              guideFetch,
            );
            recordAssertion(
              ctx,
              "The start guide offers to connect the Sofia MCP and includes the Codex setup path",
          ...
          screenshot: {
            name: "start-md-guide",
            requireText: ["Sofia Start", "Want me to connect the Sofia MCP to this agent"],
          },
        });
      },
    },
