      ...
      run: async (ctx) => {
        await ctx.prove("ScreenshotOptions exposes pretty without changing the screenshot pipeline contract", {
          ...
          action: async () => {
            ...
            await ctx.waitFor("window.__sofiaControl.listActions().some((action) => action.id === 'route.settings.general')", {
      ...
      run: async (ctx) => {
        await ctx.prove("The same screenshot primitive can write a pretty frame from the current app state", {
          ...
          action: async () => {
            ...
            await ctx.waitFor("window.__sofiaControl.listActions().some((action) => action.id === 'route.settings.appearance')", {
