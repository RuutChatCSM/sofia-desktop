...
function bootstrapPath(ctx) {
  const rawPath = ctx.env.SOFIA_DESKTOP_BOOTSTRAP_PATH?.trim();
  ctx.assert(Boolean(rawPath), "SOFIA_DESKTOP_BOOTSTRAP_PATH must be set so this flow never touches the real user config.");

  const resolvedPath = resolve(rawPath);
  const home = os.homedir();
  const unsafePrefixes = [
    resolve(home, ".config", "sofia"),
...
async function reloadCleanSettingsShell(ctx) {
  await ctx.eval(`(() => {
    localStorage.removeItem("sofia.developerMode");
    localStorage.removeItem("sofia.den.baseUrl");
    localStorage.removeItem("sofia.den.apiBaseUrl");
      ...
      run: async (ctx) => {
        await ctx.prove("Clearing the desktop server configuration returns the app to the default control plane", {
          ...
          assert: async () => {
            ...
            // With SOFIA_DESKTOP_BOOTSTRAP_PATH set, the desktop code disables the legacy path
            // instead of resolving the real user's ~/.config path. Unit coverage asserts legacy removal.
            ctx.log("Bootstrap file witness: isolated canonical file removed; legacy path is disabled under SOFIA_DESKTOP_BOOTSTRAP_PATH.");
          },
          screenshot: {
