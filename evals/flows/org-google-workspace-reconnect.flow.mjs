...
async function approveMockConsent(ctx) {
  const clicked = await ctx.eval(`(() => {
    const button = [...document.querySelectorAll('button')].find((entry) => (entry.textContent ?? '').trim() === 'Approve Sofia');
    button?.click();
    return Boolean(button);
  })()`);
  ctx.assert(clicked, "Mock OAuth consent page did not show an Approve Sofia button.");
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Clicking Reconnect opens consent for the newly requested Google scopes", {
          ...
          action: async () => {
            await ctx.eval(`(() => {
              window.__sofiaEvalLastOpen = null;
              window.open = (url) => { window.__sofiaEvalLastOpen = String(url); return null; };
              return true;
            })()`);
            ...
            await ctx.waitFor("typeof window.__sofiaEvalLastOpen === 'string'", { timeoutMs: 30_000, label: "Reconnect authorize URL opened" });
            state.reconnectAuthorizeUrl = await ctx.eval("window.__sofiaEvalLastOpen");
          ...
          screenshot: {
            ...
            requireText: ["Mock MCP OAuth", "Requested scopes", "https://www.googleapis.com/auth/gmail.readonly", "https://www.googleapis.com/auth/calendar.events", "Approve Sofia"],
