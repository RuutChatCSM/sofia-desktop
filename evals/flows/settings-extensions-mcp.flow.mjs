 ...
 * cloud sign-in. Inactive built-in Sofia MCPs are hidden by default and
  ...
  steps: [
    {
      name: "App booted",
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000 });
      },
    },
    {
    ...
    {
      name: "Default view keeps directory apps discoverable and hides inactive built-in Sofia MCPs",
      run: async (ctx) => {
        ...
        await ctx.screenshot("mcp-view-default-hidden", {
          claim: "Extensions shows public directory apps while inactive built-in Sofia MCPs are hidden by default.",
          ...
          rejectText: ["Sofia UI Control", "Something went wrong"],
          hashIncludes: "/settings/extensions/mcp",
        });
      },
    },
    {
      name: "Show hidden reveals built-in Sofia MCPs",
      run: async (ctx) => {
        await revealHidden(ctx);
        await ctx.expectText("Sofia UI Control", { timeoutMs: 15_000 });
        await ctx.screenshot("mcp-view-built-ins-revealed", {
          claim: "Show hidden reveals inactive built-in Sofia MCP entries.",
          voiceover: "Choosing Show hidden brings back Sofia UI Control for anyone who wants to manage it.",
          requireText: ["Sofia UI Control"],
