...
const FLOW_ID = "sofia-connect-status";
const vo = await loadVoiceoverParagraphs(FLOW_ID);
const DEMO_EMAIL = "alex@acme.test";
const DEMO_PASSWORD = "SofiaDemo123!";
const PROMPT = "Say hello without using connected services.";
const SAVED_TOKEN_KEY = "sofia.eval.sofiaConnectStatus.sessionToken";
...
async function authDelay(ctx, action) {
  const baseUrl = ctx.env.SOFIA_EVAL_DEN_API_URL.replace(/\/+$/, "");
  const response = await fetch(`${baseUrl}/__sofia_eval/auth-delay${action ? `?action=${action}` : ""}`, {
...
async function dismissModelUpsells(ctx) {
  if (await ctx.hasText("Use Sofia Models without API keys")) {
    await ctx.clickText("Continue without Sofia Models", { selector: "button", timeoutMs: 15_000 });
  }
...
async function ensureLocalCloudSession(ctx) {
  const denBaseUrl = ctx.env.SOFIA_EVAL_DEN_API_URL.trim();
  const denOrigin = ctx.env.SOFIA_EVAL_DEN_ORIGIN?.trim() || denBaseUrl;
  let token = ctx.env.SOFIA_EVAL_DEN_TOKEN?.trim() ?? "";
  if (!token) {
  ...
  await ctx.eval(`(() => {
    const shellConfig = JSON.parse(localStorage.getItem("sofia.shell-config") || "{}");
    localStorage.setItem("sofia.shell-config", JSON.stringify({ ...shellConfig, statusBar: true }));
    localStorage.setItem("sofia.den.baseUrl", ${quoted(denBaseUrl)});
    localStorage.setItem("sofia.den.authToken", ${quoted(token)});
    localStorage.setItem("sofia.den.activeOrgId", ${quoted(org.id)});
    ${org.slug ? `localStorage.setItem("sofia.den.activeOrgSlug", ${quoted(org.slug)});` : ""}
    ${org.name ? `localStorage.setItem("sofia.den.activeOrgName", ${quoted(org.name)});` : ""}
    return true;
  })()`);
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after Cloud session restore" });
...
async function createFreshTask(ctx) {
  ...
  while (Date.now() < deadline) {
    const result = await ctx.eval("window.__sofiaControl.execute('session.create_task', null)", { awaitPromise: true });
...
async function transcriptCount(ctx) {
  const result = await ctx.eval("window.__sofiaControl.execute('session.read_transcript', { count: 30 })", { awaitPromise: true });
...
const CONNECT_ROW = '[data-testid="sofia-connect-status"]';
...
async function waitForConnectState(ctx, expected, timeoutMs = 90_000) {
  ...
  throw new Error(`Sofia Connect did not reach "${expected}" after background retries.`);
}
...
async function beginSavedSessionRestore(ctx) {
  ...
  await ctx.eval("location.reload()");
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API during saved-session restore" });
  await dismissModelUpsells(ctx);
...
async function installHealthFailureProbe(ctx) {
  const result = await ctx.eval(`(() => {
    ...
    const existing = window.__sofiaConnectStatusProbe;
    ...
    const failedHealth = (health) => ({
      ...
      firstFailure: {
        code: "sofia_connect_eval_failure",
        stage: "engine_delivery",
        retryable: true,
        recommendedAction: "Run diagnostics",
        message: "Sofia Connect could not verify connected service tools.",
      },
    });
    window.fetch = async (input, init) => {
      const response = await originalFetch(input, init);
      const url = typeof input === "string" ? input : input?.url || String(input);
      if (!url.includes("/mcp/sofia-cloud/")) return response;
      let health;
    ...
    bridge.invokeDesktop = async (command, ...args) => {
      ...
      if (command !== "__fetch" || !url.includes("/mcp/sofia-cloud/") || !response?.body) return response;
      let health;
    ...
    window.__sofiaConnectStatusProbe = { originalInvoke, originalFetch };
    return true;
  })()`);
...
async function restoreHealthProbe(ctx) {
  await ctx.eval(`(() => {
    const bridge = window.__SOFIA_ELECTRON__;
    const probe = window.__sofiaConnectStatusProbe;
    ...
    delete window.__sofiaConnectStatusProbe;
    return true;
  })()`);
}
...
async function setup(ctx) {
  ...
  state.workspaceId = await ctx.eval(`(() => {
    ...
    return hashWorkspace || (localStorage.getItem("sofia.react.activeWorkspace") || "").trim();
  })()`);
...
export default {
  id: FLOW_ID,
  title: "Signed-in users see non-blocking Sofia Connect lifecycle health",
  kind: "user-facing",
  ...
  steps: [
    { name: "Setup signed-in Connect status", run: setup },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A retained signed-in session shows Sofia Connect checking while authentication is restored", {
          voiceover: vo[0],
          action: async () => {
            await beginSavedSessionRestore(ctx);
            await openAccountMenu(ctx);
            await ctx.waitForText("Sofia Connect: Checking", { timeoutMs: 10_000 });
          },
          assert: async () => {
            const delay = await authDelay(ctx);
          ...
          screenshot: {
            name: "sofia-connect-checking",
            claim: "The signed-in status bar shows Sofia Connect checking during session restoration.",
            requireText: ["Sofia Connect: Checking", "Ready for new tasks"],
      ...
      run: async (ctx) => {
        await ctx.prove("The shared background lifecycle leaves normal message submission unblocked", {
          ...
          assert: async () => {
            ...
            ctx.expectText("Sofia Connect: Checking");
          },
          screenshot: {
            name: "sofia-connect-nonblocking",
            claim: "A normal task is submitted while Sofia Connect continues checking in the background.",
            requireText: ["Sofia Connect: Checking", PROMPT],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Successful reconciliation changes Sofia Connect to Ready", {
          voiceover: vo[2],
          action: async () => {
            await closeAccountMenu(ctx);
          ...
          assert: async () => {
            ctx.expectNoText("Sofia Connect: Needs attention");
          },
          screenshot: {
            name: "sofia-connect-ready",
            claim: "The status bar reports Sofia Connect ready after background restoration succeeds.",
            requireText: ["Sofia Connect: Ready", "Ready for new tasks"],
            hashIncludes: `/workspace/${state.workspaceId}/session/`,
          },
        });
      },
      ...
      run: async (ctx) => {
        await ctx.prove("A persistent lifecycle failure turns the status red and offers diagnostics", {
          ...
          assert: async () => {
            ...
            witness(ctx, red, "The failed Sofia Connect status uses the red error indicator.", { red });
          },
          screenshot: {
            name: "sofia-connect-needs-attention",
            claim: "The failed status is red and explains how to run diagnostics.",
            requireText: ["Sofia Connect: Needs attention", "Run diagnostics"],
            hashIncludes: `/workspace/${state.workspaceId}/session/`,
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The Sofia Connect status is absent when the user is signed out", {
          voiceover: vo[4],
          action: async () => {
            await closeAccountMenu(ctx);
            await restoreHealthProbe(ctx);
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after sign-out" });
            await ctx.waitFor("!document.body.innerText.includes('Sofia Connect:')", { timeoutMs: 15_000, label: "Connect status hidden" });
            await openAccountMenu(ctx);
          },
          assert: async () => {
            ctx.expectNoText("Sofia Connect:");
          },
          screenshot: {
            name: "sofia-connect-signed-out-hidden",
            claim: "Signed-out users do not see an Sofia Connect lifecycle status.",
            requireText: ["Ready for new tasks"],
            rejectText: ["Sofia Connect:"],
            hashIncludes: `/workspace/${state.workspaceId}/session/`,
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          ...
          if (token) localStorage.setItem("sofia.den.authToken", token);
