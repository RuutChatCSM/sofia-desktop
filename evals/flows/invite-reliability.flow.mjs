...
function uniqueOrgName(ctx) {
  const stamp = ctx.env.SOFIA_EVAL_RUNSTAMP?.trim() || new Date().toISOString();
        ...
        await ctx.on(invalidWeb, async () => {
          await ctx.prove("A garbage invite token renders a clear product error instead of a blank loop", {
            ...
            action: async () => {
              ...
              invalidUrl.searchParams.set("invite", `invalid-${cleanStamp(ctx.env.SOFIA_EVAL_RUNSTAMP)}-token`);
            ...
            assert: async () => {
              ...
              await ctx.expectText("Back to Sofia Cloud", { timeoutMs: 60_000 });
              await ctx.expectNoText("Dashboard");
            },
            screenshot: { name: "invalid-invite-token", requireText: ["This invite can't be opened.", "Back to Sofia Cloud"], rejectText: ["Dashboard", "Could not join", "Something went wrong"] },
          });
        });
      },
    },
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Cleanup removes Jamie and the desktop cell is explicitly skipped", {
          ...
          action: async () => {
            await removeMemberIfPresent(ctx, ctx.actors.jamie.email);
            const desktopGate = ctx.env.SOFIA_EVAL_DESKTOP_SURFACE?.trim() ? "requested" : "not requested";
            ctx.skip(`Electron desktop surface ${desktopGate}; ${FLOW_ID} is intentionally web-only.`);
            ctx.output("desktop-surface-gate", `SOFIA_EVAL_DESKTOP_SURFACE=${ctx.env.SOFIA_EVAL_DESKTOP_SURFACE ?? ""}`);
          },
          assert: async () => {
            const org = await loadOrg(ctx);
