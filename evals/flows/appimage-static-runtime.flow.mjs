...
const DEFAULT_APPIMAGE = "/workspace/apps/desktop/dist-electron/sofia-linux-x86_64-*.AppImage";
...
function runInSandbox(ctx, script) {
  ...
  const result = spawnSync(
    "daytona",
    ["exec", ctx.env.SOFIA_EVAL_DAYTONA_SANDBOX, "--", "echo", encoded, "|", "base64", "-d", "|", "bash"],
...
`);
    ...
    {
      name: "The fixed AppImage reaches a working Sofia session",
      run: async (ctx) => {
        await ctx.prove("The static-runtime AppImage starts and serves the packaged Sofia UI", {
          voiceover: vo[1],
          assert: async () => {
            const appImage = ctx.env.SOFIA_EVAL_APPIMAGE_PATH || DEFAULT_APPIMAGE;
            const output = runInSandbox(ctx, `
set -euo pipefail
...
pgrep -af '/tmp/.mount_openwo.*/sofia'
...
`);
            ...
            record(ctx, output.includes('"title": "Sofia"'), "The packaged Electron CDP target is ready");
          ...
          screenshot: {
            ...
            requireText: ["Sofia", "New session", "Ready for new tasks"],
