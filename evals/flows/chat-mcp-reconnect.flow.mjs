...
const MOCK_PORT = Number(process.env.SOFIA_EVAL_CHAT_RECONNECT_MOCK_PORT || 3994);
const MOCK_SERVER_URL = `http://127.0.0.1:${MOCK_PORT}`;
const DEN_API_URL = (process.env.SOFIA_EVAL_DEN_API_URL ?? "").trim().replace(/\/+$/, "");
const DEN_ORIGIN = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? DEN_API_URL.replace("127.0.0.1", "localhost")).trim().replace(/\/+$/, "");
...
const WORKSPACE_PATH = "/tmp/sofia-chat-mcp-reconnect";
...
async function readRuntimeCloudControlMcp(ctx) {
  return ctx.eval(`(async () => {
    ...
    const [response, healthResponse] = await Promise.all([
      ...
      fetch(base + '/mcp/sofia-cloud/health?probe=1', { headers }),
    ]);
...
async function ensureCloudControlReady(ctx) {
  ...
  await ctx.expectText("Sofia Cloud Control", { timeoutMs: 60_000 });

  const alreadyConnected = await ctx.eval(`(() => {
    const card = [...document.querySelectorAll('button')].find((entry) => entry.textContent.includes('Sofia Cloud Control'));
  ...
  if (!alreadyConnected) {
    ...
    ctx.assert(connected, "Could not connect Sofia Cloud Control for the eval workspace.");
  }
  ...
  }, "Runtime Sofia Cloud Control MCP config never became ready.", 60_000);
...
async function advanceVisibleOnboarding(ctx) {
  ...
  if (await clickExactButtonIfPresent(ctx, "Continue without Sofia Models")) return true;
...
async function ensureWorkspace(ctx) {
  ...
  await ctx.waitFor(
    ...
    { timeoutMs: 60_000, label: "Sofia server auth for eval workspace" },
  );
...
async function createFreshTask(ctx) {
  await ctx.navigateHash(`/workspace/${state.workspaceId}/session`);
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000, label: "desktop control API" });
  let created = false;
  let lastCreateError = null;
  for (let attempt = 0; attempt < 5 && !created; attempt += 1) {
    await ctx.waitFor(
      "window.__sofiaControl.listActions().some((entry) => entry.id === 'session.create_task' && entry.disabled === false)",
...
async function sendPrompt(ctx, prompt) {
  ...
  await ctx.waitFor(
    "window.__sofiaControl.listActions().some((entry) => entry.id === 'composer.send' && entry.disabled === false)",
...
async function waitForAssistantToFinish(ctx, timeoutMs = 180_000) {
  await ctx.waitFor(
    "window.__sofiaControl.listActions().some((entry) => entry.id === 'composer.stop' && entry.disabled === false)",
    { timeoutMs: 45_000, label: "assistant run started" },
  ).catch(() => undefined);
  await ctx.waitFor(
    "window.__sofiaControl.listActions().some((entry) => entry.id === 'composer.stop' && entry.disabled === true)",
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        state.token = process.env.SOFIA_EVAL_DEN_TOKEN?.trim() || await signInDemoOwner();
        ...
        const written = await ctx.eval(`(async () => {
          ...
          localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(DEN_ORIGIN)});
          localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(`${DEN_ORIGIN}/api/den`)});
          return true;
        })()`, { awaitPromise: true });
        ctx.assert(written === true, "Could not point the desktop app at the eval Den.");
        await ctx.eval("location.reload()");
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after Den bootstrap" });
        ...
        await ctx.waitFor(
          "window.__sofiaControl.listActions().some((entry) => entry.id === 'auth.exchange-grant' && entry.disabled === false)",
        ...
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 45_000, label: "desktop Den token" });
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.activeOrgId') ?? '').trim())", { timeoutMs: 60_000, label: "desktop active org" });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Pending browser authorization remains an enabled, repeatable action", {
          voiceover: vo[1],
          action: async () => {
            await ctx.eval("window.__sofia?.clearEvents()");
          ...
          assert: async () => {
            ...
            const events = await ctx.eval("window.__sofia?.events(20) ?? []");
