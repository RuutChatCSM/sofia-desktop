...
const DOWNLOAD_DIR = "/tmp/sofia-office-attachment-downloads";
...
function runInSandbox(ctx, script, timeout = 120_000) {
  ...
  const result = spawnSync(
    "daytona",
    ["exec", ctx.env.SOFIA_EVAL_DAYTONA_SANDBOX.trim(), "--", "echo", encoded, "|", "base64", "-d", "|", "bash"],
...
set -euo pipefail
cd /workspace
if [ -f /tmp/sofia-office-attachments-mock.pid ]; then
  kill "$(cat /tmp/sofia-office-attachments-mock.pid)" >/dev/null 2>&1 || true
fi
rm -f /tmp/sofia-office-attachments-mock.log /tmp/sofia-office-attachments-mock.pid
nohup node evals/drivers/office-attachments-mock-provider.mjs --port ${MOCK_PORT} --workspace ${shellQuote(ctx.workspacePath || "/workspace")} > /tmp/sofia-office-attachments-mock.log 2>&1 < /dev/null &
echo $! > /tmp/sofia-office-attachments-mock.pid
for _ in $(seq 1 80); do
  if curl -sf http://127.0.0.1:${MOCK_PORT}/health >/tmp/sofia-office-attachments-health.json; then
    cat /tmp/sofia-office-attachments-health.json
    exit 0
  fi
  sleep 0.25
done
cat /tmp/sofia-office-attachments-mock.log >&2
exit 1
`);
  return output.trim();
}
...
set -uo pipefail
pidfile=/tmp/sofia-office-attachments-mock.pid
      ...
      opencode: {
        provider: {
          [PROVIDER_ID]: {
            ...
            options: { baseURL, apiKey: "sk-sofia-office-attachments-eval" },
            models: {
              [MODEL_ID]: {
...
async function dismissSofiaModelsModal(ctx) {
  const result = await ctx.eval(`(() => {
    ...
    const dialog = roots.find((item) => (item.textContent || "").includes("Sofia Models"));
    if (!dialog) return { dismissed: false };
    const buttons = Array.from(dialog.querySelectorAll("button"));
    const continueButton = buttons.find((button) => (button.textContent || "").trim().includes("Continue without Sofia Models"));
    const closeButton = buttons.find((button) => {
      const label = button.getAttribute("aria-label") || "";
      return label === "Close" || (button.textContent || "").trim() === "Close";
    });
    const button = continueButton || closeButton;
    if (!button) return { dismissed: false, reason: "Sofia Models modal had no dismiss button" };
    button.click();
  ...
  await ctx.waitFor(`(() => {
    const roots = Array.from(document.querySelectorAll('[role="dialog"], [data-slot="dialog-content"], [data-radix-dialog-content]'));
    return !roots.some((item) => (item.textContent || "").includes("Sofia Models"));
  })()`, { timeoutMs: 10_000, label: "Sofia Models modal dismissed" });
}

async function createFreshSession(ctx) {
  await ctx.waitFor(
    `window.__sofiaControl.listActions().some((item) => item.id === "session.create_task" && !item.disabled)`,
