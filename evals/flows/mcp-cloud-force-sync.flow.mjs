/**
 * The Extensions page Refresh button force-syncs the Sofia Cloud Control
 ...
 *
 * Proof: the `sofia.den.mcp.sync` marker (written only when a sync
 ...
 *
 * Prerequisites: a desktop app (this worktree) signed in to Sofia Cloud
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(signedIn, "Desktop app must be signed in to Sofia Cloud (run mcp-connections-desktop-e2e first).");
        const marker = await ctx.eval("localStorage.getItem('sofia.den.mcp.sync')");
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const before = await ctx.eval("localStorage.getItem('sofia.den.mcp.sync')");
        ...
        await ctx.prove("Refresh re-mints and rewrites the cloud MCP config immediately", {
          ...
          assert: async () => {
            await ctx.waitFor(
              `localStorage.getItem('sofia.den.mcp.sync') !== ${JSON.stringify(before)}`,
              { timeoutMs: 30_000, label: "sync marker rewritten by Refresh" },
            );
            ctx.log(`marker after: ${await ctx.eval("localStorage.getItem('sofia.den.mcp.sync')")}`);
            await ctx.expectText("Sofia Cloud Control");
