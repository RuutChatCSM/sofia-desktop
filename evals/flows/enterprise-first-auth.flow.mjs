 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_API_URL: Den API base URL for the enterprise sandbox.
 * - SOFIA_EVAL_DEN_WEB_URL: Den Web origin used by the desktop handoff link.
 *
 * Optional env:
 * - SOFIA_EVAL_CDP_URL or --cdp-url: CDP endpoint for a factory-fresh Electron app.
 * - SOFIA_EVAL_ENTERPRISE_ORG_NAME: organization display name (default Example Organization).
 * - SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_EMAIL: signed-in member email (default new.member@example.com).
 * - SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_WORKSPACE: workspace folder (default /workspace/enterprise-first-auth).
 * - SOFIA_EVAL_ENTERPRISE_GATEWAY_URL: gateway base URL used if the transcript asks for JIT login without a full link.
 * - SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_GATEWAY_USER: gateway login user override (default signed-in member email).
 * - SOFIA_EVAL_ENTERPRISE_PASSWORD: account password (default TutorialDemo123!).
 * - SOFIA_EVAL_ENTERPRISE_TASK_TIMEOUT_MS: chat turn timeout in milliseconds.
 *
 * Runner note: evals/runner/run.mjs chooses one CDP endpoint for a run. Point
 * SOFIA_EVAL_CDP_URL (or --cdp-url) at the freshly installed sandbox/app.
 */
...
import {
  ...
  signInByEmail,
  timeoutMs,
  waitForSofiaConnectReady,
  workspaceFolder,
} from "./enterprise-gateway-common.mjs";

const DEFAULT_NEW_MEMBER_EMAIL = "new.member@example.com";
const WORKSPACE_ENV = "SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_WORKSPACE";
const DEFAULT_WORKSPACE = "/workspace/enterprise-first-auth";
const PROMPT = "Use Sofia Cloud capabilities to find and use the `my-incidents` skill, then report the open incidents assigned to me.";
...
const JIT_COMPLETE_SENTINEL = "SOFIA_ENTERPRISE_JIT_COMPLETE_SENTINEL";
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A just-installed enterprise desktop app opens to the Sofia welcome screen", {
          action: async () => {
            await ctx.waitForText("Welcome to Sofia", { timeoutMs: 90_000 });
          },
          assert: async () => {
          ...
          screenshot: {
            ...
            claim: "The first launch starts from the generic Sofia welcome screen before the member signs in.",
            requireText: ["Welcome to Sofia"],
    ...
    {
      name: "Continue to workspace and wait for Sofia Connect",
      run: async (ctx) => {
        await clickTextStartingWith(ctx, "Continue to workspace", "button, [role=button]", 30_000);
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 60_000, label: "desktop Den auth token" });
        const shell = await ctx.waitFor(`(() => {
          const text = document.body.innerText || '';
          return text.includes('Sofia Connect') || text.includes('Run task') || location.hash.includes('/workspace') || location.hash.includes('/welcome');
        ...
        if (state.workspaceId) {
          assertEvidence(ctx, true, "A local workspace is created from the welcome route before polling Sofia Connect", {
            folder,
            workspaceId: state.workspaceId,
          });
        }
        const ready = await waitForSofiaConnectReady(ctx);
        assertEvidence(ctx, ready.ready, "Sofia Connect reaches Ready on the factory-fresh app", ready);
      },
    },
    {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Member's first task discovers the my-incidents org skill on a fresh machine", {
          action: async () => {
            const timeout = timeoutMs(ctx, "SOFIA_EVAL_ENTERPRISE_FIRST_AUTH_TIMEOUT_MS", 300_000);
            ...
            state.latestTranscript = await retryAfterGatewayLoginIfNeeded(
              ...
              PROMPT_AFTER_JIT,
              { timeout, gatewayUserEnvName: "SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_GATEWAY_USER" },
            );
          },
          assert: async () => {
...
function newMemberEmail(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_NEW_MEMBER_EMAIL") || DEFAULT_NEW_MEMBER_EMAIL;
}
...
async function dispatchDesktopHandoff(ctx, email) {
  ...
  await waitForDesktopToken(ctx, sofiaUrl);
  return token;
}

async function waitForDesktopToken(ctx, sofiaUrl) {
  try {
    await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 60_000, label: "desktop Den token after handoff" });
  } catch (error) {
...
async function desktopAuthState(ctx) {
  return ctx.eval(`(() => ({
    hasToken: Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim()),
