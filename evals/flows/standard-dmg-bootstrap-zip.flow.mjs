...
const DEN_WEB_URL = cleanUrl(process.env.SOFIA_EVAL_DEN_WEB_URL);
const INSTALL_TOKEN = process.env.SOFIA_EVAL_INSTALL_TOKEN?.trim() ?? "";
const BUNDLE_ZIP = process.env.SOFIA_EVAL_BUNDLE_ZIP?.trim() ?? "";
const BUNDLE_DIR = process.env.SOFIA_EVAL_BUNDLE_DIR?.trim() ?? "";
const BOOTSTRAP_PATH = process.env.SOFIA_EVAL_BOOTSTRAP_PATH?.trim() ?? "";
const DESKTOP_CDP_URL = cleanUrl(process.env.SOFIA_EVAL_DESKTOP_CDP_URL);
...
export default {
  id: FLOW_ID,
  title: "Organization downloads reuse the standard signed installer and configure Sofia on first launch",
  kind: "user-facing",
  requiredEnv: [
    "SOFIA_EVAL_DEN_WEB_URL",
    "SOFIA_EVAL_INSTALL_TOKEN",
    "SOFIA_EVAL_BUNDLE_ZIP",
    "SOFIA_EVAL_BUNDLE_DIR",
    "SOFIA_EVAL_BOOTSTRAP_PATH",
    "SOFIA_EVAL_DESKTOP_CDP_URL",
  ],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The downloaded ZIP has exactly the standard DMG and desktop-bootstrap.json", {
          ...
          assert: async () => {
            ...
            witness(ctx, entries.some((entry) => /^sofia-mac-arm64-.+\.dmg$/.test(entry)), "The ZIP contains the normal versioned macOS DMG", entries);
