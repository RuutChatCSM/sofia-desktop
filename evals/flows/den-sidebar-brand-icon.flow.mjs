...
export default {
  ...
  steps: [
    ...
    },
    {
      name: "Frame 2",
      run: async (ctx) => {
        await ctx.prove("The sidebar replaces the generic Sofia mark with the managed organization icon", {
          voiceover: vo[1],
          action: async () => {
            await ctx.client.send("Network.emulateNetworkConditions", {
              offline: false,
          ...
          screenshot: {
            ...
            claim: "The Den sidebar visibly shows the organization's managed square icon instead of the Sofia SVG.",
            requireText: ["Dashboard"],
            rejectText: ["Something went wrong"],
          },
        });
    ...
    },
    {
      name: "Frame 3",
      run: async (ctx) => {
        await ctx.prove("An organization without a managed icon gets the intact Sofia fallback", {
          voiceover: vo[2],
          action: async () => {
            await clearManagedIcon(ctx);
            await reloadDashboard(ctx);
          ...
          screenshot: {
            ...
            claim: "Without a custom square icon, Den shows the intact Sofia fallback and no broken image.",
            requireText: ["Dashboard"],
            rejectText: ["Something went wrong"],
          },
        });
