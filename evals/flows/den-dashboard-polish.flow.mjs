...
const EXPECTED_VERSION = process.env.SOFIA_EVAL_DEN_EXPECTED_VERSION?.trim() || "commit 8c412db";
...
function runMysql(sql) {
  ...
  const database = process.env.SOFIA_EVAL_DEN_DATABASE_NAME?.trim() || "sofia_den";
  execFileSync("docker", [
    "exec",
    container,
    "mysql",
