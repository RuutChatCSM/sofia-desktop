...
const JOB_GLOBAL = "__sofiaWindowsWorkspaceSessionPerformance";
const DEFAULT_ROOT = "C:\\ow\\sofia-perf";
const DEFAULTS = {
  workspaces: 3,
...
function readParams(ctx) {
  ...
  const runId = ctx.env.SOFIA_PERF_RUN_ID?.trim() || "default";
  const provider = ctx.env.SOFIA_PERF_PROVIDER?.trim() || "";
  const model = ctx.env.SOFIA_PERF_MODEL?.trim() || "";
  if ((provider && !model) || (!provider && model)) {
    throw new Error("SOFIA_PERF_PROVIDER and SOFIA_PERF_MODEL must be supplied together, or both omitted.");
  }
  state.params = {
    runId,
    safeRunId: safeRunSegment(runId),
    root: ctx.env.SOFIA_PERF_ROOT?.trim() || DEFAULT_ROOT,
    workspaces: requirePositiveInt(ctx.env, "SOFIA_PERF_WORKSPACES", DEFAULTS.workspaces),
    sessionsPerWorkspace: requirePositiveInt(ctx.env, "SOFIA_PERF_SESSIONS_PER_WORKSPACE", DEFAULTS.sessionsPerWorkspace),
    concurrency: requirePositiveInt(ctx.env, "SOFIA_PERF_CONCURRENCY", DEFAULTS.concurrency),
    conversations: requirePositiveInt(ctx.env, "SOFIA_PERF_CONVERSATIONS", DEFAULTS.conversations),
    timeoutMs: requirePositiveInt(ctx.env, "SOFIA_PERF_TIMEOUT_MS", DEFAULTS.timeoutMs),
    maxRouteReadyMs: requirePositiveInt(ctx.env, "SOFIA_PERF_MAX_ROUTE_READY_MS", DEFAULTS.maxRouteReadyMs),
    maxSwitchP95Ms: requirePositiveInt(ctx.env, "SOFIA_PERF_MAX_SWITCH_P95_MS", DEFAULTS.maxSwitchP95Ms),
    maxEventLoopP95Ms: requirePositiveInt(ctx.env, "SOFIA_PERF_MAX_EVENT_LOOP_P95_MS", DEFAULTS.maxEventLoopP95Ms),
    maxEventAborts: requireNonNegativeInt(ctx.env, "SOFIA_PERF_MAX_EVENT_ABORTS", DEFAULTS.maxEventAborts),
    maxJsHeapMb: requirePositiveInt(ctx.env, "SOFIA_PERF_MAX_JS_HEAP_MB", DEFAULTS.maxJsHeapMb),
    requireOutOfWindowSession: ctx.env.SOFIA_PERF_REQUIRE_OUT_OF_WINDOW_SESSION?.trim() === "1",
...
async function waitForControl(ctx, timeoutMs = 90_000) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs, label: "Sofia control API" });
}

async function dismissSofiaModelsPromo(ctx) {
  const promoText = "Use Sofia Models without API keys";
  if (!(await ctx.hasText(promoText))) return;
  await ctx.clickText("Continue without Sofia Models");
  await ctx.waitFor(`!document.body.innerText.includes(${quoted(promoText)})`, {
    timeoutMs: 20_000,
    label: "Sofia Models promo dismissed",
  });
}
...
async function waitForWorkspaceRouteReady(ctx, workspaceId, sessionId = "") {
  ...
  await ctx.waitFor(`(() => {
    ...
    const route = window.__sofiaControl?.snapshot?.().route || "";
...
async function waitForBenchmarkSessionReady(ctx, workspaceId, session) {
  ...
  return ctx.waitFor(`(() => {
    ...
    const routePath = (window.__sofiaControl?.snapshot?.().route || "").split(/[?#]/)[0];
    ...
    const routeState = window.__sofia?.slice?.("route");
...
async function activateVisibleWorkspace(ctx, setup) {
  ...
  await ctx.eval(`(async () => {
    ...
    localStorage.setItem("sofia.react.activeWorkspace", ${quoted(workspaceId)});
    let prefs = {};
    try { prefs = JSON.parse(localStorage.getItem("sofia.preferences") || "{}"); } catch { prefs = {}; }
    const nextPrefs = {
      ...prefs,
    ...
    localStorage.setItem("sofia.preferences", JSON.stringify(nextPrefs));
    ${model ? `localStorage.setItem("sofia.defaultModel", ${quoted(`${model.provider}/${model.model}`)});` : ""}
  ...
  await dismissSofiaModelsPromo(ctx);
  return Date.now() - startedAt;
}

async function installFetchProbe(ctx, label) {
  return ctx.eval(`(() => {
    const key = "__sofiaWindowsPerfFetchProbe";
    if (!window[key]) {
...
async function fetchProbeSummary(ctx) {
  return ctx.eval(`(() => {
    const probe = window.__sofiaWindowsPerfFetchProbe;
...
async function measureSessionSwitches(ctx, workspaceId, sessions) {
  ...
  for (const session of selected) {
    ...
    records.push({
      ...
      route: await ctx.eval("window.__sofiaControl.snapshot().route"),
      readiness,
    });
  }
...
function setupWorker(params, emit) {
  ...
  return (async () => {
    ...
    const bridge = window.__SOFIA_ELECTRON__?.invokeDesktop;
    if (!bridge) throw new Error("Sofia Electron desktop bridge is missing.");
    let connection = null;
    const refreshConnection = async (reason) => {
      const info = await bridge("sofiaServerInfo");
      if (!info?.running || !info.port) throw new Error(`Embedded Sofia server is not running after ${reason}.`);
      ...
      if (!clientToken && !ownerToken) throw new Error(`Sofia client token is unavailable after ${reason}.`);
      if (!hostToken && !ownerToken) throw new Error(`Sofia host authentication is unavailable after ${reason}.`);
      connection = {
    ...
    const headersFor = (auth) => {
      if (!connection) throw new Error("Sofia server connection is not initialized.");
      const headers = { "content-type": "application/json" };
      if (auth === "host") {
        if (connection.hostToken) headers["x-sofia-host-token"] = connection.hostToken;
...
function conversationWorker(params, emit) {
  ...
  return (async () => {
    ...
    if (!bridge) throw new Error("Sofia Electron desktop bridge is missing.");
    const info = await bridge("sofiaServerInfo");
    if (!info?.running || !info.port) throw new Error("Embedded Sofia server is not running.");
    const clientToken = typeof info.clientToken === "string" && info.clientToken ? info.clientToken : "";
    const ownerToken = typeof info.ownerToken === "string" && info.ownerToken ? info.ownerToken : "";
    if (!clientToken && !ownerToken) throw new Error("Sofia client token is unavailable inside the renderer.");
    ...
    for (let index = 1; index <= params.conversations; index += 1) {
      ...
      items.push({
        ...
        prompt: `Reply with exactly ${marker} and no other text. Do not call tools. This is a Windows Sofia performance isolation check.`,
        sessionId: null,
        created: false,
        accepted: false,
        promptStatus: null,
...
export default {
  ...
  precondition: async (ctx) => {
    readParams(ctx);
    await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__)", { timeoutMs: 60_000, label: "Sofia Electron bridge" });
  },
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("The eval is attached to a real packaged Windows Electron Sofia app", {
          voiceover: vo[0],
          action: async () => {
            await waitForControl(ctx);
            ...
            state.runtime = await ctx.eval(`(async () => {
              const bridge = window.__SOFIA_ELECTRON__;
              ...
              return {
                ...
                build: build ? {
                  ...
                  sofiaDevMode: build.sofiaDevMode,
                } : null,
          ...
          assert: async () => {
            recordAssertion(ctx, state.runtime?.hasBridge === true && state.runtime?.hasInvoke === true, "window.__SOFIA_ELECTRON__ and invokeDesktop are available.", state.runtime);
            ...
            recordAssertion(ctx, state.runtime?.build?.sofiaDevMode === false, "appBuildInfo reports packaged/not dev mode.", state.runtime?.build);
          },
          screenshot: {
            name: "packaged-windows-baseline",
            requireText: ["Overview of all settings"],
            rejectText: ["Something went wrong", "Use Sofia Models without API keys"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor(`(() => {
          const route = window.__sofia?.slice?.("route");
        ...
        await ctx.prove("Many real benchmark workspaces and sessions remain navigable in the visible app", {
          ...
          assert: async () => {
            ...
            recordAssertion(ctx, Number.isInteger(state.ui.fetchProbe.summary.aborted) && state.ui.fetchProbe.summary.aborted <= params.maxEventAborts, "Renderer fetch instrumentation aborted request count stayed within SOFIA_PERF_MAX_EVENT_ABORTS.", { actual: state.ui.fetchProbe.summary.aborted, budget: params.maxEventAborts, abortedByPath: state.ui.fetchProbe.summary.abortedByPath });
            ...
            recordAssertion(ctx, Number.isFinite(state.ui.routeReadyMs) && state.ui.routeReadyMs <= params.maxRouteReadyMs, "Workspace route readiness stayed within SOFIA_PERF_MAX_ROUTE_READY_MS.", { actualMs: state.ui.routeReadyMs, budgetMs: params.maxRouteReadyMs });
            recordAssertion(ctx, state.ui.switches.latencyMs.p95 !== null && state.ui.switches.latencyMs.p95 <= params.maxSwitchP95Ms, "Visible session switch p95 stayed within SOFIA_PERF_MAX_SWITCH_P95_MS.", { actualMs: state.ui.switches.latencyMs.p95, budgetMs: params.maxSwitchP95Ms, latencyMs: state.ui.switches.latencyMs });
            recordAssertion(ctx, state.ui.eventLoopLag.p95 !== null && state.ui.eventLoopLag.p95 <= params.maxEventLoopP95Ms, "Renderer event-loop lag p95 stayed within SOFIA_PERF_MAX_EVENT_LOOP_P95_MS.", { actualMs: state.ui.eventLoopLag.p95, budgetMs: params.maxEventLoopP95Ms, eventLoopLag: state.ui.eventLoopLag });
            recordAssertion(ctx, Number.isFinite(state.ui.jsHeapUsedMiB) && state.ui.jsHeapUsedMiB <= params.maxJsHeapMb, "After-setup renderer JS heap stayed within SOFIA_PERF_MAX_JS_HEAP_MB.", { actualMiB: state.ui.jsHeapUsedMiB, budgetMiB: params.maxJsHeapMb, metric: "JSHeapUsedSize" });
