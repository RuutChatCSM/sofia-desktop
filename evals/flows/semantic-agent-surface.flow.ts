...
async function setSidebarOpen(ctx: FlowContext, open: boolean) {
  const already = await ctx.eval(
    `window.__sofiaControl?.context?.().chrome.sidebarOpen === ${JSON.stringify(open)}`,
  );
...
async function semanticContext(ctx: FlowContext) {
  return ctx.eval("window.__sofiaControl?.context?.()");
}

export default defineFlow({
  id: "semantic-agent-surface",
  title: "People and agents share one semantic Sofia context",
  kind: "user-facing",
  precondition: async (ctx) => {
    await ctx.waitFor("Boolean(window.__sofiaControl)", {
      timeoutMs: 60_000,
      label: "Sofia control API",
    });
      ...
      run: async (ctx) => {
        await ctx.prove("Agent context names the complete visible and retained workbench", {
          ...
          action: async () => {
            ...
            primaryRoute = String(await ctx.eval("window.__sofiaControl.snapshot().route"));
            ...
            await sendPrompt(
              ctx,
              primarySessionId,
              "Call sofia_context. Reply with exactly: CONTEXT-SUMMARY TABS=3 SPLIT=yes FOCUSED=secondary SIDEBAR=open SETTINGS=none",
              "CONTEXT-SUMMARY",
            );
          },
          assert: async () => {
            ...
            const state = await ctx.eval(`(() => {
              const context = window.__sofiaControl.context();
              return {
        ...
        await ctx.prove("Agent navigation reuses the existing split session", {
          ...
          action: async () => {
            ...
            await sendPrompt(
              ctx,
              primary,
              "Call sofia_context, find the session resource titled Context Previous, then call sofia_execute with id workbench.session.focus, that sessionId, expectedRevision, and actor fraimz. Reply with exactly FOCUS-RESULT=secondary-reused.",
          ...
          assert: async () => {
            ...
            const state = await ctx.eval(`(() => ({
              tabs: window.__sofiaControl.context().conversations.tabs.length,
              surfaces: document.querySelectorAll(${JSON.stringify(SURFACE_SELECTOR)}).length,
              route: window.__sofiaControl.snapshot().route,
            }))()`);
            ctx.assert(
        ...
        await ctx.prove("Agent reads another session without changing the workbench", {
          ...
          action: async () => {
            ...
            await sendPrompt(
              ctx,
              primary,
              `Call sofia_context, find Context Previous, then call sofia_query with id session.read for that sessionId. Do not call a UI command. Reply with exactly PREVIOUS-SAID=${PREVIOUS_MARKER}.`,
          ...
          assert: async () => {
            const state = await ctx.eval(`(() => ({
              route: window.__sofiaControl.snapshot().route,
              layout: window.__sofiaControl.context().conversations.layout.kind,
              tabs: window.__sofiaControl.context().conversations.tabs.length,
              surfaces: document.querySelectorAll(${JSON.stringify(SURFACE_SELECTOR)}).length,
            }))()`);
            ctx.assert(
      ...
      run: async (ctx) => {
        await ctx.prove("Settings context retains tabs and split layout after chat unmounts", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              `window.__sofiaControl.context().screen.kind === "settings"
                && window.__sofiaControl.context().screen.panel === "ai"`,
        ...
        await ctx.prove("Known skills stay direct while unknown capabilities stay discoverable", {
          ...
          action: async () => {
            ...
            await sendPrompt(
              ctx,
              primary,
              "Choose one remote skill already listed in available_skills. Load it directly with its exact capability using sofia-cloud_execute_capability; do not search first and do not perform the skill workflow. Then call sofia_context. Reply with exactly these three lines: DIRECT_SKILL_TOOL=sofia-cloud_execute_capability; UNKNOWN_CAPABILITY_TOOL=sofia-cloud_search_capabilities; LOCAL_EXTENSION_COMMAND=extension.call",
              "DIRECT_SKILL_TOOL=sofia-cloud_execute_capability",
            );
          },
          assert: async () => {
            await ctx.expectText("DIRECT_SKILL_TOOL=sofia-cloud_execute_capability");
            await ctx.expectText("UNKNOWN_CAPABILITY_TOOL=sofia-cloud_search_capabilities");
          ...
          screenshot: {
            ...
            requireText: [
              "DIRECT_SKILL_TOOL=sofia-cloud_execute_capability",
              "UNKNOWN_CAPABILITY_TOOL=sofia-cloud_search_capabilities",
        ...
        await ctx.prove("Agent reports actions by effect instead of a flat tool list", {
          ...
          action: async () => {
            ...
            await sendPrompt(
              ctx,
              primary,
              "Call sofia_context and inspect availableAffordances plus execution. Reply with exactly five lines using real ids: READ=<id> data=read; UI=<id> ui=<effect>; DURABLE=<id> data=write; CONFIRMATION=<id> confirmation=destructive; CONCURRENCY=queries:parallel commands:serialized",
