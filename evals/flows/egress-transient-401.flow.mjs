...
function transient401Script(baseUrl) {
  return `
    ...
    const tokenKey = "sofia.den.authToken";
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        let verdictLab;
        let result;
        await ctx.prove("A one-off foreign 401 is treated as unavailable, not revoked, and Sofia product diagnostics report the recovered transient 401", {
          voiceover: vo[0],
          action: async () => {
