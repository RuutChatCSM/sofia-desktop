  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") {
        reject(new Error("Mock Sofia server did not bind a port."));
        return;
      }
      resolve({
...
async function runInjectedSessionTools(baseUrl) {
  ...
  const { stdout } = await execFile("bun", ["--eval", script], {
    ...
    env: {
      ...process.env,
      SOFIA_SERVER_URL: baseUrl,
      SOFIA_SERVER_TOKEN: TOKEN,
    },
    maxBuffer: 1024 * 1024,
  });
...
async function showProofPanel(ctx, title, rows) {
  await ctx.eval(
    `(() => {
      const id = "sofia-fraimz-past-chat-proof";
...
export default {
  ...
  title: "Injected tools can search and read past Sofia chats",
  spec: "Sofia injected tool surface for cross-session memory",
  steps: [
    {
      name: "Real app boots before tool proof",
      run: async (ctx) => {
        await ctx.prove("The real Sofia app is running for frame proof", {
          action: async () => {
          ...
          assert: async () => {
            ...
            ctx.assert(typeof route === "string" && route.length > 0, "Sofia route was not available.");
            ctx.log(`route: ${route}`);
          },
    ...
    },
    {
      ...
      run: async (ctx) => {
        const server = await startMockSofiaServer();
        try {
          const output = await runInjectedSessionTools(server.baseUrl);
          ctx.toolOutput = output;
          await ctx.prove("sofia_query session.search returns a transcript hit from another chat", {
            action: async () => {
              const hit = output.search.results[0];
              await showProofPanel(ctx, "Past Chat Search Tool Proof", [
                { label: "Tool called", value: "sofia_query session.search" },
            ...
            assert: async () => {
              ...
              ctx.assert(
                ...
                "Search did not load transcript messages through the Sofia server API.",
              );
            },
            screenshot: { name: "session-search-hit", requireText: ["sofia_query session.search", "raven launch", "ses_alpha"] },
          });
        } finally {
          await server.close();
        }
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("sofia_query session.read retrieves the referenced session transcript", {
          action: async () => {
            await showProofPanel(ctx, "Past Chat Read Tool Proof", [
              { label: "Tool called", value: "sofia_query session.read" },
          ...
          screenshot: { name: "session-read-transcript", requireText: ["sofia_query session.read", "Archive decisions", "ship the archive importer first"] },
        });
      },
    },
  ],
