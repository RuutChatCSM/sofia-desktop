...
function apiBase(ctx) {
  const value = (ctx.env.SOFIA_EVAL_DEN_API_URL ?? "").trim().replace(/\/+$/, "");
  ctx.assert(Boolean(value), "SOFIA_EVAL_DEN_API_URL is required.");
  return value;
}

function webBase(ctx) {
  const value = (ctx.env.SOFIA_EVAL_DEN_WEB_URL ?? "").trim().replace(/\/+$/, "");
  ctx.assert(Boolean(value), "SOFIA_EVAL_DEN_WEB_URL is required.");
  return value;
}

function adminToken(ctx) {
  const value = (ctx.env.SOFIA_EVAL_DEN_TOKEN ?? "").trim();
  ctx.assert(Boolean(value), "SOFIA_EVAL_DEN_TOKEN is required.");
  return value;
}
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("Boolean(window.__sofiaControl)", {
          timeoutMs: 60_000,
          label: "Sofia control API after clean-state reload",
        });
        const desktopContext = await ctx.eval(`({
          signedIn: Boolean((localStorage.getItem("sofia.den.authToken") ?? "").trim()),
        ...
        const workspacePath = join(tmpdir(), "sofia-composer-model-refresh-proof");
    ...
    },
    {
      name: "Frame 2",
      run: async (ctx) => {
        await ctx.prove("The newly assigned model appears without restarting Sofia", {
          voiceover: vo[1],
          action: async () => {
