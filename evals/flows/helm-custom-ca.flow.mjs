...
const CHART = join(ROOT, "packaging", "helm", "sofia-ee");
const MYSQL_TLS_TEST = join(CHART, "tests", "custom-ca-mysql-tls.mjs");
const CUSTOM_CA_PATH = "/etc/sofia/custom-ca/ca-bundle.pem";
...
function helmTemplate(args) {
  return run("helm", ["template", "sofia-ee", CHART, ...args]);
}
...
export default {
  ...
  enabled: true
  existingSecret: sofia-ca-secret
  existingConfigMap: ""
  key: corp-root.pem`;
        await ctx.prove("Operators reference an existing CA Secret without embedding PEM material in Helm values", {
          ...
          action: async () => {
            const result = helmTemplate([
              ...
              "--set",
              "customCa.existingSecret=sofia-ca-secret",
              "--set",
          ...
          assert: async () => {
            witness(ctx, rendered.includes('secretName: "sofia-ca-secret"'), "The rendered volume references the existing Secret by name");
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("The selected CA key is mounted read-only into every Helm-managed Node workload", {
          ...
          assert: async () => {
            witness(ctx, rendered.includes("name: sofia-ee-den-api"), "Den API renders");
            witness(ctx, rendered.includes("name: sofia-ee-den-web"), "Den Web renders");
            witness(ctx, rendered.includes("name: sofia-ee-inference"), "Inference renders when enabled");
            witness(ctx, rendered.includes("name: sofia-ee-migrate"), "The migration Job renders");
            witness(ctx, countOccurrences(rendered, 'mountPath: "/etc/sofia/custom-ca"') === 4, "All four workloads mount the chart-controlled CA directory");
            ...
            ctx.output(
              ...
              selectedLines(rendered, ["sofia-ee-den-api", "sofia-ee-den-web", "sofia-ee-inference", "sofia-ee-migrate", "custom-ca", "mountPath", "readOnly", "ca-bundle.pem", "subPath"]),
            );
          },
        });
      },
        ...
        await ctx.prove("Every rendered Node workload receives NODE_EXTRA_CA_CERTS pointing at the mounted CA file", {
          ...
          action: async () => {
            const result = helmTemplate([
              ...
              "customCa.enabled=true",
              "--set",
              "customCa.existingConfigMap=sofia-ca-config",
            ]);
          ...
          assert: async () => {
            ...
            witness(ctx, rendered.includes('name: "sofia-ca-config"'), "The ConfigMap source is rendered when selected");
            ctx.output(
              "Rendered NODE_EXTRA_CA_CERTS",
              selectedLines(rendered, ["NODE_EXTRA_CA_CERTS", CUSTOM_CA_PATH, "sofia-ca-config"]),
            );
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        let output = "";
        await ctx.prove("The actual migration bootstrap and an Sofia/mysql2 query connect to a strict-TLS MySQL endpoint signed by the private CA", {
          voiceover: vo[3],
          action: async () => {
          ...
          assert: async () => {
            ...
            witness(ctx, output.includes("Strict Sofia/mysql2 without custom CA: rejected"), "Strict Sofia/mysql2 connection fails without the custom CA");
            witness(ctx, output.includes("Sofia migration bootstrap completed with NODE_EXTRA_CA_CERTS and strict DATABASE_URL"), "The actual Sofia migration bootstrap completes with the mounted custom CA");
            witness(ctx, output.includes("Sofia/mysql2 strict query succeeded after migration"), "Sofia/mysql2 connects successfully after migration using strict TLS");
          ...
          action: async () => {
            const cases = [
              ...
              },
              {
                ...
                args: ["--set", "customCa.enabled=true", "--set", "customCa.existingSecret=sofia-ca-secret", "--set", "customCa.existingConfigMap=sofia-ca-config"],
                message: "customCa.existingSecret and customCa.existingConfigMap are mutually exclusive when customCa.enabled=true",
              },
              {
                name: "empty key",
                args: ["--set", "customCa.enabled=true", "--set", "customCa.existingSecret=sofia-ca-secret", "--set-string", "customCa.key="],
                message: "customCa.key is required when customCa.enabled=true",
              },
              {
                name: "Den API env conflict",
                args: ["--set", "customCa.enabled=true", "--set", "customCa.existingSecret=sofia-ca-secret", "--set", "denApi.env.NODE_EXTRA_CA_CERTS=/tmp/ca.pem"],
              ...
              },
              {
                ...
                args: ["--set", "customCa.enabled=true", "--set", "customCa.existingSecret=sofia-ca-secret", "--set", "denWeb.env.NODE_EXTRA_CA_CERTS=/tmp/ca.pem"],
              ...
              },
              {
                ...
                args: ["--set", "customCa.enabled=true", "--set", "customCa.existingSecret=sofia-ca-secret", "--set", "inference.env.NODE_EXTRA_CA_CERTS=/tmp/ca.pem"],
