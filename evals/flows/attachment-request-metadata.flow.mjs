...
function runTests(pattern) {
  return spawnSync(
    "pnpm",
    ["--filter", "@sofia/app", "exec", "bun", "test", "tests/attachment-file-part.test.ts", "--test-name-pattern", pattern],
      ...
      run: async (ctx) => {
        await ctx.prove("Automated coverage locks filename, MIME, extension, and byte preservation together", {
          ...
          assert: async () => {
            const run = spawnSync(
              "pnpm",
              ["--filter", "@sofia/app", "exec", "bun", "test", "tests/attachment-file-part.test.ts", "tests/ollama-local-provider.test.ts"],
              { cwd: ROOT, encoding: "utf8", timeout: 60_000 },
            );
