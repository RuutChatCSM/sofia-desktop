...
const MOCK_SERVER_URL = (process.env.SOFIA_EVAL_CLOUD_CONNECT_MOCK_URL ?? "http://127.0.0.1:3979")
  .trim()
  .replace(/\/+$/, "");
const TENANT_ID = "11111111-2222-3333-4444-555555555555";
const CLIENT_ID = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee";
const CLIENT_SECRET = "sofia-microsoft-mock-secret";
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        if (!state.adminSession && ctx.env.SOFIA_EVAL_DEN_TOKEN?.trim()) {
          state.adminSession = ctx.env.SOFIA_EVAL_DEN_TOKEN.trim();
        }
      ...
      run: async (ctx) => {
        await ctx.prove("The calling member completes delegated OAuth and sees their tenant plus exact approved scopes", {
          ...
          action: async () => {
            ...
            witness(ctx, Boolean(closed?.ok), "The completed OAuth popup closes before returning to Sofia.", {
        ...
        await ctx.prove("An admin enables broader capabilities without replacing credentials and connected members are clearly asked to reconnect", {
          ...
          assert: async () => {
            ...
            const file = await authenticatedApi("/v1/capabilities/microsoft-365/drive-files", {
              method: "PUT",
              body: JSON.stringify({ path: "Sofia/permission-parity.txt", content: "Microsoft 365 permission parity verified." }),
            });
            ...
            witness(
              ctx,
              draft.response.ok && draft.body.draft?.id === "draft-sofia-test"
                && event.response.ok && event.body.event?.id === "event-sofia-test"
            ...
            witness(
              ctx,
              chats.response.ok && chatId === "chat-sofia-test"
            ...
            witness(
              ...
              requests.some((request) => request.path === "/graph/v1.0/me/messages" && request.method === "POST")
                ...
                && requests.some((request) => request.path === "/graph/v1.0/me/drive/root:/Sofia/permission-parity.txt:/content" && request.method === "PUT")
                && requests.some((request) => request.path === "/graph/v1.0/chats/chat-sofia-test/messages" && request.method === "POST"),
