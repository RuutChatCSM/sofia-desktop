...
| Sofia server | 8 hours | 24 hours | Genpact |
...
async function openMarkdownArtifact(ctx) {
  const hasSession = await ctx.eval(`String(window.__sofiaControl.snapshot().route || "").includes("/session/ses_")`);
  if (!hasSession) {
    await ctx.control("session.create_task");
    await ctx.waitFor(
      `String(window.__sofiaControl.snapshot().route || "").includes("/session/ses_")`,
  ...
  const seedReady = `window.__sofiaControl.listActions().some((item) => item.id === "eval.artifact_tabs.seed_overflow" && !item.disabled)`;
