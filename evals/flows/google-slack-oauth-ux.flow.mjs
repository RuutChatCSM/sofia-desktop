...
function denWebUrl(ctx, path = "/") {
  const base = ctx.env.SOFIA_EVAL_DEN_WEB_URL.trim().replace(/\/+$/, "");
  ctx.assert(base.length > 0, "SOFIA_EVAL_DEN_WEB_URL was empty.");
  if (path.startsWith("http")) return path;
  return `${base}${path.startsWith("/") ? path : `/${path}`}`;
}

function adminSessionToken(ctx) {
  const token = ctx.env.SOFIA_EVAL_DEN_TOKEN.trim();
  ctx.assert(token.length > 0, "SOFIA_EVAL_DEN_TOKEN was empty.");
  return token;
}

function mysqlContainer(ctx) {
  return ctx.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER || "sofia-web-local-mysql";
}
...
function orgHeaders(session) {
  ...
  return { authorization: `Bearer ${session}`, "x-sofia-legacy-org-id": state.orgId };
}
...
async function signInAdminBrowserWithToken(ctx) {
  ...
  await ctx.eval(`(() => {
    ...
    localStorage.setItem('sofia:web:auth-token', token);
...
function reauthReadyScript() {
  ...
  return `(() => {
    ...
    const text = dialog?.textContent ?? '';
    const helperOk = text.includes('Sofia retries the pending action automatically');
...
async function completeVisibleReauth(ctx) {
  ...
  await ctx.eval(`window.postMessage({ type: 'sofia:reauth-complete', nonce: ${JSON.stringify(reauthState.nonce)}, error: null }, window.location.origin); true`);
}
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_TOKEN", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_DEN_MYSQL_CONTAINER"],
      ...
      run: async (ctx) => {
        await ctx.prove("A stale Google Workspace save opens the security check with a clear retry path", {
          ...
          assert: async () => {
            ...
            ctx.assert(actual.text.includes("Sofia retries the pending action automatically"), `Reauth helper missing automatic retry copy: ${actual.text}`);
          ...
          screenshot: {
            ...
            requireText: state.authProviders.includes("google")
              ? [SECURITY_MESSAGE, "SECURITY CHECK", "Sofia retries the pending action automatically", "Continue with Google"]
              : [SECURITY_MESSAGE, "SECURITY CHECK", "Sofia retries the pending action automatically"],
      ...
      run: async (ctx) => {
        await ctx.prove("Creating a Slack-style OAuth-client connection shows the exact redirect URL handoff and Copy button", {
          ...
          screenshot: {
            ...
            claim: "After creation, Sofia shows the exact redirect URL and a Copy button before teammates connect.",
