 ...
 *
 * Required env:
 * - SOFIA_EVAL_DEN_API_URL    Den API base (e.g. http://127.0.0.1:8788)
 * - SOFIA_EVAL_DEN_TOKEN      Bearer session token for the demo owner
 * - SOFIA_EVAL_WORKSPACE_PATH A folder to use as the workspace
 */
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const response = await fetch(`${apiBase}/v1/auth/desktop-handoff`, {
          method: "POST",
          headers: { authorization: `Bearer ${ctx.env.SOFIA_EVAL_DEN_TOKEN.trim()}`, "content-type": "application/json" },
          body: JSON.stringify({}),
        });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.activeOrgId') ?? '').trim())", {
          timeoutMs: 60_000,
          label: "active org",
        });
        const onWelcome = await ctx.eval("location.hash.includes('/welcome')");
        if (onWelcome) {
          await ctx.fill("input", ctx.env.SOFIA_EVAL_WORKSPACE_PATH.trim());
      ...
      run: async (ctx) => {
        await ctx.prove("The memory saved via the agent capability appears in the desktop Memory management panel.", {
          ...
          screenshot: {
            ...
            rejectText: ["Something went wrong", "Sign in to your Sofia account"],
