...
async function desktopWorkspaceList(ctx) {
  ...
  return ctx.eval("window.__SOFIA_ELECTRON__.invokeDesktop('workspaceBootstrap')", {
    awaitPromise: true,
  });
}
...
async function resetToFreshWelcome(ctx) {
  ...
  for (const workspace of list?.workspaces ?? []) {
    await ctx.eval(`(async () => {
      const info = await window.__SOFIA_ELECTRON__.invokeDesktop("sofiaServerInfo").catch(() => null);
      const baseUrl = info?.baseUrl || info?.connectUrl || localStorage.getItem("sofia.server.active");
      const token = info?.ownerToken || info?.clientToken || localStorage.getItem("sofia.server.token") || "";
      const hostToken = info?.hostToken || localStorage.getItem("sofia.server.hostToken") || "";
      if (baseUrl) {
        const headers = {};
  ...
  const cleanup = await ctx.eval(`(async () => {
    ...
    const info = await invoke("sofiaServerInfo");
    const baseUrl = info?.baseUrl || info?.connectUrl;
    if (!baseUrl) throw new Error("Sofia server URL is unavailable");
    const headers = {};
    const token = info?.ownerToken || info?.clientToken || "";
    if (token) headers.authorization = "Bearer " + token;
    if (info?.hostToken) headers["x-sofia-host-token"] = info.hostToken;
  ...
  await ctx.eval(`(() => {
    ...
    localStorage.removeItem("sofia.react.sessionByWorkspace");
  ...
  await ctx.waitForText("Welcome to Sofia", { timeoutMs: 30_000 });
}
...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("Desktop shows welcome whenever no workspaces exist", {
          ...
          screenshot: {
            ...
            requireText: ["Welcome to Sofia", "Pick a folder to get started"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        const workspaceDir = await mkdtemp(join(homedir(), ".sofia-eval-user-workspace-"));
        ctx.workspaceDir = workspaceDir;
        await ctx.prove("Sofia creates a workspace only in the chosen folder", {
          voiceover: vo[1],
          action: async () => {
      ...
      run: async (ctx) => {
        await ctx.prove("Provider and attribution onboarding run before entering the workspace", {
          ...
          action: async () => {
            ...
            await ctx.waitForText("How did you hear about Sofia?", { timeoutMs: 30_000 });
          },
          assert: async () => {
            await ctx.expectText("How did you hear about Sofia?");
          ...
          screenshot: {
            ...
            requireText: ["How did you hear about Sofia?", "Skip"],
            hashIncludes: ["/welcome"],
          },
        });
      },
    ...
    },
    {
      ...
      run: async (ctx) => {
        const emptyDir = await mkdtemp(join(homedir(), ".sofia-eval-empty-workspace-"));
        await ctx.prove("Opening an empty workspace does not create a session", {
          ...
          action: async () => {
            const created = await ctx.eval(`(async () => {
              ...
              if (token) headers.authorization = "Bearer " + token;
              if (hostToken) headers["x-sofia-host-token"] = hostToken;
              const response = await window.__SOFIA_ELECTRON__.invokeDesktop(
                "__fetch",
