...
const ADMIN_TOKEN = (process.env.SOFIA_EVAL_DEN_TOKEN ?? "").trim();
...
function mysqlQuery(sql) {
  return execFileSync("docker", ["exec", MYSQL_CONTAINER, "mysql", "-uroot", "-ppassword", "sofia_den", "-N", "-e", sql], {
    encoding: "utf8",
    maxBuffer: 1024 * 1024,
  }).trim();
}
...
export default {
  id: FLOW_ID,
  title: "SCIM groups create and safely manage Sofia teams alongside SAML",
  kind: "user-facing",
  requiresApp: false,
  preserveTheme: true,
  requiredEnv: [
  ...
  steps: [
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => withAdminBrowser(ctx, async () => {
        await ctx.prove("A SCIM membership move updates Sofia teams without another user login", {
          voiceover: vo[4],
          action: async () => {
    ...
    },
    {
      name: "cleanup",
      run: async () => {
        mysqlQuery("DELETE FROM sso_connection; DELETE FROM account WHERE provider_id LIKE 'sofia-scim-%'; DELETE FROM scim_provider;");
      },
    },
  ],
};
