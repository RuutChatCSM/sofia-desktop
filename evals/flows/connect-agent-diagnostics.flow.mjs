...
async function navigateToDebugDiagnostics(ctx) {
  ...
  const workspaceId = await ctx.eval(`(() => {
    const route = window.__sofiaControl?.snapshot?.().route ?? location.hash;
...
async function installForwardingDiagnosticsObserver(ctx) {
  await ctx.eval(`(() => {
    const previous = window.__sofiaAgentDiagnosticsEval;
    const originalFetch = previous?.originalFetch ?? window.fetch.bind(window);
    window.__sofiaAgentDiagnosticsEval = {
      originalFetch,
      requests: [],
    };
    window.fetch = async (input, init) => {
      const state = window.__sofiaAgentDiagnosticsEval;
...
async function realClickSelector(ctx, selector, label) {
  ...
  await ctx.eval(`(() => {
    const element = document.querySelector(${JSON.stringify(selector)});
    window.__sofiaFraimzClickObserved = false;
    element?.addEventListener("click", () => { window.__sofiaFraimzClickObserved = true; }, { once: true });
  ...
  const observed = await ctx.eval("window.__sofiaFraimzClickObserved === true");
  if (!observed) {
...
async function observerLog(ctx) {
  return ctx.eval(`(() => ({
    entries: (window.__sofiaAgentDiagnosticsEval?.requests ?? []).map((entry) => ({ ...entry })),
  }))()`);
}
      ...
      run: async (ctx) => {
        await ctx.prove("Run agent diagnostics forwards one real bounded server request without direct mutation calls", {
          ...
          screenshot: {
            ...
            claim: "Settings Debug displays the report returned by the real Sofia diagnostics route.",
      ...
      run: async (ctx) => {
        await ctx.prove("The real report distinguishes effective engine evidence from configured fallback intent", {
          ...
          assert: async () => {
            ...
            ctx.assert(report.agent.configuredSofiaAgent.state === "present", "The reported Sofia agent was absent or disabled.");
            ctx.assert(report.agent.configuredSofiaAgent.mode === "primary", `Unexpected agent mode: ${report.agent.configuredSofiaAgent.mode}`);
            ctx.assert(
              Object.values(report.agent.configuredSofiaAgent.prompt.markers).every(Boolean),
              `Required context markers were absent: ${JSON.stringify(report.agent.configuredSofiaAgent.prompt.markers)}`,
            );
            ctx.assert(
              report.agent.pluginLabels.includes("sofia-extensions-preview"),
            ...
            ctx.assert(
              rendered.report.includes(effective ? "Effective Sofia agent" : "Configured Sofia agent"),
            ...
            ctx.assert(rendered.plugins.includes("sofia-extensions-preview"), "Connect plugin evidence was not rendered.");
            ctx.assert(
          ...
          screenshot: {
            ...
            requireText: ["Sofia agent", "search_capabilities", "execute_capability", "sofia-extensions-preview", "MCP inventory"],
        ...
        await ctx.prove("Cloud and organization evidence exactly mirror the real active-or-fail-closed response", {
          ...
          assert: async () => {
            ...
            if (performed && cloudCheck.code === "cloud_catalog_exact_match") {
              const runtimeCloud = report.mcps.find((mcp) => mcp.source === "config.remote" && mcp.name === "sofia-cloud");
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Copy report exports the exact sanitized server response with actionable ownership", {
          ...
          screenshot: {
            ...
            requireText: ["Sofia Cloud tool catalog", "Owner", "Recommended action"],
