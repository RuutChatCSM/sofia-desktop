 ...
 * (window.__sofia.record("analytics.<event>")), so this flow asserts
  ...
  steps: [
    {
      name: "App booted",
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__sofiaControl) && Boolean(window.__sofia)", {
          timeoutMs: 60_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__sofia.events(200).some((e) => e.name === 'analytics.app_opened')",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__sofiaControl.listActions().some((a) => a.id === 'session.create_task')",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__sofia.events(200).some((e) => e.name === 'analytics.task_created')",
          { timeoutMs: 30_000, label: "analytics.task_created in inspector" },
        );
        const events = await ctx.eval(
          "window.__sofia.events(200).filter((e) => e.name.startsWith('analytics.')).map((e) => e.name)",
        );
