...
const ADMIN_PASSWORD = "SofiaDemo123!";
...
function orgSettingsUrl(ctx) {
  return `${ctx.env.SOFIA_EVAL_DEN_WEB_URL.replace(/\/$/, "")}${ORG_SETTINGS_PATH}`;
}
...
async function ensureDesktopSession(ctx) {
  ...
  await ctx.eval(`(() => {
    localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(process.env.SOFIA_EVAL_DEN_WEB_URL)});
    localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(process.env.SOFIA_EVAL_DEN_API_URL)});
    localStorage.setItem('sofia.den.authToken', ${JSON.stringify(process.env.SOFIA_EVAL_DEN_TOKEN)});
    localStorage.setItem('sofia.den.activeOrgId', ${JSON.stringify(activeOrg.id)});
    localStorage.setItem('sofia.den.activeOrgSlug', ${JSON.stringify(activeOrg.slug ?? "example-corp")});
    localStorage.setItem('sofia.den.activeOrgName', 'Example Corp');
    window.dispatchEvent(new CustomEvent('sofia-den-settings-changed', { detail: {} }));
    window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: { token: ${JSON.stringify(process.env.SOFIA_EVAL_DEN_TOKEN)} } }));
    return true;
  })()`);
  ...
  await panelEval(ctx, `(() => {
    ...
    window.fetch = async (...args) => {
      ...
      if (method === 'POST' && url.includes('/api/den/v1/org/brand-assets')) {
        ...
        window.__sofiaBrandReauthIntercepted = true;
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 30_000, label: "window.__sofiaControl" });
          ...
          assert: async () => {
            const state = await panelEval(ctx, `(() => {
              ...
              return {
                ...
                interceptedReauth: window.__sofiaBrandReauthIntercepted === true,
        ...
        await ctx.prove("Verifying once resumes the queued brand upload without reselecting files", {
          ...
          assert: async () => {
            ...
            for (const asset of [firstAssets.logo, firstAssets.icon]) {
              ...
              ctx.assert(assetUrl.origin === new URL(ctx.env.SOFIA_EVAL_DEN_API_URL).origin, `Asset escaped the Den origin: ${asset.url}`);
      ...
      run: async (ctx) => {
        await ctx.prove("A member desktop loads both brand URLs only from its Den", {
          ...
          assert: async () => {
            ...
            const iconState = await ctx.eval("window.__SOFIA_ELECTRON__?.brandIcon?.getState?.()", { awaitPromise: true });
            const denOrigin = new URL(ctx.env.SOFIA_EVAL_DEN_API_URL).origin;
