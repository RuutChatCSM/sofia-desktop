...
function runMysql(sql) {
  const container = process.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER?.trim() || "sofia-web-local-mysql";
  execFileSync("docker", ["exec", container, "mysql", "-uroot", "-ppassword", "sofia_den", "-e", sql], { stdio: "ignore" });
}
...
async function readMemberHome(ctx) {
  return ctx.eval(`(() => {
    ...
    return {
      ...
      sofiaModels: text.includes("Sofia Models"),
      ...
      run: async (ctx) => {
        await ctx.prove("Riley's member home opens on the real sections with no Available resources strip", {
          ...
          assert: async () => {
            ...
            witness(ctx, actual.yourWorkspace === true && actual.llmProviders === true && actual.sofiaModels === true && actual.marketplaces === true && actual.plugins === true, "The real sections are still on the page", actual);
          },
          screenshot: {
            name: "member-home-no-overview",
            requireText: ["Your workspace", "LLM providers", "Sofia Models", "Marketplaces", "Plugins"],
      ...
      run: async (ctx) => {
        await ctx.prove("Empty states stay inside LLM providers and the other real sections", {
          ...
          assert: async () => {
            ...
            witness(ctx, actual.sofiaModels === true && actual.marketplaces === true && actual.plugins === true, "The real sections still describe what Riley can use", actual);
          },
          screenshot: {
            name: "member-sections-without-summary",
            requireText: ["LLM providers", "Sofia Models"],
            rejectText: ["Available resources", "Assigned directly to you", "Something went wrong"],
            hashIncludes: "/dashboard",
          },
        });
