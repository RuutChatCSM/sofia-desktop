 ...
 * - a standard win-x64 desktop artifact available through SOFIA_INSTALLER_ARTIFACTS_DIR
 *
...
const MEMBER_EMAIL = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "riley.downloads@acme.test";
...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("The admin dashboard offers Acme's configured installer instead of public release assets", {
          ...
          action: async () => {
            ...
            await ctx.waitForText(`Download Sofia for ${ORGANIZATION_NAME}`, { timeoutMs: 30_000 });
          },
          assert: async () => {
            await ctx.expectText(`Download Sofia for ${ORGANIZATION_NAME}`);
            ...
            const evidence = await ctx.eval(`(() => {
              ...
              return {
                ...
                githubRequests: performance.getEntriesByType('resource').map((entry) => entry.name).filter((url) => url.includes('api.github.com/repos/RuutChatCSM/sofia-desktop/releases')),
              };
            })()`);
          ...
          screenshot: {
            ...
            requireText: [`Download Sofia for ${ORGANIZATION_NAME}`, "Download for this workspace"],
      ...
      run: async (ctx) => {
        await ctx.prove("Alex reaches an Acme install page that clearly requires sign-in", {
          ...
          screenshot: {
            ...
            requireText: [`Download Sofia for ${ORGANIZATION_NAME}`, "Run it, then sign in", "Download for"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("An ordinary member sees the download action without admin distribution controls", {
          ...
          screenshot: {
            ...
            requireText: ["WORKSPACE MEMBER", `Download Sofia for ${ORGANIZATION_NAME}`, "Download for this workspace"],
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Riley receives the configured Windows installer from Acme's Sofia server", {
          voiceover: vo[3],
          action: async () => {
          ...
          assert: async () => {
            ...
            witness(ctx, disposition.includes("sofia-win-x64-"), "The Windows filename is the standard desktop installer asset", disposition);
          ...
          screenshot: {
            ...
            requireText: [`Download Sofia for ${ORGANIZATION_NAME}`, "Download for Windows", "Run it, then sign in"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("Possessing the installer config does not grant Acme workspace access", {
          ...
          screenshot: {
            ...
            requireText: [`Download Sofia for ${ORGANIZATION_NAME}`, "Run it, then sign in"],
