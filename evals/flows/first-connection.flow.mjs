...
const DEN_API_URL = cleanBaseUrl(process.env.SOFIA_EVAL_DEN_API_URL);
const DEN_WEB_URL = cleanBaseUrl(process.env.SOFIA_EVAL_DEN_WEB_URL);
...
const MEMBER_EMAIL = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || `riley.first.connection+${RUN_TAG}@acme.test`;
const MEMBER_PASSWORD = process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || "SofiaDemo123!";
const BOOTSTRAP_PATH = process.env.SOFIA_EVAL_BOOTSTRAP_PATH?.trim()
  || path.join(makeTempDir("sofia-first-connection-bootstrap-"), "desktop-bootstrap.json");
  ...
  steps: [
      ...
      run: async (ctx) => {
        ...
        await withClient(ctx, ADMIN_CDP_URL, async () => {
          await ctx.prove("Alex copies a workspace install link from the dashboard and the token resolves to Acme's required sign-in config", {
            voiceover: vo[0],
            // "On the Sofia dashboard home, the admin clicks Download for this workspace"
            action: async () => {
      ...
      run: async (ctx) => {
        ...
        await withClient(ctx, INVITEE_CDP_URL, async () => {
          await ctx.prove("Riley opens the install link and sees Acme's three-step checklist with the same link pinned for installer fallback", {
            ...
            assert: async () => {
              await ctx.expectText("Download Sofia for Acme Robotics");
            ...
            screenshot: {
              ...
              requireText: ["Download Sofia for Acme Robotics", "Download the Sofia installer", "Open the installer and paste this link:", "Waiting for sign-in"],
            },
          });
        ...
        await withClient(ctx, INVITEE_CDP_URL, async () => {
          await ctx.prove("The Acme install download redirects to the standard desktop release asset", {
            ...
            screenshot: {
              ...
              requireText: ["Download Sofia for Acme Robotics", "Open the installer and paste this link:", "Waiting for sign-in"],
            },
          });
        }, { targetId: state.installPageTargetId });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("A plain first-run desktop asks whether to use Sofia Cloud or join an organization, and pasting Acme's link binds it to Acme's server", {
          voiceover: vo[3],
          // "Suppose someone skips all that and installs the plain Sofia app instead: "
          action: async () => {
            ...
            await ctx.waitForText("Sign in to Sofia", { timeoutMs: 60_000 });
          },
          assert: async () => {
            await ctx.expectText("Sign in to Sofia");
            ...
            await ctx.screenshot("plain-desktop-join-org-paste-forced-signin", {
              claim: "A plain first-run desktop asks whether to use Sofia Cloud or join an organization, and pasting Acme's link binds it to Acme's server.",
              voiceover: vo[4],
              requireText: ["Welcome to Sofia", "Sign in to Sofia", `Connected to ${serverHost}`],
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Desktop sign-in completes through Acme's browser handoff, while a later sign-in link for another server asks before switching", {
          ...
          action: async () => {
            ...
            await clickExactText(ctx, "Sign in to Sofia", "button");
            ...
            await withClient(ctx, INVITEE_CDP_URL, async () => {
              ...
              state.copiedDesktopUrl = await ctx.waitFor(
                "typeof window.__capturedSignin === 'string' && window.__capturedSignin.startsWith('sofia://den-auth') && window.__capturedSignin",
                { timeoutMs: 45_000, label: "captured browser-minted Sofia sign-in link" },
              );
              state.copiedDesktopGrant = new URL(state.copiedDesktopUrl).searchParams.get("grant") ?? "";
              witness(ctx, state.copiedDesktopGrant.length > 0, "The browser-minted Sofia URL carries a handoff grant", redactUrlParam(state.copiedDesktopUrl, "grant"));
            }, { targetId: state.authTargetId });

            useDesktopClient(ctx);
            await deliverDeepLinkToDesktop(ctx, requireStateValue(state.copiedDesktopUrl, "browser-minted Sofia sign-in URL"));
            await ctx.waitFor("(localStorage.getItem('sofia.den.activeOrgName') ?? '').includes('Acme Robotics')", {
              timeoutMs: 60_000,
          ...
          assert: async () => {
            ...
            witness(ctx, (await ctx.eval("localStorage.getItem('sofia.den.activeOrgName') ?? ''")).includes("Acme Robotics"), "Cancel leaves the active organization as Acme Robotics", await ctx.eval("localStorage.getItem('sofia.den.activeOrgName') ?? ''"));
          ...
          screenshot: {
            ...
            requireText: ["Sofia Cloud", "Acme Robotics", "Sign out"],
      ...
      run: async (ctx) => {
        await withClient(ctx, INVITEE_CDP_URL, async () => {
          await ctx.prove("The browser handoff and original install page both flip to Connected for Acme Robotics", {
            voiceover: vo[5],
            // "Back on the install page, step three flips to Connected — Sofia is set up"
            action: async () => {
              await withClient(ctx, INVITEE_CDP_URL, async () => {
            ...
            assert: async () => {
              ...
              await ctx.expectText("Sofia is set up for Acme Robotics");
            ...
            screenshot: {
              ...
              requireText: ["Download the Sofia installer", "Open the installer and paste this link:", "Connected", "Sofia is set up for Acme Robotics"],
            },
          });
        }, { targetId: state.installPageTargetId });
      },
...
async function ensureAdminToken(ctx) {
  ...
  const token = process.env.SOFIA_EVAL_DEN_TOKEN?.trim() ?? "";
  ctx.assert(token.length > 0, `Admin sign-in failed and SOFIA_EVAL_DEN_TOKEN is missing: ${signedIn.response.status}`);
...
function markEmailVerified(ctx, email) {
  ctx.assert(
    ...
    "Invitation acceptance requires a verified email; set SOFIA_EVAL_MARK_VERIFIED_CMD (shell template with {email}).",
  );
...
async function signInOnCurrentDenWebPage(ctx, email, password, { captureDesktopHandoff = false } = {}) {
  await ctx.waitFor(
    `document.body.innerText.includes('Sign in')
      || document.body.innerText.includes('Start using Sofia')
  ...
  if (captureDesktopHandoff) {
    await ctx.waitFor(
      "typeof window.__capturedSignin === 'string' && window.__capturedSignin.startsWith('sofia://den-auth')",
  ...
  await ctx.eval(`(() => {
    ...
    window.fetch = async (...args) => {
      ...
      if (requestUrl.includes('/v1/auth/desktop-handoff')) {
        try {
          ...
          if (typeof payload?.sofiaUrl === 'string') window.__capturedSignin = payload.sofiaUrl;
        } catch {}
      }
      return response;
    };
    try {
      ...
      Object.defineProperty(window.location, 'assign', {
        configurable: true,
        value(url) {
          if (String(url).startsWith('sofia://')) {
...
async function ensureDesktopReady(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "desktop control API" });
  await ctx.waitFor("Boolean(window.__SOFIA_ELECTRON__?.invokeDesktop)", { timeoutMs: 60_000, label: "desktop bridge" });
}

async function invokeDesktop(ctx, command, input) {
  await ensureDesktopReady(ctx);
  return ctx.eval(`window.__SOFIA_ELECTRON__.invokeDesktop(${JSON.stringify(command)}, ${JSON.stringify(input ?? null)})`, { awaitPromise: true });
}
...
async function stubDesktopExternalOpenCapture(ctx) {
  await ctx.eval(`(() => {
    window.__capturedBrowserSigninUrl = '';
    window.__SOFIA_ELECTRON__ = window.__SOFIA_ELECTRON__ || {};
    window.__SOFIA_ELECTRON__.shell = window.__SOFIA_ELECTRON__.shell || {};
    window.__SOFIA_ELECTRON__.shell.openExternal = async (url) => {
...
async function deliverDeepLinkToDesktop(ctx, sofiaUrl) {
  await ctx.eval(`(() => {
    ...
    window.dispatchEvent(new CustomEvent('sofia:deep-link-native', { detail: [url] }));
...
async function fetchAndVerifyMacInstallerRedirect(ctx) {
  ...
  const fileName = `sofia-mac-arm64-${version}.dmg`;
  const expectedLocation = `https://github.com/RuutChatCSM/sofia-desktop/releases/download/${encodeURIComponent(releaseTag)}/${encodeURIComponent(fileName)}`;
...
function buildMismatchedDenAuthUrl() {
  const url = new URL("sofia://den-auth");
