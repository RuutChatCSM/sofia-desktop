...
export function enterpriseOrgName(ctx) {
  return envText(ctx, "SOFIA_EVAL_ENTERPRISE_ORG_NAME") || "Example Organization";
}
...
export function denApiBase(ctx) {
  const value = cleanBase(envText(ctx, "SOFIA_EVAL_DEN_API_URL"));
  ctx.assert(Boolean(value), "Missing SOFIA_EVAL_DEN_API_URL for the enterprise eval flow.");
  return value;
}

export function denWebBase(ctx) {
  const value = cleanBase(envText(ctx, "SOFIA_EVAL_DEN_WEB_URL"));
  ctx.assert(Boolean(value), "Missing SOFIA_EVAL_DEN_WEB_URL for the enterprise desktop handoff deep link.");
  return value;
}
...
export function timeoutMs(ctx, envName, fallback) {
  const raw = envText(ctx, envName) || envText(ctx, "SOFIA_EVAL_ENTERPRISE_TASK_TIMEOUT_MS");
...
export async function signInByEmail(ctx, email) {
  const result = await denApiFetch(ctx, "/api/auth/sign-in/email", {
    method: "POST",
    body: JSON.stringify({ email, password: envText(ctx, "SOFIA_EVAL_ENTERPRISE_PASSWORD") || DEFAULT_PASSWORD }),
  });
...
export async function createDesktopHandoff(ctx, token) {
  ...
  const sofiaUrl = result.body?.sofiaUrl;
  ctx.assert(typeof sofiaUrl === "string" && sofiaUrl.length > 0, "Desktop handoff response did not include sofiaUrl.");
  const url = new URL(sofiaUrl);
  url.searchParams.set("denBaseUrl", denWebBase(ctx));
  return url.toString();
}

export async function configureDesktopForDen(ctx) {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 120_000, label: "Sofia control API" });
  ...
  const result = await ctx.eval(`(async () => {
    ...
    localStorage.setItem("sofia.den.baseUrl", ${JSON.stringify(webBase)});
    localStorage.setItem("sofia.den.apiBaseUrl", ${JSON.stringify(apiBase)});
    let prefs = {};
    try { prefs = JSON.parse(localStorage.getItem("sofia.preferences") || "{}"); } catch {}
...
export async function resetDesktopDenSession(ctx) {
  await ctx.eval(`(() => {
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith("sofia.den.mcp")) localStorage.removeItem(key);
    }
    for (const key of ["sofia.den.authToken", "sofia.den.activeOrgId", "sofia.den.activeOrgSlug", "sofia.den.activeOrgName"]) {
...
export async function deliverDesktopDeepLink(ctx, sofiaUrl) {
  const webBase = denWebBase(ctx);
  await ctx.eval(`(() => {
    ...
    window.__SOFIA__.deepLinks = [...(window.__SOFIA__.deepLinks || []), url];
    window.dispatchEvent(new CustomEvent("sofia:deep-link", { detail: { urls: [url], denBaseUrl: ${JSON.stringify(webBase)} } }));
    return true;
  })()`);
}

async function waitForDesktopDenToken(ctx, sofiaUrl) {
  try {
    await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 60_000, label: "desktop Den token" });
  } catch (error) {
    const diagnostics = await ctx.eval(`(() => ({
      authToken: Boolean((localStorage.getItem("sofia.den.authToken") ?? "").trim()),
      baseUrl: localStorage.getItem("sofia.den.baseUrl") || "",
      apiBaseUrl: localStorage.getItem("sofia.den.apiBaseUrl") || "",
    ...
    const redactedUrl = sofiaUrl.replace(/([?&]grant=)[^&]+/, "$1<redacted>");
...
export async function desktopHandoffSignIn(ctx, email) {
  ...
  const sofiaUrl = await createDesktopHandoff(ctx, token);
  await deliverDesktopDeepLink(ctx, sofiaUrl);
  await waitForDesktopDenToken(ctx, sofiaUrl);
  await completeEnterpriseOrgOnboarding(ctx);
  await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.activeOrgId') ?? '').trim())", { timeoutMs: 60_000, label: "desktop active organization" });
  return token;
}

function localServerExpr() {
  return `(() => {
    const urlOverride = (localStorage.getItem("sofia.server.urlOverride") || "").trim();
    const port = (localStorage.getItem("sofia.server.port") || "").trim();
    const token = (localStorage.getItem("sofia.server.token") || "").trim();
    const hostToken = (localStorage.getItem("sofia.server.hostToken") || "").trim();
    const base = urlOverride.replace(/\\/+$/, "") || (port ? "http://127.0.0.1:" + port : "");
    return { base, token, hostToken };
  })()`;
}

export async function waitForSofiaConnectReady(ctx, timeout = 90_000) {
  let last = null;
  ...
  while (Date.now() < deadline) {
    ...
    last = await ctx.eval(`(() => {
      const text = document.body.innerText || "";
      const match = text.match(/Sofia Connect: (Ready|Checking|Needs attention)/);
      return { ready: text.includes("Sofia Connect: Ready"), status: match?.[0] || "", hash: window.location.hash };
    })()`);
  ...
  throw new Error(`Sofia Connect did not become Ready in the status bar within ${timeout}ms: ${JSON.stringify(last)}`);
}
...
export async function ensureLocalWorkspace(ctx, folderPath) {
  await ctx.waitFor(`(() => { const s = ${localServerExpr()}; return Boolean(s.base && s.token); })()`, { timeoutMs: 60_000, label: "local Sofia server URL/token" });
  let last = null;
...
async function clickThroughWorkspaceOnboarding(ctx, folderPath) {
  ...
  for (const text of ["Skip and use the free model", "Continue without Sofia Models", "Skip"]) {
...
export async function readTranscriptSnapshot(ctx) {
  return ctx.eval(`(async () => {
    ...
    const substantive = (value) => {
      ...
      return text.length > 0 && text !== "Sofia";
    };
    const bodyText = document.body.innerText || "";
    let transcript = null;
    try {
      const result = await window.__sofiaControl?.execute?.("session.read_transcript", { count: 30 });
...
function gatewayLoginUrl(ctx, transcript, desiredUser) {
  ...
  const gatewayBase = cleanBase(envText(ctx, "SOFIA_EVAL_ENTERPRISE_GATEWAY_URL"));
...
export async function completeGatewayLogin(ctx, email, transcript, gatewayUserEnvName) {
  ...
  ctx.assert(Boolean(loginUrl), "Gateway requested login but no login URL was visible. Set SOFIA_EVAL_ENTERPRISE_GATEWAY_URL or expose the gateway login link in the transcript.");
...
export async function retryAfterGatewayLoginIfNeeded(ctx, email, transcript, expectedText, retryPrompt, options = {}) {
  ...
  const gatewayUserEnvName = options.gatewayUserEnvName ?? "SOFIA_EVAL_ENTERPRISE_GATEWAY_USER";
  const desiredUser = normalizeGatewayPrincipal(envText(ctx, gatewayUserEnvName) || email);
