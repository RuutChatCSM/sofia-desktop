    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Attention notifications use text that reflects the triggering event", {
          voiceover: vo[2],
          action: async () => {
            await ctx.eval(`window.__SOFIA_ELECTRON__?.invokeDesktop?.("desktopNotificationShow", {
          ...
          assert: async () => {
            const bridgeAvailable = await ctx.eval(`Boolean(window.__SOFIA_ELECTRON__?.invokeDesktop)`);
