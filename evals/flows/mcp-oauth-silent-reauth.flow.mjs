 ...
 *   2. The connector's API goes down and Sofia "reopens" (engine
...
async function restartEngine(ctx) {
  await ctx.eval(
    'window.__SOFIA_ELECTRON__.invokeDesktop("engineRestart", {})',
    { awaitPromise: true },
  );
  ctx.log("Engine restarted (simulates quitting and reopening Sofia).");
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        if (!(await cardStatus(ctx))) {
          ...
          await ctx.fill('input[placeholder="Paste the OAuth client ID"]', "sofia-eval-preregistered-client");
    ...
    {
      name: "Token expires and the API is unreachable while Sofia reopens",
      run: async (ctx) => {
    ...
    {
      name: "API returns; Sofia silently re-authenticates with the stored refresh token",
      run: async (ctx) => {
