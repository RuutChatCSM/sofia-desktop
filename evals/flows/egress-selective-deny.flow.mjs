...
async function fetchDenied(lab) {
  ...
  url.searchParams.set("url", "https://github.com/RuutChatCSM/sofia-desktop/releases/latest");
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        let denied;
        let manifest;
        await ctx.prove("github.com is blocked by policy, the lab error names the host, and Sofia product diagnostics classify the HTTP 451 allowlist deny", {
          voiceover: vo[0],
          action: async () => {
