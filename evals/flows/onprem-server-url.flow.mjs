...
async function currentDenBaseUrls(ctx) {
  return ctx.eval(`(() => ({
    baseUrl: localStorage.getItem("sofia.den.baseUrl") || ${JSON.stringify(DEFAULT_DEN_BASE_URL)},
    apiBaseUrl: localStorage.getItem("sofia.den.apiBaseUrl") || ${JSON.stringify(DEFAULT_DEN_API_BASE_URL)},
  }))()`);
}
...
async function resetToDefaultWelcome(ctx) {
  ...
  await ctx.waitForText("Using Sofia on-premises?", { timeoutMs: 60_000 });
}
  ...
  if (onWelcome) {
    ...
    if (hasManualFolder) {
      ...
      const state = await ctx.waitFor(`(() => {
        ...
        if (text.includes("How did you hear about Sofia?")) return "attribution";
        if (location.hash.includes("/workspace/") || location.hash.includes("#/session")) return "done";
        if (text.includes("Sofia server is unavailable") || text.includes("Failed to create workspace")) return "fallback";
        return null;
      ...
      const attribution = await ctx.waitFor(`(() => {
        const text = document.body.innerText;
        if (text.includes("How did you hear about Sofia?")) return "ready";
...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("The fresh welcome screen offers an on-premises server link under Get started", {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Using Sofia on-premises?");
            await ctx.expectNoText(`Connected to ${ORG_HOST}`);
          },
          screenshot: {
            name: "frame-1",
            requireText: ["Pick a folder to get started", "Using Sofia on-premises?"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The on-premises link opens a focused organization server dialog", {
          ...
          action: async () => {
            await closeDialogs(ctx);
            await ctx.clickText("Using Sofia on-premises?");
            await ctx.fill('input[placeholder="https://sofia.yourcompany.com"]', ORG_URL);
          },
          assert: async () => {
      ...
      run: async (ctx) => {
        await ctx.prove("Saving the URL leaves a connected organization server state on welcome", {
          ...
          assert: async () => {
            ...
            const stored = await ctx.eval(`localStorage.getItem("sofia.den.baseUrl")`);
          ...
          screenshot: {
            ...
            rejectText: ["Using Sofia on-premises?", "Something went wrong"],
          },
        });
      },
    },
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("Reset returns to standard Sofia Cloud and the welcome link state", {
          voiceover: vo[4],
          action: async () => {
            await assertAdvancedOrganizationServerFirst(ctx);
            await ctx.clickText("Reset", { selector: "[data-section] button" });
            await ctx.waitForText("Using standard Sofia Cloud.", { timeoutMs: 30_000 });
          ...
          screenshot: {
            name: "frame-5",
            requireText: ["Using Sofia on-premises?"],
            rejectText: [`Connected to ${ORG_HOST}`, "Something went wrong"],
          },
        });
      },
        ...
        try {
          await ctx.prove("The forced sign-in gate exposes the same on-premises server option without developer mode", {
            ...
            action: async () => {
              ...
              await ctx.waitForText("Sign in with Sofia Cloud", { timeoutMs: 60_000 });
