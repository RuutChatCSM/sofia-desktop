...
async function denFetch(ctx, path, options = {}) {
  const base = ctx.env.SOFIA_EVAL_DEN_API_URL.replace(/\/$/, "");
...
async function findPanelTarget(ctx) {
  ...
  const denHost = new URL(ctx.env.SOFIA_EVAL_DEN_WEB_URL).host;
...
async function memberRefresh(ctx) {
  await ctx.eval(`(() => {
    ...
    window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: {} }));
    return true;
  })()`);
...
async function getBrandIconState(ctx) {
  return ctx.eval("window.__SOFIA_ELECTRON__?.brandIcon?.getState?.()", { awaitPromise: true });
}
...
async function daytonaExec(ctx, label, script) {
  ...
  if (!sandbox) {
    ctx.log(`Skipping Daytona ${label}: SOFIA_EVAL_DAYTONA_SANDBOX is not set.`);
    return null;
  }
  try {
...
set -euo pipefail
for candidate in "$HOME/.config"/com.differentai.sofia* "$HOME/.config"/*Sofia* "$HOME/.config"/*sofia*; do
  if [ -d "$candidate" ] && [ -f "$candidate/brand-icon.png" ]; then
    printf '%s\n' "$candidate/brand-icon.png"
    exit 0
  fi
done
printf 'brand-icon.png not found under ~/.config sofia dirs\n' >&2
exit 1
`);
  if (result) {
...
printf 'brand-icon.png absent from sofia config dirs\n'
`);
  if (result) {
...
export DISPLAY="\${SOFIA_EVAL_DISPLAY:-:99}"
window_id=""
for candidate in $(xprop -root _NET_CLIENT_LIST 2>/dev/null | grep -o '0x[0-9a-f]*'); do
  if xprop -id "$candidate" WM_NAME 2>/dev/null | grep -qi sofia; then
    window_id="$candidate"
  fi
done
if [ -z "$window_id" ]; then
...
async function ensureWorkspaceReady(ctx) {
  const workspacePath = ctx.env.SOFIA_EVAL_WORKSPACE_PATH?.trim() || "/workspace";
...
`).catch((error) => {
              ...
              await ctx.waitFor(
                "window.__sofiaControl.listActions().some((action) => action.id === 'eval.app.relaunch' && !action.disabled)",
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl)", {
              timeoutMs: 60_000,
              label: "window.__sofiaControl after relaunch",
            });
