...
const EXA_API_KEY = process.env.SOFIA_EVAL_EXA_API_KEY?.trim() || "exa-eval-placeholder-key";
          ...
          assert: async () => {
            ...
            if (HAS_REAL_EXA_API_KEY) {
              const execute = await mcpAgentCall(mcpToken, "tools/call", {
                ...
                arguments: {
                  name: exaToolName,
                  body: { query: "Sofia AI agents", numResults: 1 },
                },
              }, ctx);
              ...
              ctx.log("Skipping real Exa web_search_exa execution because real execution requires SOFIA_EVAL_EXA_API_KEY; placeholder keys only validate create and tool discovery.");
            }
