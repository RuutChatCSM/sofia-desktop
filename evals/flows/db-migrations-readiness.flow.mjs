...
const DEFAULT_DATABASE_URL = "mysql://root:password@127.0.0.1:3306/sofia_den";
...
function readDatabaseUrl() {
  return process.env.SOFIA_EVAL_DATABASE_URL?.trim() || process.env.DATABASE_URL?.trim() || DEFAULT_DATABASE_URL;
}
...
function temporaryDatabaseName() {
  return `sofia_eval_db_migrations_${Date.now()}_${randomUUID().replaceAll("-", "").slice(0, 8)}`;
}
      ...
      run: async (ctx) => {
        await ctx.prove("Dockerfile.den builds Den DB dist assets with a deterministic current-schema snapshot", {
          ...
          assert: async () => {
            ...
            witness(ctx, denApiPackage.includes('"build:den-db": "pnpm --filter @sofia-ee/den-db build"'), "hosted Den API build exposes the den-db build step");
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The production Helm migration path invokes node dist only, with no deploy-time TypeScript toolchain", {
          voiceover: vo[1],
          assert: async () => {
            const values = await readFile(join(ROOT, "packaging", "helm", "sofia-ee", "values.yaml"), "utf8");
      ...
      run: async (ctx) => {
        await ctx.prove("The precompiled runner finishes by repairing required FULLTEXT indexes after migration work", {
          ...
          assert: async () => {
            ...
            witness(ctx, migrateWorkflow.includes("run_with_ddl_retry pnpm --filter @sofia-ee/den-db db:migrate"), "Den DB Migrate explicitly invokes db:migrate");
            witness(ctx, tests.includes("schema snapshot contains SQL only"), "focused tests cover build artifact wiring without a live DB");
            rejectForbiddenDeployTools(ctx, sliceBetween(await readFile(join(ROOT, "packaging", "helm", "sofia-ee", "values.yaml"), "utf8"), "migrations:\n", "\ningress:"), "Final Helm migration path");
