...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The Browse the web icon src is a relative path rebased by resolveExtensionIconSrc, not a bare absolute /sofia-mark.png", {
          voiceover: vo[1],
          action: async () => {
            const result = await ctx.eval(`(() => {
              const buttons = Array.from(document.querySelectorAll("button"));
          ...
          assert: async () => {
            ...
            ctx.assert(!result.src.startsWith("/sofia-mark"), `Icon src is absolute "${result.src}" — should be rebased to relative.`);
            ctx.assert(result.src.includes("sofia-mark.png"), `Icon src "${result.src}" does not reference sofia-mark.png.`);
          },
        });
      },
    },
