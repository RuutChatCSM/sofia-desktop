...
function mysqlContainer() {
  return process.env.SOFIA_EVAL_DEN_MYSQL_CONTAINER?.trim() || "sofia-web-local-mysql";
}
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The dashboard keeps the workspace download while removing the extensions promotion", {
          ...
          screenshot: {
            name: "focused-dashboard",
            requireText: ["Dashboard", "Download Sofia", "Download for this workspace"],
      ...
      run: async (ctx) => {
        await ctx.prove("An ordinary member gets a compact resource dashboard with no admin controls", {
          ...
          screenshot: {
            ...
            requireText: ["Your workspace", "Sofia Models", "Marketplaces", "Plugins"],
        ...
        await ctx.prove("Stripe presents one truthful billing state with clear prices, counts, and actions", {
          ...
          assert: async () => {
            const actual = await ctx.eval(`(() => ({
              ...
              hasModels: document.body.innerText.includes('Sofia Models'),
            }))()`);
          ...
          screenshot: {
            ...
            requireText: ["Stripe", "Included users", "Active users", "Billable users", "Sofia Models", "Refresh"],
