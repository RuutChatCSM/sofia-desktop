/**
 * After cloud sign-in, the Den cloud MCP ("Sofia Cloud Control") is
 ...
 * - SOFIA_EVAL_DEN_API_URL    Den API base, e.g. http://127.0.0.1:8790
...
export default {
  ...
  steps: [
    ...
    {
      name: "Sofia Cloud Control is hidden by default and revealed as a configured app",
      run: async (ctx) => {
        ...
        await ctx.screenshot("cloud-mcp-configured", {
          claim: "Sofia Cloud Control is auto-configured while hidden by default, then appears after Show hidden.",
          voiceover: "After signing in to Sofia Cloud the connection is already configured and enabled, and it only shows up once the user reveals hidden extensions.",
          requireText: ["Sofia Cloud Control", "Showing hidden"],
