...
const LONG_TITLE = "Sofia diagnostics authorization with a deliberately long session title";
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        if (choosingModel || surveySkip) {
          ...
          await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label: "control API after onboarding" });
        }
        ...
        const sessionId = await ctx.waitFor(`(() => {
          const match = /session\\/([^/?#]+)/.exec(window.__sofiaControl.snapshot().route);
        ...
        await ctx.prove("A long active session title truncates cleanly beside its left spinner", {
          ...
          screenshot: {
            ...
            requireText: ["Sofia diagnostics authorization"],
          },
        });
      },
    },
