...
const UI_STATE_KEY = "sofia:ui-state:v1";
      ...
      run: async (ctx) => {
        await ctx.prove("Clicking the Extensions rail button opens the panel next to the chat", {
          ...
          screenshot: {
            ...
            rejectText: ["Use Sofia Models without API keys"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Reloading the app returns to the same task without restoring the side panel", {
          ...
          action: async () => {
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl) && !window.__sidePanelReloadSentinel", {
              timeoutMs: 60_000,
              label: "control API after reload",
            });
            await waitForRenderedBody(ctx);
            const routeHasSession = await ctx.eval(
              `window.__sofiaControl.snapshot().route.includes(${JSON.stringify(sessionId)})`,
            );
            if (!routeHasSession) {
              await ctx.waitFor(
                `window.__sofiaControl.listActions().some((a) => a.id === "session.open" && !a.disabled)`,
