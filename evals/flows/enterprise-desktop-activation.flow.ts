  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        const distribution = await ctx.eval(
          "window.__SOFIA_ELECTRON__?.meta?.distribution ?? null",
    ...
    },
    {
      ...
      run: async (ctx) => {
        const result = await ctx.eval(`(async () => {
          try {
            await window.__SOFIA_ELECTRON__.invokeDesktop("engineInfo");
    ...
    },
    {
      ...
      run: async (ctx) => {
        const denApiUrl = cleanBaseUrl(ctx.env.SOFIA_EVAL_DEN_API_URL);
        ...
        const sofiaUrl = typeof body.sofiaUrl === "string" ? body.sofiaUrl : "";
        ctx.assert(sofiaUrl.length > 0, "Den returned no enterprise deep link.");
        ctx.assert(sofiaUrl.startsWith("sofia:"), "Den returned the wrong desktop scheme.");
        const parsed = new URL(sofiaUrl);
        state.grant = body.grant ?? parsed.searchParams.get("grant") ?? "";
        state.sofiaUrl = sofiaUrl;
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.eval(`(() => {
          window.dispatchEvent(new CustomEvent("sofia:deep-link-native", {
            detail: [${JSON.stringify(state.sofiaUrl)}],
          }));
          return true;
        })()`);
        await ctx.waitFor(
        ...
        const bootstrap = await ctx.eval(
          "window.__SOFIA_ELECTRON__.invokeDesktop('getDesktopBootstrapConfig')",
          { awaitPromise: true },
        ) as {
          baseUrl?: string;
