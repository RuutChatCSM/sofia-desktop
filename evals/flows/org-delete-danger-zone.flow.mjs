 ...
 *   2. SOFIA_EVAL_DEN_WEB_URL=http://127.0.0.1:3005 SOFIA_EVAL_WEB_CDP_ADMIN=http://127.0.0.1:9855 pnpm fraimz --flow org-delete-danger-zone --stack den
 *      (the stack exports SOFIA_EVAL_DEN_API_URL and SOFIA_EVAL_DEN_TOKEN)
 *   3. In another shell, run den-web against the stack API:
 *      DEN_WEB_PORT=3005 DEN_API_BASE=http://127.0.0.1:8790 DEN_AUTH_ORIGIN=http://127.0.0.1:3005 DEN_AUTH_FALLBACK_BASE=http://127.0.0.1:8790 pnpm --filter @sofia-ee/den-web dev:local
...
const ADMIN_CDP_URL = (process.env.SOFIA_EVAL_WEB_CDP_ADMIN ?? "").trim().replace(/\/+$/, "");
const MYSQL_CONTAINER = "sofia-web-local-mysql";
const MYSQL_ARGS = ["exec", MYSQL_CONTAINER, "mysql", "-uroot", "-ppassword", "sofia_den", "-N", "-e"];
...
const OWNER_PASSWORD = `Sofia-${RUN_TAG}!`;
...
async function signInBrowser(ctx, email, password, label) {
  ...
  await goToDenWeb(ctx, "/");
  await ctx.waitFor(`localStorage.getItem('sofia:web:auth-token') === ${JSON.stringify(session.token)}`, { timeoutMs: 30_000, label: `${label} den-web token persisted` });
  return session;
}
...
export default {
  ...
  kind: "user-facing",
  requiresApp: false,
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_TOKEN", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_WEB_CDP_ADMIN"],
  steps: [
    {
