...
const MOBILE_CDP_URL = cleanBaseUrl(process.env.SOFIA_EVAL_WEB_CDP_MOBILE);
...
const MEMBER_PASSWORD = "SofiaDemo123!";
const DOWNLOAD_URL = "https://sofia.ruut.chat/download";
...
export default {
  ...
  steps: [
        ...
        await ctx.prove("Maya joins and lands on the desktop-app success step instead of the dashboard", {
          ...
          assert: async () => {
            await withClient(ctx, INVITEE_CDP_URL, async () => {
              ...
              await ctx.expectText("Open Sofia");
          ...
          screenshot: {
            ...
            requireText: ["Open Sofia", "Download the desktop app", "Edit spreadsheets"],
        ...
        await ctx.prove("The desktop welcome screen offers a Joining a team sign-in affordance", {
          ...
          assert: async () => {
            ...
            ctx.output(
              ...
              "WelcomeRoute.handleTeamSignIn calls platform.openLink(buildDenAuthUrl(settings.baseUrl || DEFAULT_DEN_BASE_URL, 'sign-in')); buildDenAuthUrl adds desktopAuth=1 and desktopScheme=sofia in desktop builds.",
            );
          },
          screenshot: {
    ...
    },
    {
      name: "Frame 6",
      run: async (ctx) => {
        await ctx.prove("Maya clicks Open Sofia and the Electron app signs into Acme", {
          voiceover: vo[5],
          action: async () => {
            await withClient(ctx, INVITEE_CDP_URL, async () => {
              ...
              state.copiedDesktopUrl = await ctx.waitFor(
                ...
                { timeoutMs: 30_000, label: "captured Sofia sign-in link" },
              );
              const clicked = await ctx.eval(`(() => {
                const button = document.querySelector('[data-testid="join-org-open-sofia"]');
                button?.scrollIntoView({ block: "center" });
                button?.click();
                return Boolean(button);
              })()`);
              ctx.assert(clicked, "Open Sofia button was not available on Maya's success page.");
            });

            useDesktopClient(ctx);
            await ensureDesktopReady(ctx);
            await resetDesktopDenSession(ctx);
            // Dev Electron does not register the OS sofia:// protocol handler;
            ...
            ctx.output(
              ...
              "Dev Electron lacks OS protocol registration in evals, so the flow dispatches sofia:deep-link with { detail: { urls: [sofiaUrl] } } — the same renderer bridge DenAuthProvider consumes.",
            );
          },
          assert: async () => {
            useDesktopClient(ctx);
            await ctx.waitFor("Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", { timeoutMs: 60_000, label: "persisted Den auth token" });
            await ctx.waitFor("(localStorage.getItem('sofia.den.activeOrgName') ?? '').includes('Acme Robotics')", { timeoutMs: 60_000, label: "Acme active org" });
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Mobile success is honest: email the link, no dead-end app open or download buttons", {
          ...
          assert: async () => {
            await withClient(ctx, MOBILE_CDP_URL, async () => {
              await applyMobileEmulation(ctx);
              await ctx.expectText("Sofia runs on your computer.");
              await ctx.expectText("Email me the download link");
              await ctx.expectNoText("Open Sofia");
              const hiddenDesktopButtons = await ctx.eval(`(() => {
                return document.querySelector('[data-testid="join-org-open-sofia"]') === null
          ...
          screenshot: {
            ...
            requireText: ["Sofia runs on your computer", "Email me the download link"],
            rejectText: ["Open Sofia", "Download the desktop app", "Something went wrong"],
          },
        });
        });
      },
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Riley's download-link email is sent and shows the desktop value", {
          ...
          assert: async () => {
            ...
            ctx.assert(html.includes("Download Sofia"), "Download email is missing its primary heading/CTA.");
            ...
            await withClient(ctx, MOBILE_CDP_URL, async () => {
              ...
              await ctx.waitForText("Download Sofia", { timeoutMs: 20_000 });
              await ctx.expectText("Edit spreadsheets");
            });
          },
          screenshot: {
            name: "riley-download-email",
            requireText: ["Download Sofia", "Edit spreadsheets"],
            rejectText: ["Something went wrong"],
          },
        });
        });
...
function blockedDownloadOrInstallHrefs(html) {
  return htmlHrefs(html).filter((href) => {
    ...
    return lowerHref.includes("/install") || lowerHref.includes("sofia.ruut.chat/download") || lowerHref.includes("download");
  });
}
...
function rewriteInviteLink(inviteLink) {
  ...
  // explicitly trust SOFIA_EVAL_DEN_WEB_URL for the browser navigation.
  const parsed = new URL(inviteLink, DEN_WEB_URL);
...
async function showDesktopWelcome(ctx) {
  await ctx.eval(`(() => {
    ...
    localStorage.setItem('sofia.preferences', JSON.stringify({ ...prefs, hasCompletedOnboarding: false }));
