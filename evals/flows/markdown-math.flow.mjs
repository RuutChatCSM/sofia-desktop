...
async function seedMathMessage(ctx, stage, expectedFormulas) {
  await ctx.waitFor(
    `window.__sofiaControl.listActions().some((action) => action.id === ${escapeUnsafeChars(JSON.stringify(MATH_SEED_ACTION))} && !action.disabled)`,
...
async function readMathState(ctx) {
  ...
  return ctx.eval(`(() => {
    ...
    const errors = Array.from(root.querySelectorAll("[data-sofia-math-error]"))
      .map((node) => (node.textContent || "").trim());
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Sofia displays a normal session before the math proof starts", {
          voiceover: vo[0],
          action: async () => {
