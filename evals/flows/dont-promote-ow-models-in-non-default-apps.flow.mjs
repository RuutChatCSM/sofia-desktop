...
const PROMO_REJECT_TEXT = [
  "Use Sofia Models",
  "Use Sofia Models without API keys",
  "Continue without Sofia Models",
...
function prefsPatchScript(patch) {
  return `(() => {
    ...
    localStorage.setItem("sofia.preferences", JSON.stringify({ ...prefs, ...${JSON.stringify(patch)} }));
    return true;
  })()`;
}
...
function promoResetScript() {
  return `(() => {
    for (const key of [
      "sofia.sofiaModelsPromo.hidden",
      "sofia.sofiaModelsPromo.lastShownAt",
      "sofia.sofiaModelsPromo.startupShown",
    ]) {
      localStorage.removeItem(key);
    }
    window.dispatchEvent(new Event("sofia-sofia-models-promo-changed"));
    return true;
  })()`;
}
...
function shellDefaultsScript() {
  return `(() => {
    let shellConfig = {};
    try {
      const raw = localStorage.getItem("sofia.shell-config");
      shellConfig = raw ? JSON.parse(raw) : {};
    } catch {
      shellConfig = {};
    }
    localStorage.setItem("sofia.shell-config", JSON.stringify({
      ...shellConfig,
      addWorkspace: true,
      cloudSignin: true,
      modelPicker: true,
...
function sendJson(response, status, payload) {
  response.writeHead(status, {
    "access-control-allow-headers": "authorization, content-type, x-sofia-legacy-org-id",
...
async function waitForControl(ctx, label = "Sofia control API") {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 60_000, label });
}
...
async function setDesktopBootstrapConfig(ctx, config) {
  ...
  await ctx.eval(`(async () => {
    ...
    const persisted = await window.__SOFIA_ELECTRON__.invokeDesktop("setDesktopBootstrapConfig", config);
    ...
    localStorage.setItem("sofia.den.baseUrl", baseUrl);
    localStorage.setItem("sofia.den.apiBaseUrl", apiBaseUrl);
...
async function resetToDefaultDen(ctx) {
  ...
  await ctx.eval(`(async () => {
    ...
    if (${quoted(state.workspaceId)}) {
      await window.__SOFIA_ELECTRON__?.invokeDesktop?.("workspaceSetSelected", ${quoted(state.workspaceId)});
      await window.__SOFIA_ELECTRON__?.invokeDesktop?.("workspaceSetRuntimeActive", ${quoted(state.workspaceId)});
      localStorage.setItem("sofia.react.activeWorkspace", ${quoted(state.workspaceId)});
    }
...
async function createDedicatedWorkspace(ctx) {
  ...
  state.workspaceDir = ctx.env.SOFIA_EVAL_WORKSPACE_DIR?.trim() || join(tmpdir(), `${FLOW_ID}-${Date.now()}`);
  ...
  try {
    ...
    throw new Error(`Could not reach Sofia server at ${baseUrl}: ${error?.message ?? error}`);
  }
  ...
  await ctx.eval(`(async () => {
    ${shellDefaultsScript()};
    ${prefsPatchScript({ hasCompletedOnboarding: true, providerStepCompleted: true })};
    await window.__SOFIA_ELECTRON__?.invokeDesktop?.("workspaceSetSelected", ${quoted(workspaceId)});
    await window.__SOFIA_ELECTRON__?.invokeDesktop?.("workspaceSetRuntimeActive", ${quoted(workspaceId)});
...
async function assertNoPromoText(ctx, surface) {
  ...
  ctx.recordEvidence({
    ...
    assertion: `${surface} does not contain Sofia Models subscribe or promo copy.`,
    actual: found,
  });
...
async function signInToCustomDen(ctx) {
  await ctx.eval(`(() => {
    localStorage.setItem("sofia.den.authToken", ${quoted(CUSTOM_TOKEN)});
    localStorage.setItem("sofia.den.activeOrgId", ${quoted(CUSTOM_ORG.id)});
    localStorage.setItem("sofia.den.activeOrgSlug", ${quoted(CUSTOM_ORG.slug)});
    localStorage.setItem("sofia.den.activeOrgName", ${quoted(CUSTOM_ORG.name)});
    ${promoResetScript()};
    ${prefsPatchScript({ hasCompletedOnboarding: true, providerStepCompleted: true })};
...
export default {
  id: FLOW_ID,
  title: "Custom Den deployments do not promote Sofia Models",
  kind: "user-facing",
  steps: [
    {
    ...
    },
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The default Sofia Cloud Den still exposes Sofia Models normally", {
          voiceover: vo[0],
          action: async () => {
            ...
            await ctx.waitForText("Sofia Models", { timeoutMs: 30_000 });
          },
          assert: async () => {
            await ctx.expectText("Sofia Models");
            await ctx.expectText("Providers");
            await ctx.expectText("Connected");
          },
          screenshot: {
            name: "frame-1-default-den-sofia-models",
            requireText: ["Providers", "Sofia Models", "Connected"],
    ...
    },
    {
      name: "Frame 2",
      run: async (ctx) => {
        await ctx.prove("The default Den model picker still lists Sofia Models for eligible users", {
          voiceover: vo[1],
          action: async () => {
            await closeDialogs(ctx);
          ...
          screenshot: {
            ...
            requireText: ["Models", "Sofia Models"],
            rejectText: ["Something went wrong"],
          },
        });
      },
        ...
        await ctx.prove("The desktop app can be switched from default Den to the custom Den URL", {
          ...
          assert: async () => {
            ...
            const stored = await ctx.eval(`(async () => {
              const config = await window.__SOFIA_ELECTRON__.invokeDesktop("getDesktopBootstrapConfig");
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("On the custom Den, the model picker shows normal available models without Sofia Models promos", {
          voiceover: vo[3],
          action: async () => {
            await ctx.eval(`(() => {
              ${clearDenSessionScript()};
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("Custom Den suppresses Sofia Models promos across startup, status, AI settings, provider setup, welcome, and first-task surfaces", {
          voiceover: vo[4],
          action: async () => {
            await closeDialogs(ctx);
            await ctx.eval(`(() => {
