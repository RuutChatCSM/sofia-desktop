 ...
 *   2. SOFIA_EVAL_WEB_CDP_MEMBER=http://127.0.0.1:9856 pnpm fraimz --flow join-org-already-joined --stack den
 */
...
export default {
  ...
  kind: "user-facing",
  requiresApp: false,
  requiredEnv: ["SOFIA_EVAL_DEN_API_URL", "SOFIA_EVAL_DEN_TOKEN", "SOFIA_EVAL_DEN_WEB_URL", "SOFIA_EVAL_WEB_CDP_MEMBER"],
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        // generic "Sofia" name, which degenerates every copy line in the
        // demo ("Join Sofia.", "welcome to Sofia's Sofia"). Rename
