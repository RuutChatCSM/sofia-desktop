...
const DEFAULT_DIAGNOSTICS_ORIGIN = "https://sofia-diagnostic.ruut.chat";
...
function denEnv(extra = {}) {
  return {
    ...
    DATABASE_URL: "mysql://root:password@127.0.0.1:3306/sofia_test",
    DB_MODE: "mysql",
    DEN_DB_ENCRYPTION_KEY: "x".repeat(32),
    BETTER_AUTH_SECRET: "y".repeat(32),
    BETTER_AUTH_URL: "https://den.sofia.test",
    SOFIA_DEV_MODE: "0",
        ...
        await ctx.prove("Den blocks private MCP URLs by default and exposes only the documented opt-out switches", {
          ...
          assert: async () => {
            ...
            ctx.output("DEN_ALLOW_PRIVATE_MCP_URLS env probes", [
              ...
              commandOutput("bun", ["--conditions", "development", "--eval", "<SOFIA_DEV_MODE=1 env probe>"], DEN_API_ROOT, devEnvProbe),
            ].join("\n\n"));
            ...
            witness(ctx, devEnvProbe.status === 0 && devEnv?.allowPrivateMcpUrls === true, "SOFIA_DEV_MODE=1 is the local-dev opt-out switch", commandOutput("bun", ["--conditions", "development", "--eval", "<SOFIA_DEV_MODE=1 env probe>"], DEN_API_ROOT, devEnvProbe));
          },
        });
      },
    },
        ...
        await ctx.prove("Cloud MCP diagnostics skip untrusted Den origins before any credentialed request", {
          ...
          action: async () => {
            ...
            result = run("bun", args, SERVER_ROOT, {
              ...process.env,
              SOFIA_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS: "",
            });
            output = commandOutput("bun", args, SERVER_ROOT, result);
            untrustedProbe = run("bun", ["--eval", `
              const { probeSofiaCloudCatalog } = await import("./src/agent-context-cloud-probe.ts");
              let calls = 0;
              const observed = await probeSofiaCloudCatalog({
        ...
        await ctx.prove("Den diagnostics use an operator-configurable origin and require a 24-character bearer token", {
          ...
          action: async () => {
            const args = [
              ...
              "-t",
              "defaults to the Sofia Labs diagnostic host and accepts an operator override",
            ];
