...
const MEMBER_TOKEN = process.env.SOFIA_EVAL_MEMBER_DEN_TOKEN?.trim() || DEN_TOKEN;
...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Clicking download returns Sofia Enterprise 0.17.38 instead of the disallowed 0.17.39 release", {
          voiceover: vo[2],
          action: async () => {
      ...
      run: async (ctx) => {
        await ctx.prove("Legacy desktop-version pins download the exact version the organization allows", {
          ...
          assert: async () => {
            ...
            witness(ctx, state.legacyDownload.location?.includes("/sofia-enterprise-win-x64-"), "The legacy-pinned download still selects the enterprise app artifact", state.legacyDownload);
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("Removing the allowed-version restriction redirects to Den's configured enterprise app release", {
          ...
          assert: async () => {
            ...
            witness(ctx, state.unrestrictedDownload.location?.includes("/RuutChatCSM/sofia-desktop/releases/download/"), "The unrestricted download uses the same GitHub release path as the public app", state.unrestrictedDownload);
            witness(ctx, state.unrestrictedDownload.location?.includes("/sofia-enterprise-win-x64-"), "The unrestricted download selects the enterprise app artifact", state.unrestrictedDownload);
