 ...
 *   1. Signs in to Sofia Cloud via desktop handoff and creates a workspace.
 ...
 * seeded demo org (alex@acme.test / SofiaDemo123!).
 */
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Owner is signed in to Sofia Cloud", {
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const wsPath = join(homedir(), ".sofia", "two-electron-demo", "eval-workspace-a");
