...
const MAYA_EMAIL = process.env.SOFIA_EVAL_MAYA_EMAIL?.trim() || process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "maya.support@acme.test";
const MAYA_PASSWORD = process.env.SOFIA_EVAL_MAYA_PASSWORD?.trim() || process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || "SofiaDemo123!";
...
const MOCK_PORT = Number(process.env.SOFIA_EVAL_MARKETPLACE_SLACK_MOCK_PORT ?? 4537);
const MOCK_BASE = `http://127.0.0.1:${MOCK_PORT}`;
const MOCK_MCP_URL = process.env.SOFIA_EVAL_MARKETPLACE_SLACK_MCP_URL?.trim() || `${MOCK_BASE}/mcp`;
...
export default {
  ...
  steps: [
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Maya's MCP-compatible harness searches Sofia and finds Create shift handoff from the assigned marketplace", {
          voiceover: vo[3],
          action: async () => {
            ...
            await renderHarness(ctx, "Search result from Sofia MCP", [
              { title: "Connected MCPs", body: { connected: ["Sofia MCP"], exposedTools: toolNames(state.toolsListPayload) } },
          ...
          assert: async () => {
            ...
            ctx.assert(JSON.stringify(exposedTools) === JSON.stringify(["execute_capability", "search_capabilities"]), `Harness should expose only Sofia MCP search/execute tools: ${JSON.stringify(exposedTools)}`);
          ...
          screenshot: {
            ...
            claim: "Maya's visible harness finds Create shift handoff through Sofia MCP only.",
            requireText: ["MAYA'S MCP-COMPATIBLE HARNESS", "Sofia MCP", "search_capabilities", "Create shift handoff", PLUGIN_NAME],
        ...
        await ctx.prove("Maya retries the same request and then executes the discovered Slack MCP tool with her connected account", {
          ...
          action: async () => {
            await renderHarness(ctx, "Retry after Maya connects Slack", [
              { title: "Connected MCPs", body: { connected: ["Sofia MCP"], note: "Maya retries the same search and execute inputs." } },
            ]);
...
function authHeaders(token) {
  ...
  if (state.orgId) {
    headers["x-sofia-org-id"] = state.orgId;
    headers["x-sofia-legacy-org-id"] = state.orgId;
  }
  return headers;
}
...
async function ensureMcpConnectionsCapability(ctx) {
  ...
  ctx.assert(
    ...
    "MCP Connections are disabled for this org. Provide SOFIA_EVAL_PLATFORM_ADMIN_EMAIL/PASSWORD to enable the eval capability, or run against a seeded org with MCP Connections on.",
  );
...
function markEmailVerified(ctx, email) {
  ctx.assert(
    ...
    `Maya account bootstrap needs email verification. Set SOFIA_EVAL_MARK_VERIFIED_CMD with an {email} placeholder, or pre-seed ${email}.`,
  );
...
async function startMock(ctx) {
  if (mockChild) return;
  ctx.assert(!(await mockHealthy()), `Port ${MOCK_PORT} is already serving. Stop that process or set SOFIA_EVAL_MARKETPLACE_SLACK_MOCK_PORT.`);
...
async function assertAllSkillsSearchableForMaya(ctx) {
  ...
  recordAssertion(ctx, "All three Support Operations skills remain searchable for Maya through Sofia MCP", searchable.every((entry) => entry.found), searchable);
}
...
async function prepareMayaHarness(ctx) {
  ...
  await renderHarness(ctx, "Maya's MCP-compatible harness", [
    { title: "Connection", body: { connected: ["Sofia MCP"], token: "Maya MCP token minted from /v1/mcp/token (redacted)" } },
  ]);
}
...
async function renderHarness(ctx, title, panels) {
  ...
  await ctx.eval(`(() => {
    document.title = 'Sofia MCP harness';
  ...
  return `
    <main style="min-height:100vh;background:#eef6ff;color:#0f172a;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:36px;">
      <section style="max-width:1040px;margin:0 auto;">
        ...
        <p style="font-size:15px;color:#475569;margin:0 0 24px;">Only the Sofia MCP is connected here. The harness calls <code>/mcp/agent</code> and renders the structured responses it receives.</p>
...
async function clickFocusedConnect(ctx) {
  const selector = '[data-sofia-eval-focused-connect="true"]';
  const marked = await ctx.eval(`(() => {
    ...
    button?.setAttribute('data-sofia-eval-focused-connect', 'true');
    return Boolean(button);
  })()`);
...
async function routeLocalSplitOriginCallback(ctx) {
  ...
  })()`, { timeoutMs: 30_000, label: "provider redirect to Sofia callback" });
