...
function currentSessionIdExpression(previousSessionId = "") {
  return `(() => {
    const route = window.__sofiaControl?.snapshot?.().route ?? window.location.hash ?? "";
      ...
      run: async (ctx) => {
        await ctx.prove("The selected conversation shows left-lane activity during active work", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              "window.__sofiaControl.listActions().some((action) => action.id === 'composer.set_text' && !action.disabled)",
      ...
      run: async (ctx) => {
        await ctx.prove("Completed conversation activity stays cleared after navigation", {
          ...
          action: async () => {
            ...
            await ctx.waitFor(
              "window.__sofiaControl.listActions().some((action) => action.id === 'session.open' && !action.disabled)",
              { timeoutMs: 30_000, label: "open conversation action" },
            );
            await ctx.control("session.open", { sessionId: state.sessionId });
            await ctx.waitFor(
              `window.__sofiaControl.snapshot().route.includes(${JSON.stringify(`/session/${state.sessionId}`)})`,
