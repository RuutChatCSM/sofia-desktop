...
const APPIMAGE = process.env.SOFIA_EVAL_APPIMAGE_PATH;
const MOVED_APPIMAGE = process.env.SOFIA_EVAL_MOVED_APPIMAGE_PATH;
const DATA_HOME = process.env.SOFIA_EVAL_XDG_DATA_HOME;
const CONFIG_HOME = process.env.SOFIA_EVAL_XDG_CONFIG_HOME;
const CACHE_HOME = process.env.SOFIA_EVAL_XDG_CACHE_HOME;
const USER_DATA = process.env.SOFIA_EVAL_ELECTRON_USERDATA;
const CDP_URL = process.env.SOFIA_EVAL_CDP_URL ?? "http://127.0.0.1:9223";
const CDP_PORT = new URL(CDP_URL).port;
const DESKTOP_ID = "com.differentai.sofia.desktop";
const MANAGER_ID = "gearlever-sofia.desktop";
const MIME = "x-scheme-handler/sofia";
...
async function desktopStatus(ctx) {
  return ctx.eval(
    `window.__SOFIA_ELECTRON__.invokeDesktop("desktopIntegrationStatus")`,
    { awaitPromise: true },
  );
}
...
async function closeApp(ctx) {
  ...
  throw new Error("Sofia did not close before the AppImage relaunch.");
}
...
function launchAppImage(appImagePath) {
  const child = spawn(appImagePath, [], {
    ...
    env: {
      ...xdgEnvironment(),
      SOFIA_ELECTRON_USERDATA: USER_DATA,
      SOFIA_ELECTRON_REMOTE_DEBUG_PORT: CDP_PORT,
    },
  });
  child.unref();
}
...
async function dismissPromptFor(appImagePath) {
  const statePath = path.join(CONFIG_HOME, "sofia", "desktop-integration.json");
...
export default {
  ...
  kind: "user-facing",
  requiredEnv: [
    "SOFIA_EVAL_APPIMAGE_PATH",
    "SOFIA_EVAL_MOVED_APPIMAGE_PATH",
    "SOFIA_EVAL_XDG_DATA_HOME",
    "SOFIA_EVAL_XDG_CONFIG_HOME",
    "SOFIA_EVAL_XDG_CACHE_HOME",
    "SOFIA_EVAL_ELECTRON_USERDATA",
    "SOFIA_EVAL_CDP_URL",
  ],
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The packaged AppImage shows an opt-in desktop integration control", {
          voiceover: vo[0],
          action: async () => {
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 45_000 });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Integrate installs the launcher, icon, and sofia:// handler", {
          voiceover: vo[1],
          action: async () => {
          ...
          assert: async () => {
            ...
            record(ctx, handler.status === 0 && handler.stdout.trim() === DESKTOP_ID, "The Sofia launcher is the selected sofia:// handler", handler.stdout.trim());

            const iconPath = path.join(DATA_HOME, "icons", "hicolor", "512x512", "apps", "com.differentai.sofia.png");
            ...
            const launch = run("gtk-launch", ["com.differentai.sofia"]);
      ...
      run: async (ctx) => {
        await ctx.prove("A moved AppImage silently repairs its owned entry, then defers to a manager-owned launcher without duplication", {
          ...
          action: async () => {
            ...
            await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs: 45_000 });
            // The launcher is stale after the move. Sofia repairs the entry it
...
`;
          ...
          assert: async () => {
            const status = await desktopStatus(ctx);
            record(ctx, status.state === "managed_externally", "Sofia recognizes the manager-owned launcher", status.state);
            ...
            record(ctx, !ownEntryExists, "Sofia does not create a duplicate launcher");
