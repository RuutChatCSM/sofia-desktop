        ...
        try {
          await ctx.prove("each demo gets an independent folder under the temporary demo root", {
            ...
            assert: async () => {
              ...
              const productionRoot = path.join(os.homedir(), ".sofia");
              witness(ctx, run.admin.root !== run.consumer.root, "Demo A and Demo B folders differ");
              witness(ctx, !run.admin.root.startsWith(productionRoot), "Demo A does not use the production .sofia folder", run.admin.root);
              witness(ctx, !run.consumer.root.startsWith(productionRoot), "Demo B does not use the production .sofia folder", run.consumer.root);
