 ...
 *   - MEMBER: the Sofia desktop app in Electron (CDP :9823) — signed into
...
/** Pick the den-web admin page target (title "Sofia Cloud"). */
async function connectAdmin() {
  const targets = await listTargets(ADMIN_CDP);
  const page = targets.find((t) => t.type === "page" && t.title.includes("Sofia Cloud"))
...
async function memberRefreshAndWait(member, condition, label, timeoutMs = 25000) {
  await evaluate(member, `window.dispatchEvent(new CustomEvent('sofia-den-settings-changed', { detail: {} }))`);
  await evaluate(member, `window.dispatchEvent(new CustomEvent('sofia-den-session-updated', { detail: {} }))`);
...
async function main() {
  ...
  await evaluate(member, `localStorage.setItem('sofia.react.settings.theme-mode','light')`);
  ...
  await sleep(800);
  await evaluate(member, `(() => {
    const store = window.__sofia?.notificationStore;
    // Use the public notify path if exposed; otherwise write an unread entry.
    try {
      const raw = localStorage.getItem('sofia:notifications:v1');
      ...
      localStorage.setItem('sofia:notifications:v1', JSON.stringify(data));
    } catch {}
    return true;
  })()`);
