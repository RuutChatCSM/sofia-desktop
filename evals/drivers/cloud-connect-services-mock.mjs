...
const MICROSOFT_MESSAGES = [
  {
    ...
    toRecipients: [{ emailAddress: { name: "Sofia Tester", address: "tester@example.test" } }],
...
const MICROSOFT_EVENTS = [
  {
    ...
    attendees: [],
    location: { displayName: "Sofia Room" },
    isCancelled: false,
...
const MICROSOFT_Q3_FILE = {
  ...
  parentReference: { driveId: "drive-sofia-test", path: "/drive/root:" },
};
...
function sendJson(response, status, body, headers = {}) {
  response.writeHead(status, {
    ...
    "access-control-allow-headers": "authorization,content-type,x-sofia-host-token",
...
function handleMicrosoftGraph(request, response, url) {
  ...
  if (url.pathname === "/graph/v1.0/me/messages" && request.method === "POST") {
    sendJson(response, 201, {
      id: "draft-sofia-test",
      subject: "Sofia permission parity draft",
      bodyPreview: "Drafted by the deterministic Microsoft Graph mock.",
      webLink: "https://outlook.office.test/mail/draft-sofia-test",
      isDraft: true,
    });
    return true;
  }
  ...
  if (url.pathname === "/graph/v1.0/me/events" && request.method === "POST") {
    sendJson(response, 201, {
      id: "event-sofia-test",
      subject: "Sofia permission parity review",
      start: { dateTime: "2026-07-13T10:00:00Z", timeZone: "UTC" },
      end: { dateTime: "2026-07-13T10:30:00Z", timeZone: "UTC" },
      webLink: "https://outlook.office.test/calendar/event-sofia-test",
    });
    return true;
  }
  ...
  if (url.pathname === "/graph/v1.0/me/drive/root:/Sofia/permission-parity.txt:/content" && request.method === "PUT") {
    sendJson(response, 201, {
  ...
  if (url.pathname === "/graph/v1.0/me/chats" && request.method === "GET") {
    sendJson(response, 200, graphEnvelope([{
      ...
      topic: "Sofia launch",
      chatType: "group",
      webUrl: "https://teams.office.test/chats/chat-sofia-test",
      lastUpdatedDateTime: "2026-07-13T09:00:00Z",
    }]));
    return true;
  }

  if (url.pathname === "/graph/v1.0/chats/chat-sofia-test/messages" && request.method === "GET") {
    sendJson(response, 200, graphEnvelope([{
      ...
      from: { user: { id: "microsoft-user-sofia-test", displayName: "Sofia Tester" } },
      webUrl: "https://teams.office.test/messages/teams-message-existing",
    }]));
    return true;
  }

  if (url.pathname === "/graph/v1.0/chats/chat-sofia-test/messages" && request.method === "POST") {
    sendJson(response, 201, {
...
function hasWorkerDualAuth(request) {
  return request.headers["x-sofia-host-token"] === MOCK_WORKER_HOST_TOKEN
...
function workerUnauthorized(response) {
  sendJson(response, 401, {
    ...
    message: "Both the Sofia host token and client bearer token are required.",
  });
}
...
function workerSnapshot(session, status) {
  ...
  if (status === "idle" && session.prompt) {
    messages.push(workerMessage(session, "assistant", `${session.messageId}-assistant`, `Sofia worker reply: ${session.prompt}`, session.messageId));
  }
  return {
    item: {
      session: {
