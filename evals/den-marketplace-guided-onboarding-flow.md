...
handoff explicit: install Sofia Desktop, sign in, then access organization
marketplaces from the app.

## Preconditions

- Run Den Web and Den API from a fresh Daytona server sandbox or a clean local DB.
- Use `SOFIA_DEV_MODE=1` for local/Daytona email-password sign-up without email delivery.
...
- `Sofia Marketplace`
  - Description explains built-in Sofia AI capabilities available in desktop after sign-in.
...
6. Confirm the onboarding screen includes:
   ...
   - Desktop guidance: download Sofia Desktop, sign in with the same account, then open Marketplace.
...
1. Start Sofia Desktop in dev mode pointed at the fresh Den Web/API URLs.
2. Open Settings -> Extensions -> Marketplace while signed out.
3. Confirm signed-out copy says Sofia is usable without an account and sign-in unlocks Marketplace/built-ins/org marketplaces.
4. Sign in to Sofia Cloud with the same Den account.
5. Return to Marketplace and refresh if needed.
6. Confirm:
   - `Sofia Marketplace` appears as a marketplace source/filter.
...
- Den sandbox: `sofia-server-onboarding-20260603-2108`.
- Electron sandbox: `sofia-test-20260603-211949`.
...
- Desktop Marketplace showed `Sofia Marketplace` and `Anthropic-Compatible Plugins`.
...
- Marketplace list shows only `Sofia Marketplace`.
