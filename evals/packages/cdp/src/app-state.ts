...
/**
 * Readiness of the Sofia desktop is defined by the UI being *interactive*,
 ...
 * layer (@sofia/hosts) and the behaviours specs call (@sofia/behaviors)
 * cannot drift apart. It lives in @sofia/cdp because both already depend on
...
const PROBE_EXPRESSION = `(() => {
  ...
  const welcome = text.includes("Welcome to Sofia");
  ...
  const stored = localStorage.getItem("sofia.react.activeWorkspace");
  ...
  const surface = welcome
    ...
    : (taskUi || settingsSurface) && workspaceId && !needsWorkspace
      ...
      : taskUi || needsWorkspace
        ...
        : null;
  return {
    controlReady: Boolean(window.__sofiaControl),
    transitional,
    surface,
