...
test("defer mode judges a caller-provided deterministic witness inline", async () => {
  const previousMode = process.env.SOFIA_EVAL_VISION;
  try {
    process.env.SOFIA_EVAL_VISION = "defer";
    ...
    if (previousMode === undefined) delete process.env.SOFIA_EVAL_VISION;
    else process.env.SOFIA_EVAL_VISION = previousMode;
  }
});

test("deferred validation records pending expectations that the judge resolves", async () => {
  const dir = await mkdtemp(join(tmpdir(), "sofia-deferred-vision-"));
  const previousMode = process.env.SOFIA_EVAL_VISION;
  try {
    const expectation = `Synthetic screenshot is visible ${randomUUID()}`;
