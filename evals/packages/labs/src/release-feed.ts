...
const DEFAULT_REPO = "RuutChatCSM/sofia-desktop";
...
export function desktopReleaseAssetFileName(
  ...
  const prefix = distribution === "public" ? "sofia" : `sofia-${distribution}`;
...
function assetBody(input: ReleaseAssetInput, version: string): Uint8Array {
  ...
  return textEncoder.encode(`sofia release ${version} ${input.platform}\n`);
}
