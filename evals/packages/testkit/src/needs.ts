...
export function unmetNeeds(requirements: TestNeeds, env: NodeJS.ProcessEnv): string[] {
  ...
  if (requirements.model === "tool-capable") {
    if (!present(env, "SOFIA_EVAL_MODEL")) missing.push("set SOFIA_EVAL_MODEL");
    if (!present(env, "OPENAI_API_KEY") && !present(env, "ANTHROPIC_API_KEY")) {
      missing.push("set OPENAI_API_KEY or ANTHROPIC_API_KEY");
    }
  }
  if (requirements.daytona && env.SOFIA_EVAL_DAYTONA?.trim() !== "1") {
    missing.push("set SOFIA_EVAL_DAYTONA=1");
  }
  if (requirements.placement === "daytona" && env.SOFIA_EVAL_DAYTONA?.trim() !== "1") {
    missing.push("set SOFIA_EVAL_DAYTONA=1");
  }
  if (
    requirements.placement === "local"
    && (env.SOFIA_EVAL_DAYTONA?.trim() === "1" || present(env, "SOFIA_EVAL_DEN_API_URL"))
  ) {
    missing.push("use local placement without SOFIA_EVAL_DEN_API_URL");
  }
  return missing;
}
