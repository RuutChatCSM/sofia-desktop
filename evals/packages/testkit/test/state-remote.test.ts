...
test("readConnectStateFile reads Daytona candidates in order", async () => {
  ...
  assert(calls[0]?.script.includes("/workspace/profiles/desktop/electron-userdata/sofia-dev-data/xdg/config/sofia/connect-state.json"));
  assert(calls[1]?.script.includes("/workspace/profiles/desktop/xdg-config/sofia/connect-state.json"));
...
test("readConnectStateFile preserves local profile reads", async () => {
  const root = await mkdtemp(join(tmpdir(), "sofia-state-test-"));
  try {
    const paths = electronProfilePaths(root);
    const statePath = join(paths.configHome, "sofia", "connect-state.json");
