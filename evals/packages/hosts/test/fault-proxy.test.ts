...
test("fault proxy script exposes authenticated controls and preserves local fault semantics", async () => {
  ...
  const directory = await mkdtemp(join(tmpdir(), "sofia-fault-proxy-"));
  ...
  const auth = { "x-sofia-fault-token": token };
  ...
  try {
    ...
    while (Date.now() < deadline) {
      try {
        health = await fetch(`${url}/__sofia_faults/health`);
        if (health.ok) break;
      } catch {
    ...
    assert.equal((await fetch(`${url}/__sofia_faults/requests`)).status, 401);
    assert.equal((await fetch(`${url}/__sofia_faults/clear`, { method: "POST" })).status, 401);
    assert.equal((await post("/__sofia_faults/rules", {
      kind: "status",
      pathPrefix: "/flaky",
      statusCode: 429,
      times: 2,
    ...
    assert.equal((await post("/__sofia_faults/clear")).status, 204);
    assert.equal((await fetch(`${url}/cleared`)).status, 200);

    const logResponse = await fetch(`${url}/__sofia_faults/requests`, { headers: auth });
