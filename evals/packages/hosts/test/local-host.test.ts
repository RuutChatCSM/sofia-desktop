...
const ENV_KEYS = [
  ...
  "SOFIA_ELECTRON_APP_NAME",
  "SOFIA_ELECTRON_DISABLE_PROTOCOL_REGISTRATION",
  "SOFIA_ELECTRON_REMOTE_DEBUG_PORT",
  "SOFIA_ELECTRON_SKIP_SHARED_PREPARE",
  "SOFIA_ELECTRON_USE_MOCK_KEYCHAIN",
  "SOFIA_ELECTRON_USERDATA",
  "SOFIA_ENV_STORE",
  "PORT",
  "VITE_DISABLE_SOFIA_MODELS",
  "XDG_CACHE_HOME",
  "XDG_CONFIG_HOME",
  "XDG_DATA_HOME",
  "XDG_STATE_HOME",
...
test("electronProfilePaths returns all expected paths under the profile root", () => {
  const root = join(tmpdir(), "sofia-local-host-profile");
...
test("electronSurfaceEnv matches the isolated Electron demo contract", () => {
  const root = join(tmpdir(), "sofia-local-host-env");
  const paths = electronProfilePaths(root);
  const env = electronSurfaceEnv(paths, {
    appName: "Sofia App Eval probe",
    appIdentifier: "com.differentai.sofia.eval.probe",
    port: 5123,
    cdpPort: 9123,
  });
  ...
  assert.equal(env.SOFIA_ELECTRON_APP_NAME, "Sofia App Eval probe");
  assert.equal(env.SOFIA_ELECTRON_APP_IDENTIFIER, "com.differentai.sofia.eval.probe");
  assert.equal(env.SOFIA_ELECTRON_SKIP_SHARED_PREPARE, "1");
  assert.equal(env.SOFIA_ELECTRON_USE_MOCK_KEYCHAIN, "1");
...
test("resolveChromeBinary finds Linux Chrome on PATH and reports a helpful error otherwise", async () => {
  const binDir = await mkdtemp(join(tmpdir(), "sofia-chrome-bin-"));
