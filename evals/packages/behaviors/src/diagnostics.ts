...
import { parseClientHelloVersions } from "@sofia/labs";
import type { ClientHelloParseResult, EgressLabHandle, EgressLabProfile } from "@sofia/labs";
import type { TlsFacts, TlsVersionFacts } from "@sofia/matchers";

export type { TlsFacts, TlsVersionFacts } from "@sofia/matchers";
...
async function main() {
  ...
  const cloudProbe = await cloudModule.probeSofiaCloudCatalog({
...
export function productDiagnosticsPrecondition(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  if (bun.status !== 0) {
    ...
    return `Bun is required to import Sofia's shipped TypeScript diagnostics modules for product-verdict egress proofs (${reason}).`;
  }
...
async function diagnoseTlsLab(lab: EgressLabHandle): Promise<DiagnosticVerdict> {
  ...
  if (lab.profile === "intercept" && /corporate|interception|proxy|mitm/u.test(issuerLower)) {
    return {
      profile: lab.profile,
      text: `LIKELY TLS INTERCEPTION: endpoint leaf is re-signed by ${servedIssuer}, not the expected public issuer. Install or pass the corporate root, or bypass TLS inspection for Sofia hosts.`,
      evidence: lines.join("\n"),
...
export async function readDeniedHostFacts(
  lab: EgressLabHandle,
  targetUrl = "https://github.com/RuutChatCSM/sofia-desktop/releases/latest",
...
async function diagnoseDenyLab(lab: EgressLabHandle): Promise<DiagnosticVerdict> {
  ...
  const facts = await readDeniedHostFacts(lab, `https://${host}/RuutChatCSM/sofia-desktop/releases/latest`);
