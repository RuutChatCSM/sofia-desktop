...
export async function captureOpenedUrls(): Promise<UrlCapture> {
  const dir = await mkdtemp(join(tmpdir(), "sofia-open-external-"));
...
export async function signInInBrowser(
  ...
  await timed("browser.signIn", async () => {
    ...
    await waitFor(browser, `(() => {
      ...
      // Plain web sign-in redirects into the dashboard; handoff shows "signed in" or the sofia:// code.
      if (location.pathname.startsWith("/dashboard")) return true;
      if (/signed in/i.test(text)) return true;
      return [...document.querySelectorAll("input")]
        .some((input) => (input.value ?? "").startsWith("sofia://"));
 ...
 *
 * Observed shape: the app opens `<den>/?mode=sign-up&desktopAuth=1&desktopScheme=sofia`,
 * the person signs in there, and Den shows "You're signed in" with an
 * "Open Sofia" button plus a readonly input holding the sign-in code —
 * the full `sofia://den-auth?grant=…&denBaseUrl=…` URL a person would
...
export async function readHandoffDeepLink(browser: Surface, { timeoutMs = 60_000 } = {}): Promise<string> {
  const found = await waitFor(browser, `(() => {
    const fromInput = [...document.querySelectorAll("input")]
      ...
      .find((value) => typeof value === "string" && value.startsWith("sofia://") && value.includes("grant="));
    if (fromInput) return fromInput;
    const fromAnchor = [...document.querySelectorAll('a[href^="sofia://"]')]
      .map((anchor) => anchor.getAttribute("href"))
      .find((href) => typeof href === "string" && href.includes("grant="));
    if (fromAnchor) return fromAnchor;
    const inText = (document.body?.innerText ?? "").match(/sofia:\\/\\/[^\\s"']+grant=[^\\s"']+/);
 ...
 *
 * A real OS dispatches `sofia://den-auth?grant=…` to the app. A container has
