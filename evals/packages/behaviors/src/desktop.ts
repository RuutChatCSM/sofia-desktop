import { describeAppState, dumpScreenState, evaluateOnSurface, isInteractive, probeAppStateOnSurface } from "@sofia/cdp";
import type { AppStateProbe, EvaluateOptions, Surface } from "@sofia/cdp";
...
export async function evalIn(app: Surface, expression: string, opts: EvaluateOptions = {}): Promise<unknown> {
  // Target healing lives in @sofia/cdp; behaviours just evaluate.
...
export async function waitForConnectionCard(app: Surface, name: string, workspaceId: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    await evalIn(app, "window.__sofiaControl.execute('extensions.refresh-marketplace', null)", { awaitPromise: true, timeoutMs: 15_000 })
      .catch(() => undefined);
    await evalIn(app, `(() => {
      const button = [...document.querySelectorAll('button')]
...
/** Invoke a registered `window.__sofiaControl` action, the product's own automation seam. */
