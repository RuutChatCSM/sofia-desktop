import { dumpScreenState, readActiveWorkspaceId } from "@sofia/cdp";
...
export async function signInDesktopAs(app: Surface, den: DenRef, member: DenSession): Promise<void> {
  await waitFor(app, "Boolean(window.__sofiaControl?.listActions?.().some((action) => action.id === 'auth.exchange-grant'))", {
    timeoutMs: 60_000,
  ...
  await waitForDenState(app, den, "Boolean((localStorage.getItem('sofia.den.authToken') ?? '').trim())", {
    timeoutMs: 45_000,
    label: "persisted den auth token",
  });
  await waitForDenState(app, den, "Boolean((localStorage.getItem('sofia.den.activeOrgId') ?? '').trim())", {
    timeoutMs: 60_000,
...
async function completeOrganizationOnboarding(app: Surface): Promise<void> {
  ...
  while (Date.now() < deadline && (await currentHash(app)).includes("/onboarding")) {
    const label = await evalIn(app, `(() => {
      ...
      return ["Continue with organization", "Continue to workspace", "Continue without Sofia Models", "Continue"]
