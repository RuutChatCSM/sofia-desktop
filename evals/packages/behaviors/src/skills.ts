...
export async function readLoadedExtensions(app: Surface): Promise<string[]> {
  ...
  await waitFor(app, 'document.body.innerText.includes("Sofia Browser")', {
    timeoutMs: 10_000,
    label: "Sofia Browser extension",
  });
  const value = await waitFor(app, `([...document.querySelectorAll("button")]
    .map((button) => (button.textContent ?? "").replace(/\\s+/g, " ").trim())
    .filter((label) => label.includes("Sofia Browser")))`, { timeoutMs: 60_000, label: "Sofia Browser extension rows" });
