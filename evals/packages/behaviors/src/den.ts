...
import { notImplemented } from "@sofia/labs";
...
export function doInternalMarkEmailVerified(command: string, email: string): void {
  if (!command.trim()) throw new Error("SOFIA_EVAL_MARK_VERIFIED_CMD is required to verify a newly-created member.");
  try {
...
export async function provisionOrg(den: DenRef, input: ProvisionOrgInput): Promise<ProvisionedOrg> {
  ...
  const email = `sofia-eval-admin-${unique}@example.test`;
  const name = `Sofia Eval ${unique}`;
  const signUp = await denFetch(den, "/api/auth/sign-up/email", {
    method: "POST",
  ...
  for (const connector of connectors) {
    const configured = await denFetch(den, `/v1/oauth-providers/${connector}/client`, {
      ...
      headers: auth(admin),
      body: JSON.stringify({
        clientId: `sofia-eval-google-client-${unique}`,
        clientSecret: `sofia-eval-google-secret-${unique}`,
      }),
    });
...
export async function createDesktopHandoffGrant(member: DenSession, desktopScheme = "sofia"): Promise<string> {
