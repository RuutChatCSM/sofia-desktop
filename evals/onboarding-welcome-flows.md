...
```text
Create a short welcome checklist for this Sofia workspace. Use exactly three bullets and mention one thing I can do next.
```
...
**Why**: When a user opens Sofia for the first time with zero
...
Pass criteria:
- URL is `/welcome`.
- Heading "Welcome to Sofia" is visible.
...
3. Expect: the local panel shows:
   ...
   - Explanation text: "This folder becomes your workspace. Sofia
     will be able to:"
...
**Why**: Users connecting to a remote Sofia server from the welcome
flow should also have onboarding marked complete.

Steps:
1. From `/welcome`, click "Get started".
2. Click "Connect custom remote".
3. Enter a valid Sofia server URL.
...
## Flow 28 — Sofia Models path explains payment before value

**Why**: Sofia Models is the preferred business path, but it must make the tradeoff clear: pay/sign in through Sofia Cloud to skip API keys, then return to task execution.

Steps:
1. Start from a clean first-run app at `/welcome` with no workspaces.
2. Create a local workspace through the welcome CTA.
3. On “Power your first task,” choose “Use Sofia Models.”
4. Expect: the user sees the Sofia Models value proposition and sign-in/payment CTA.
5. If a paid/signed-in test account is available, complete sign-in, select an Sofia model, and run the recommended first task prompt.
