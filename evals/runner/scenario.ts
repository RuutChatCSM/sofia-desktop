...
function denPrecondition(stageDen: DenServiceOptions): (ctx: FlowContext) => Promise<string | false> {
  return async (ctx) => {
    const apiUrl = ctx.env.SOFIA_EVAL_DEN_API_URL?.trim();
    const webUrl = ctx.env.SOFIA_EVAL_DEN_WEB_URL?.trim();
    ...
    try {
      ...
      if (stageDen.orgMode === "multi_org" && ctx.env.SOFIA_EVAL_DEN_MULTI_ORG?.trim()) {
        ctx.log(`Runtime config probe failed at ${runtimeConfigUrl(webUrl)}; honoring SOFIA_EVAL_DEN_MULTI_ORG fallback: ${messageText(error)}`);
        return false;
      }
