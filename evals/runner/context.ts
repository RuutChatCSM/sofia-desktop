...
export class EvalContext implements FlowContext {
  ...
  async ensureLightMode(): Promise<void> {
    await this.waitFor("Boolean(window.__sofiaControl)", {
      timeoutMs: 30_000,
  ...
  /**
   * Execute a registered window.__sofiaControl action.
   */
  async control(actionId: string, args?: unknown): Promise<unknown> {
    const result = await this.eval(
      `window.__sofiaControl.execute(${JSON.stringify(actionId)}, ${JSON.stringify(args ?? null)})`,
      { awaitPromise: true },
    );
  ...
  async screenshot(name: string, options: ScreenshotOptions = {}): Promise<string> {
    ...
    const sandbox = options.sandboxCapture === "computer-use"
      ? (this.env.SOFIA_EVAL_DAYTONA_SANDBOX_ID || this.env.SOFIA_EVAL_DAYTONA_SANDBOX)?.trim() ?? null
      : options.sandboxCapture
        ? this.env.SOFIA_EVAL_DAYTONA_SANDBOX?.trim() ?? null
        : null;
