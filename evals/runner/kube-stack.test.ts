...
function imagePlan(mode: "published" | "local"): KubeImagePlan {
  return {
    mode,
    denApiRepository: mode === "local" ? "sofia-den-api" : "ghcr.io/ruutchatcsm/sofia-den-api",
    denWebRepository: mode === "local" ? "sofia-den-web" : "ghcr.io/ruutchatcsm/sofia-den-web",
...
function withCleanImageEnv<T>(fn: () => Promise<T>): Promise<T> {
  const previous = process.env.SOFIA_EVAL_KUBE_IMAGES;
  delete process.env.SOFIA_EVAL_KUBE_IMAGES;
  return fn().finally(() => {
    if (previous === undefined) delete process.env.SOFIA_EVAL_KUBE_IMAGES;
    else process.env.SOFIA_EVAL_KUBE_IMAGES = previous;
  });
}
...
test("helm upgrade argv includes release, chart, profile, context, and local image overrides", () => {
  ...
  assert(args.includes("denApi.image.repository=sofia-den-api"));
  assert(args.includes("denWeb.image.repository=sofia-den-web"));
  assert(args.includes("--kube-context"));
  assert(args.includes("kind-sofia-kube-lab"));
});

test("kubectl rollout and port-forward argv use the kind context", () => {
  assert.deepEqual(rolloutStatusArgs("sofia-ee-den-api", "300s"), [
    "--context",
    "kind-sofia-kube-lab",
    "rollout",
    "status",
    "deployment/sofia-ee-den-api",
    "--timeout=300s",
  ]);
  assert.deepEqual(portForwardArgs("sofia-ee-den-web", 3005, 3005), [
    "--context",
    "kind-sofia-kube-lab",
    "port-forward",
    "service/sofia-ee-den-web",
    "3005:3005",
  ]);
});
...
test("kubeStackDown stops port-forwards before uninstalling the release", async () => {
  const stateDir = await mkdtemp(join(tmpdir(), "sofia-kube-stack-test-"));
  const order: string[] = [];
...
test("rollout failure surfaces pod status and recent logs", async () => {
  const { exec } = createExec((call) => {
    ...
    if (text.includes("rollout status deployment/sofia-ee-den-api")) return { stdout: "", stderr: "rollout failed", code: 1 };
    if (text.includes("get pods")) return success("pod/sofia-ee-den-api pending");
