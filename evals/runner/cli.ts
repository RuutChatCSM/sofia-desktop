...
const FLOWS_DIR = process.env.SOFIA_EVAL_FLOWS_DIR?.trim() || join(RUNNER_DIR, "..", "flows");
...
function manifestDaytonaSandbox(manifest: EnvManifest | null): string | null {
  ...
  return manifest.env?.SOFIA_EVAL_DAYTONA_SANDBOX?.trim() || null;
}

export function resolveHostPlacement(manifest: EnvManifest | null, env: NodeJS.ProcessEnv = process.env): HostPlacement {
  const daytonaSandboxId = manifestDaytonaSandbox(manifest) ?? (env.SOFIA_EVAL_DAYTONA_SANDBOX?.trim() || null);
  return {
    daytonaSandboxId,
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  const envCdp = process.env.SOFIA_EVAL_CDP_URL?.trim();
  ...
  for (const flow of selected) {
    ...
    if (args.repeat === 1) {
      ...
      const repeatRunStamp = process.env.SOFIA_EVAL_RUNSTAMP?.trim() || process.env.SOFIA_EVAL_RUN_STAMP?.trim() || runId;
