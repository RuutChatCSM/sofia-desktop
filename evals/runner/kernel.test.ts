...
test("env manifests round-trip and apply without overwriting explicit env", async () => {
  ...
  try {
    ...
    const env: NodeJS.ProcessEnv = { SOFIA_EVAL_DEN_API_URL: "preset-api" };
    applyManifestToEnv(manifest, env);
    assert.equal(env.SOFIA_EVAL_DEN_API_URL, "preset-api");
    assert.equal(env.SOFIA_EVAL_DEN_WEB_URL, "http://den-web.test");
    assert.equal(env.SOFIA_EVAL_DEN_TOKEN, "manifest-token");
    assert.equal(env.SOFIA_EVAL_DEN_MULTI_ORG, "1");
    ...
    assert.equal(singleOrgEnv.SOFIA_EVAL_DEN_MULTI_ORG, undefined);
  } finally {
...
test("defineScenario returns a flow, preserves steps, gates Den env, and passes actors", async () => {
  ...
  const ctx = new EvalContext({ client: null, outDir: tmpdir(), flowId: flow.id, env: { SOFIA_EVAL_RUNSTAMP: "abc123" } });
  ...
  assert.equal(ctx.state.freshEmail, "teammate-abc123@eval.sofia.test");
});
...
test("org-invite-two-desktops scenario loads with Den env gates and one step per narrated frame", async () => {
  ...
  assert(flow.requiredEnv?.includes("SOFIA_EVAL_DEN_API_URL"));
  assert(flow.requiredEnv?.includes("SOFIA_EVAL_DEN_WEB_URL"));
});
    ...
    steps: [
      {
        ...
        run: (ctx) => {
          attempt += 1;
          runstamps.push(ctx.env.SOFIA_EVAL_RUNSTAMP ?? "");
          ctx.recordEvidence({
            type: "frame",
            status: "passed",
...
test("EvalContext.on swaps clients, restores through nesting, and labels frames", async () => {
  const outDir = await mkdtemp(join(tmpdir(), "sofia-kernel-context-"));
