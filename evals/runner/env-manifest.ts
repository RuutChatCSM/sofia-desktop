...
export function applyManifestToEnv(manifest: EnvManifest, env: NodeJS.ProcessEnv): void {
  if (manifest.den) {
    setIfMissing(env, "SOFIA_EVAL_DEN_API_URL", manifest.den.apiUrl);
    setIfMissing(env, "SOFIA_EVAL_DEN_WEB_URL", manifest.den.webUrl);
    setIfMissing(env, "SOFIA_EVAL_DEN_TOKEN", manifest.den.token);
    if (manifest.den.orgMode === "multi_org") setIfMissing(env, "SOFIA_EVAL_DEN_MULTI_ORG", "1");
  }
