...
export const hostileGatewayFaultCases: readonly HostileGatewayFaultCase[] = [
  {
    ...
    claim: "Sofia's real connect path shows today's redirect-whitelist surface, including the exact generated callback but not the provider's rejected-URI message.",
  ...
  },
  {
    ...
    claim: "Sofia's real callback path reaches token exchange and currently collapses a per-connector redirect failure to the generic token-exchange diagnostic.",
  ...
  },
  {
    ...
    claim: "Sofia's real connect path attempts dynamic client registration when advertised, then currently maps an authorization-time DCR-required error generically.",
  ...
  {
    faultId: "method-405",
    claim: "Sofia's real connect path does not issue a GET to the MCP endpoint, so a GET-only 405 fault is latent in this user flow.",
    expectedProductSnippets: ["GET /mcp count 0", "needs_auth"],
  },
  {
    faultId: "duplicate-amplification",
    claim: "Sofia sends one tools/call for one user action, but today's Den diagnostic loses the duplicate-upstream-request signal.",
    expectedProductSnippets: ["one MCP tools/call", "provider status 409", "MCP_PROVIDER_TOOL_ERROR"],
  },
  {
    faultId: "refresh-expired",
    claim: "Sofia's real runtime attempts a refresh-token grant for expired credentials and currently reports a generic token/refresh exchange rejection.",
  ...
  },
  {
    ...
    claim: "Sofia's real client canonicalizes the trailing-dot authorization server before issuer discovery and still reaches authorization.",
    expectedProductSnippets: ["canonicalized", "needs_auth", "trailingDotRequests"],
  },
  {
    faultId: "per-user-403",
    claim: "Sofia's real callback validation classifies a per-user 403 as provider authorization, but today's message does not name the subject.",
