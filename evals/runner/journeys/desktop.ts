...
async function ensureDesktopControl(ctx: FlowContext, timeoutMs = 60_000): Promise<void> {
  await ctx.waitFor("Boolean(window.__sofiaControl)", { timeoutMs, label: "desktop control API" });
}
...
async function currentRoute(ctx: FlowContext): Promise<string> {
  const route = await ctx.eval(`(() => {
    try {
      const snapshotRoute = window.__sofiaControl?.snapshot?.().route;
...
async function chatFirstSessionReady(ctx: FlowContext): Promise<boolean> {
  ...
  return Boolean(await ctx.eval(`(() => {
    const route = window.__sofiaControl?.snapshot?.().route || window.location.hash.replace(/^#/, '') || window.location.pathname;
...
async function createWorkspaceFromWelcome(ctx: FlowContext, workspacePath: string, timeoutMs: number): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (!triedControl) {
      triedControl = true;
      const canControlCreate = await ctx.eval("Boolean(window.__sofiaControl?.listActions?.().find((action) => action.id === 'workspace.create' && !action.disabled))").catch(() => false);
      if (canControlCreate) {
...
async function getDesktopDenState(ctx: FlowContext): Promise<DesktopDenState> {
  const state = await ctx.eval(`(() => ({
    ...
    baseUrl: (localStorage.getItem('sofia.den.baseUrl') ?? '').trim(),
    apiBaseUrl: (localStorage.getItem('sofia.den.apiBaseUrl') ?? '').trim(),
    activeOrgName: (localStorage.getItem('sofia.den.activeOrgName') ?? '').trim(),
  }))()`);
...
async function writeDesktopBootstrap(ctx: FlowContext, denWebUrlValue: string, denApiUrlValue: string): Promise<void> {
  ...
  // desktop bootstrap through the Electron bridge, mirrors sofia.den.*
  ...
  const written = await ctx.eval(`(async () => {
    ...
    localStorage.setItem('sofia.den.baseUrl', ${JSON.stringify(denWebUrlValue)});
    localStorage.setItem('sofia.den.apiBaseUrl', ${JSON.stringify(denApiUrlValue)});
    for (const key of [
      'sofia.den.authToken',
...
async function completeDesktopCloudOnboardingIfNeeded(ctx: FlowContext): Promise<void> {
  ...
  const workspacePath = await mkdtemp(join(tmpdir(), "sofia-den-workspace-"));
...
async function waitForAssistantActivity(ctx: FlowContext, timeoutMs: number): Promise<void> {
  await ctx.waitFor(`(() => {
    ...
    const assistant = [...document.querySelectorAll('[data-message-role="assistant"], .markdown-content')]
      .some((element) => normalize(element.textContent).length > 0 && normalize(element.textContent) !== 'Sofia');
...
export async function firstBoot(ctx: FlowContext, options: FirstBootOptions): Promise<FirstBootResult> {
  const workspacePath = options.workspacePath?.trim() || await mkdtemp(join(tmpdir(), "sofia-workspace-"));
...
export async function connectDen(ctx: FlowContext, options: ConnectDenOptions): Promise<ConnectDenResult> {
  ...
  return ctx.on(options.surface, async () => {
    ...
    await ctx.waitFor("Boolean(window.__sofiaControl?.listActions?.().some((action) => action.id === 'auth.exchange-grant' && !action.disabled))", {
      timeoutMs: 30_000,
...
export async function runPrompt(ctx: FlowContext, options: RunPromptOptions): Promise<RunPromptResult> {
  ...
  return ctx.on(options.surface, async () => {
    ...
    await ctx.waitFor("window.location.hash.includes('/session') || window.__sofiaControl?.snapshot?.().route?.includes('/session')", {
      timeoutMs: 60_000,
...
export async function openSettings(ctx: FlowContext, options: OpenSettingsOptions): Promise<OpenSettingsResult> {
  return ctx.on(options.surface, async () => {
    ...
    const workspaceId = await ctx.eval(`(() => {
      ...
      return match?.[1] || localStorage.getItem('sofia.react.activeWorkspace') || '';
    })()`);
