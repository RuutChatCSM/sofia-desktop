...
function authToken(ctx: FlowContext, org?: SsoOrganizationRef): string {
  const token = org?.token?.trim() || ctx.env.SOFIA_EVAL_DEN_TOKEN?.trim() || "";
  if (!token) {
    throw new EvalError("SOFIA_EVAL_DEN_TOKEN is required to configure the mock IdP through the Den SSO API.");
  }
  return token;
}
...
function authHeaders(token: string, organizationId?: string, cookie?: string): Headers {
  ...
  if (organizationId) {
    headers.set("x-sofia-org-id", organizationId);
  }
  return headers;
}
...
function demoOwnerPassword(ctx: FlowContext): string {
  return ctx.env.DEN_DEMO_OWNER_PASSWORD?.trim() || "SofiaDemo123!";
}
