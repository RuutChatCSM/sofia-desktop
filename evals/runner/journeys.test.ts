...
test("Den URL helpers trim bases and build invite URLs", () => {
  const env: NodeJS.ProcessEnv = {
    SOFIA_EVAL_DEN_API_URL: " http://127.0.0.1:3004/api/den/// ",
    SOFIA_EVAL_DEN_WEB_URL: " http://127.0.0.1:3005/// ",
  };
...
test("Den env resolution reports missing URLs clearly", () => {
  assert.throws(
    () => resolveDenApiUrl({}),
    (error) => error instanceof EvalError && error.message.includes("SOFIA_EVAL_DEN_API_URL"),
  );
  assert.throws(
    () => resolveDenWebUrl({ SOFIA_EVAL_DEN_API_URL: "http://api.test" }),
    (error) => error instanceof EvalError && error.message.includes("SOFIA_EVAL_DEN_WEB_URL"),
  );
});
...
test("hostile gateway precondition reports an actionable bun skip reason", async () => {
  const reason = await hostileGatewayBunPrecondition("__sofia_missing_bun__");
