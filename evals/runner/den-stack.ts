 ...
 *   6. A demo-owner session token, exported as SOFIA_EVAL_DEN_API_URL /
 *      SOFIA_EVAL_DEN_TOKEN so env-gated flows run without manual setup.
 *
...
const EVAL_ELECTRON_USERDATA = process.env.SOFIA_EVAL_ELECTRON_USERDATA?.trim()
...
const MYSQL_CONTAINER = "sofia-web-local-mysql";
const MYSQL_STATE_DIR = join(STATE_DIR, "mysql");
const MYSQL_SOCKET = join(MYSQL_STATE_DIR, "mysql.sock");
const COMPOSE_ARGS = ["compose", "-p", "sofia-den-local", "-f", "packaging/docker/docker-compose.web-local.yml"];
const DEFAULT_MOCK_IDP_ISSUER = "http://127.0.0.1:19190";
const MOCK_IDP_ISSUER = (process.env.SOFIA_EVAL_MOCK_IDP_ISSUER?.trim() || DEFAULT_MOCK_IDP_ISSUER).replace(/\/+$/, "");
const DEN_WEB_ORIGIN = (process.env.SOFIA_EVAL_DEN_WEB_URL ?? "http://localhost:3005").replace(/\/+$/, "");
...
const DEN_TRUSTED_ORIGINS = [
  ...
  "http://127.0.0.1:5173",
  MOCK_IDP_ISSUER,
  ...envCsv("SOFIA_EVAL_DEN_EXTRA_TRUSTED_ORIGINS"),
].filter((origin, index, origins) => origins.indexOf(origin) === index).join(",");

// Override with SOFIA_EVAL_DATABASE_URL to isolate a run from the shared
// dev database (e.g. a dedicated schema on the same MySQL container).
const DEN_DATABASE_URL = process.env.SOFIA_EVAL_DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/sofia_den";
const DEN_DATABASE_NAME = new URL(DEN_DATABASE_URL).pathname.replace(/^\//, "") || "sofia_den";
...
export function denEvalEnvironment(options: DenEvalEnvironmentOptions = {}): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = {
    ...
    STRIPE_SECRET_KEY: process.env.STRIPE_SECRET_KEY ?? "sk_test_sofia_eval",
    STRIPE_INFERENCE_PRICE_ID: process.env.STRIPE_INFERENCE_PRICE_ID ?? "price_sofia_models_eval",
    STRIPE_WEBHOOK_SECRET: process.env.STRIPE_WEBHOOK_SECRET ?? "whsec_sofia_eval",
...
function devBootstrapPath(): string {
  return join(devUserDataHome(), "sofia-dev-data", "home", ".config", "sofia", "desktop-bootstrap.json");
}
...
async function ensureMysql(log: (message: string) => void): Promise<void> {
  if (process.env.SOFIA_EVAL_DATABASE_URL) {
    if (!(await nativeMysqlHealthy())) {
      throw new Error("SOFIA_EVAL_DATABASE_URL is configured but not reachable.");
    }
...
async function ensureSchema(log: (message: string) => void): Promise<void> {
  ...
  await run("pnpm", ["--filter", "@sofia-ee/den-db", "build"]);
...
async function ensureDenApi(log: (message: string) => void, orgMode?: DenOrgMode): Promise<void> {
  ...
  if (publicHealthOk) {
    throw new Error(
      ...
      "The desktop app cannot use a bare den-api there; rerun with SOFIA_EVAL_DEN_PORT=<free port>.",
    );
  }
  ...
  const proxyPid = spawnDetached(process.execPath, [join(RUNNER_DIR, "den-proxy.ts")], {
    ...
    env: {
      ...
      SOFIA_EVAL_DEN_PROXY_CONTROL: "1",
    },
  });
...
async function ensureApp(log: (message: string) => void, cdpCandidates: string[]): Promise<void> {
  ...
  const pid = spawnDetached("pnpm", ["dev"], {
    logName: "app",
    env: { SOFIA_ELECTRON_USERDATA: EVAL_ELECTRON_USERDATA },
  });
...
export async function ensureDenStack({ log, cdpCandidates, skipApp = false, orgMode }: EnsureDenStackOptions): Promise<void> {
  ...
  process.env.SOFIA_EVAL_DEN_WEB_URL = DEN_WEB_ORIGIN;
  process.env.SOFIA_EVAL_DEN_TOKEN = token;
  log(`Den stack ready — flows get SOFIA_EVAL_DEN_API_URL=${DEN_API_URL}, SOFIA_EVAL_DEN_WEB_URL=${DEN_WEB_ORIGIN}, and a fresh ${DEMO_EMAIL} token.`);
}
