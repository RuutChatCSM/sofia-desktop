...
function fakeSurface(name: string, kind: "electron" | "chrome"): SurfaceHandle {
  const handle: SurfaceHandle = {
    ...
    pid: 987654321,
    profileDir: `/tmp/sofia-fake-${name}`,
  };
  if (kind === "electron") handle.meta = { log: `/tmp/sofia-fake-${name}/electron.log` };
  return handle;
}
...
test("owt manifest lifecycle writes fake surfaces, shares links, and tolerates ESRCH on down", async () => {
  ...
  const previousToken = process.env.SOFIA_EVAL_DEN_TOKEN;
  delete process.env.SOFIA_EVAL_DEN_TOKEN;
  const host: Host = {
    kind: "fake-local",
  ...
  try {
    ...
    assert(printed.some((line) => line === "electron desk log: /tmp/sofia-fake-desk/electron.log"));
    ...
    if (previousToken === undefined) delete process.env.SOFIA_EVAL_DEN_TOKEN;
    else process.env.SOFIA_EVAL_DEN_TOKEN = previousToken;
...
test("owt up adopts existing Daytona surfaces without spawning", async () => {
  ...
  try {
    ...
    assert.equal(manifest.env?.SOFIA_EVAL_DAYTONA_SANDBOX, "sandbox-9");
    assert.equal(manifest.env?.SOFIA_EVAL_CDP_URL, "https://primary.example.test");
