...
SOFIA_EVAL_DEN_* exports. The generated trusted origins always include the
...
function printEnvironment(state: DevDenState): void {
  ...
  console.log(`export SOFIA_EVAL_DEN_API_URL="${state.apiUrl}"`);
  console.log(`export SOFIA_EVAL_DEN_WEB_URL="${state.webUrl}"`);
  console.log(`# DEN_BETTER_AUTH_TRUSTED_ORIGINS="${trustedOrigins}"`);
  console.log("# The trusted origins include SOFIA_EVAL_DEN_WEB_URL; otherwise sign-in fails with 403 INVALID_ORIGIN.");
...
async function applySchema(state: DevDenState): Promise<void> {
  ...
  await run("pnpm", ["--filter", "@sofia-ee/den-db", "db:bootstrap"], denEnvironment(state));
}

async function seedDemoOrg(state: DevDenState): Promise<void> {
  console.log("Seeding the demo organization...");
  await run("pnpm", ["--filter", "@sofia-ee/den-api", "seed:demo-org"], {
...
async function up(portValue: string | undefined, databaseValue: string | undefined, seed: boolean): Promise<void> {
  ...
  const database = validateDatabase(databaseValue ?? `sofia_den_eval_${process.pid}_${Date.now().toString(36)}`);
  ...
  const child = spawn("pnpm", ["--filter", "@sofia-ee/den-api", "dev:local"], {
    cwd: REPO_ROOT,
    detached: true,
