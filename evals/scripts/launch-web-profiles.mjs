...
async function killPidfile(userDataDir) {
  const pidPath = join(userDataDir, "sofia-eval-chrome.pid");
  let pid = 0;
  try {
...
async function ensureProfile(port) {
  const userDataDir = `/tmp/sofia-evals-web-${port}`;
  ...
  child.unref();
  await writeFile(join(userDataDir, "sofia-eval-chrome.pid"), `${child.pid}\n`);
...
for (const [name, port] of PROFILES) {
  console.log(`export SOFIA_EVAL_WEB_CDP_${name}=http://127.0.0.1:${port}`);
}
