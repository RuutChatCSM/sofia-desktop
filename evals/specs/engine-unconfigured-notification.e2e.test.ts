...
test(title, async ({ evidence }) => {
  ...
  await using app = await desktop({ name: "engine-unconfigured-notification" });
  ...
  const engineConfig = isRecord(config) && isRecord(config.engine) ? config.engine : {};
  const providerConfig = isRecord(engineConfig.provider) ? engineConfig.provider : {};
  expect(configResponse.status).toBe(200);
  expect(engineConfig.model).toBeUndefined();
  expect(Object.keys(providerConfig)).toEqual([]);

  const engineResponse = await fetch(
    `${server.directBaseUrl}/workspace/${workspace.workspaceId}/engine/session`,
    {
      method: "POST",
      headers: {
  ...
  expect(isRecord(engineError) ? engineError.code : null).toBe("engine_unconfigured");
