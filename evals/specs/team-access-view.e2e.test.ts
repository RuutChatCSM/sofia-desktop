...
const title = !apiUrl
  ? "team access view skipped: set SOFIA_EVAL_DEN_API_URL to a running Den API"
  : !webUrl
    ? "team access view skipped: set SOFIA_EVAL_DEN_WEB_URL to a running Den Web"
...
test.skipIf(!apiUrl || !webUrl)(title, async () => {
  ...
  const nova = await ensureMemberSession(den, admin, {
    email: process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "nova.spec@acme.test",
    password: process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || password,
    name: "Nova Spec",
    markVerifiedCmd: process.env.SOFIA_EVAL_MARK_VERIFIED_CMD?.trim(),
  });
  ...
  await using visualEvidence = createVisualEvidence("p3-team-access");
