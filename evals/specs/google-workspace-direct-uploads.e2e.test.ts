...
const title = !localPlacement
  ? "google workspace direct uploads skipped — needs: local placement without SOFIA_EVAL_DEN_API_URL"
  : !mysqlOpen
...
test.skipIf(!localPlacement || !mysqlOpen)(title, async ({ evidence, place }) => {
  needs({});
  const account = "uploader@sofia.test";
  const root = await mkdtemp(join(tmpdir(), "sofia-direct-upload-spec-"));
  ...
  try {
    ...
    const cloudActions = actions.filter((action): action is Record<string, unknown> => (
      isRecord(action) && action.extensionId === "sofia-cloud-uploads"
    ));
