...
async function writeSignInStampedBootstrap(denBaseUrl: string): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "sofia-enterprise-activation-"));
...
async function resolveCaEnvFromActivationRecord(options: {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), "sofia-chain-repair-spec-"));
  const logs: string[] = [];
...
test("enterprise TLS chain repair unlocks exactly the sign-in-stamped activation origin", async ({ evidence }) => {
  ...
  // Claim 3 — kill switch: with SOFIA_DISABLE_CHAIN_REPAIR=1 the same
  ...
  evidence.recordAssertionEvidence(
    ...
    `With SOFIA_DISABLE_CHAIN_REPAIR=1 and the same activation record, the runtime logged "chain repair disabled", never logged "chain repaired", and the child fetch failed again with ${chainErrorPattern.exec(killSwitched.output)?.[0] ?? "a chain error"}; the exported bundle carried the root only.`,
    true,
  );
