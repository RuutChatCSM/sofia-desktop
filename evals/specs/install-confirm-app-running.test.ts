...
test("the enterprise install guide is three steps ending in one copy-paste link", async ({ evidence }) => {
  ...
  // sentence; step 3 connects with the workspace address, keeping the Sofia
  ...
  expect(source).not.toContain("Copy Sofia link");
  ...
  expect(source).toContain("Set up Sofia Enterprise");
