...
import { denFetch, ensureMemberSession, signIn } from "@sofia/behaviors";
...
const title = apiUrl
  ...
  : "skill grant access skipped: set SOFIA_EVAL_DEN_API_URL";
let requestId = 0;
...
test.skipIf(!apiUrl)(title, async () => {
  ...
  const creator = await ensureMemberSession(den, admin, {
    email: process.env.SOFIA_EVAL_CREATOR_EMAIL?.trim() || "casey.spec@acme.test",
  ...
  const deniedEmail = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "nova.spec@acme.test";
  ...
  const thirdEmail = process.env.SOFIA_EVAL_THIRD_MEMBER_EMAIL?.trim() || "riley.spec@acme.test";
