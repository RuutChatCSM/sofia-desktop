...
import { denFetch, ensureMemberSession, evalIn, fill, signIn, waitFor } from "@sofia/behaviors";
...
const title = !apiUrl
  ? "plugin access panel skipped: set SOFIA_EVAL_DEN_API_URL to a running Den API"
  : !webUrl
    ? "plugin access panel skipped: set SOFIA_EVAL_DEN_WEB_URL to a running Den Web"
...
test.skipIf(!apiUrl || !webUrl)(title, async () => {
  ...
  const caseyEmail = process.env.SOFIA_EVAL_CREATOR_EMAIL?.trim() || "casey.spec@acme.test";
  caseySession = await ensureMemberSession(den, admin, {
    email: caseyEmail,
    password: process.env.SOFIA_EVAL_MEMBER_PASSWORD?.trim() || password,
    name: "Casey Spec",
  ...
  const novaEmail = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "nova.spec@acme.test";
  ...
  const tokenStored = await evalIn(browser, `(() => {
    localStorage.setItem("sofia:web:auth-token", ${JSON.stringify(admin.token)});
    return localStorage.getItem("sofia:web:auth-token") === ${JSON.stringify(admin.token)};
  })()`);
  ...
  await using visualEvidence = createVisualEvidence("p3-plugin-access");
