...
import type { MockMcpHandle, MockAuthorizeRequest } from "@sofia/labs";
import { eventually, sleep, test } from "@sofia/testkit";

const KUBE_CONTEXT = "kind-sofia-kube-lab";
const apiUrl = process.env.SOFIA_EVAL_DEN_API_URL?.trim().replace(/\/+$/, "") ?? "";
const webUrl = process.env.SOFIA_EVAL_DEN_WEB_URL?.trim().replace(/\/+$/, "") ?? "";
const allowedHostIp = process.env.SOFIA_EVAL_KUBE_ALLOWED_HOST_IP?.trim() ?? "";
...
const allowedPort = envPort("SOFIA_EVAL_KUBE_ALLOWED_MOCK_PORT", 4791);
const deniedPort = envPort("SOFIA_EVAL_KUBE_DENIED_MOCK_PORT", 4792);

function precondition(): string | null {
  if (!apiUrl) return "set SOFIA_EVAL_DEN_API_URL";
  if (!webUrl) return "set SOFIA_EVAL_DEN_WEB_URL";
  if (process.env.SOFIA_EVAL_KUBE_EGRESS_TEST?.trim() !== "1") return "set SOFIA_EVAL_KUBE_EGRESS_TEST=1";
  if (!allowedHostIp) return "set SOFIA_EVAL_KUBE_ALLOWED_HOST_IP";
  if (allowedPort === null) return "SOFIA_EVAL_KUBE_ALLOWED_MOCK_PORT must be a valid port";
  if (deniedPort === null) return "SOFIA_EVAL_KUBE_DENIED_MOCK_PORT must be a valid port";
