...
import { allocateFreePort, navigate } from "@sofia/cdp";
...
const title = !apiUrl
  ? "library view skipped: set SOFIA_EVAL_DEN_API_URL to a running Den API"
  : !webUrl
    ? "library view skipped: set SOFIA_EVAL_DEN_WEB_URL to a running Den Web"
...
test.skipIf(!apiUrl || !webUrl)(title, async () => {
  ...
  await using mock = await startMockMcp({
    port: mockPort,
    publicUrl: process.env.SOFIA_EVAL_LIBRARY_MOCK_PUBLIC_URL?.trim() || undefined,
  });
  ...
  const tokenStored = await evalIn(browser, `(() => {
    localStorage.setItem("sofia:web:auth-token", ${JSON.stringify(nova.token)});
    return localStorage.getItem("sofia:web:auth-token") === ${JSON.stringify(nova.token)};
  })()`);
