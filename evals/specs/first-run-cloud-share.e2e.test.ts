 ...
 *  - Only the OS protocol dispatch of `sofia://den-auth?grant=…` is bridged,
...
const denApiUrl = process.env.SOFIA_EVAL_DEN_API_URL?.trim().replace(/\/+$/, "") ?? "";
const denWebUrl = (process.env.SOFIA_EVAL_DEN_WEB_URL?.trim() || denApiUrl.replace("127.0.0.1", "localhost")).replace(/\/+$/, "");
const password = process.env.SOFIA_EVAL_DEMO_PASSWORD?.trim() || "SofiaDemo123!";
const adminEmail = process.env.SOFIA_EVAL_DEMO_EMAIL?.trim() || "alex@acme.test";
const colleagueEmail = process.env.SOFIA_EVAL_MEMBER_EMAIL?.trim() || "jordan.demo@acme.test";

const title = !e2eTestsEnabled
  ? "first-run cloud sharing skipped: set SOFIA_EVAL_E2E_TESTS=1 to opt in"
  : !denApiUrl
    ? "first-run cloud sharing skipped: set SOFIA_EVAL_DEN_API_URL to a running Den"
...
test.skipIf(!e2eTestsEnabled || !denApiUrl)(title, async () => {
  ...
  const signInLabel = ["Sign in to Sofia Cloud", "Sign in"].find((label) => buttons.includes(label));
  ...
  await signInInBrowser(browser, handoffUrl, { email: adminEmail, password });
  {
    ...
    const seen = await validate(shot, [
      "A browser page shows an Sofia Cloud sign-in result or dashboard, not a sign-in form error",
  ...
  expect(deepLink.startsWith("sofia://"), `unexpected deep link: ${deepLink}`).toBe(true);
  ...
  ).toBe(true);
  {
    const shot = await screenshot(app);
    const seen = await validate(shot, [
      "The app is back in focus and no longer offers a bare 'Sign in to Sofia Cloud' as the only action",
