...
// Proves the docs' "Verified" claim for Sofia engine: the real `engine` CLI
...
// Launcher names engine may shell out to for opening the authorize URL.
...
const requirements: TestNeeds = {
  ...
  commands: [placementCommand, SOFIA_ENGINE_BIN],
};
const missingRequirements = unmetNeeds(requirements, process.env);
const title = missingRequirements.length > 0
  ? `engine OAuth connection to the Den MCP agent skipped — needs: ${missingRequirements.join(", ")}`
  : "engine connects to the Den MCP agent through the full OAuth flow and survives token rotation";
...
function childEnvironment(home: { root: string; binDir: string; browserShim: string }): NodeJS.ProcessEnv {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    // The spec may itself run under an engine session; parent SOFIA_ENGINE_*
    // variables must not redirect the child CLI away from the isolated home.
    if (key.startsWith("SOFIA_ENGINE")) continue;
...
async function prepareWorkspaceEngineHome(mcpUrl: string): Promise<WorkspaceEngineHome> {
  const root = await mkdtemp(path.join(os.tmpdir(), "sofia-engine-oauth-"));
  const binDir = path.join(root, "bin");
  const configDir = path.join(root, "xdg-config", "engine");
  const dataDir = path.join(root, "xdg-data", "engine");
  const capturedUrlsPath = path.join(root, "captured-urls.txt");
  await mkdir(binDir, { recursive: true });
  await mkdir(configDir, { recursive: true });
  await mkdir(dataDir, { recursive: true });
  await mkdir(path.join(root, "xdg-cache", "engine"), { recursive: true });
  ...
  await writeFile(path.join(configDir, "engine.json"), `${JSON.stringify(config, null, 2)}\n`, "utf8");
  return {
    root,
...
function runWorkspaceEngine(home: WorkspaceEngineHome, args: string[], label: string, timeoutMs = COMMAND_TIMEOUT_MS): Promise<CommandResult> {
...
function startAuthProcess(home: WorkspaceEngineHome): AuthProcess {
  const child: ChildProcess = spawn(SOFIA_ENGINE_BIN, ["mcp", "auth", MCP_NAME], {
    cwd: home.root,
    env: home.env,
    stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  const exited = new Promise<CommandResult>((resolve) => {
    const finish = (exitCode: number | null) => {
      const out = stripAnsi(Buffer.concat(stdout).toString("utf8"));
      const err = stripAnsi(Buffer.concat(stderr).toString("utf8"));
      resolve({ label: "engine mcp auth", exitCode, stdout: out, stderr: err, combined: `${out}\n${err}` });
    };
    child.on("error", () => finish(null));
    child.on("close", (code) => finish(code));
  });
...
async function waitForAuthorizeUrl(home: WorkspaceEngineHome, auth: AuthProcess): Promise<string> {
  ...
  throw new Error(`engine mcp auth never surfaced an authorize URL. Output so far:\n${lastOutput.slice(0, 2_000)}`);
}
...
// organization, consent accept, then the front-channel hop to engine's
...
async function readStoredCredential(home: WorkspaceEngineHome): Promise<StoredCredential> {
...
async function poisonStoredCredential(home: WorkspaceEngineHome, credential: StoredCredential): Promise<{ expiryPoisoned: boolean; accessPoisoned: boolean }> {
...
test.skipIf(missingRequirements.length > 0)(title, { timeout: 30 * 60_000 }, async ({ evidence, place }) => {
  ...
  if (process.env.SOFIA_EVAL_DEN_API_URL?.trim()) {
    throw new SkipError("The engine OAuth proof requires a cold managed Den");
  }
  ...
  // engine validates that match per RFC 9728 and refuses drifted hosts, so
  ...
  expect(advertisedResource, "The advertised MCP resource identity must match the URL clients connect to (RFC 9728); strict OAuth clients like engine refuse drifted hosts.").toBe(mcpUrl);

  const home = await prepareWorkspaceEngineHome(mcpUrl);
  ...
  // Claim 2 — the real engine binary drives a spec-compliant PKCE
  ...
  if (!authResult) {
    authProcess.kill();
    throw new Error(`engine mcp auth did not exit after the loopback callback. Output:\n${authProcess.liveOutput().slice(0, 2_000)}`);
  }
  const authClean = authResult.exitCode === 0 && !containsTokenMaterial(authResult.combined);
  evidence.recordAssertionEvidence(
    "engine completed the full OAuth PKCE flow after real consent approval",
    `Authorize URL used S256 + loopback ${redirectUri} pinned to ${mcpUrl}; consent hop ${approval.consentPageUrl} redirected to ${approval.callbackUrl.origin}${approval.callbackUrl.pathname} (HTTP ${approval.callbackStatus}) carrying only code+state; engine exited ${String(authResult.exitCode)} without leaking token material.`,
  ...
  expect(authResult.exitCode, `engine mcp auth failed:\n${authResult.combined.slice(0, 2_000)}`).toBe(0);
  expect(containsTokenMaterial(authResult.combined)).toBe(false);

  // Claim 3 — engine persisted rotating OAuth credentials, accepts Den's
  ...
  const standaloneRejected = standaloneGet.status === 405
    ...
    && standaloneBody === "";
  const authList = await runWorkspaceEngine(home, ["mcp", "auth", "list"], "engine mcp auth list");
  const mcpList = await runWorkspaceEngine(home, ["mcp", "list"], "engine mcp list");
  ...
  evidence.recordAssertionEvidence(
    "engine accepts Den's 405 standalone-SSE rejection and remains authenticated and connected",
  ...
  expect(authListSound, `engine mcp auth list did not report authenticated:\n${authList.combined.slice(0, 1_000)}`).toBe(true);
  expect(mcpListSound, `engine mcp list did not report connected:\n${mcpList.combined.slice(0, 1_000)}`).toBe(true);

  // Claim 4 — the engine-minted OAuth token works on the MCP surface:
  ...
  evidence.recordAssertionEvidence(
    "The OAuth token engine obtained drives the real capability surface",
  ...
  const mcpListAfterExpiry = await runWorkspaceEngine(home, ["mcp", "list"], "engine mcp list after forced expiry");
  ...
  evidence.recordAssertionEvidence(
    "engine transparently refreshes an expired credential with refresh-token rotation while the stale token stays rejected",
    `After poisoning the stored credential, engine mcp list exited ${String(mcpListAfterExpiry.exitCode)} and reported connected; the stale bearer got HTTP ${staleRejected.status}; the store rotated to a new access token (changed: ${String(refreshed.accessToken !== credential.accessToken)}) and new refresh token (changed: ${String(refreshed.refreshToken !== credential.refreshToken)}); tools/list with the refreshed token returned ${JSON.stringify(refreshedToolNames)}.`,
    refreshSound,
  );
  expect(staleRejected.status).toBe(401);
  expect(mcpListAfterExpiry.exitCode, `engine mcp list failed after forced expiry:\n${mcpListAfterExpiry.combined.slice(0, 1_000)}`).toBe(0);
  ...
  // Claim 6 — logout removes the stored credential and engine stops
  // reporting the server as authenticated.
  const logout = await runWorkspaceEngine(home, ["mcp", "logout", MCP_NAME], "engine mcp logout");
  const authListAfterLogout = await runWorkspaceEngine(home, ["mcp", "auth", "list"], "engine mcp auth list after logout");
  ...
  evidence.recordAssertionEvidence(
    ...
    `engine mcp logout exited ${String(logout.exitCode)}; auth list then said: ${authListAfterLogout.combined.trim().slice(0, 300)}; the credential store no longer contains a refresh token.`,
    logoutSound,
  );
  expect(logout.exitCode, `engine mcp logout failed:\n${logout.combined.slice(0, 1_000)}`).toBe(0);
