...
import { denFetch, freshSession } from "@sofia/behaviors";
import type { DenSession } from "@sofia/behaviors";
import { eventually, localMysqlIsRunning, needs, server, test } from "@sofia/testkit";
...
const title = !e2eTestsEnabled
  ? "role change session survival skipped — needs: set SOFIA_EVAL_E2E_TESTS=1"
  : !localPlacement
    ? "role change session survival skipped — needs local placement without SOFIA_EVAL_DEN_API_URL"
    : !mysqlOpen
