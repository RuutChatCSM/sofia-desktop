...
description: "Reliable HTML-to-MP4 rendering via headless browser recording (Playwright/Puppeteer) + ffmpeg. Trigger when building or debugging pipelines that turn HTML pages into video files, including headless recording, screen capture, deterministic frame capture, multi-scene concatenation, or mixed-engine video output. Also trigger on symptoms like FOUT, frozen opening frames, truncated animations, wrong duration after concat, soft output, or choppy playback."
---
