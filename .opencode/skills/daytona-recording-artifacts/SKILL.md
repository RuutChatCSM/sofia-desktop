...
journey. Pass/fail evidence comes from an `@sofia/testkit` spec and its
...
imports `test` from `@sofia/testkit`; screenshots and validation claims are
...
```text
sofia-eval-artifacts:/daytona-artifacts
```
