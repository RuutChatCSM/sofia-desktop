...
Initial Helm chart for the Sofia EE Den stack:
...
```yaml
image:
  tag: "REPLACE_SOFIA_VERSION"

config:
  tenancy:
    # Default chart behavior is single-org for private/self-hosted installs.
    # Hosted Sofia Cloud should set this to "multi_org" explicitly.
    mode: "single_org"
    singleOrgName: "Sofia"
  ...
  public:
    webOrigin: "https://sofia.example.com"
    apiOrigin: "https://api.sofia.example.com"
    mcpResourceUrl: "https://api.sofia.example.com/mcp"
    mcpClaimNamespace: "https://sofia.example.com"
    desktopDenBaseUrl: "https://sofia.example.com"
    corsOrigins: "https://sofia.example.com,https://api.sofia.example.com"
    betterAuthTrustedOrigins: "https://sofia.example.com"
    webAppHosts: "sofia.example.com"
    ...
    authCallbackUrl: "https://sofia.example.com"
  githubConnector:
    appId: ""
    clientId: ""
...
secret:
  values:
    ...
    databaseUrl: "mysql://sofia:REPLACE_ME@mysql.example.internal:3306/sofia_den?sslaccept=accept"
...
ingress:
  ...
  className: nginx
  web:
    host: sofia.example.com
  api:
    host: api.sofia.example.com
```
...
```bash
helm template sofia-ee ./packaging/helm/sofia-ee -f values.prod.yaml
helm upgrade --install sofia-ee ./packaging/helm/sofia-ee -f values.prod.yaml
```
...
unambiguous. Hosted Sofia Cloud explicitly enables availability.
...
secret:
  create: false
  existingSecret: sofia-ee-secrets
```
...
Set optional `DATABASE_REDIS_URL` to enable Den API Redis-backed session and query caching. Set `DAYTONA_API_KEY` when `config.provisioner.mode` is `daytona`. Set `POLAR_ACCESS_TOKEN` when Polar feature gating is enabled. Set `OPENROUTER_MANAGEMENT_API_KEY` when enabling Sofia Models management.
...
secret:
  values:
    databaseRedisUrl: "rediss://redis-master.sofia.svc.cluster.local:6379"
```
...
Use `customCa` when Sofia must trust a private certificate authority for
...
```bash
kubectl create secret generic sofia-custom-ca \
  --namespace sofia \
...
customCa:
  enabled: true
  existingSecret: sofia-custom-ca
  existingConfigMap: ""
  key: ca.crt
```
...
```bash
kubectl create configmap sofia-custom-ca \
  --namespace sofia \
  --from-file=ca.crt=./corp-root-ca.pem
```
...
customCa:
  enabled: true
  existingSecret: ""
  existingConfigMap: sofia-custom-ca
  key: ca.crt
```
...
`/etc/sofia/custom-ca/ca-bundle.pem` and sets `NODE_EXTRA_CA_CERTS` to that
...
secret:
  values:
    databaseUrl: "mysql://sofia:REPLACE_DB_PASSWORD@mysql.example.internal:3306/sofia_den?sslmode=verify-full"
```
...
roots your Sofia deployment should trust. On CA rotation, update the existing
Secret or ConfigMap and restart the running workloads so Node reloads the CA
file, for example:

```bash
kubectl rollout restart deployment/sofia-ee-den-api --namespace sofia
kubectl rollout restart deployment/sofia-ee-den-web --namespace sofia
kubectl rollout restart deployment/sofia-ee-inference --namespace sofia
```
...
The chart configures telemetry export from Sofia; it does not install an
...
`localhost`, because that would refer to the Sofia container itself.

First create the namespace used by this example:

```bash
kubectl create namespace sofia
```
...
```bash
kubectl create secret generic sofia-otel-headers \
  --namespace sofia \
...
observability:
  backend: otel
  serviceNames:
    denApi: sofia-den-api
    denWeb: sofia-den-web
  otel:
    ...
    tracesSamplerArg: ""
    headers:
      existingSecret: sofia-otel-headers
...
Install or upgrade Sofia with the values file:
...
The commands below assume the Helm release is named `sofia-ee`. If you use a
different release name, run `kubectl get deployments,services --namespace
sofia` to find the generated resource names.

Confirm that the workloads are ready:

```bash
kubectl get pods --namespace sofia
kubectl rollout status deployment/sofia-ee-den-api --namespace sofia
kubectl rollout status deployment/sofia-ee-den-web --namespace sofia
```
...
```bash
kubectl describe deployment/sofia-ee-den-api --namespace sofia
kubectl describe deployment/sofia-ee-den-web --namespace sofia
```
...
```bash
kubectl port-forward service/sofia-ee-den-web 3005:3005 --namespace sofia
```
...
Your observability backend should show `sofia-den-web` and
`sofia-den-api`, with one connected trace for the request. Logs from both
...
- `observability.otel.endpoint` is a base endpoint. Sofia appends
...
- The Secret must be in the Sofia release namespace, and its key must match
...
observability:
  ...
  sentry:
    dsnSecret:
      existingSecret: sofia-sentry-runtime
      key: SENTRY_DSN
...
- Setup URL: `https://sofia.example.com/dashboard/integrations/github`
- Webhook URL: `https://api.sofia.example.com/v1/webhooks/connectors/github`
...
- `DEN_API_BASE=http://<release>-sofia-ee-den-api:8788`
- `DEN_AUTH_FALLBACK_BASE=http://<release>-sofia-ee-den-api:8788`
- `INFERENCE_PROXY_BASE_URL=http://<release>-sofia-ee-inference:8791` when `inference.enabled=true`
...
config:
  public:
    ...
    installerReleaseRepo: "RuutChatCSM/sofia-desktop"

installerArtifacts:
  enabled: true
  existingClaim: sofia-desktop-artifacts
  mountPath: /var/lib/sofia/installer-artifacts
```
...
The organization download page hands the normal Sofia app its Den
...
setup stays in the **Open Sofia** deep-link step after installation.
...
- `sofia-mac-arm64-<version>.dmg`
- `sofia-mac-x64-<version>.dmg`
- `sofia-win-x64-<version>.exe`
- `sofia-linux-x86_64-<version>.AppImage`
- `sofia-linux-arm64-<version>.AppImage`
