...
const tmp = mkdtempSync(join(tmpdir(), "sofia-custom-ca-"));
...
https.get(process.argv[1], (response) => {
  ...
  response.on("end", () => {
    if (response.statusCode === 200 && body === "sofia-custom-ca-ok") {
      console.log(body);
      process.exit(0);
    }
...
`,
  ...
  runOpenSsl(["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", "/CN=Sofia Test CA", "-keyout", caKey, "-out", caCert]);
  ...
  const server = createServer(
    ...
    (_request, response) => {
      response.end("sofia-custom-ca-ok");
    },
  );
