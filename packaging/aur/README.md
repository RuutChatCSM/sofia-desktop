...
Use this directory to build and install the Sofia package locally on Arch Linux without Docker.
...
- Installs the Electron bundle under `/opt/sofia`.
- Adds `/usr/bin/sofia`, a desktop entry, and the Sofia icon.
...
```bash
sudo pacman -U sofia-<version>-1-x86_64.pkg.tar.zst
```
